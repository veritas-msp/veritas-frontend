/** Options de contrat par défaut (fallback si l'API est indisponible). */
export const FALLBACK_CONTRACT_MODULES = [
  { id: 1, moduleKey: "Support", label: "Support", icon: "mdi:headset", enabled: true, sortOrder: 10 },
  { id: 2, moduleKey: "Curatif", label: "Curatif", icon: "tabler:truck-filled", enabled: true, sortOrder: 20 },
  { id: 3, moduleKey: "Preventif", label: "Préventif", icon: "fluent-mdl2:documentation", enabled: true, sortOrder: 30 },
  { id: 4, moduleKey: "Monitoring", label: "Monitoring", icon: "eos-icons:monitoring", enabled: true, sortOrder: 40 },
  { id: 5, moduleKey: "Hebergement", label: "Hébergement", icon: "carbon:data-center", enabled: true, sortOrder: 50 },
];
...
export function getAllActiveModuleKeys(options, moduleDefinitions) {
  ...
  const legacy = Object.keys(options || {}).filter(
    (key) => options[key] && !known.has(key)
  );

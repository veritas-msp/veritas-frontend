...
export default function LicencesTab({ licences, dashboardMetrics, theme }) {
  ...
  const getSortIcon = (column) => {
    ...
    return sortDirection === 'asc'
      ? <FaSortUp style={{ fontSize: '0.75rem', marginLeft: '0.25rem', color: '#15D1A0' }} />
      : <FaSortDown style={{ fontSize: '0.75rem', marginLeft: '0.25rem', color: '#15D1A0' }} />;
  };
  ...
  const filteredAndSortedLicences = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const totalA = a.total || 0, totalB = b.total || 0;
      const usedA = a.utilisees || 0, usedB = b.utilisees || 0;
      const availA = Math.max(0, totalA - usedA), availB = Math.max(0, totalB - usedB);
  ...
  return (
    <section className={styles.kpiSection}>
      <h2 className={styles.sectionTitle}>Informations de licence</h2>
      {licences.length > 0 ? (
        <>
          {/* KPI en cartes */}
          {dashboardMetrics && (
            <div className={styles.statsCards} style={{ marginBottom: '1rem' }}>
              <div className={styles.statCard}>
                <div className={styles.statCardContent}>
                  <div className={styles.statCardLabel}>Total licences</div>
              ...
              <div className={styles.statCard}>
                <div className={styles.statCardContent}>
                  ...
                  <div className={styles.statCardValue} style={{ color: '#f59e0b' }}>{usedLicenses.toLocaleString()}</div>
                </div>
              </div>
              <div className={styles.statCard}>
                <div className={styles.statCardContent}>
                  <div className={styles.statCardLabel}>Licences disponibles</div>
                  <div className={styles.statCardValue}>{availableLicenses.toLocaleString()}</div>
                </div>
              </div>
              <div className={styles.statCard}>
                <div className={styles.statCardContent}>
                  <div className={styles.statCardLabel}>Taux d'utilisation</div>
          ...
          <div className={styles.licensesTableContainer}>
            <table className={styles.licensesTable}>
              <thead>
                <tr>
                  <th onClick={() => handleLicenseSort('type')} style={{ cursor: 'pointer' }}>
                    Type de licence {getSortIcon('type')}
                  </th>
                  <th onClick={() => handleLicenseSort('used')} style={{ cursor: 'pointer' }}>
                    Utilisées {getSortIcon('used')}
                  </th>
                  <th onClick={() => handleLicenseSort('total')} style={{ cursor: 'pointer' }}>
                    Total {getSortIcon('total')}
                  </th>
                  <th onClick={() => handleLicenseSort('available')} style={{ cursor: 'pointer' }}>
                    Disponibles {getSortIcon('available')}
                  </th>
                  <th onClick={() => handleLicenseSort('usageRate')} style={{ cursor: 'pointer' }}>
                    Taux d'utilisation {getSortIcon('usageRate')}
                  </th>
                </tr>
              </thead>
              <tbody>
                {filteredAndSortedLicences.map((lic, idx) => {
                    const total = lic.total || 0;
                      ...
                      <tr key={idx}>
                        <td>
                          <div style={{ fontWeight: 500, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            {freeLicense && (
                              <SmartTooltip as="span" content="Licence gratuite">
                                <span style={{ display: 'inline-flex' }}>
                                  <FaGift style={{ fontSize: '0.875rem', color: '#15D1A0', flexShrink: 0 }} />
                                </span>
                              </SmartTooltip>
                            )}
                          </div>
          ...
          {/* Licences expirant bientôt (même logique que dans O365.js) */}
          {(() => {
            ...
            return (
              <div style={{
                ...
                <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem', fontWeight: 600, color: '#ef4444' }}>
                  Licences expirant bientôt
                </h3>
                <ul style={{ margin: 0, paddingLeft: '1.5rem', color: theme === 'dark' ? '#fca5a5' : '#dc2626', fontSize: '0.85rem' }}>
                  {expiringLicenses.map((lic, idx) => (
                    <li key={idx}>
                      {getLicenseDisplayName(lic.nom || lic.displayName)} - Expire le{' '}
                      {new Date(lic.expirationDate).toLocaleDateString('fr-FR')}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className={styles.noDataMessage}>
          <p>Aucune licence disponible. Veuillez synchroniser les données.</p>
        </div>
      )}
    </section>
  );

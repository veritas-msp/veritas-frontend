...
// Icônes des méthodes MFA (même mapping que table utilisateurs / onglet Utilisateurs)
...
function priorityColor(label) {
  ...
  if (label === 'Élevée') return '#ef4444';
  if (label === 'Moyenne') return '#f59e0b';
  if (label === 'Faible') return '#10b981';
  return '#6b7280';
}

/** Affichage des points restants : arrondi si besoin, au plus 2 décimales */
...
export default function SecuriteTab({ securityData, users, mfaDetails = [], clientId, theme }) {
  const [secureRecommendations, setSecureRecommendations] = useState(
    () => (Array.isArray(securityData?.secureScoreRecommendations) ? securityData.secureScoreRecommendations : [])
  );
  const [recsLoading, setRecsLoading] = useState(false);
  const [recsError, setRecsError] = useState(null);
  /** Par défaut : plus de points encore à gagner (remaining) en premier · ordre décroissant */
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/office365/secure-score-recommendations?clientId=${encodeURIComponent(clientId)}`, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) {
          setRecsError(json.error || 'Impossible de charger les recommandations');
        }
      })
      .catch((err) => {
        if (!cancelled) {
          setRecsError(err?.message || 'Erreur réseau');
      ...
      switch (recSortColumn) {
        ...
        case 'recommendation':
          return (
            (a.titleFr || a.title || '')
              .toLowerCase()
              .localeCompare((b.titleFr || b.title || '').toLowerCase(), 'fr') * dir
          );
        case 'category':
          return (
            (a.categoryFr || a.category || '')
              .toLowerCase()
              .localeCompare((b.categoryFr || b.category || '').toLowerCase(), 'fr') * dir
          );
        case 'state':
          return (
            (a.stateLabel || '')
              .toLowerCase()
              .localeCompare((b.stateLabel || '').toLowerCase(), 'fr') * dir
          );
        case 'score': {
  ...
  // Calculs pour l'onglet Sécurité
  ...
  const identityScorePercentage = securityData?.secureScore?.percentage ?? (
    ...
    return (
      <div>
        ...
        <div className={styles.noDataMessage}>
          <p>Aucune donnée de sécurité disponible. Veuillez synchroniser les données.</p>
        </div>
      </div>
    );
  }
  ...
  if (securityData.success === false) {
    return (
      <div>
        <h2 className={styles.sectionTitle}>Sécurité</h2>
        <div className={styles.noDataMessage}>
          <p style={{ color: '#ef4444' }}>❌ Erreur lors du chargement des données de sécurité</p>
    ...
    <div>
      ...
      <div style={{
        ...
        {identityScoreCurrent !== null && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem', marginBottom: '0.5rem' }}>
              ...
              <div style={{ fontSize: '1rem', color: '#6b7280', fontWeight: '500', marginBottom: '0.25rem' }}>
                / {identityScoreMax || 100}
              </div>
            </div>
            {identityScorePercentage !== null ? (
              <>
                <div style={{ fontSize: '0.9rem', fontWeight: '600', color: '#374151', marginTop: '0.5rem' }}>
                  {Math.round(identityScorePercentage)}% des points obtenus
                </div>
                <div style={{ marginTop: '0.75rem', width: '100%', height: '6px', background: '#e5e7eb', borderRadius: '999px' }}>
                  <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#111827', marginBottom: '1rem' }}>
            Total utilisateur
          </div>
          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#111827', marginBottom: '1rem' }}>
            {kpiStats.totalUsers.toLocaleString()}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Avec MFA</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#10b981' }}>{kpiStats.usersWithMFA.toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Sans MFA</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#ef4444' }}>{kpiStats.usersWithoutMFA.toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Taux d'adoption</div>
          ...
          <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid #e5e7eb' }}>
            <div style={{ marginBottom: '0.35rem' }}>Top 3 méthodes préférées</div>
            {kpiStats.top3Total.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                {kpiStats.top3Total.map(({ key, label, count }, i) => (
                  <span key={key} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.75rem', color: '#111827' }}>
                    <Icon icon={getMfaMethodIconName(key)} style={{ fontSize: '1rem', color: '#10b981' }} />
                    {label}
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Avec MFA</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#10b981' }}>{kpiStats.adminsWithMFA.toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Sans MFA</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#ef4444' }}>{kpiStats.adminsWithoutMFA.toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Taux d'adoption</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#111827' }}>
          ...
          <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid #e5e7eb' }}>
            <div style={{ marginBottom: '0.35rem' }}>Top 3 méthodes préférées</div>
            {kpiStats.top3Admin.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                {kpiStats.top3Admin.map(({ key, label, count }, i) => (
                  <span key={key} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.75rem', color: '#111827' }}>
                    <Icon icon={getMfaMethodIconName(key)} style={{ fontSize: '1rem', color: '#10b981' }} />
                    {label}
                    <span style={{ color: '#6b7280', fontWeight: 600 }}>({count})</span>
        ...
        {/* Carte Total utilisateur non admin */}
        <div style={{
          flex: '1',
          minWidth: '280px',
          padding: '1.25rem',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Avec MFA</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#10b981' }}>{kpiStats.nonAdminWithMFA.toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Sans MFA</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#ef4444' }}>{kpiStats.nonAdminWithoutMFA.toLocaleString()}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.5rem' }}>Taux d'adoption</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#111827' }}>
          ...
          <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid #e5e7eb' }}>
            <div style={{ marginBottom: '0.35rem' }}>Top 3 méthodes préférées</div>
            {kpiStats.top3NonAdmin.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                {kpiStats.top3NonAdmin.map(({ key, label, count }, i) => (
                  <span key={key} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.75rem', color: '#111827' }}>
                    <Icon icon={getMfaMethodIconName(key)} style={{ fontSize: '1rem', color: '#10b981' }} />
                    {label}
                    <span style={{ color: '#6b7280', fontWeight: 600 }}>({count})</span>
      ...
      {/* Recommandations Secure Score (Microsoft Graph · endpoint dédié) */}
      <div style={{ marginBottom: '1.5rem' }}>
        <h3 className={styles.subsectionTitle} style={{ marginBottom: '0.75rem' }}>
          Recommandations pour gagner des points (Secure Score)
        </h3>
        {!clientId && (
          <p style={{ fontSize: '0.875rem', color: '#6b7280' }}>
            Client non identifié · impossible de charger les recommandations Graph.
          </p>
        )}
        {clientId && recsLoading && (
          <p style={{ fontSize: '0.875rem', color: '#6b7280' }}>Chargement des recommandations Microsoft…</p>
        )}
        {clientId && recsError && !recsLoading && (
          <p style={{ fontSize: '0.875rem', color: '#b45309' }}>{recsError}</p>
        )}
        {clientId && !recsLoading && sortedSecureRecommendations.length > 0 && (
          <div className={enterpriseStyles.equipmentTableSection}>
            <div className={enterpriseStyles.tableWrapper}>
              <div className={enterpriseStyles.tableScroll}>
                <table className={enterpriseStyles.equipmentTable} style={{ minWidth: '720px' }}>
                  <thead>
                    <tr>
                      <th>
                        <button
                          type="button"
                          className={enterpriseStyles.equipmentTableSortButton}
                          onClick={() => handleRecSort('priority')}
                        >
                          Priorité <RecSortIcon column="priority" />
                        </button>
                      </th>
                      <th>
                        <button
                          type="button"
                          className={enterpriseStyles.equipmentTableSortButton}
                          onClick={() => handleRecSort('recommendation')}
                        >
                          Recommandation <RecSortIcon column="recommendation" />
                        </button>
                      </th>
                      <th>
                        <button
                          type="button"
                          className={enterpriseStyles.equipmentTableSortButton}
                          onClick={() => handleRecSort('category')}
                        >
                          Catégorie <RecSortIcon column="category" />
                        </button>
                      </th>
                      <th>
                        <button
                          type="button"
                          className={enterpriseStyles.equipmentTableSortButton}
                          onClick={() => handleRecSort('state')}
                        >
                          État <RecSortIcon column="state" />
                        </button>
                      </th>
                      <th>
                        <button
                      ...
                      <th>
                        <button
                          type="button"
                          className={enterpriseStyles.equipmentTableSortButton}
                          onClick={() => handleRecSort('remaining')}
                        >
                  ...
                  <tbody>
                    {paginatedSecureRecommendations.map((rec, idx) => {
                      ...
                      return (
                        <tr key={rowKey}>
                          <td>
                            <span style={{ fontWeight: 600, color: priorityColor(rec.priorityLabel), fontSize: '0.8125rem' }}>
                              {rec.priorityLabel || '-'}
                              {typeof rec.rank === 'number' ? (
                                <span className={enterpriseStyles.equipmentTableRank}>#{rec.rank}</span>
                              ) : null}
                            </span>
                          </td>
                          <td className={enterpriseStyles.equipmentTableRecommendationCell}>
                            ...
                            {remediation ? (
                              <div
                                className={enterpriseStyles.equipmentTableRemediation}
                          ...
                          <td style={{ fontWeight: 600 }}>{formatRemainingPoints(remaining)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div className={enterpriseStyles.paginationBar}>
              <div className={enterpriseStyles.paginationLeft}>
                <span className={enterpriseStyles.paginationLabel}>Lignes par page</span>
                <select
                  className={enterpriseStyles.paginationSelect}
                  value={recPageSize}
                  onChange={(e) => setRecPageSize(Number(e.target.value))}
                >
              ...
              <div className={enterpriseStyles.paginationRight}>
                <button
                  type="button"
                  className={enterpriseStyles.paginationButton}
        ...
        {clientId && !recsLoading && !recsError && sortedSecureRecommendations.length === 0 && (
          <p style={{ fontSize: '0.875rem', color: '#6b7280' }}>
            Aucune recommandation retournée par Microsoft pour ce tenant.
          </p>
        )}
      </div>

      {/* Statistiques des méthodes MFA */}
      {(() => {
        // Calculer les statistiques des méthodes MFA depuis les utilisateurs
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{
                ...
                {[
                  ...
                  .filter(([methodType, count]) => count > 0)
                    ...
                    return (
                      ...
                      >
                        <div style={{
                          ...
                          <Icon icon={methodInfo.icon} style={{ fontSize: '1rem', color: '#10b981' }} />
                          {methodInfo.name}
                        </div>
                        <div style={{
                          fontSize: '2rem',
      ...
      {securityData?.defenderSecureScore && (
        <div style={{ 
          marginBottom: '1.5rem',
          padding: '1.25rem',
          borderRadius: '12px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            <div style={{ minWidth: '160px', flex: '1' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#111827', marginBottom: '0.75rem' }}>
                Score de sécurité Microsoft 365 Defender
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem' }}>
                <div style={{ fontSize: '3rem', fontWeight: '700', color: '#10b981', lineHeight: '1' }}>
              ...
              {securityData.defenderSecureScore.percentage !== null && (
                <div style={{ fontSize: '0.85rem', color: '#374151', marginTop: '0.25rem', fontWeight: '600' }}>
            ...
            {/* Scores comparatifs */}
            {securityData.defenderSecureScore.averageComparativeScores && securityData.defenderSecureScore.averageComparativeScores.length > 0 && (
              <div style={{ flex: '1', minWidth: '200px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
                {securityData.defenderSecureScore.averageComparativeScores.map((comparison, idx) => (
                  <div key={idx} style={{ fontSize: '0.8rem', color: '#374151' }}>
                    <div style={{ fontWeight: '600', marginBottom: '0.25rem' }}>
                      {comparison.basis === 'AllTenants' ? 'Toutes les organisations' : 
                       comparison.basis === 'TotalSeats' ? 'Organisations similaires' : 
                       comparison.basis || 'Comparaison'}
                    </div>
                    <div style={{ fontSize: '1.25rem', fontWeight: '700', color: comparison.averageScore > (securityData.defenderSecureScore.currentScore || 0) ? '#ef4444' : '#10b981' }}>
          ...
          {/* Historique du score */}
          {securityData?.secureScoreHistory && securityData.secureScoreHistory.length > 0 && (
            <div style={{ marginTop: '1rem' }}>
              <div style={{ fontSize: '0.8rem', fontWeight: '600', marginBottom: '0.5rem', color: '#374151' }}>
                Historique du score (30 derniers jours)
              </div>
              <div style={{ 
                height: '100px', 
                display: 'flex', 
                ...
                {securityData.secureScoreHistory.map((entry, idx) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${new Date(entry.date).toLocaleDateString('fr-FR')}: ${entry.percentage}% (${entry.score}/${entry.maxScore})`}
                    />
                  );
                })}
              </div>

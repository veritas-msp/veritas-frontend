import {
  ...
  getSslHostLabel,
  SSL_STATUS_ORDER,
} from "../EnterprisesPage/sslCertificateUtils";

export function buildSslStatusLabels(statusMeta = {}) {
  return {
    error: statusMeta.error || "Erreur",
    unknown: statusMeta.unknown || "Non vérifié",
    unknownInvalid: statusMeta.unknownInvalid || "Inconnu",
    expired: statusMeta.expired || "Expiré",
    warning: statusMeta.warning || "Expire bientôt",
    active: statusMeta.active || "Valide",
  };
}
...
export function buildSslFleetStats(rows = []) {
  ...
  const statusCounts = { active: 0, warning: 0, expired: 0, error: 0, unknown: 0 };

  list.forEach((row) => {
    if (row.clientId != null) clientIds.add(row.clientId);
    if (statusCounts[row.statusKey] != null) statusCounts[row.statusKey] += 1;
...
export function filterSslFleetRows(rows, { search = "", statusFilter = "all" } = {}) {
  ...
  if (query) {
    filtered = filtered.filter(
      (row) =>
        ...
        row.hostLabel?.toLowerCase().includes(query) ||

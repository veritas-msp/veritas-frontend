...
export default function MicrosoftTenantMspDashboard({
    ...
    if (query) {
      rows = rows.filter(
        (row) =>
          ...
          (row.tenantId || "").toString().toLowerCase().includes(query)
      );
    }
    if (statusFilter !== "all") {
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      switch (sortBy) {
        case "clientName": {
          const cmp = getClientNameForSort(a.clientName).localeCompare(
            getClientNameForSort(b.clientName)
          );
          return cmp * dir;
        }
        case "status": {
          const order = { actif: 0, inactif: 1 };
          return ((order[a.status] ?? 2) - (order[b.status] ?? 2)) * dir;
        }
        case "tenantId":
          return (a.tenantId || "")
            .toString()
            .localeCompare((b.tenantId || "").toString(), undefined, { sensitivity: "base" }) * dir;
        case "userCount":
    ...
    <div className={styles.dashboard}>
      <section className={styles.hero}>
        ...
        <div className={styles.kpiRow}>
          ...
          <KpiCard
            icon="mdi:shield-off"
            label={copy.kpi.inactive}
            value={formatNumber(stats.inactive)}
            tone="bad"
            active={statusFilter === "inactif"}
            onClick={() => setStatusFilter(statusFilter === "inactif" ? "all" : "inactif")}
          />
          <KpiCard
          ...
          <div className={styles.priorityList}>
            {priorityRows.map((item) => {
              ...
              return (
                ...
                >
                  <span className={styles.priorityDot} style={{ background: dotColor }} />
      ...
      {loading ? (
        ...
        <MspEmptyState
          icon={tenants.length === 0 ? "mdi:microsoft-azure" : "mdi:magnify"}
          title={tenants.length === 0 ? copy.emptyTitleNone : copy.emptyTitleNoMatch}
          text={tenants.length === 0 ? copy.emptyTextNone : copy.emptyTextNoMatch}
        />
      ) : (
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <SortableHeader
                    column="mfaAdminPct"
                    label={copy.table.mfaAdmins}
                    sortBy={sortBy}
                    sortDirection={sortDirection}
                    onSort={handleSort}
                  />
                  <SortableHeader
                    column="mfaNonAdminPct"
                    label={copy.table.mfaUsers}
                    sortBy={sortBy}
                    sortDirection={sortDirection}
                    onSort={handleSort}
                  />
                    ...
                    <tr
                      ...
                      onMouseDown={(e) => {
                        if (e.button === 1) {
                          e.preventDefault();
                          e.stopPropagation();
                          onOpenTenant?.(row, { background: true });
                        }
                      }}
                    ...
                    >
                      ...
                      <td>
                        {row.clientId && onOpenClient ? (
                          ...
                          <span className={styles.cellName}>{formatClientDisplay(row.clientName)}</span>
                        )}
                      </td>
                      <td>
                        <span
                          className={`${styles.chip} ${row.status === "actif" ? styles.chip_good : styles.chip_bad}`}
                        >
                      ...
                      <td className={styles.cellMuted}>
                        {row.lastSync
                          ? new Date(row.lastSync).toLocaleString(bcp47)
                          : "-"}
                      </td>
                    </tr>
                  );

...
function StatusChip({ statusKey, copy }) {
...
function FleetTableRow({ row, onOpenClient, copy, bcp47 }) {
  ...
  const dotColor =
    meta.tone === "bad" ? "#dc2626" : meta.tone === "warn" ? "#d97706" : "#2b5fab";
        ...
        <div className={styles.solutionCell}>
          ...
          <div className={styles.solutionText}>
            ...
            {row.issuer && row.issuer !== "-" ? (
              <span className={styles.cellSub} title={row.issuer}>
                {row.issuer}
              </span>
            ) : null}
          </div>
...
export default function SslMspDashboard({
  ...
  const filteredRows = useMemo(
    () => filterSslFleetRows(fleetRows, { search, statusFilter }),
    [fleetRows, search, statusFilter]
  );

  const sortedRows = useMemo(
    () => sortSslFleetRows(filteredRows, sortBy, sortDirection),
  ...
  const priorityRows = useMemo(
    () =>
      fleetRows
        .filter((row) => isSslFleetIssue(row.statusKey))
        .sort((a, b) => {
          const order = { error: 0, expired: 1, warning: 2 };
  ...
  return (
    <div className={styles.dashboard}>
      <section className={styles.hero}>
        ...
        <div className={styles.kpiRow}>
          ...
          <KpiCard
            ...
            active={statusFilter === "warning" || statusFilter === "expired" || statusFilter === "error"}
              ...
              return (
                ...
                >
                  ...
                  <span className={styles.priorityVerb}>
                    {row.statusKey === "expired" || row.statusKey === "error"
                      ? copy.priorityVerbRenew
                      : copy.priorityVerbAnticipate}
                  </span>
                </button>
              );
            })}
      ...
      {loading ? (
        ...
        <MspEmptyState
          icon={fleetRows.length === 0 ? "mdi:certificate-outline" : "mdi:magnify"}

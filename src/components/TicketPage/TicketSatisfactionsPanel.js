...
import {
  SATISFACTION_SENTIMENT_FILTERS,
  formatSatisfactionDate,
  getSatisfactionSentiment,
} from "../../utils/ticketSatisfactionUi";

function SatisfactionStars({ rating }) {
  const safeRating = Math.max(0, Math.min(5, Number(rating) || 0));
  return (
    <span className={styles.satisfactionStarsInline} aria-label={`${safeRating} sur 5`}>
      {[1, 2, 3, 4, 5].map((star) => (
        <Icon
          key={star}
          icon={star <= safeRating ? "mdi:star" : "mdi:star-outline"}
          className={star <= safeRating ? styles.satisfactionStarActive : styles.satisfactionStarMuted}
          aria-hidden
        />
      ))}
    </span>
...
function SentimentBadge({ averageRating }) {
  const sentiment = getSatisfactionSentiment(averageRating);
  return (
    <span className={`${styles.satisfactionSentimentBadge} ${styles[`satisfactionSentiment_${sentiment.tone}`]}`}>
      {sentiment.label}
    </span>
  );
}
...
export default function TicketSatisfactionsPanel({
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== searchRef.current) return;
      toast.error(error.message || "Impossible de charger les retours clients.");
      setItems([]);
      setTotalCount(0);
    } finally {
    ...
    <>
      <div className={`${layout.toolbar} ${styles.toolbarGrow}`}>
        ...
        <div className={`${layout.searchWrap} ${styles.searchWrapFull}`}>
          ...
          <input
            ...
            placeholder="Ticket, client, commentaire…"
            aria-label="Rechercher un retour client"
          />
          {search ? (
            <button
              type="button"
        ...
        >
          {SATISFACTION_SENTIMENT_FILTERS.map((item) => (
      ...
      {loading ? (
        <div className={layout.stateBox}>
          ...
          <span>Chargement des retours clients…</span>
        </div>
      ) : totalCount === 0 ? (
        <div className={layout.emptyState}>
          <Icon icon="mdi:star-outline" className={layout.emptyStateIcon} aria-hidden />
          <p className={layout.emptyStateTitle}>Aucun retour client</p>
          <p className={layout.emptyStateHint}>
            {scope === "mine"
              ? "Les évaluations laissées sur vos tickets apparaîtront ici."
                ...
                <tbody>
                  {items.map((row) => {
                    ...
                    return (
                      <tr
                        key={row.id}
                        className={styles.satisfactionRow}
                        onClick={() =>
                      ...
                      >
                        ...
                        <td>
                          <div className={styles.satisfactionCriteriaCompact}>
                            {satisfactionCriteria.map(({ key, label }) => (
          ...
          <div className={layout.pagination}>
            ...
            <div className={layout.paginationRight}>
              ...
              <span className={layout.paginationInfo}>
                Page {currentPage} / {totalPages} · {totalCount} retour{totalCount > 1 ? "s" : ""}
              </span>
              <button
                type="button"
                className={layout.pageBtn}

...
import {
  searchTickets,
  fetchAllMatchingTickets,
  restoreTicket,
  permanentlyDeleteTicket,
  fetchTicketViews,
  fetchTicketViewCounts,
  fetchTicketSatisfactionCounts,
  bulkUpdateTickets,
  updateTicketView,
} from "../../api/tickets";
...
import {
  getBuiltinTicketViews,
  DEFAULT_TICKET_VIEW_ID,
  canUserEditTicketView,
...
import {
  TICKET_SATISFACTION_VIEWS,
  TICKET_SATISFACTION_VIEW_IDS,
  isTicketSatisfactionViewId,
  resolveTicketSatisfactionScope,
} from "../../utils/ticketSatisfactionViewConstants";
...
function writeViewsPaneCollapsed(collapsed) {
  try {
    if (collapsed) localStorage.setItem(VIEWS_PANE_COLLAPSED_KEY, "1");
    else localStorage.removeItem(VIEWS_PANE_COLLAPSED_KEY);
  } catch {
    /* ignore */
  }
}
...
function SortableViewRow({
  ...
  return (
    <div
      ...
      style={style}
      className={`${styles.viewItem} ${isActive ? styles.viewItemActive : ""} ${
        isDragging ? styles.viewItemDragging : ""
      } ${reorderMode ? styles.viewItemReorderMode : ""}`}
      onClick={reorderMode ? undefined : () => onSelect(view)}
      onKeyDown={
        reorderMode
          ? undefined
          : (e) => {
      ...
      aria-pressed={reorderMode ? undefined : isActive}
    >
      {canDrag ? (
        <button
          type="button"
          className={styles.viewItemDragHandle}
          aria-label={copy.formatDragViewAria(view.name)}
      ...
      {editable && !reorderMode ? (
        <button
          type="button"
          className={styles.viewItemEditBtn}
          title={copy.views.editView}
          aria-label={copy.formatEditViewAria(view.name)}
          onClick={(event) => onEdit(view, event)}
        >
...
function SortableViewsSection({ sectionKey, views, reorderMode, sensors, onDragEnd, children }) {
...
function buildTicketSubjectTooltip(ticket, labels, copy) {
  ...
  const meta = [
    ...
    labels.isMajor ? copy.tooltip.majorIncident : null,
    labels.category,
    labels.clientLabel,
    labels.requesterLabel,
    labels.commentsCount != null ? copy.formatCommentCount(labels.commentsCount) : null,
  ].filter(Boolean);

  return (
    <div className={styles.subjectTooltip}>
...
export default function TicketPage({ onNavigate }) {
  const { user, userRole } = useAuthContext();
  const { isCommunity } = useVeritasEdition();
  const { formatDate: formatAppDate, formatDateTime: formatAppDateTime } = useAppFormatters();
  ...
  const [satisfactionCounts, setSatisfactionCounts] = useState({ mine: 0, all: 0 });
  ...
  const visibleSatisfactionViews = useMemo(
    () => TICKET_SATISFACTION_VIEWS.filter((view) => !view.adminOnly || isAdmin),
    [isAdmin]
  );
  ...
  const customTicketViews = useMemo(
    () => ticketViews.filter((view) => !view.isBuiltin),
    [ticketViews]
  );

  const activeView = useMemo(() => {
    const satisfactionView = TICKET_SATISFACTION_VIEWS.find(
    ...
    if (String(activeViewId) === "__all__") {
      return builtinTicketViews.find((view) => view.id === DEFAULT_TICKET_VIEW_ID)
        || builtinTicketViews[builtinTicketViews.length - 1];
    }
    const found = ticketViews.find((view) => String(view.id) === String(activeViewId));
    return found
      || builtinTicketViews.find((view) => view.id === DEFAULT_TICKET_VIEW_ID)
      || builtinTicketViews[builtinTicketViews.length - 1];
  ...
  const loadSatisfactionCounts = useCallback(async () => {
    try {
      ...
      setSatisfactionCounts({ mine: 0, all: 0 });
    }
  }, []);
  ...
  const loadViewCounts = useCallback(async () => {
    try {
      const result = await fetchTicketViewCounts({ scope: "views" });
  ...
  const loadReferenceData = useCallback(async () => {
    try {
      const [userRows, contactRows, clientRows] = await Promise.all([
        fetchUsers().catch(() => []),
  ...
  const loadTicketsPage = useCallback(async () => {
    ...
    try {
      const includeViewRules =
        viewMode !== "trash" || activeView?.rules?.viewMode === "trash";

      const result = await searchTickets({
        ...(includeViewRules && activeView?.rules ? { viewRules: activeView.rules } : {}),
        viewMode,
        ticketType,
  ...
  const hasActiveSubFilters = Boolean(
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      ...
      if (result.failureCount > 0) {
        toast.warn(
          interpolate(pageCopy.toasts.bulkRestorePartial, {
  ...
  const handleBulkPurge = () => {
    ...
    setConfirmModal({
      title: interpolate(pageCopy.bulk.purgeTitle, { label }),
      message: pageCopy.bulk.purgeMessage,
      confirmLabel: pageCopy.bulk.confirmPurge,
      icon: "mdi:delete-forever-outline",
      variant: "danger",
      onConfirm: async () => {
        ...
        try {
          ...
          if (result.failureCount > 0) {
            toast.warn(
              interpolate(pageCopy.toasts.bulkPurgePartial, {
                success: String(result.successCount),
                failure: String(result.failureCount),
              })
            );
  ...
  const handleExportCsv = async () => {
    ...
    setExporting(true);
    try {
      const { items, truncated } = await fetchAllMatchingTickets({
        ...(viewMode !== "trash" || activeView?.rules?.viewMode === "trash"
          ? activeView?.rules
  ...
  const resolveRequesterContactId = (ticket) =>
    ticket?.requester_contact_id || ticket?.requesterContactId || "";

  const resolveRequesterFallback = (ticket) =>
    ticket?.requester_name ||
    ticket?.requester_full_name ||
    ticket?.requester_contact_name ||
    ticket?.requester_email ||
    ticket?.requester_contact_email ||
    "-";
      ...
      const labels = assignees
        .map((a) => {
          ...
          return resolveUserLabel(
            a.user_id || a.userId || a.id || a.value,
            a.name || a.nom || a.email || ""
          );
        })
  ...
  const resolveFollowersLabel = (ticket) => {
    ...
    const labels = followers
      .map((w) => {
        ...
        return resolveUserLabel(
          w.user_id || w.userId || w.id || w.value,
          w.name || w.nom || w.email || ""
        );
      })
      .filter((label) => label && label !== "-");
  ...
  const getPriorityVisual = (priority, ticket) =>
    pageCopy.getPriorityVisual(priority, isMajorIncidentTicket(ticket));
  ...
  const openTicket = (ticket, background = false) => {
    ...
    onNavigate(
      "TicketDetail",
      { ticketId: ticket.id, ticketNumber: ticket.ticket_number, title: ticket.title },
  ...
  const openEditViewModal = (view, event) => {
    ...
    if (!canUserEditTicketView(view, { userId: user?.id, isAdmin })) return;
    setEditingView(view);
    setViewModalOpen(true);
  };

  const viewReorderSensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 6 } }),
    useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates })
  );

  const hasReorderableViews = useMemo(
    () => customTicketViews.some((view) => canUserEditTicketView(view, { userId: user?.id, isAdmin })),
  ...
  const persistCustomViewsOrder = useCallback(async (nextOrder) => {
    ...
    const updates = orderedIds
      .map((id, index) => {
        ...
        return { id: view.id, displayOrder: nextDisplayOrder };
      })
      .filter(Boolean);
    ...
    await Promise.all(
      updates.map((entry) => updateTicketView(entry.id, { displayOrder: entry.displayOrder }))
    );

    setTicketViews((prev) =>
      prev.map((view) => {
        const update = updates.find((entry) => String(entry.id) === String(view.id));
        return update ? { ...view, displayOrder: update.displayOrder } : view;
      })
    );
  }, [ticketViews]);
  ...
  const handleViewsSectionDragEnd = useCallback(
    async (sectionKey, event) => {
  ...
  const renderViewItem = (view, { reorderMode = false } = {}) => {
    const isActive = String(activeViewId) === String(view.id);
    const count = getViewTicketCount(view);
    const editable = canUserEditTicketView(view, { userId: user?.id, isAdmin });
      ...
      >
        ...
        {editable ? (
          <button
            type="button"
            className={styles.viewItemEditBtn}
            title={pageCopy.views.editView}
            aria-label={pageCopy.formatEditViewAria(view.name)}
            onClick={(event) => openEditViewModal(view, event)}
          >
            <Icon icon="mdi:pencil-outline" aria-hidden />
          </button>
        ) : null}
  ...
  const renderCustomViewsSection = (sectionKey, label, views) => {
    ...
    const orderedViews =
      viewsReorderMode && customViewsOrder
        ? orderViewsByIds(views, customViewsOrder[sectionKey])
        : views;
    ...
    return (
      <>
        ...
        <SortableViewsSection
          ...
          sensors={viewReorderSensors}
          onDragEnd={handleViewsSectionDragEnd}
        >
          {orderedViews.map((view) => renderViewItem(view, { reorderMode: viewsReorderMode }))}
        </SortableViewsSection>
      </>
    );
  };
  ...
  const publicViews = useMemo(
    () => customTicketViews.filter((view) => view.visibility === "public"),
    [customTicketViews]
  );
  const assignedViews = useMemo(
    () => customTicketViews.filter(
      (view) => view.visibility === "assigned" || view.visibility === "profile"
    ),
    [customTicketViews]
  );
  const privateViews = useMemo(
    () => customTicketViews.filter((view) => view.visibility === "private"),
    [customTicketViews]
  );
  ...
  const handleBulkDelete = () => {
    ...
    const label = pageCopy.formatBulkTicketLabel(selectedCount);
    setConfirmModal({
      title: interpolate(pageCopy.bulk.deleteTitle, { label }),
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (result.failureCount > 0) {
            toast.warn(
              interpolate(pageCopy.toasts.bulkDeletePartial, {
                success: String(result.successCount),
                failure: String(result.failureCount),
              })
            );
  ...
  const handleBulkActionSuccess = async (result) => {
    if (result?.failureCount > 0) {
      toast.warn(
        interpolate(pageCopy.toasts.bulkUpdatePartial, {
          success: String(result.successCount),
          failure: String(result.failureCount),
        })
      );
  ...
  const handleRandomTicketClick = async (event) => {
    ...
    try {
      setPlayMode(true);
      await navigateToRandomTicket(onNavigate, { enablePlayMode: true });
    } catch {
  ...
  const pageSubtitle = isSatisfactionView
    ? pageCopy.formatSatisfactionCount(getViewTicketCount(activeView))
    : loading
      ? pageCopy.loadingTickets
      : viewMode === "trash"
        ? pageCopy.formatTrashSubtitle(totalCount)
  ...
  return (
    <div className={`${mspStyles.mspPage} ${layout.page} msp-page-grid`}>
      <div className={mspStyles.mspLayout}>
        <div className={mspStyles.mspMain}>
          <MspPageHero
            ...
            actions={
              <>
                <label
                  className={`${styles.trashSwitch} ${viewMode === "trash" ? styles.trashSwitchOn : ""}`}
                  title={pageCopy.trashSwitchTitle}
                >
                  ...
                  <input
                    type="checkbox"
                    className={styles.trashSwitchInput}
                    checked={viewMode === "trash"}
                    onChange={(e) => {
                ...
                <SmartTooltip content={pageCopy.playModeTooltip}>
                  <button
                    type="button"
                    className={`${playModeStyles.diceBtn} ${playMode ? playModeStyles.diceBtnActive : ""}`}
                    onClick={handleRandomTicketClick}
                    disabled={loadingRandom || viewMode === "trash"}
                    aria-label={pageCopy.playModeAria}
                ...
                <button
                  ...
                  onClick={() => onNavigate?.("TicketCreate")}
                  disabled={viewMode === "trash"}
                  aria-label={pageCopy.newTicketAria}
                >
                  <FaPlus />
                </button>
              </>
        ...
        <div
          className={`${styles.viewsLayout} ${viewMode === "trash" ? styles.viewsLayoutTrash : ""} ${viewsPaneCollapsed ? styles.viewsLayoutCollapsed : ""}`.trim()}
        >
          {viewMode !== "trash" && !viewsPaneCollapsed && (
          <aside className={styles.viewsPane} aria-label={pageCopy.views.paneAria}>
            <div className={styles.viewsPaneHeader}>
              ...
              <div className={styles.viewsHeaderActions}>
                <SmartTooltip content={pageCopy.views.collapsePane}>
                  <button
                    type="button"
                    className={styles.viewsCollapseBtn}
                    onClick={() => setViewsPaneCollapsedState(true)}
                    title={pageCopy.views.collapsePane}
                    aria-label={pageCopy.views.collapsePane}
                    disabled={viewsReorderMode}
                  >
                ...
                {hasReorderableViews ? (
                  <button
                    type="button"
                    className={`${styles.viewsEditBtn} ${viewsReorderMode ? styles.viewsEditBtnActive : ""}`}
                    onClick={toggleViewsReorderMode}
                    title={viewsReorderMode ? pageCopy.views.reorderFinish : pageCopy.views.reorderStart}
                    aria-label={viewsReorderMode ? pageCopy.views.reorderFinish : pageCopy.views.reorderStart}
                    aria-pressed={viewsReorderMode}
                ...
                <button
                  type="button"
                  className={styles.viewsAddBtn}
                  onClick={openCreateViewModal}
                  title={pageCopy.views.create}
                  aria-label={pageCopy.views.create}
                  disabled={viewsReorderMode}
                >
                  <FaPlus />
                </button>
            ...
            {viewsReorderMode ? (
              <p className={styles.viewsReorderHint}>
            ...
            <div className={styles.viewsList}>
              {viewsLoading ? (
                ...
                <>
                  ...
                  {customTicketViews.length === 0 && !viewsReorderMode && !visibleSatisfactionViews.length ? (
                    <div className={styles.viewsEmpty}>
          ...
          <div className={styles.mainColumn}>
            {isSatisfactionView ? (
              <TicketSatisfactionsPanel
                scope={satisfactionScope}
                onNavigate={onNavigate}
                leadingToolbarContent={
                  viewMode !== "trash" && viewsPaneCollapsed ? (
                    <SmartTooltip content={pageCopy.views.expandPane}>
                      <button
                        type="button"
                        className={styles.viewsExpandBtn}
                        onClick={() => setViewsPaneCollapsedState(false)}
                        aria-label={pageCopy.views.expandPane}
                      >
            ...
            <div className={`${layout.toolbar} ${styles.toolbarGrow}`}>
              ...
              <div className={`${layout.searchWrap} ${styles.searchWrapFull}`}>
                <Icon
                  icon={isSearchPending || refreshing ? "mdi:loading" : "mdi:magnify"}
                  className={`${layout.searchIcon} ${isSearchPending || refreshing ? layout.spinning : ""}`.trim()}
                  aria-hidden
                />
                <input
                  type="text"
                  ...
                  placeholder={pageCopy.search.placeholder}
                  aria-label={pageCopy.search.aria}
                />
                {search && !isSearchPending && !refreshing ? (
                  <button type="button" className={layout.clearButton} onClick={() => setSearch("")} aria-label={pageCopy.search.clear}>
                    <FaTimes />
                  </button>
                ) : null}
              </div>
              ...
              <select
                ...
                aria-label={pageCopy.search.filterType}
              >
                {pageCopy.typeFilterOptions.map((opt) => (
          ...
          <div className={layout.emptyState}>
            ...
            {viewMode !== "trash" && (
              <button type="button" className={layout.primaryBtn} onClick={() => onNavigate?.("TicketCreate")}>
                <Icon icon="mdi:plus" />
                {pageCopy.newTicket}
              </button>
            )}
          ...
          <>
            ...
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort("type")}>{pageCopy.table.type}{getSortIndicator("type")}</th>
                ...
                {!isCommunity && (
                  <th onClick={() => handleSort("sla")}>{pageCopy.table.sla}{getSortIndicator("sla")}</th>
                )}
            ...
            <tbody>
              {tickets.length === 0 && refreshing ? (
                <tr><td colSpan={viewMode === "trash" ? (isCommunity ? 14 : 15) : (isCommunity ? 13 : 14)} className={styles.empty}>{pageCopy.table.loading}</td></tr>
              ) : tickets.length === 0 ? (
                <tr><td colSpan={viewMode === "trash" ? (isCommunity ? 14 : 15) : (isCommunity ? 13 : 14)} className={styles.empty}>{pageCopy.table.noRows}</td></tr>
              ) : (
                paginatedTickets.map((t) => {
                  ...
                  return (
                    <tr
                      key={t.id}
                      className={`${majorIncident ? styles.majorIncidentRow : ""} ${isSelected ? styles.selectedRow : ""}`.trim() || undefined}
                    ...
                    >
                      <td className={styles.checkboxCell} onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          className={styles.rowCheckbox}
                          checked={isSelected}
                          onChange={(e) => toggleTicketSelection(t.id, e.target.checked)}
                          aria-label={pageCopy.formatSelectTicketAria(t.ticket_number || t.id)}
                        />
                      </td>
                      <td>
                        <span className={styles.ticketIdCell}>
                          {majorIncident ? (
                            <Icon icon="mdi:alert-octagon" className={styles.majorIncidentIcon} aria-hidden />
                          ) : null}
                          #{t.ticket_number || "-"}
                        </span>
                      </td>
                      <td>
                        <SmartTooltip
                          as="span"
                          className={styles.subjectCellWrap}
                          content={buildTicketSubjectTooltip(t, {
                            ...
                            requesterLabel: resolveContactLabel(
                              resolveRequesterContactId(t),
                              resolveRequesterFallback(t)
                            ),
                        ...
                        >
                          <span className={styles.subjectCell}>
                            {majorIncident ? (
                              <span className={styles.majorIncidentBadge}>
                      ...
                      <td>
                        {resolveRequesterContactId(t) ? (
                          ...
                          <span>{resolveContactLabel(resolveRequesterContactId(t), resolveRequesterFallback(t))}</span>
                        )}
                      </td>
                      <td>
                        {resolveClientId(t) ? (
                          ...
                          <span>{resolveClientLabel(t)}</span>
                        )}
                      </td>
                      <td>{resolveAssigneesLabel(t)}</td>
                      <td>{resolveFollowersLabel(t)}</td>
                      <td><span className={styles.statusBadge}>{pageCopy.getStatusBadge(ticketStatus) || t.status}</span></td>
                      <td>
                        <span
                          className={styles.priorityIndicator}
                          title={getPriorityVisual(t.priority, t).label}
                        >
                      ...
                      <td>
                        {(() => {
                          ...
                          return (
                            <span
                              className={`${styles.slaBadge} ${styles[`slaBadge_${sla.tone}`] || styles.slaBadge_neutral}`}
                              title={
                                sla.phase === "first_response"
                                  ? pageCopy.table.slaFirstResponse
                                  : sla.phase === "resolution"
                                    ? pageCopy.table.slaResolution
                                    : pageCopy.table.sla
                              }
                            >
                        ...
                        <td>
                          <div className={styles.actionRow}>
                            <button
                              ...
                              title={pageCopy.bulk.restore}
                              onClick={(e) => { e.stopPropagation(); handleRestoreTicket(t.id); }}
                            >
                              <Icon icon="mdi:restore" />
                            ...
                            {isAdmin && (
                              <button
                                ...
                                title={pageCopy.table.purgeTitle}
                                onClick={(e) => { e.stopPropagation(); handlePermanentDelete(t.id); }}
                              >
                                <Icon icon="mdi:delete-forever-outline" />
            ...
            <div className={layout.pagination}>
              ...
              <div className={layout.paginationRight}>
                <button
                  ...
                  aria-label={commonCopy.prevPage}
                >
                  <FaChevronLeft />
                </button>
                ...
                <button
                  ...
                  aria-label={commonCopy.nextPage}
                >
                  <FaChevronRight />
                </button>
              </div>

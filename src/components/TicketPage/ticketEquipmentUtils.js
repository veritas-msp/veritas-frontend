...
const STORAGE_TYPE_KEYS = new Set(["NAS", "SAN", "Stockage"]);
...
export function resolveEquipmentModuleKey(type) {
  ...
  if (!key) return "";
  if (STORAGE_TYPE_KEYS.has(key)) return "Stockage";
...
export function getEquipmentPickerLabel(
  equipment,
  { serialPrefix = "SN: ", locale, separator = " · " } = {}
) {
  if (!equipment) return "";
  ...
  const base =
    [typeLabel, name].filter(Boolean).join(separator) ||
    `${getHardwareFallbackLabel(locale)} #${equipment.id || equipment.equipment_id}`;
...
export function formatLinkedEquipmentEventLabel(event, { locale, separator = " · " } = {}) {
  if (!event) return "";
...
export function buildLinkedEquipmentComment(equipment, clientId) {
  ...
  const licensesText = Array.isArray(equipment?.licenses)
    ? equipment.licenses.join(", ")
    : String(equipment?.licenses || "");

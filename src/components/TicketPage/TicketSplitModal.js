...
export default function TicketSplitModal({
  ...
  const filteredTargets = useMemo(() => {
    ...
    return base
      .filter((row) => getTicketLabel(row).toLowerCase().includes(query))
      .slice(0, 40);
  }, [targets, search]);

  const selectedTarget = useMemo(
    () => (Array.isArray(targets) ? targets : []).find((row) => String(row.id) === String(targetId)) || null,
    [targets, targetId]
  );
  ...
  return createPortal(
    <div className={layout.overlay} onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="ticket-split-modal-title"
      >
          ...
          <div className={layout.headerMain}>
            ...
            <div className={layout.headerText}>
              ...
              <p className={layout.subtitle}>
                Clôture ce ticket et transfère le fil vers un ticket existant.
              </p>
            </div>
          </div>
          <button
        ...
        <div className={layout.content}>
          ...
          <div className={layout.field}>
            ...
            <div
              className={`${styles.pickerWrap} ${dropdownOpen ? styles.pickerWrapOpen : ""} ${fs.contactPicker}`}
              ref={pickerRef}
            >
              ...
              {dropdownOpen && (
                <div className={fs.contactDropdown} role="listbox">
                  {filteredTargets.length === 0 ? (
                    <div className={fs.contactEmpty}>Aucun ticket trouvé</div>
                  ) : (
                    filteredTargets.map((row, idx) => (
                      <button
                        ...
                        type="button"
                        role="option"
                        className={`${fs.contactOption} ${highlight === idx ? fs.contactOptionActive : ""}`}
          ...
          {selectedTarget ? (
            <div className={styles.selectedTargetCard}>
          ...
          <div className={styles.warningCallout}>
            ...
            <span>
              Le ticket courant sera <strong>clôturé</strong>. Un commentaire interne avec lien sera ajouté
        ...
        <footer className={layout.footer}>
          <span className={layout.footerHint}>Action irréversible sur le statut</span>

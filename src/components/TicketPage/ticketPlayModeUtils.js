...
export function setTicketPlayModeEnabled(enabled) {
  try {
    if (enabled) sessionStorage.setItem(PLAY_MODE_STORAGE_KEY, "1");
    else sessionStorage.removeItem(PLAY_MODE_STORAGE_KEY);
  } catch {
    /* ignore */
  }
}

export async function navigateToRandomTicket(
  onNavigate,
  { excludeTicketId, enablePlayMode = false, showToast = true } = {}
) {
  if (enablePlayMode) setTicketPlayModeEnabled(true);

  try {
    const filters = excludeTicketId ? { excludeId: excludeTicketId } : {};
    ...
    if (showToast) {
      toast.success(
        enablePlayMode
          ? `Mode aléatoire · ticket #${ticket.ticket_number || ticket.id}`
    ...
    if (showToast) {
      toast.error(error.message || "Aucun ticket à traiter");
    }
    throw error;
  }
}
...
export async function advanceToNextRandomTicket(onNavigate, excludeTicketId) {
  ...
  try {
    ...
    toast.info("Plus de tickets à traiter · mode aléatoire terminé");
    return null;
  }
}

...
function TicketCreateTipsTooltip({ copy }) {
  return (
    <div className={s.tipsTooltip}>
      <p className={s.tipsTooltipTitle}>{copy.tipsTitle}</p>
      <ul className={s.tipList}>
        {copy.tips.map((tip) => (
          <li key={tip.icon} className={s.tipItem}>
...
function SectionPanel({ title, description, children, className, headerExtra }) {
  return (
    <section className={`${account.sectionPanel} ${className || ""}`.trim()}>
      {(title || description || headerExtra) && (
        <header
          className={`${account.sectionHeader} ${headerExtra ? s.sectionHeaderInline : ""}`.trim()}
        >
...
function buildAvailabilityContactSlots({ mode, date, startTime, endTime, note }) {
  if (mode === "none") return [];
  ...
  if (mode === "from") {
    ...
    return [{ date: slotDate, startTime: start, endTime: "", note: slotNote, mode: "from" }];
  }
  if (!slotDate && !start && !end && !slotNote) return [];
  return [{ date: slotDate, startTime: start, endTime: end, note: slotNote, mode: "range" }];
}

function serializeContactSlots(slots) {
  return slots
    .map(({ date, startTime, endTime, note }) => ({
...
function RequesterOpenTicketsStat({ tickets, openCount, onNavigate, copy }) {
  ...
  const openTickets = useMemo(
    () =>
      (Array.isArray(tickets) ? tickets : [])
        .filter((ticket) => isTicketOpenStatus(ticket?.status))
        .sort(
          (a, b) =>
  ...
  const popover =
    open && coords && hasPreview
      ? createPortal(
          <div
            className={s.requesterOpenTicketsPopover}
            style={{
              position: "fixed",
              top: coords.top,
              left: coords.left,
              ...
              {openTickets.map((ticket) => {
                ...
                return (
                  <li key={ticket.id}>
                    <button
                      type="button"
                      className={s.requesterOpenTicketItem}
                      onClick={() =>
                    ...
                    >
                      <span
                        className={`${s.requesterOpenTicketStatus} ${
                          s[`requesterOpenTicketStatus_${statusKey}`] || ""
                        }`}
                      >
  ...
  return (
    <>
      <div
        ref={anchorRef}
        className={`${s.requesterTicketStat} ${openCount > 0 ? s.requesterTicketStatWarn : ""} ${
          hasPreview ? s.requesterTicketStatHoverable : ""
        }`}
        ...
        aria-label={hasPreview ? copy.formatViewOpenTicketsAria(openCount) : undefined}
      >
...
function TicketCreateRecap({
  ...
  const optionalRows = [
    { label: copy.recapEquipment, value: equipmentSummary },
    { label: copy.recapLinkedTicket, value: linkedTicketSummary },
    { label: copy.recapPreAssign, value: preAssigneesSummary },
    { label: copy.followers, value: preFollowersSummary },
    { label: copy.recapDocuments, value: attachmentsSummary },
  ].filter((row) => !isEmptyValue(row.value));

  return (
    <div className={s.recapBody}>
      <div className={s.recapHero}>
        ...
        {description.trim() ? (
          <p className={s.recapDescText}>{description.trim()}</p>
        ) : null}
      </div>
      ...
      {filledContactSlots.length > 0 ? (
        <ul className={s.recapAvailList}>
          {filledContactSlots.map((slot, index) => (
            <li key={`${slot.date}-${slot.startTime}-${index}`} className={s.recapAvailItem}>
...
export default function TicketCreatePage({ onNavigate, initialData }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoadingData(true);
      try {
        const [contactRows, clientRows, categoryRows, userRows] = await Promise.all([
  ...
  const selectedContact = useMemo(
    () => contacts.find((c) => String(c.id) === String(requesterContactId)) || null,
    [contacts, requesterContactId]
  );
  ...
  const selectedClient = useMemo(
    () => clients.find((c) => String(c.id) === String(selectedContact?.client_id)) || null,
    [clients, selectedContact?.client_id]
  );

  const clientContractSummary = useMemo(
    () => buildClientContractSummary(selectedClient),
    [selectedClient]
  );
  ...
  const requesterTicketStats = useMemo(
    () => buildRequesterTicketStats(clientTickets),
    [clientTickets]
  );

  const selectedEquipment = useMemo(
    () => clientEquipments.find((eq) => String(eq.id) === String(equipmentId)) || null,
    [clientEquipments, equipmentId]
  );
  ...
  const filteredEquipments = useMemo(() => {
    ...
    const base = q
      ? clientEquipments.filter((eq) => getEquipmentSearchText(eq, locale).includes(q))
      : clientEquipments;
  ...
  const filteredLinkableTickets = useMemo(() => {
    ...
    const base = q
      ? clientTickets.filter((ticket) => getTicketLinkSearchText(ticket).includes(q))
      : clientTickets;
    return base.slice(0, 50);
  ...
  const enabledCategories = useMemo(
    () => (Array.isArray(categories) ? categories : []).filter((item) => item?.enabled !== false),
    [categories]
  );
  ...
  const supportCreditTotals = useMemo(
    () => computeSupportCreditTotals(supportCreditBalance, supportCreditPacks),
    [supportCreditBalance, supportCreditPacks]
  );
  ...
  const contractCreditsLabel = useMemo(() => {
    ...
    const { remaining, total } = supportCreditTotals;
  ...
  const selectContact = useCallback((contact) => {
    ...
    setContactSearch(getContactLabel(contact, copy));
    setShowContactDropdown(false);
    setFieldErrors((prev) => ({ ...prev, requester: undefined }));
  }, [copy]);
  ...
  const removeAttachmentFile = useCallback((fileKey) => {
    setAttachmentFiles((prev) =>
      prev.filter((file) => `${file.name}-${file.size}-${file.lastModified || 0}` !== fileKey)
    );
  }, []);
  ...
  const enrichContactRow = useCallback(
    (contact) => {
  ...
  const reloadContacts = useCallback(async () => {
    const contactRows = await fetchContactsList(null, { bypassCache: true }).catch(() => []);
    const nextContacts = Array.isArray(contactRows)
      ? contactRows.map((row) => enrichContactRow(row) || row)
      : [];
  ...
  const handleContactSaved = useCallback(
    async (savedContact) => {
      ...
      const contact =
        rows.find((row) => String(row.id) === String(savedContact?.id)) || enriched;
  ...
  const handleAvailabilityModeChange = useCallback((mode) => {
    ...
    setFieldErrors((prev) => ({ ...prev, contactSlots: undefined }));
  ...
  const selectEquipment = useCallback(
    (equipment) => {
      ...
      setEquipmentSearch(
        getEquipmentPickerLabel(equipment, { serialPrefix: copy.serialPrefix, locale })
      );
      setShowEquipmentDropdown(false);
      setFieldErrors((prev) => ({ ...prev, equipmentId: undefined }));
    },
  ...
  const handleLinkedTicketEnabledChange = useCallback((enabled) => {
    ...
    setFieldErrors((prev) => ({ ...prev, linkedTicketId: undefined }));
    if (!enabled) {
      setLinkedTicketId("");
      setLinkedTicketSearch("");
      setShowLinkedTicketDropdown(false);
  ...
  const scrollToFirstFieldError = useCallback(() => {
    requestAnimationFrame(() => {
      const errorClassNames = [
        s.inputError,
        s.contactInputWrapError,
        s.fieldShellError,
        s.attachmentDropZoneError,
        s.availabilitySlotBarError,
      ].filter(Boolean);
    ...
    try {
      ...
      for (const userId of preAssigneeUserIds) {
        try {
          ...
          // Le ticket est créé ; l'assignation pourra être faite depuis la fiche.
        }
      }
      ...
      for (const userId of preFollowerUserIds) {
        try {
          ...
          // Le ticket est créé ; les followers pourront être ajoutés depuis la fiche.
        }
      }
      ...
      if (equipmentConcerned && equipmentSource === "veritas" && selectedEquipment) {
        try {
          await addTicketComment(
            created.id,
            buildLinkedEquipmentComment(selectedEquipment, selectedContact?.client_id),
            true
          );
        } catch {
          // Le ticket est créé ; la liaison matériel pourra être faite depuis la fiche.
        }
      }
      ...
      if (linkedTicketEnabled && selectedLinkedTicket) {
        try {
          ...
          // Le ticket est créé ; la liaison pourra être faite depuis la fiche.
        }
      }
  ...
  const equipmentSummary = useMemo(() => {
    ...
    if (equipmentSource === "veritas") {
      return selectedEquipment
        ? getEquipmentPickerLabel(selectedEquipment, { serialPrefix: copy.serialPrefix, locale })
  ...
  const resolveUserIdLabel = useCallback(
    (userId) => {
  ...
  if (createdTicket) {
    return (
      <div className={`${layout.page} msp-page-grid`}>
          ...
          <SectionPanel>
            <div className={s.successPanel}>
              ...
              <p className={s.successText}>
                {copy.formatSuccessCreatedFor(
                  selectedContact ? getContactLabel(selectedContact, copy) : "-",
                  clientLabel
                )}
              </p>
  ...
  return (
    <div
      className={`${layout.page} msp-page-grid`}
    ...
    >
      {isAttachmentDragOver && (
        <div className={s.pageDropOverlay} aria-hidden>
        ...
        <header className={layout.hero}>
          ...
          <div className={layout.heroActions}>
            ...
            <button
              ...
              title={loadingData ? copy.loadingData : undefined}
            >
              <Icon icon="mdi:check" />
        ...
        <div className={s.typeKpiRow}>
          {copy.ticketTypes.map((item) => (
            <button
              key={item.key}
              type="button"
              className={`${layout.kpiCard} ${type === item.key ? layout.kpiCardActive : ""}`}
            ...
            <div className={s.formStack}>
              <SectionPanel title={copy.sections.requester} className={s.panelAllowOverflow}>
                <div className={s.demandeurBlock}>
                  ...
                  <div
                    data-pulse={fieldErrors.requester ? errorPulseTick : undefined}
                    className={`${s.contactInputWrap} ${showContactDropdown ? s.contactInputWrapOpen : ""} ${fieldErrors.requester ? s.contactInputWrapError : ""} ${fieldErrors.requester ? s.fieldErrorPulse : ""}`}
                  >
                    ...
                    <input
                      ...
                      aria-label={copy.searchContactAria}
                  ...
                  {showContactDropdown && (
                    <div className={s.contactDropdown}>
                      {filteredContacts.length === 0 ? (
                        <div className={s.contactEmpty}>
                          <span>{copy.noContactFound}</span>
                          <button
                            type="button"
                            className={s.contactEmptyAction}
                            onClick={openContactCreateModal}
                          >
                            {copy.createContact}
                          </button>
                        </div>
                        ...
                        filteredContacts.map((c, idx) => {
                          const company =
                            ...
                            clients.find((cl) => String(cl.id) === String(c.client_id))?.nom ||
                            "";
                          return (
                            <button
                              key={c.id}
                              type="button"
                              className={`${s.contactOption} ${idx === contactHighlight ? s.contactOptionActive : ""}`}
                              onMouseEnter={() => setContactHighlight(idx)}
                  ...
                  {selectedContact && (
                    <div className={s.contactSummaryCard}>
                      <div className={s.contactSummaryMain}>
                        ...
                        <div className={s.contactSummaryTop}>
                          ...
                          <button
                            type="button"
                            className={s.contactEditBtn}
                            onClick={openContactEditModal}
                            aria-label={copy.editContactAria}
                          >
                            <Icon icon="mdi:pencil-outline" aria-hidden />
                          </button>
                        </div>
                        {(selectedContact.poste || selectedContact.telephone || selectedContact.email) && (
                          <div className={s.contactSummaryMetaGrid}>
                            {selectedContact.poste && (
                              <p className={s.contactSummaryMeta} title={selectedContact.poste}>
                                <Icon icon="mdi:briefcase-outline" aria-hidden />
                                <span>{selectedContact.poste}</span>
                              </p>
                            )}
                            {selectedContact.telephone && (
                              <p className={s.contactSummaryMeta} title={selectedContact.telephone}>
                                <Icon icon="mdi:phone-outline" aria-hidden />
                                <span>{selectedContact.telephone}</span>
                              </p>
                            )}
                            {selectedContact.email && (
                              <p className={s.contactSummaryMeta} title={selectedContact.email}>
                      ...
                      <div className={s.requesterTicketStats} aria-live="polite">
                        ...
                        {loadingClientTickets ? (
                          <p className={s.requesterTicketStatsLoading}>{copy.loading}</p>
                        ) : (
                          <>
                            <div className={s.requesterTicketStatsGrid}>
                              <RequesterOpenTicketsStat
                                tickets={clientTickets}
                                openCount={requesterTicketStats.openCount}
                  ...
                  <div className={s.availabilityComposer}>
                    <div className={s.availabilityModeChips} role="radiogroup" aria-label={copy.availabilityAria}>
                      <button
                        type="button"
                        role="radio"
                        aria-checked={availabilityMode === "none"}
                        className={`${s.availabilityModeChip} ${availabilityMode === "none" ? s.availabilityModeChipActive : ""}`}
                        onClick={() => handleAvailabilityModeChange("none")}
                      >
                      ...
                      <button
                        type="button"
                        role="radio"
                        aria-checked={availabilityMode === "from"}
                        className={`${s.availabilityModeChip} ${availabilityMode === "from" ? s.availabilityModeChipActive : ""}`}
                        onClick={() => handleAvailabilityModeChange("from")}
                      >
                      ...
                      <button
                        type="button"
                        role="radio"
                        aria-checked={availabilityMode === "range"}
                        className={`${s.availabilityModeChip} ${availabilityMode === "range" ? s.availabilityModeChipActive : ""}`}
                        onClick={() => handleAvailabilityModeChange("range")}
                      >
                    ...
                    {availabilityMode !== "none" && (
                      <div
                        data-pulse={fieldErrors.contactSlots ? errorPulseTick : undefined}
                        className={`${s.availabilitySlotBar} ${fieldErrors.contactSlots ? s.availabilitySlotBarError : ""} ${fieldErrors.contactSlots ? s.fieldErrorPulse : ""}`}
                      >
                        ...
                        {availabilityMode === "from" ? (
                          ...
                          <div
                            className={`${s.slotField} ${s.slotFieldTimeRange}`}
                            role="group"
                            aria-label={copy.timeRangeAria}
                          >
                            <Icon icon="mdi:clock-outline" className={s.slotFieldIcon} aria-hidden />
                            <input
                              type="time"
              ...
              <SectionPanel
                title={copy.sections.ticketDetails}
                headerExtra={
                  <SmartTooltip
                    content={<TicketCreateTipsTooltip copy={copy} />}
                    tooltipClassName={s.tipsPortalTooltip}
                    trigger="click"
                    as="button"
                    type="button"
                    className={s.tipsHelpBtn}
                    aria-label={copy.showTipsAria}
                  >
              ...
              >
                <div className={s.fieldBlock}>
                  ...
                  <div
                    data-pulse={fieldErrors.title ? errorPulseTick : undefined}
                    className={`${s.fieldShell} ${fieldErrors.title ? s.fieldShellError : ""} ${fieldErrors.title ? s.fieldErrorPulse : ""}`}
                  >
                    <input
                      type="text"
                      ...
                      placeholder={copy.getTitlePlaceholder(type)}
                      maxLength={200}
                    />
                  </div>
                ...
                <div className={s.fieldBlock}>
                  ...
                  <div
                    data-pulse={fieldErrors.description ? errorPulseTick : undefined}
                    className={`${s.fieldShell} ${s.fieldShellMultiline} ${fieldErrors.description ? s.fieldShellError : ""} ${fieldErrors.description ? s.fieldErrorPulse : ""}`}
                  >
                    <textarea
                      className={`${s.fieldShellControl} ${s.fieldShellTextarea}`}
                      value={description}
                      onChange={(e) => {
                        setDescription(e.target.value);
                        setFieldErrors((prev) => ({ ...prev, description: undefined }));
                      }}
                      placeholder={copy.getDescriptionPlaceholder(type)}
                      rows={8}
                      maxLength={5000}
                    />
                  </div>
                ...
                <div className={s.fieldBlock}>
                  ...
                  <div
                    data-pulse={fieldErrors.attachments ? errorPulseTick : undefined}
                    className={`${s.attachmentDropZone} ${isAttachmentDragOver ? s.attachmentDropZoneActive : ""} ${fieldErrors.attachments ? s.attachmentDropZoneError : ""} ${fieldErrors.attachments ? s.fieldErrorPulse : ""}`}
                  >
                    ...
                    <label className={s.attachmentUploadBtn}>
                      ...
                      <input
                        ...
                        aria-labelledby="ticket-create-attachments-label"
                        onChange={(e) => {
                  ...
                  {attachmentFiles.length > 0 && (
                    <ul className={s.attachmentFileList}>
                      {attachmentFiles.map((file) => {
                        const fileKey = `${file.name}-${file.size}-${file.lastModified || 0}`;
                        return (
                          <li key={fileKey} className={s.attachmentFileItem}>
                            ...
                            <button
                              type="button"
                              className={s.attachmentFileRemove}
                              onClick={() => removeAttachmentFile(fileKey)}
                              aria-label={copy.formatRemoveFileAria(file.name)}
                            >
            ...
            <aside className={`${s.formStack} ${s.sideColumn}`}>
              <SectionPanel title={copy.sections.contract}>
                <dl className={s.contractFacts}>
                    ...
                    <dd
                      className={
                        contractFactEmpty
                          ? s.contractFactEmpty
                          : clientContractSummary
                            ? s[`contractFact_${clientContractSummary.validity.status}`] || ""
                            : s.contractFactEmpty
                      }
                    >
                      {contractFactLabel}
                  ...
                  <div className={s.contractFactRow}>
                    ...
                    <dd
                      className={contractOptionsEmpty ? s.contractFactEmpty : ""}
              ...
              <SectionPanel title={copy.sections.settings} className={s.panelAllowOverflow}>
                <div className={s.settingsPanel}>
                  {type === "incident" && (
                    <div className={s.settingsMajorTop}>
                      <div className={s.majorSwitchRow}>
                        ...
                        <label className={s.switch}>
                          <input
                            ...
                            onChange={(e) => handleMajorIncidentChange(e.target.checked)}
                          />
                  ...
                  <div className={s.settingsFieldRow}>
                    <div className={s.equipmentField}>
                      ...
                      <div className={s.contactPicker} ref={categoryDropdownRef}>
                        <div
                          data-pulse={fieldErrors.category ? errorPulseTick : undefined}
                          className={`${s.contactInputWrap} ${showCategoryDropdown ? s.contactInputWrapOpen : ""} ${fieldErrors.category ? s.contactInputWrapError : ""} ${fieldErrors.category ? s.fieldErrorPulse : ""}`}
                        >
                          ...
                          <input
                            ...
                            aria-labelledby="ticket-create-category-label"
                        ...
                        {showCategoryDropdown && (
                          <div className={s.contactDropdown} role="listbox" aria-labelledby="ticket-create-category-label">
                            {filteredCategoryOptions.length === 0 ? (
                              <div className={s.contactEmpty}>{copy.noCategoryFound}</div>
                            ) : (
                              (() => {
                                let optionIndex = -1;
                                    ...
                                    {items.map((item) => {
                                      ...
                                      return (
                                        <button
                                          ...
                                          type="button"
                                          role="option"
                                          aria-selected={category === String(item.name || "")}
                                          className={`${s.contactOption} ${categoryHighlight === idx ? s.contactOptionActive : ""}`}
                    ...
                    <div className={s.equipmentField}>
                      ...
                      <select
                        data-pulse={fieldErrors.priority ? errorPulseTick : undefined}
                        id="ticket-create-priority"
                        className={`${s.select} ${fieldErrors.priority ? s.inputError : ""} ${fieldErrors.priority ? s.fieldErrorPulse : ""}`}
                        value={priority}
                        disabled={isMajorIncident}
                  ...
                  <div className={s.equipmentField}>
                    ...
                    <div
                      className={s.channelIconBar}
                      role="radiogroup"
                      aria-labelledby="ticket-create-channel-label"
                    >
                      {copy.channelOptions.map((item) => {
                        ...
                        <button
                          ...
                          className={`${s.channelIconBtn} ${channel === item.key ? s.channelIconBtnActive : ""} ${channelDisabled ? s.channelIconBtnDisabled : ""}`}
                  ...
                  <div className={s.settingsFieldRow}>
                    <div className={s.equipmentField}>
                      ...
                      <div className={s.contactPicker} ref={assigneeDropdownRef}>
                        <div
                          className={`${s.contactInputWrap} ${showAssigneeDropdown ? s.contactInputWrapOpen : ""}`}
                        >
                          <Icon icon="mdi:magnify" className={s.contactInputIcon} aria-hidden />
                          <input
                            className={s.contactInput}
                            ...
                            aria-label={copy.searchAgentAssignAria}
                        ...
                        {showAssigneeDropdown && (
                          <div className={s.contactDropdown} role="listbox" aria-label={copy.assignAgentsAria}>
                            {filteredAssigneeOptions.length === 0 ? (
                              <div className={s.contactEmpty}>{copy.noAgentFound}</div>
                            ) : (
                              filteredAssigneeOptions.map((opt, idx) => (
                                <button
                                  ...
                                  role="option"
                                  aria-selected={false}
                                  className={`${s.contactOption} ${assigneeHighlight === idx ? s.contactOptionActive : ""}`}
                                  onMouseEnter={() => setAssigneeHighlight(idx)}
                                  onClick={() => addPreAssignee(opt.id)}
                                >
                      ...
                      <div className={s.chipsWrap}>
                        {preAssigneeUserIds.length === 0 ? (
                          <span className={s.emptyChipHint}>{copy.noAssignee}</span>
                        ) : (
                          preAssigneeUserIds.map((userId) => (
                            <span key={userId} className={s.chip}>
                              {resolveUserIdLabel(userId)}
                              <button
                                type="button"
                                onClick={() => removePreAssignee(userId)}
                                aria-label={copy.formatRemoveAgentAria(resolveUserIdLabel(userId))}
                              >
                                ×
                              </button>
                            </span>
                    ...
                    <div className={s.equipmentField}>
                      ...
                      <div className={s.contactPicker} ref={followerDropdownRef}>
                        <div
                          className={`${s.contactInputWrap} ${showFollowerDropdown ? s.contactInputWrapOpen : ""}`}
                        >
                          <Icon icon="mdi:magnify" className={s.contactInputIcon} aria-hidden />
                          <input
                            className={s.contactInput}
                            ...
                            placeholder={copy.searchAgent}
                            aria-label={copy.searchFollowerAria}
                        ...
                        {showFollowerDropdown && (
                          <div className={s.contactDropdown} role="listbox" aria-label={copy.followerAgentsAria}>
                            {filteredFollowerOptions.length === 0 ? (
                              ...
                              filteredFollowerOptions.map((opt, idx) => (
                                <button
                                  ...
                                  role="option"
                                  aria-selected={false}
                                  className={`${s.contactOption} ${followerHighlight === idx ? s.contactOptionActive : ""}`}
                                  onMouseEnter={() => setFollowerHighlight(idx)}
                                  onClick={() => addPreFollower(opt.id)}
                                >
                                  <span className={s.contactOptionName}>{opt.label}</span>
                                </button>
                              ))
                      ...
                      <div className={s.chipsWrap}>
                        {preFollowerUserIds.length === 0 ? (
                          <span className={s.emptyChipHint}>{copy.noFollower}</span>
                        ) : (
                          preFollowerUserIds.map((userId) => (
                            <span key={userId} className={s.chip}>
                              {resolveUserIdLabel(userId)}
                              <button
                                type="button"
                                onClick={() => removePreFollower(userId)}
                                aria-label={copy.formatRemoveAgentAria(resolveUserIdLabel(userId))}
                              >
                                ×
                              </button>
                ...
                <div className={s.equipmentPanel}>
                  <div className={s.segmentedGroup} role="radiogroup" aria-label={copy.equipmentConcernedAria}>
                    <button
                      type="button"
                      role="radio"
                      aria-checked={!equipmentConcerned}
                      className={`${s.segmentedBtn} ${!equipmentConcerned ? s.segmentedBtnActive : ""}`}
                      onClick={() => handleEquipmentConcernedChange(false)}
                    >
                    ...
                    <button
                      ...
                      aria-checked={equipmentConcerned}
                      className={`${s.segmentedBtn} ${equipmentConcerned ? s.segmentedBtnActive : ""}`}
                      onClick={() => handleEquipmentConcernedChange(true)}
                    >
                    ...
                    <div className={s.equipmentSubPanel}>
                      <div className={s.segmentedGroup} role="radiogroup" aria-label={copy.equipmentSourceAria}>
                        <button
                          type="button"
                          role="radio"
                          aria-checked={equipmentSource === "veritas"}
                          className={`${s.segmentedBtn} ${equipmentSource === "veritas" ? s.segmentedBtnActive : ""}`}
                          onClick={() => {
                            setEquipmentSource("veritas");
                            setEquipmentBrand("");
                            setEquipmentModel("");
                        ...
                        <button
                          type="button"
                          role="radio"
                          aria-checked={equipmentSource === "external"}
                          className={`${s.segmentedBtn} ${equipmentSource === "external" ? s.segmentedBtnActive : ""}`}
                          onClick={() => {
                      ...
                      {equipmentSource === "veritas" ? (
                        <div className={s.equipmentField}>
                          <label className={s.equipmentFieldLabel}>
                            {copy.equipment}<span className={s.requiredMark}>*</span>
                          </label>
                          {!selectedContact?.client_id ? (
                            <p className={s.equipmentHint}>{copy.selectRequesterFirst}</p>
                          ) : loadingEquipments ? (
                            <p className={s.equipmentHint}>{copy.loadingFleet}</p>
                          ) : clientEquipments.length === 0 ? (
                            <p className={s.equipmentHint}>{copy.noFleetEquipment}</p>
                          ) : (
                            <div className={s.linkTicketPicker} ref={equipmentDropdownRef}>
                              <div
                                data-pulse={fieldErrors.equipmentId ? errorPulseTick : undefined}
                                className={`${s.contactInputWrap} ${showEquipmentDropdown ? s.contactInputWrapOpen : ""} ${fieldErrors.equipmentId ? s.contactInputWrapError : ""} ${fieldErrors.equipmentId ? s.fieldErrorPulse : ""}`}
                              >
                                <Icon icon="mdi:magnify" className={s.contactInputIcon} aria-hidden />
                                <input
                                  type="text"
                              ...
                              {showEquipmentDropdown && (
                                <div className={s.contactDropdown} role="listbox" aria-label={copy.equipment}>
                                  {filteredEquipments.length === 0 ? (
                                    ...
                                    filteredEquipments.map((eq, idx) => (
                                      <button
                                        ...
                                        type="button"
                                        role="option"
                                        aria-selected={idx === equipmentHighlight}
                                      ...
                                      >
                                        <span className={s.contactOptionName}>
                                          {getEquipmentPickerLabel(eq, { serialPrefix: copy.serialPrefix, locale })}
                                        </span>
                                      </button>
                                    ))
                                  )}
                        ...
                        <div className={s.equipmentExternalGrid}>
                          <div className={s.equipmentField}>
                            ...
                            <input
                              data-pulse={fieldErrors.equipmentBrand ? errorPulseTick : undefined}
                              type="text"
                              className={`${s.input} ${fieldErrors.equipmentBrand ? s.inputError : ""} ${fieldErrors.equipmentBrand ? s.fieldErrorPulse : ""}`}
                              value={equipmentBrand}
                              placeholder={copy.brandPlaceholder}
                              onChange={(e) => {
                                setEquipmentBrand(e.target.value);
                                setFieldErrors((prev) => ({ ...prev, equipmentBrand: undefined }));
                              }}
                            />
                          </div>
                          <div className={s.equipmentField}>
                            ...
                            <input
                              data-pulse={fieldErrors.equipmentModel ? errorPulseTick : undefined}
                              type="text"
                              className={`${s.input} ${fieldErrors.equipmentModel ? s.inputError : ""} ${fieldErrors.equipmentModel ? s.fieldErrorPulse : ""}`}
                              value={equipmentModel}
                              placeholder={copy.modelPlaceholder}
                              onChange={(e) => {
                                setEquipmentModel(e.target.value);
                                setFieldErrors((prev) => ({ ...prev, equipmentModel: undefined }));
                              }}
                            />
                          </div>
                          <div className={`${s.equipmentField} ${s.equipmentExternalFull}`}>
                            ...
                            <input
                              data-pulse={fieldErrors.equipmentSerial ? errorPulseTick : undefined}
                              type="text"
                              className={`${s.input} ${fieldErrors.equipmentSerial ? s.inputError : ""} ${fieldErrors.equipmentSerial ? s.fieldErrorPulse : ""}`}
                              value={equipmentSerial}
                              placeholder={copy.serialPlaceholder}
                              onChange={(e) => {
                                setEquipmentSerial(e.target.value);
                                setFieldErrors((prev) => ({ ...prev, equipmentSerial: undefined }));
                              }}
                            />
                          </div>
                        </div>
                ...
                <div className={s.linkTicketPanel}>
                  <div className={s.segmentedGroup} role="radiogroup" aria-label={copy.ticketLinkAria}>
                    <button
                      type="button"
                      role="radio"
                      aria-checked={!linkedTicketEnabled}
                      className={`${s.segmentedBtn} ${!linkedTicketEnabled ? s.segmentedBtnActive : ""}`}
                      onClick={() => handleLinkedTicketEnabledChange(false)}
                    >
                    ...
                    <button
                      ...
                      aria-checked={linkedTicketEnabled}
                      className={`${s.segmentedBtn} ${linkedTicketEnabled ? s.segmentedBtnActive : ""}`}
                      onClick={() => handleLinkedTicketEnabledChange(true)}
                    >
                  ...
                  {linkedTicketEnabled && (
                    <div className={s.linkTicketSubPanel}>
                      {!selectedContact ? (
                        <p className={s.equipmentHint}>{copy.selectRequesterFirst}</p>
                      ) : loadingClientTickets ? (
                        <p className={s.equipmentHint}>{copy.loadingRequesterTickets}</p>
                      ) : clientTickets.length === 0 ? (
                        <p className={s.equipmentHint}>{copy.noRequesterTickets}</p>
                      ) : (
                        <>
                          <div className={s.equipmentField}>
                            ...
                            <div className={s.linkTicketPicker} ref={linkedTicketDropdownRef}>
                              <div
                                data-pulse={fieldErrors.linkedTicketId ? errorPulseTick : undefined}
                                className={`${s.contactInputWrap} ${fieldErrors.linkedTicketId ? s.contactInputWrapError : ""} ${fieldErrors.linkedTicketId ? s.fieldErrorPulse : ""}`}
                              >
                                <Icon icon="mdi:magnify" className={s.contactInputIcon} />
                                <input
                                  id="ticket-create-linked-ticket"
                                  className={s.contactInput}
                                  type="text"
                                  value={linkedTicketSearch}
                                  onChange={(e) => handleLinkedTicketSearchChange(e.target.value)}
                                  onFocus={() => {
                                  ...
                                  onKeyDown={(e) => {
                                    ...
                                    if (e.key === "ArrowDown") {
                                      ...
                                      setLinkedTicketHighlight((h) =>
                                        Math.min(h + 1, filteredLinkableTickets.length - 1)
                                      );
                                    } else if (e.key === "ArrowUp") {
                                      e.preventDefault();
                                  ...
                                  placeholder={copy.searchTicketPlaceholder}
                                  disabled={loadingData}
                              ...
                              {showLinkedTicketDropdown && (
                                <div className={s.contactDropdown} role="listbox">
                                  {filteredLinkableTickets.length === 0 ? (
                                    <div className={s.contactEmpty}>{copy.noTicketFound}</div>
                                  ) : (
                                    filteredLinkableTickets.map((ticket, idx) => (
                                      <button
                                        key={ticket.id}
                                        type="button"
                                        role="option"
                                        aria-selected={idx === linkedTicketHighlight}
                                        className={`${s.contactOption} ${idx === linkedTicketHighlight ? s.contactOptionActive : ""}`}
                                        onMouseEnter={() => setLinkedTicketHighlight(idx)}
                                        onClick={() => selectLinkedTicket(ticket)}
                                      >
                          ...
                          {selectedLinkedTicket ? (
                            <div className={s.linkTicketCard}>
                            ...
                            <div className={s.linkTicketEmpty}>
      ...
      {confirmModalOpen &&
        createPortal(
          ...
          >
            <div
              className={s.confirmShell}
              onClick={(e) => e.stopPropagation()}
              role="dialog"
              aria-modal="true"
              aria-labelledby="ticket-create-recap-title"
            >
              ...
              <div className={s.confirmBody}>
                <TicketCreateRecap
                  ...
                  filledContactSlots={filledContactSlots}
                  equipmentSummary={equipmentSummary}
                  linkedTicketSummary={linkedTicketSummary}
                  preAssigneesSummary={preAssigneesSummary}
                  preFollowersSummary={preFollowersSummary}
      ...
      <ContactFormModal
        ...
        clients={clients}
        defaultClientId={contactModalInitial ? null : contactCreateDefaultClientId}

...
function ConnectorToggle({ value, onChange, connectors, ariaLabel }) {
  return (
    <div className={styles.filterConnectorRow} role="group" aria-label={ariaLabel}>
      {connectors.map((opt) => (
        <button
          key={opt.value}
          type="button"
          className={`${styles.filterConnectorBtn} ${
            value === opt.value ? styles.filterConnectorBtnActive : ""
          }`}
...
function SortableFilterRow({
  ...
  return (
    <Fragment>
      {showConnector && (
        <div className={styles.filterConnectorWrap} style={{ marginLeft: `${depth * 14 + 24}px` }}>
          <ConnectorToggle
            value={connector || "and"}
            onChange={onConnectorChange}
            connectors={connectors}
            ariaLabel={logicalConnectorAria}
          />
        </div>
      )}
      <li
        ref={setNodeRef}
        style={style}
        className={`${macroModalStyles.actionsListRow} ${
          isDragging ? macroModalStyles.actionsListRowDragging : ""
        } ${isGroup ? styles.filterGroupRow : ""}`}
      >
        <button
          type="button"
          className={macroModalStyles.actionDragHandle}
        ...
        <button
          type="button"
          className={`${macroModalStyles.actionListItem} ${
            isSelected ? macroModalStyles.actionListItemActive : ""
          }`}
          onClick={onSelect}
        >
        ...
        <div className={macroModalStyles.actionListTools}>
          <button
            type="button"
            className={`${macroModalStyles.listToolBtn} ${macroModalStyles.listToolBtnDanger}`}
            title={deleteTitle}
            onClick={onDelete}
          >
  ...
  const renderChild = (child, index) => {
      ...
      return (
        <Fragment key={child.id}>
          <SortableFilterRow
            {...commonProps}
            isGroup
            icon="mdi:folder-filter-outline"
            title={copy.filterGroupTitle}
            subtitle={copy.formatFilterGroupSubtitle(ruleCount)}
          />
          <FilterGroupList
            group={child}
            depth={depth + 1}
    ...
    return (
      <SortableFilterRow
        key={child.id}
        {...commonProps}
        icon={ruleIcons[child.field] || "mdi:filter-variant"}
        title={copy.ruleLabel}
        subtitle={describeRuleBrief(child)}
      />
    );
  };
...
export default function TicketViewFilterBuilder({
  ...
  const connectors = useMemo(
    () => [
      { value: "and", label: viewConstants.describe.connectorAnd },
      { value: "or", label: viewConstants.describe.connectorOr },
    ],
    [viewConstants]
  );
  ...
  return (
    <div className={macroModalStyles.actionsListPanel}>

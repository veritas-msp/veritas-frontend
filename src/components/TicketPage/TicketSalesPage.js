...
export default function TicketSalesPage({ onNavigate }) {
  ...
  const activeView = useMemo(
    () => pageCopy.salesViews.find((view) => view.id === activeViewId) || pageCopy.salesViews[0],
    [activeViewId, pageCopy.salesViews]
  );

  const categoryLabels = useMemo(
    () =>
      Object.fromEntries(
        salesForms.filter((form) => form.enabled !== false).map((form) => [form.categorySlug, form.label])
      ),
    [salesForms]
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [ticketRows, userRows, contactRows, clientRows, formRows] = await Promise.all([
        ticketFetcher({ limit: 200 }),
        fetchUsers().catch(() => []),
  ...
  const resolveChannelMeta = (channel) =>
    pageCopy.channelMeta[channel] || { label: channel || "-", icon: "mdi:help-circle-outline" };
    ...
    const rows = [...ticketsForView]
      ...
      .filter((row) => {
        ...
        const searchable = [
          ...
          getTicketKindLabel(row),
          resolveClientLabel(row),
          resolveContactLabel(resolveRequesterContactId(row), resolveRequesterFallback(row)),
          resolveAssigneesLabel(row),
          resolveFollowersLabel(row),
  ...
  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(sortedTickets.length / pageSize)),
  ...
  const openCreatePage = (kind = "prestation") => {
    onNavigate?.("TicketSalesCreate", { kind });
  };
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      ...
      if (result.failureCount > 0) {
        ...
        toast.success(
          interpolate(
            result.successCount > 1 ? pageCopy.toasts.bulkRestoreOkPlural : pageCopy.toasts.bulkRestoreOk,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (result.failureCount > 0) {
            ...
            toast.success(
              interpolate(
                result.successCount > 1 ? pageCopy.toasts.bulkPurgeOkPlural : pageCopy.toasts.bulkPurgeOk,
                { count: String(result.successCount) }
              )
            );
          }
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (result.failureCount > 0) {
            ...
            toast.success(
              interpolate(
                result.successCount > 1 ? pageCopy.toasts.bulkDeleteOkPlural : pageCopy.toasts.bulkDeleteOk,
                { count: String(result.successCount) }
              )
            );
          }
  ...
  const handleBulkActionSuccess = async (result) => {
    if (result?.failureCount > 0) {
      ...
      toast.success(
        interpolate(count > 1 ? pageCopy.toasts.bulkUpdateOkPlural : pageCopy.toasts.bulkUpdateOk, {
          count: String(count),
        })
      );
    }
  ...
  const handleExportCsv = () => {
    ...
    const rows = sortedTickets.map((t) => {
      ...
      return [
        `#${t.ticket_number || "-"}`,
        t.title || "",
        getTicketKindLabel(t),
        getCategoryLabel(t.category, categoryLabels),
        t.channel || "-",
        ...
        resolvePriorityLabel(t.priority),
        pageCopy.formatDateTime(t.created_at),
        pageCopy.formatDateTime(t.updated_at),
      ];
    });
  ...
  const categoryFilterOptions = useMemo(() => {
    ...
    return [
      { value: "", label: pageCopy.allCategories },
      ...forms.map((form) => ({ value: form.categorySlug, label: form.label })),
    ];
      ...
      <div className={mspStyles.mspLayout}>
        <div className={mspStyles.mspMain}>
          <MspPageHero
            ...
            subtitle={pageCopy.formatSubtitle(loading, viewMode, activeView.name, sortedTickets.length)}
            ...
            actions={
              <>
                ...
                <button
                  ...
                  onClick={() => openCreatePage(createKind)}
                  disabled={viewMode === "trash"}
                  aria-label={pageCopy.newRequestAria}
                >
                  <FaPlus />
                </button>
              </>
        ...
        <div className={`${styles.viewsLayout} ${viewMode === "trash" ? styles.viewsLayoutTrash : ""}`}>
          ...
          <div className={styles.mainColumn}>
            {viewMode !== "trash" ? (
              ...
              <div className={`${styles.activeViewBanner} ${styles.trashBanner}`}>
                <span className={styles.activeViewBannerMain}>
            ...
            <div className={`${layout.toolbar} ${styles.toolbarGrow}`}>
              ...
              <select
                ...
                aria-label={pageCopy.search.filterCategory}
              >
            ...
            {loading ? (
              ...
              <div className={layout.emptyState}>
                ...
                {viewMode !== "trash" && (
                  <button type="button" className={layout.primaryBtn} onClick={() => openCreatePage(createKind)}>
                    <Icon icon="mdi:plus" />
                            ...
                            >
                              <td className={styles.checkboxCell} onClick={(e) => e.stopPropagation()}>
                                <input
                                  ...
                                  aria-label={pageCopy.formatSelectOne(t.ticket_number || t.id)}
                                />
                              </td>
                              ...
                              <td>
                                <span
                                  ...
                                  title={getPriorityVisual(t.priority).label}
                                >
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <span
                                      ...
                                      title={pageCopy.getSlaTitle(sla.phase)}
                                    >
                                ...
                                <td>
                                  <div className={styles.actionRow}>
                                    <button
                                      ...
                                      title={pageCopy.table.restoreTitle}
                                      onClick={(e) => {
                                        e.stopPropagation();
                ...
                <div className={layout.pagination}>
                  ...
                  <div className={layout.paginationRight}>
                    <button
                      ...
                      aria-label={pageCopy.pagination.prev}
                    >
                      <FaChevronLeft />
                    </button>
                    ...
                    <button
                      ...
                      aria-label={pageCopy.pagination.next}
                    >
                      <FaChevronRight />
                    </button>
                  </div>

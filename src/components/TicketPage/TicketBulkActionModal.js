...
function resolveContactLabel(contact, fallbackTemplate) {
  ...
  return contact?.id ? interpolate(fallbackTemplate, { id: String(contact.id) }) : "";
}
...
export default function TicketBulkActionModal({
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!q) return rows.slice(0, 80);
    return rows
      .filter((user) => resolveUserLabel(user, pageCopy.bulkModal.userFallback).toLowerCase().includes(q))
      .slice(0, 80);
  }, [users, userSearch]);
  ...
  const validateForm = () => {
    const hasFieldUpdate =
      ...
      form.enablePriority ||
      form.enableType ||
      form.enableRequester ||
      form.enableAssignees ||
      form.enableWatchers;
          ...
          <section className={styles.section}>
            <div className={styles.sectionHead}>
              <label>
                <input
                  type="checkbox"
                  checked={form.enableStatus}
                  onChange={(e) => setForm((prev) => ({ ...prev, enableStatus: e.target.checked }))}
                />
            ...
            {form.enableStatus && (
              <div className={styles.sectionBody}>
                ...
                >
                  {pageCopy.statusOptions.map((option) => (
          ...
          <section className={styles.section}>
            <div className={styles.sectionHead}>
              <label>
                <input
                  type="checkbox"
                  checked={form.enablePriority}
                  onChange={(e) => setForm((prev) => ({ ...prev, enablePriority: e.target.checked }))}
                />
            ...
            {form.enablePriority && (
              <div className={styles.sectionBody}>
                ...
                >
                  {pageCopy.priorityOptions.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
              </div>
          ...
          <section className={styles.section}>
            <div className={styles.sectionHead}>
              <label>
                <input
                  type="checkbox"
                  checked={form.enableType}
                  onChange={(e) => setForm((prev) => ({ ...prev, enableType: e.target.checked }))}
                />
            ...
            {form.enableType && (
              <div className={styles.sectionBody}>
                ...
                >
                  {pageCopy.ticketTypeOptions.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
              </div>
          ...
          <section className={styles.section}>
            <div className={styles.sectionHead}>
              <label>
                <input
                  type="checkbox"
                  checked={form.enableRequester}
                  onChange={(e) => setForm((prev) => ({ ...prev, enableRequester: e.target.checked }))}
                />
            ...
            {form.enableRequester && (
              <div className={styles.sectionBody}>
                <select
                  ...
                  onChange={(e) => setForm((prev) => ({ ...prev, requesterKind: e.target.value }))}
                >
                ...
                {form.requesterKind === "contact" ? (
                  <>
                    <input
                      ...
                      placeholder={pageCopy.bulkModal.searchContact}
                    />
                    <select
                      className={styles.select}
                      value={form.requesterContactId}
                      onChange={(e) => setForm((prev) => ({ ...prev, requesterContactId: e.target.value }))}
                    >
                  ...
                  <>
                    <input
                      ...
                      placeholder={pageCopy.bulkModal.searchUser}
                    />
                    <select
                      className={styles.select}
                      value={form.requesterUserId}
                      onChange={(e) => setForm((prev) => ({ ...prev, requesterUserId: e.target.value }))}
                    >
          ...
          <section className={styles.section}>
            <div className={styles.sectionHead}>
              <label>
                <input
                  type="checkbox"
                  checked={form.enableAssignees}
                  onChange={(e) => setForm((prev) => ({ ...prev, enableAssignees: e.target.checked }))}
                />
            ...
            {form.enableAssignees && (
              <div className={styles.sectionBody}>
                <select
                  ...
                  onChange={(e) => setForm((prev) => ({ ...prev, assigneeMode: e.target.value }))}
                >
                  {pageCopy.assigneeModes.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
                <input
                ...
                <div className={styles.userList}>
                  {filteredAssigneeUsers.map((user) => (
                    <label key={user.id} className={styles.userOption}>
                      <input
                        type="checkbox"
                        checked={form.assigneeUserIds.includes(String(user.id))}
                        onChange={() => toggleUserId("assigneeUserIds", user.id)}
                      />
                ...
                {form.assigneeMode === "replace" && (
                  <p className={styles.hint}>{pageCopy.bulkModal.assigneeReplaceHint}</p>
                )}
              </div>
            )}
          </section>
          ...
          <section className={styles.section}>
            <div className={styles.sectionHead}>
              <label>
                <input
                  type="checkbox"
                  checked={form.enableWatchers}
                  onChange={(e) => setForm((prev) => ({ ...prev, enableWatchers: e.target.checked }))}
                />
            ...
            {form.enableWatchers && (
              <div className={styles.sectionBody}>
                <select
                  className={styles.select}
                  value={form.watcherMode}
                  onChange={(e) => setForm((prev) => ({ ...prev, watcherMode: e.target.value }))}
                >
                  {pageCopy.watcherModes.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
                <input
                ...
                <div className={styles.userList}>
                  {filteredWatcherUsers.map((user) => (
                    <label key={user.id} className={styles.userOption}>
                      <input
                        type="checkbox"
                        checked={form.watcherUserIds.includes(String(user.id))}
                        onChange={() => toggleUserId("watcherUserIds", user.id)}
                      />
                      <span>{resolveUserLabel(user, pageCopy.bulkModal.userFallback)}</span>
                    </label>
                  ))}
                ...
                {form.watcherMode === "replace" && (
                  <p className={styles.hint}>{pageCopy.bulkModal.watcherReplaceHint}</p>
                )}
              </div>
            )}
          </section>

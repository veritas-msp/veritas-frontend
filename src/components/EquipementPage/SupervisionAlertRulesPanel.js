...
export default function SupervisionAlertRulesPanel({
  catalog,
  rules: rulesProp,
  isAdmin = false,
  onSaved,
  ...
  const [draft, setDraft] = useState(rulesProp || buildDefaultSupervisionAlertRules());
  ...
  const isDirty = useMemo(() => {
    return JSON.stringify(draft) !== JSON.stringify(rulesProp || buildDefaultSupervisionAlertRules());
  }, [draft, rulesProp]);
  ...
  const handleResetAll = useCallback(() => {
    setDraft(buildDefaultSupervisionAlertRules());
  }, []);
    ...
    <div className={styles.panel}>
      ...
      <div className={styles.familyList}>
        {families.map((family) => {
          ...
          return (
            <section key={family.key} className={styles.familyCard}>
              <button
                type="button"
                ...
                <div className={styles.criteriaList}>
                  {isAdmin ? (
                    <button
                      ...
                      onClick={() => handleResetFamily(family.key)}
                    >
                  ...
                  {criteria.map((criterion) => {
                    ...
                    const criterionDescription = copy.getCriterionDescription(
                      criterion.key,
                      meta.description
                    );
                    return (
                      <div key={criterion.key} className={styles.criterionRow}>
                        <div className={styles.criterionText}>
                          ...
                          {criterionDescription ? (
                            <span className={styles.criterionDesc}>{criterionDescription}</span>
                          ) : null}
                        </div>
                        <Toggle
                          checked={enabled}
                          onChange={(value) => handleToggle(family.key, criterion.key, value)}

...
import {
  buildContractFleetRows,
  buildContractFleetStats,
  filterContractFleetRows,
  groupContractFleetByCategory,
  sortContractFleetRows,
} from "./contractMspUtils";
...
function FleetTableRow({ row, onOpenClient, copy, bcp47 }) {
  ...
  return (
    <tr className={styles.tableRow}>
      ...
      <td>
        ...
        {row.subtitle ? (
          <span className={styles.cellSub} title={row.subtitle}>
            {row.subtitle}
          </span>
        ) : null}
      </td>
      ...
      <td>
        {row.clientId ? (
          <button
            ...
            aria-label={copy.viewEnterprise}
            onClick={() =>
              onOpenClient?.({
                id: row.clientId,
                name: row.clientName,
...
export default function ContractMspDashboard({
  ...
  const fleetRows = useMemo(
    () =>
      buildContractFleetRows(alerts, licenseAlerts, {
  ...
  const filteredRows = useMemo(
    () => filterContractFleetRows(fleetRows, { search, statusFilter, categoryFilter }),
    [fleetRows, search, statusFilter, categoryFilter]
  );

  const sortedRows = useMemo(
    () => sortContractFleetRows(filteredRows, sortBy, sortDirection),
  ...
  const categoryTabs = useMemo(() => {
    ...
    return Array.from(counts.entries())
      .map(([id, count]) => {
        ...
        return { id, label: sample?.categoryLabel || id, count };
      })
  ...
  const priorityRows = useMemo(
    () =>
      fleetRows
        .filter((row) => row.status === "expired" || row.status === "suspended" || row.status === "expiring")
        .sort((a, b) => {
    ...
    <div className={styles.dashboard}>
      <section className={styles.hero}>
        ...
        <div className={styles.kpiRow}>
          <KpiCard
            ...
            label={copy.kpi.alerts}
            value={stats.total}
            tone={stats.total > 0 ? "warn" : "good"}
            active={statusFilter === "all" && categoryFilter === "all" && !search}
            onClick={() => {
              setStatusFilter("all");
          ...
          <KpiCard
            ...
            label={copy.kpi.expired}
            value={stats.statusCounts.expired}
            tone={stats.statusCounts.expired > 0 ? "bad" : "good"}
            active={statusFilter === "expired"}
            onClick={() => setStatusFilter(statusFilter === "expired" ? "all" : "expired")}
          />
          <KpiCard
            icon="mdi:calendar-clock"
            label={copy.kpi.toRenew}
            value={stats.statusCounts.expiring}
            tone={stats.statusCounts.expiring > 0 ? "warn" : "good"}
              ...
              return (
                ...
                >
                  ...
                  <span className={styles.priorityVerb}>
                    {row.status === "expired" || row.status === "suspended"
                      ? copy.priorityVerbFix
                      : copy.priorityVerbReview}
                  </span>
                </button>
              );
            })}
      ...
      <div className={styles.toolbar}>
        ...
        {categoryTabs.length > 1 ? (
          <div className={styles.filterGroup} role="tablist" aria-label={copy.categoryFilterAria}>
            <button
              type="button"
            ...
            {categoryTabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                className={`${styles.filterChip} ${categoryFilter === tab.id ? styles.filterChipActive : ""}`}
                onClick={() => setCategoryFilter(categoryFilter === tab.id ? "all" : tab.id)}
              >
                {tab.label} {tab.count}
              </button>
            ))}
      ...
      {loading ? (
        ...
        <MspEmptyState
          icon={fleetRows.length === 0 ? "mdi:file-check-outline" : "mdi:magnify"}

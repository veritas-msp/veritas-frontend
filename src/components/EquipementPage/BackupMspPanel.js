...
import {
  getBackupJobStatus,
  getBackupJobStatusLabel,
  getBackupJobStatusTitle,
  getBackupJobRowStyle,
  compareBackupJobsByStatus,
  computeBackupJobStats,
...
export default function BackupMspPanel({
  ...
  const [instanceSauvegardeModal, setInstanceSauvegardeModal] = useState({
    open: false,
    mode: "add",
    instanceType: null,
    clientId: null,
  ...
  const loadClients = useCallback(async ({ force = false, skipCache = false } = {}) => {
    ...
    const request = (async () => {
      try {
          ...
          try {
            ...
            if (raw) {
              ...
              const isFresh =
                ...
                Date.now() - parsed.savedAt < CYBER_PAGE_DATA_CACHE_TTL_MS;
              if (isFresh) {
        ...
        const payload = await fetchCyberPageData({ signal: controller.signal });
        ...
        try {
          sessionStorage.setItem(
            CYBER_PAGE_DATA_CACHE_KEY,
            JSON.stringify({
              savedAt: Date.now(),
              data: { clients: clientsData, campaigns: payload?.campaigns || [] },
            })
          );
        } catch {
  ...
  const loadAllBackups = useCallback(
    async (clientsList = null, options = {}) => {
      const skipCache = options?.skipCache === true;
      const fromRetry = options?.fromRetry === true;
      const list = clientsList != null ? clientsList : clients;
      const hasModulesData =
        Array.isArray(list) && list.some((client) => client?.equipements && typeof client.equipements === "object");

      try {
        if (!hasModulesData && !fromRetry) {
          const enrichedClients = await loadClients({ force: true, skipCache: true });
          if (Array.isArray(enrichedClients)) {
            return await loadAllBackups(enrichedClients, { skipCache: true, fromRetry: true });
          }
        }
    ...
    try {
      ...
      toast.success(
        data.message && data.updated != null
          ...
          : copy.toasts.syncDone
      );
      if (data.lastSync) setJobsLastSyncDate(data.lastSync);
      await loadClients({ force: true, skipCache: true });
      invalidateBackupsCache();
      await loadAllBackups(null, { skipCache: true });
    } catch (err) {
  ...
  const handleDeleteJob = useCallback(
    async (item) => {
      ...
      if (!window.confirm(interpolate(cyberCopy.deleteBackupJob, { name: item.nom || item.id }))) return;
      try {
        ...
        const clientsData = await loadClients({ force: true, skipCache: true });
        if (Array.isArray(clientsData)) {
          invalidateBackupsCache();
          loadAllBackups(clientsData, { skipCache: true });
        }
      } catch (err) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const modulesClients = await loadClients({ force: false });
      if (cancelled) return;
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (item) =>
          ...
          (item.logiciel && item.logiciel.toLowerCase().includes(query)) ||
  ...
  const filteredActiveBackup = useMemo(() => {
    ...
    let filtered = backupData.filter(
      (item) => item.type === "instance" && item.logiciel === "Active Backup for Microsoft 365"
    );
    if (searchQuery.trim()) {
      const query = searchQuery.toLowerCase();
      filtered = filtered.filter(
  ...
  const sortedHyperBackup = useMemo(
    () => sortByKey(filteredHyperBackup, hyperBackupSortBy, hyperBackupSortOrder),
    [filteredHyperBackup, hyperBackupSortBy, hyperBackupSortOrder]
  );
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (item) =>
          ...
          (item.typeSauvegarde && item.typeSauvegarde.toLowerCase().includes(query)) ||
          (item.instanceLogiciel && item.instanceLogiciel.toLowerCase().includes(query)) ||
          (item.serveurLie && item.serveurLie.toLowerCase().includes(query)) ||
  ...
  const displayedJobs = useMemo(() => {
    ...
    return jobsAfterStatusFilter.filter(
      (item) =>
        item.clientName.toLowerCase().includes(query) ||
        (item.nom && item.nom.toLowerCase().includes(query)) ||
        (item.typeSauvegarde && item.typeSauvegarde.toLowerCase().includes(query)) ||
        ...
        (item.serveurLie && item.serveurLie.toLowerCase().includes(query)) ||
        (item.destination && item.destination.toLowerCase().includes(query)) ||
        (item.regularite && item.regularite.toLowerCase().includes(query)) ||
        (item.horaire && String(item.horaire).toLowerCase().includes(query)) ||
        (item.retention && item.retention.toLowerCase().includes(query))
    );
  ...
  const backupTableCounts = useMemo(
    () => ({
      ...
      activeBackup: backupData.filter(
        (item) => item.type === "instance" && item.logiciel === "Active Backup for Microsoft 365"
      ).length,
  ...
  const handleRefreshAfterSave = async () => {
    const data = await loadClients({ force: true, skipCache: true });
    if (Array.isArray(data)) {
      invalidateBackupsCache();
      loadAllBackups(data, { skipCache: true });
    }
  };
    ...
    return (
      <>
        <BackupMspDashboard
          ...
          syncing={jobsSyncLoading}
          onSync={syncJobsFromCheckMK}
          copy={dashboardCopy}
          bcp47={localeTag}
          onOpenJob={(job) => openMappingModal(job)}
          onOpenClient={(row) => {
            if (!onNavigate || !row?.clientId) return;
            onNavigate("ContratDetail", { clientId: row.clientId, name: row.clientName });
          }}
        />
          ...
          <EquipmentMappingModal
            ...
            onMappingSaved={(mapping) => {
              if (mappingModalEquipment) {
                setBackupData((prev) =>
                  prev.map((item) =>
                    item.id === mappingModalEquipment.id &&
                    item.clientId === mappingModalEquipment.clientId
                      ? {
                          ...item,
        ...
        {instanceSauvegardeModal.open ? (
          <InstanceSauvegardeModal
            open={instanceSauvegardeModal.open}
            onClose={() => setInstanceSauvegardeModal((prev) => ({ ...prev, open: false }))}
            mode={instanceSauvegardeModal.mode}
            instanceType={instanceSauvegardeModal.instanceType}
            clientId={instanceSauvegardeModal.clientId}
            clientName={instanceSauvegardeModal.clientName}
            instance={instanceSauvegardeModal.instance}
            clients={clients}
            onSaved={handleRefreshAfterSave}
          />
        ) : null}
  ...
  const panelContent = (
    <div className={styles.tablesContainer}>
          {backupJobStats.total > 0 && (
            <div
              className={`${styles.backupAlertBar} ${backupJobStats.issues > 0 ? styles.backupAlertBarHasIssues : ""}`}
            >
              ...
              <div className={styles.backupAlertPills}>
                <button
                  type="button"
                  className={`${styles.backupAlertPill} ${styles.backupAlertPillIssues} ${backupJobStatusFilter === "issues" ? styles.backupAlertPillActive : ""}`}
                  onClick={() => toggleBackupJobStatusFilter("issues")}
                  disabled={backupJobStats.issues === 0}
                  title={copy.alertBar.alertsTitle}
                >
                ...
                <button
                  type="button"
                  className={`${styles.backupAlertPill} ${styles.backupAlertPillCritical} ${backupJobStatusFilter === "critical" ? styles.backupAlertPillActive : ""}`}
                  onClick={() => toggleBackupJobStatusFilter("critical")}
                  disabled={backupJobStats.critical === 0}
                  title={copy.alertBar.criticalTitle}
                >
                ...
                <button
                  type="button"
                  className={`${styles.backupAlertPill} ${styles.backupAlertPillWarning} ${backupJobStatusFilter === "warning" ? styles.backupAlertPillActive : ""}`}
                  onClick={() => toggleBackupJobStatusFilter("warning")}
                  disabled={backupJobStats.warning === 0}
                  title={copy.alertBar.warningTitle}
                >
                  <Icon icon="mdi:alert" />
                ...
                <button
                  type="button"
                  className={`${styles.backupAlertPill} ${styles.backupAlertPillOk} ${backupJobStatusFilter === "ok" ? styles.backupAlertPillActive : ""}`}
                  onClick={() => toggleBackupJobStatusFilter("ok")}
                  disabled={backupJobStats.ok === 0}
                  title={copy.alertBar.okTitle}
                >
                ...
                {backupJobStatusFilter && (
                  <button
                    type="button"
                    className={styles.backupAlertPillClear}
                    onClick={() => setBackupJobStatusFilter(null)}
                  >
              ...
              <div className={styles.backupAlertActions}>
                <SmartTooltip as="span" content={copy.alertBar.syncTooltip}>
                  <button
                    type="button"
                    className={styles.backupAlertSyncBtn}
                ...
                {jobsLastSyncDate && (
                  <span className={styles.backupAlertMeta}>
                ...
                {backupJobStats.issues > 0 && !backupJobStatusFilter && (
                  <span className={styles.backupAlertHint}>
          ...
          <div className={styles.backupJobsSection}>
            <div className={styles.tableSectionHeaderJobs}>
              ...
              <div className={styles.jobsHeaderActionsCenter}>
                <div className={styles.jobsSearchBox}>
                  ...
                  <input
                    type="text"
                    placeholder={copy.jobsSearchPlaceholder}
                    value={jobsSearchQuery}
                    onChange={(e) => setJobsSearchQuery(e.target.value)}
                    className={styles.searchInput}
                    aria-label={copy.jobsSearchAria}
                  />
                  {jobsSearchQuery && (
                    <button
                      type="button"
                ...
                <button
                  ...
                  title={copy.addJobTitle}
                  aria-label={copy.addJobAria}
                >
                  <FaPlus />
                </button>
              </div>
            ...
            {loadingBackups || loadingClients ? (
              ...
              <MspEmptyState
                ...
                title={copy.empty.noJobsTitle}
                text={copy.empty.noJobsText}
              />
            ) : displayedJobs.length === 0 ? (
              <MspEmptyState
                icon="mdi:magnify"
                title={copy.empty.noResultsTitle}
                text={copy.empty.noResultsText}
              />
            ) : (
                ...
                <table className={styles.equipmentTable}>
                  <thead>
                    <tr>
                      ...
                      <th onClick={() => handleJobsSort("clientName")} style={{ cursor: "pointer" }}>
                        {copy.table.client} <JobsSortIcon column="clientName" />
                      </th>
                      <th onClick={() => handleJobsSort("nom")} style={{ cursor: "pointer" }}>
                        {copy.table.name} <JobsSortIcon column="nom" />
                      </th>
                      <th onClick={() => handleJobsSort("typeSauvegarde")} style={{ cursor: "pointer" }}>
                        {copy.table.type} <JobsSortIcon column="typeSauvegarde" />
                      </th>
                      <th onClick={() => handleJobsSort("serveurLie")} style={{ cursor: "pointer" }}>
                        {copy.table.server} <JobsSortIcon column="serveurLie" />
                      </th>
                      <th onClick={() => handleJobsSort("destination")} style={{ cursor: "pointer" }}>
                        {copy.table.destination} <JobsSortIcon column="destination" />
                      </th>
                      <th onClick={() => handleJobsSort("last_backup_start")} style={{ cursor: "pointer" }}>
                        {copy.table.lastBackup} <JobsSortIcon column="last_backup_start" />
                      </th>
                      <th onClick={() => handleJobsSort("last_backup_duration")} style={{ cursor: "pointer" }}>
                  ...
                  <tbody>
                    {sortedDisplayedJobs.map((item, index) => {
                      ...
                      const badgeClass =
                        jobStatus === "critical"
                          ? styles.jobStatusBadgeCritical
                          : jobStatus === "warning"
                            ? styles.jobStatusBadgeWarning
                            : jobStatus === "ok"
                              ? styles.jobStatusBadgeOk
                              : jobStatus === "unmapped"
                                ? styles.jobStatusBadgeUnmapped
                                : styles.jobStatusBadgeHycu;
                      return (
                        <tr
                          key={`${item.id}-${index}`}
                          className={styles.equipmentRow}
                          style={{ cursor: "default", ...getBackupJobRowStyle(jobStatus) }}
                          title={statusTitle}
                        >
                          <td>
                            ...
                            <div
                              ...
                              onClick={(e) => {
                                ...
                                if (onNavigate && item.clientId) {
                                  onNavigate("ContratDetail", { clientId: item.clientId, name: item.clientName });
                                }
                              }}
                              style={item.clientId ? { cursor: "pointer" } : {}}
                          ...
                          <td>
                            {item.typeSauvegarde
                              ? item.typeSauvegarde.charAt(0).toUpperCase() + item.typeSauvegarde.slice(1)
                              : "-"}
                          </td>
                            ...
                            {item.last_backup_start ?? item.rawData?.last_backup_start
                              ? (() => {
                                  try {
                                    return new Date(
                                      item.last_backup_start ?? item.rawData?.last_backup_start
                                    ).toLocaleString(localeTag, { dateStyle: "short", timeStyle: "short" });
                                  } catch {
                          ...
                          <td>
                            {jobStatus === "hycu" ? (
                              <span
                                className={styles.unmappedBadge}
                                style={{ opacity: 0.6 }}
                              ...
                              >
                                {item.isMapped ? (
                                  <span
                                    ...
                                    title={`Mappé: ${item.checkmkMapping?.checkmk_host_name || "CheckMK"}`}
                                  >
                                    <Icon icon="simple-icons:checkmk" style={{ width: "18px", height: "18px" }} />
                                  </span>
                                ) : (
                                  <span className={styles.unmappedBadge} title={copy.table.mapCheckmk}>
                                    <Icon
                          ...
                          <td>
                            <button
                              ...
                              title={copy.table.deleteJob}
                              aria-label={copy.table.deleteJob}
                            >
                              <FaTrash />
                            </button>
                          </td>
          ...
          <div className={styles.backupInstancesCollapsible}>
            <button
              type="button"
              className={styles.backupInstancesToggle}
              onClick={() => setShowBackupInstances((open) => !open)}
            >
              <Icon icon={showBackupInstances ? "mdi:chevron-up" : "mdi:chevron-down"} />
              {copy.formatInstancesToggle(
                backupTableCounts.hycu +
                  backupTableCounts.veeam +
                  backupTableCounts.activeBackup +
                  backupTableCounts.hyperBackup
              )}
            </button>
            {showBackupInstances && (
              <div className={styles.backupInstancesGrid}>
                {(selectedBackupTables.size === 0 || selectedBackupTables.has("HYCU")) && (
                  <div className={styles.equipmentTableSection}>
                    <div className={styles.tableSectionHeader}>
                      <div className={styles.tableSectionTitle}>
                        <img
                          src="/assets/icons/hycu.png"
                          alt="HYCU Backup"
                          style={{
                            width: "24px",
                            height: "24px",
                      ...
                      <button
                        ...
                        title="Ajouter une instance HYCU"
                        aria-label="Ajouter une instance HYCU"
                      >
                        <FaPlus />
                      </button>
                    </div>
                    ...
                    ) : filteredHYCU.length === 0 ? (
                      <MspEmptyState
                        icon="mdi:server"
                        title={copy.instances.noHycuTitle}
                        text={copy.instances.noHycuText}
                      />
                    ) : (
                      <div className={styles.tableWrapper}>
                        <table className={styles.equipmentTable}>
                          <thead>
                            <tr>
                              <th onClick={() => handleHycuSort("clientName")} style={{ cursor: "pointer" }}>
                                Client <HycuSortIcon column="clientName" />
                              </th>
                              <th onClick={() => handleHycuSort("server")} style={{ cursor: "pointer" }}>
                                Nom <HycuSortIcon column="server" />
                              </th>
                            </tr>
                          </thead>
                          <tbody>
                            {sortedHYCU.map((item, index) => (
                              <tr
                                key={`${item.id}-${index}`}
                                className={styles.equipmentRow}
                ...
                {(selectedBackupTables.size === 0 || selectedBackupTables.has("Veeam")) && (
                  <div className={styles.equipmentTableSection}>
                    <div className={styles.tableSectionHeader}>
                      <div className={styles.tableSectionTitle}>
                        <img
                          src="/assets/icons/veeam.png"
                          alt="Veeam"
                          style={{
                            width: "24px",
                            height: "24px",
                      ...
                      <button
                        ...
                        title="Ajouter une instance Veeam"
                        aria-label="Ajouter une instance Veeam"
                      >
                        <FaPlus />
                      </button>
                    </div>
                    ...
                    ) : filteredVeeam.length === 0 ? (
                      <MspEmptyState
                        icon="mdi:server"
                        title={copy.instances.noVeeamTitle}
                        text={copy.instances.noVeeamText}
                      />
                    ) : (
                      <div className={styles.tableWrapper}>
                        <table className={styles.equipmentTable}>
                          <thead>
                            <tr>
                              <th onClick={() => handleVeeamSort("clientName")} style={{ cursor: "pointer" }}>
                                Client <VeeamSortIcon column="clientName" />
                              </th>
                              <th onClick={() => handleVeeamSort("server")} style={{ cursor: "pointer" }}>
                                Nom <VeeamSortIcon column="server" />
                              </th>
                              <th onClick={() => handleVeeamSort("expiration")} style={{ cursor: "pointer" }}>
                          ...
                          <tbody>
                            {sortedVeeam.map((item, index) => {
                              ...
                              const isExpiringSoon =
                                expirationMs != null && expirationMs >= now && expirationMs - now <= J30;
                              const rowStyle = {
                                cursor: "pointer",
                              ...
                              return (
                                <tr
                                  ...
                                  title={
                                    isExpired
                                      ? "Licence expirée"
                                      : isExpiringSoon
                                        ? "Licence expire dans les 30 jours"
                                        : ""
                                  }
                                  onClick={() =>
                                    setInstanceSauvegardeModal({
                                ...
                                >
                                  ...
                                  <td className={styles.dateCell}>
                                    {item.expiration ? new Date(item.expiration).toLocaleDateString("fr-FR") : "-"}
                                  </td>
                                </tr>
                              );
                            })}
                ...
                {(selectedBackupTables.size === 0 || selectedBackupTables.has("Active Backup")) && (
                  <div className={styles.equipmentTableSection}>
                    <div className={styles.tableSectionHeader}>
                      <div className={styles.tableSectionTitle}>
                        <img
                          src="/assets/icons/active-backup.png"
                      ...
                      <button
                        ...
                        title="Ajouter une instance Active Backup"
                        aria-label="Ajouter une instance Active Backup"
                      >
                        <FaPlus />
                      </button>
                    </div>
                    ...
                    ) : filteredActiveBackup.length === 0 ? (
                      <MspEmptyState
                        icon="mdi:sync"
                        title={copy.instances.noActiveBackupTitle}
                        text={copy.instances.noActiveBackupText}
                      />
                    ) : (
                      <div className={styles.tableWrapper}>
                        <table className={styles.equipmentTable}>
                          <thead>
                            <tr>
                              <th onClick={() => handleActiveBackupSort("clientName")} style={{ cursor: "pointer" }}>
                                Client <ActiveBackupSortIcon column="clientName" />
                              </th>
                              <th
                                onClick={() => handleActiveBackupSort("activeBackupModules")}
                                style={{ cursor: "pointer" }}
                              >
                                Modules activés <ActiveBackupSortIcon column="activeBackupModules" />
                              </th>
                              <th
                                onClick={() => handleActiveBackupSort("activeBackupStorage")}
                                style={{ cursor: "pointer" }}
                              >
                                Stockage de destination <ActiveBackupSortIcon column="activeBackupStorage" />
                              </th>
                            </tr>
                          </thead>
                          <tbody>
                            {sortedActiveBackup.map((item, index) => {
                              ...
                              const allModules = [
                                { key: "oneDrive", label: "OneDrive", icon: "entypo-social:onedrive" },
                                { key: "sharePoint", label: "SharePoint", icon: "mdi:microsoft-sharepoint" },
                                { key: "exchange", label: "Exchange", icon: "simple-icons:microsoftexchange" },
                                { key: "teams", label: "Teams", icon: "simple-icons:microsoftteams" },
                                { key: "calendar", label: "Calendar", icon: "mdi:calendar" },
                                { key: "contacts", label: "Contacts", icon: "mdi:contacts" },
                              ];
                                      ...
                                      {allModules.map((module) => {
                                        ...
                                        return (
                                          <span
                                            key={module.key}
                                            className={
                                              isActive ? styles.activeBackupModuleOn : styles.activeBackupModuleOff
                                            }
                                            title={`${module.label} : ${isActive ? "activé" : "inactif"}`}
                                          >
                                            <Icon icon={module.icon} style={{ width: "18px", height: "18px" }} />
                                          </span>
                                        );
                                      })}
                                    </div>
                ...
                {(selectedBackupTables.size === 0 || selectedBackupTables.has("HyperBackup")) && (
                  <div className={styles.equipmentTableSection}>
                    <div className={styles.tableSectionHeader}>
                      <div className={styles.tableSectionTitle}>
                        <img
                          src="/assets/icons/hyperbackup.png"
                          alt="HyperBackup"
                          style={{
                            width: "24px",
                            height: "24px",
                      ...
                      <button
                        ...
                        title="Ajouter une instance HyperBackup"
                        aria-label="Ajouter une instance HyperBackup"
                      >
                        <FaPlus />
                      </button>
                    </div>
                    ...
                    ) : filteredHyperBackup.length === 0 ? (
                      <MspEmptyState
                        icon="mdi:sync"
                        title={copy.instances.noHyperBackupTitle}
                        text={copy.instances.noHyperBackupText}
                      />
                    ) : (
                      <div className={styles.tableWrapper}>
                        <table className={styles.equipmentTable}>
                          <thead>
                            <tr>
                              <th onClick={() => handleHyperBackupSort("clientName")} style={{ cursor: "pointer" }}>
                                Client <HyperBackupSortIcon column="clientName" />
                              </th>
                              <th
                                onClick={() => handleHyperBackupSort("hyperbackupSource")}
                                style={{ cursor: "pointer" }}
                              >
                                Nas d&apos;origine <HyperBackupSortIcon column="hyperbackupSource" />
                              </th>
                              <th
                                onClick={() => handleHyperBackupSort("hyperbackupDestination")}
                                style={{ cursor: "pointer" }}
                              >
                          ...
                          <tbody>
                            {sortedHyperBackup.map((item, index) => {
                              const rawData = item.rawData || {};
                              const nasSource = rawData.hyperbackupSource
                                ? rawData.hyperbackupSource.replace(/^(NAS|SAN)-/, "")
                                : "-";
                              const nasDestination = rawData.hyperbackupDestination
                                ? rawData.hyperbackupDestination.replace(/^(NAS|SAN|DISQUE)-/, "").replace(/-\d+$/, "")
                                : "-";
        ...
        <EquipmentMappingModal
          ...
          onMappingSaved={(mapping) => {
            if (mappingModalEquipment) {
              setBackupData((prev) =>
                prev.map((item) =>
                  item.id === mappingModalEquipment.id && item.clientId === mappingModalEquipment.clientId
                    ? {
                        ...item,
                        checkmkMapping: mapping || null,
                        isMapped: !!(mapping && mapping.checkmk_host_name && mapping.is_active !== false),

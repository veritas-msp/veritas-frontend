export const WIFI_SSID_BAND_OPTIONS = [
  { value: "dual", label: "Dual (2,4 + 5 GHz)" },
...
export const WIFI_SSID_TYPE_OPTIONS = [
  { value: "prive", label: "Privé (entreprise)" },
  { value: "public", label: "Public / invité" },
];
...
export function normalizeWifiSsidEntry(entry, index = 0) {
  ...
  if (!entry || typeof entry !== "object") {
    return createWifiSsidEntry({ id: `ssid-empty-${index}` });
  }
  const nom =
    entry.nom ||
    entry.name ||
...
/** Catalogue client · conserve les lignes en cours de saisie (nom vide). */
...
/** @deprecated Utiliser normalizeWifiSsidCatalog pour l'éditeur. */
export function normalizeWifiSsidList(ssids) {
  return normalizeWifiSsidCatalog(ssids).filter(
    (entry) => entry.nom || entry.vlan
  );
}

export function serializeWifiSsidCatalogForPersistence(ssids) {
  return normalizeWifiSsidCatalog(ssids)
    .filter((entry) => entry.nom.trim())
    .map(({ id, nom, vlan, bande, type, portailCaptif }) => ({
      id,
      nom: nom.trim(),
      vlan: vlan.trim(),
      bande,
      type,
      ...(type === "public" ? { portailCaptif: !!portailCaptif } : {}),
    }));
}

export function wifiSsidCatalogsEqual(a, b) {
  return (
    JSON.stringify(serializeWifiSsidCatalogForPersistence(a || [])) ===
    JSON.stringify(serializeWifiSsidCatalogForPersistence(b || []))
  );
}
...
export function resolveAssignedSsidIds(rawAssigned, clientCatalog = []) {
  ...
  const catalogById = new Map(
    normalizeWifiSsidCatalog(clientCatalog).map((entry) => [entry.id, entry])
  );
  const catalogByNom = new Map(
    normalizeWifiSsidCatalog(clientCatalog)
      .filter((entry) => entry.nom)
      .map((entry) => [entry.nom.toLowerCase(), entry.id])
  );
...
export function buildBorneWifiSsidFormState(equipment, client) {
  ...
  normalizeWifiSsidCatalog(rawAssigned).forEach((entry) => {
    ...
    const exists = clientSsids.some(
      (ssid) => ssid.id === entry.id || ssid.nom.toLowerCase() === entry.nom.toLowerCase()
    );
    if (!exists) {
  ...
  return { clientSsids, assignedSsidIds };
}
...
function resolveSsidDisplayName(catalog, ssidId) {
  ...
  const byObject = normalized.find(
    (item) =>
      item.id === raw &&
      item.nom &&
      !isInternalSsidToken(item.nom)
  );
...
/** Persiste les SSID assignés avec leur nom pour l'affichage hors catalogue client. */
  ...
  return ids
    .map((item) => {
      if (item && typeof item === "object" && item.id) {
        ...
        if (embeddedNom && !isInternalSsidToken(embeddedNom)) {
          return { id: item.id, nom: String(embeddedNom).trim() };
        }
        const entry = getWifiSsidById(normalizedCatalog, item.id);
        if (entry?.nom) return { id: entry.id, nom: entry.nom };
        return item.id;
      }
...
export function formatAssignedSsidsDisplay(formData) {
  ...
  if (!names.length && Array.isArray(rawAssigned) && rawAssigned.length) {
    names = collectSsidDisplayNames(
      catalog,
      resolveAssignedSsidIds(rawAssigned, catalog)
    );
  }

...
export default function EquipmentDocumentsPanel({
  ...
  return <div className={embedded ? styles.embeddedRoot : styles.panel}>
      <div className={styles.toolbar}>
        <div className={styles.uploadFields}>
          <div className={styles.field}>
            ...
            <select id="equipment-doc-category" value={category} onChange={e => setCategory(e.target.value)}>
          ...
          <div className={`${styles.field} ${styles.fieldGrow}`}>
            ...
            <input id="equipment-doc-description" type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder={docCopy.descriptionPlaceholder} />
          </div>
        </div>
        ...
        <input ref={fileInputRef} type="file" className={styles.hiddenInput} onChange={handleUpload} accept=".pdf,.png,.jpg,.jpeg,.gif,.webp,.doc,.docx,.xls,.xlsx,.txt,.csv" />
      </div>
      ...
      {loading ? <p className={styles.hint}>{docCopy.loading}</p> : documents.length === 0 ? <div className={styles.empty}>
          ...
          {filteredDocuments.length === 0 ? <div className={styles.empty}>
          ...
          {filteredDocuments.map(doc => <li key={doc.id} className={styles.row}>
              ...
              <div className={styles.rowActions}>
                {IMAGE_MIMES.has(doc.mime_type) || doc.mime_type === "application/pdf" ? <button type="button" className={styles.actionBtn} onClick={() => setPreviewDoc(doc)} title={docCopy.preview}>
                    <Icon icon="mdi:eye-outline" aria-hidden />
                  </button> : null}
                <a className={styles.actionBtn} href={getEquipmentDocumentDownloadUrl(doc.id)} title={docCopy.download}>
                  <Icon icon="mdi:download" aria-hidden />
                </a>
                <button type="button" className={`${styles.actionBtn} ${styles.actionBtnDanger}`} onClick={() => handleDelete(doc)} title={docCopy.delete}>
      ...
      {previewDoc ? <div className={styles.previewOverlay} onClick={() => setPreviewDoc(null)} role="presentation">
          <div className={styles.previewModal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.previewBody}>
              {IMAGE_MIMES.has(previewDoc.mime_type) ? <img src={getEquipmentDocumentPreviewUrl(previewDoc.id)} alt={previewDoc.file_name} className={styles.previewImage} /> : <iframe title={previewDoc.file_name} src={getEquipmentDocumentPreviewUrl(previewDoc.id)} className={styles.previewFrame} />}
            </div>
          </div>
        </div> : null}

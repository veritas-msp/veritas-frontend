...
import { buildEquipmentSupervisionSummary } from "./equipmentSupervisionUtils";
...
export const EQUIPMENT_MONITOR_META = {
  critical: { label: "Critique", color: "#ef4444", soft: "rgba(239, 68, 68, 0.14)", icon: "mdi:alert-octagon" },
  warning: { label: "Warning", color: "#f59e0b", soft: "rgba(245, 158, 11, 0.14)", icon: "mdi:alert" },
  ok: { label: "OK", color: "#16a34a", soft: "rgba(34, 197, 94, 0.12)", icon: "mdi:check-circle" },
  unmapped: { label: "Non mappé", color: "#94a3b8", soft: "rgba(148, 163, 184, 0.14)", icon: "mdi:eye-off-outline" },
  neutral: { label: "Actif", color: "#2b5fab", soft: "rgba(43, 95, 171, 0.1)", icon: "mdi:check" },
  no_data: { label: "Sans données", color: "#94a3b8", soft: "rgba(148, 163, 184, 0.14)", icon: "mdi:database-off-outline" },
  manual: { label: "Manuel", color: "#64748b", soft: "rgba(100, 116, 139, 0.12)", icon: "mdi:hand-back-right" },
  offline: { label: "Hors ligne", color: "#ef4444", soft: "rgba(239, 68, 68, 0.14)", icon: "mdi:laptop-off" },
};

const PRIORITY = { critical: 0, warning: 1, offline: 1, unmapped: 2, no_data: 3 };

export function resolveEquipmentMonitorStatus(equipment, summary, options = {}) {
  const { checkmkEnabled = true, isMappable = false, isMapped = false } = options;
...
export function buildEquipmentPriorityActions(items = [], resolveStatus, { limit = 10, alertRules = null } = {}) {
  ...
  return [...(Array.isArray(items) ? items : [])]
    ...
    .filter((row) => actionable.includes(row.status))
    .filter((row) => {
...
/**
 * Points d'attention combinés (statut supervision + critères RMM / garanties / etc.)
 * en respectant les règles d'alerte globales.
 */
export function buildSupervisionTodoActions(
  items = [],
  resolveStatus,
  {
    limit = 12,
...
export function buildMonitorStatusCounts(items = [], resolveStatus, { alertRules = null } = {}) {

...
import {
  filterSupervisionIssues,
  resolveEquipmentFamilyKey,
} from "./supervisionAlertRulesConfig";

const WARRANTY_TYPES = new Set(["Firewalls", "Serveurs", "Stockage", "NAS"]);
const NETWORK_IP_TYPES = new Set(["Firewalls", "Switch", "Routeur", "Serveurs", "BorneWifi", "TOIP"]);
...
function readMaintenanceDate(equipment) {
  ...
  return getMaintenanceLicenceExpiration(licences) || null;
}

function expirationIssue(keyPrefix, labelPrefix, rawDate, { priorityExpired = 1, prioritySoon = 2 } = {}) {
...
/**
 * Analyse un périphérique et retourne les points de vigilance
 * (supervision, garanties, licences, données saisies manquantes…).
 */
export function buildEquipmentSupervisionSummary(
  equipment,
  { monitorStatus, checkmkEnabled = true, isMkMapped = false, alertRules = null } = {}
) {
  const issues = [];
  const type = equipment?.type === "NAS" ? "Stockage" : equipment?.type;
  const displayType = type || "Périphérique";
  ...
  if (WARRANTY_TYPES.has(displayType)) {
    const serverType = normalizeServerType(
      equipment?.typeServer || equipment?.rawData?.typeServer || equipment?.rawData?.type || ""
    );
    const skipWarranty = displayType === "Serveurs" && serverType === "virtuel";
    if (!skipWarranty) {
  ...
  if (displayType === "Alimentation") {
    ...
    if (batteryIssue) {
      batteryIssue.label =
        batteryIssue.key === "battery_expired" ? "Batterie à remplacer" : "Batterie à surveiller";
  ...
  if (displayType === "Ordinateurs" && isRmmManagedEquipment(equipment)) {
    ...
    if ((worstDisk?.pct ?? 0) >= 90) {
      pushIssue(issues, {
        key: "disk_critical",
        label: `Disque ${worstDisk.pct}% utilisé`,
  ...
  const displayTypeForRules = equipment?.type === "NAS" ? "Stockage" : equipment?.type;
  const filteredIssues = alertRules
    ? filterSupervisionIssues(issues, displayTypeForRules, alertRules)
    : issues;

  filteredIssues.sort((a, b) => a.priority - b.priority);

  const tone = filteredIssues.some((i) => i.tone === "bad")
    ? "bad"
    : filteredIssues.some((i) => i.tone === "warn")
      ? "warn"
      : "good";

  const warrantyStatus = WARRANTY_TYPES.has(displayType)
    ? getExpirationStatus(readWarrantyDate(equipment))
    : "unknown";
  const maintenanceStatus =
    displayType === "Firewalls" ? getExpirationStatus(readMaintenanceDate(equipment)) : "unknown";
...
export function formatExpirationChip(status, rawDate) {
  if (status === "unknown" || !rawDate) return { label: "-", tone: "neutral" };
  const formatted = formatDateFr(rawDate);
  if (status === "expired") return { label: formatted ? `Exp. ${formatted}` : "Expirée", tone: "bad" };
  if (status === "soon") return { label: formatted ? `Expire ${formatted}` : "Expire bientôt", tone: "warn" };
  return { label: formatted ? `OK · ${formatted}` : "OK", tone: "good" };
}

export function equipmentHasSupervisionIssues(summary) {
  return Boolean(summary?.issues?.length);
}

/** Dernière remontée RMM ou sync CheckMK, si disponible. */
export function resolveEquipmentLastCollectionAt(equipment, mkSummary) {
  if (isRmmManagedEquipment(equipment)) {
    ...
    if (at) return { at, source: "RMM" };
  }
  if (mkSummary?.lastSyncedAt) {
    return { at: mkSummary.lastSyncedAt, source: "CheckMK" };
  }
  return null;
}

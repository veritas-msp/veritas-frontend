      ...
      <div className={styles.activityPresetGroup} role="group" aria-label={copy?.dateRange?.aria}>
        {presets.map((row) => (
          <button
            key={row.value}
            type="button"
            className={`${styles.activityPresetBtn} ${preset === row.value ? styles.activityPresetBtnActive : ""}`}
            onClick={() => onPresetChange(row.value)}
          >
            {row.label}
          </button>
        ))}
        <button
          type="button"
          className={`${styles.activityPresetBtn} ${preset === "custom" ? styles.activityPresetBtnActive : ""}`}
          onClick={() => onPresetChange("custom")}
        >
          {copy?.dateRange?.custom || "Personnalisé"}
        </button>
      </div>

      {preset === "custom" ? (
        <div className={styles.activityCustomRange}>
          <label className={styles.activityDateField}>
            <span>{copy?.dateRange?.from || "Du"}</span>
            <input
              type="date"
              value={customStart || ""}
          ...
          <label className={styles.activityDateField}>
            <span>{copy?.dateRange?.to || "Au"}</span>
            <input
              type="date"
              value={customEnd || ""}

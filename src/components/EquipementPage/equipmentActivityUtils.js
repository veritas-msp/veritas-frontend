...
export function resolveEquipmentActivityRange({ preset = "30d", customStart = "", customEnd = "" } = {}) {
  if (preset === "custom") {
...
export function buildActivityFeedItems({ events = [], tickets = [], copy }) {
  const planningRows = (Array.isArray(events) ? events : []).map((event) => ({
    kind: "planning",
    id: `planning-${event.id}`,
    title: event.title || copy?.events?.fallbackTitle || "Événement",
    subtitle: event.type || "",
    date: event.start,
    endDate: event.end,
    raw: event,

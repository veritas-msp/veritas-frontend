import { formatDateFr, getMaintenanceLicenceExpiration } from "./constants/firewallLicenceUtils";
import {
  buildInitialFormData,
  getEquipmentFormSections,
  getFirewallTypeLabel,
...
function formatBool(value, { yes = "Oui", no = "Non" } = {}) {
  return value ? yes : no;
}
...
function formatLicences(licences) {
  if (!Array.isArray(licences) || !licences.length) return null;
  return licences
    .map((lic, index) => {
      const label = lic?.nom || `Licence ${index + 1}`;
...
function formatOtherLicences(licences) {
  if (!Array.isArray(licences) || !licences.length) return null;
...
const FIELD_FORMATTERS = {
  ...
  netbios: (f, equipment) =>
    f.netbios ||
    getRmmNetbiosName(equipment) ||
    null,
  location: (f) => f.location,
  ip: (f, eq) => {
    if (eq?.type === "Internet" && f.ipNonFixe) return "IP non fixe";
    return f.ip;
  },
  vlan: (f) => f.vlan,
  ...
  categorie: (f) => f.categorie || "Principale",
  ...
  licenceMaintenance: (f) => formatDateFr(getMaintenanceLicenceExpiration(f.licences)),
  autresLicences: (f) => formatOtherLicences(f.licences),
...
const SECTION_FIELDS = {
  ...
  licences: ["licenceMaintenance", "autresLicences"],
...
const ORDINATEUR_SECTIONS = [
  { id: "identity", label: "Identité", icon: "mdi:tag-text-outline" },
  { id: "hardware", label: "Matériel", icon: "mdi:laptop" },
  { id: "network", label: "Réseau", icon: "mdi:lan" },
  { id: "system", label: "Système", icon: "mdi:microsoft-windows" },
...
/** Poste agent RMM : inventaire live dans le panneau RMM, fiche = saisie Veritas uniquement. */
...
function mergeRmmIntoFormData(equipment, formData) {
  ...
  return {
    ...
    windowsLicenseStatus:
      formData.windowsLicenseStatus ||
      editionInfo.licenseLabel ||
      (editionInfo.licenseName ? editionInfo.licenseName : ""),
...
function shouldShowField(fieldKey, formData, equipment) {
  ...
  if (fieldKey === "adresseMac") {
    ...
    const showsMacInNetwork =
      sections.some((section) => section.id === "network") &&
      (SECTION_FIELDS.network || []).includes("mac");
    if (showsMacInNetwork) return false;
  }
  if (fieldKey === "hypervisor" && moduleKey === "Serveurs") {
    return getServerFormProfile(formData.typeServer).showHypervisor;
  }
  if (["processeur", "memoire", "stockage"].includes(fieldKey) && moduleKey === "Serveurs") {
    return getServerFormProfile(formData.typeServer).showHardware;
  }
  if (["manufacturer", "model", "serial", "expirationGarantie"].includes(fieldKey) && moduleKey === "Serveurs") {
    return getServerFormProfile(formData.typeServer).showHardware;
  }
  if (["manufacturer", "model", "serial"].includes(fieldKey) && moduleKey === "Stockage") {
  ...
  if (
    (fieldKey === "nbDisquesActuels" || fieldKey === "nbDisquesMax") &&
    moduleKey === "Stockage"
  ) {
    const profile = getStorageFormProfile(formData.storageType || formData.type);
    return !shouldShowStorageDiskBays(formData, { showDisques: profile.showDisques });
  }
  if (fieldKey === "raid" && moduleKey === "Stockage") {
    return getStorageFormProfile(formData.storageType || formData.type).showRaid;
  }
  if (fieldKey === "capacite" && moduleKey === "Stockage") {
    return getStorageFormProfile(formData.storageType || formData.type).showCapacite;
  }
  if (fieldKey === "numeroDisque" && moduleKey === "Stockage") {
    const profile = getStorageFormProfile(formData.storageType || formData.type);
    ...
    const rmmValue =
      fieldKey === "manufacturer"
        ...
        : fieldKey === "model"
          ...
          : fieldKey === "serial"
            ? chassis.serial
            : fieldKey === "editionWindows"
              ? editionInfo.edition
              : fieldKey === "windowsFeatureVersion"
                ? editionInfo.displayVersion
                : fieldKey === "windowsBuild"
                  ? editionInfo.build
                  : fieldKey === "windowsLicenseStatus"
                    ? editionInfo.licenseLabel || editionInfo.licenseName
                    : fieldKey === "systeme"
                      ? editionInfo.osCaption || inventory.systeme || inventory.os?.name
                      : fieldKey === "domaine"
                        ? inventory.domaine
                        : fieldKey === "mac"
                          ? inventory.mac || inventory.network?.mac
                          : fieldKey === "ip"
                            ? inventory.ip || inventory.network?.ip
                            : fieldKey === "processeur"
                              ? inventory.hardware?.cpu
                              : fieldKey === "memoire"
                                ? formatRmmRam(inventory)
                                : null;
...
function buildSectionFields(sectionId, formData, equipment, fieldKeys, locale) {
  ...
  keys.forEach((fieldKey) => {
    ...
    fields.push({
      ...
      label,
      value,
      mono: ["ip", "vlan", "mac", "adresseMac", "serial", "gateway", "adminUrl", "stormshieldWanUrl", "quickConnect", "windowsBuild", "netbios"].includes(
        fieldKey
      ),
...
export function buildDetailFormData(equipment, options = {}) {
  ...
  if (equipment?.type === "BorneWifi") {
    const catalog = normalizeWifiSsidCatalog(
      Array.isArray(clientSsids) ? clientSsids : merged.clientSsids || []
    );
    const rawSsids =
      equipment?.ssids ?? equipment?.rawData?.data?.ssids ?? equipment?.rawData?.ssids ?? merged.ssids ?? [];
    const { clientSsids: mergedCatalog, assignedSsidIds } = buildBorneWifiSsidFormState(
      { ...equipment, ssids: rawSsids },
      { ssids: catalog }
    );
    return {
      ...merged,
  ...
  if (equipment?.type === "Ordinateurs") {
    ...
    sections = sectionDefs.map((section) => {
      const localized = locale
        ? localizeEquipmentDetailSection(section, "Ordinateurs", sectionOptions, locale)
        : section;
      return {
        ...localized,
    ...
    sections = formSections.map((section) => {
      const localized = locale
        ? localizeEquipmentDetailSection(section, moduleKey, sectionOptions, locale)
        : section;
      return {
        id: localized.id,
...
export function getEquipmentTypeLabel(equipment) {
  ...
  if (moduleKey === "Stockage") {
    ...
    return storageType === "NAS" ? "Stockage NAS" : storageType;
  }

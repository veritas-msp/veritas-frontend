...
const HOST_STATE_COLORS = { UP: '#13BA8E', DOWN: '#ef4444', UNREACHABLE: '#f59e0b' };
...
export default function CheckMKMonitoringPanel({
  ...
  const filteredServices = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      ...
      else if (serviceSort.col === 'service') {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (eventServiceFilter) {
      list = list.filter(
        (e) => (e.service ?? e.log_service_description) === eventServiceFilter
      );
    }
    ...
    return [...list].sort((a, b) => {
      let cmp = 0;
      if (eventsSort.col === 'state') cmp = getEventStateNum(a) - getEventStateNum(b);
      else if (eventsSort.col === 'timestamp') {
    ...
    return (
      <section className={styles.panel}>
        <div className={styles.emptyMsg}>
          <Icon icon="simple-icons:checkmk" style={{ fontSize: '2.5rem', color: '#15D1A0', marginBottom: '0.75rem' }} />
          <h5 style={{ margin: '0 0 0.5rem', color: 'var(--text-primary)' }}>Équipement non mappé</h5>
          <p style={{ margin: '0 0 1rem', fontSize: '0.85rem' }}>
            Mappez cet équipement avec CheckMK pour afficher le monitoring.
          </p>
          <button type="button" className={styles.periodPillActive} style={{ border: 'none', cursor: 'pointer' }} onClick={onOpenMapping}>
            Mapper avec CheckMK
          </button>
        </div>
      </section>
  ...
  return (
    <section className={styles.panel}>
      ...
      <div className={styles.header}>
        <div className={styles.titleBlock}>
          <h2 className={styles.title}>
            ...
            {checkmkHostDetails?.alias && (
              <span className={styles.alias}>({checkmkHostDetails.alias})</span>
            )}
          </h2>
          <div className={styles.metaRow}>
            {hostState && (
              <span className={styles.hostStateBadge} style={{ background: `${hostStateColor}22`, color: hostStateColor, border: `1px solid ${hostStateColor}55` }}>
                <Icon icon={hostState === 'UP' ? 'mdi:check-circle' : 'mdi:alert-circle'} />
                {hostState}
              </span>
            )}
            {checkmkHostDetails?.labels &&
              Object.entries(checkmkHostDetails.labels).slice(0, 6).map(([k, v]) => (
        ...
        <div className={styles.gaugeWrap}>
          <div className={styles.gaugeContainer}>
            <div className={styles.gaugeTrack}>
              <div className={styles.gaugeNeedle} style={{ transform: `rotate(${gaugeRotation}deg)` }} />
            </div>
          ...
          <div className={styles.periodButtons}>
            {CHECKMK_AVAILABILITY_PERIOD_OPTIONS.map(({ value, label }) => (
              <button
                key={value}
                type="button"
                className={`${styles.periodBtn} ${checkmkAvailabilityPeriod === value ? styles.periodBtnActive : ''}`}
      ...
      {/* KPIs calculés depuis les données en base */}
      <div className={styles.kpiGrid}>
        <div className={styles.kpiCard}>
          <span className={styles.kpiLabel}>Santé services</span>
          <span className={styles.kpiValue} style={{ color: kpis.healthScore >= 90 ? '#13BA8E' : kpis.healthScore >= 70 ? '#f59e0b' : '#ef4444' }}>
        ...
        <div className={styles.kpiCard}>
          ...
          <span className={styles.kpiSub}>
            ~{kpis.eventsPerDay}/jour · <TrendBadge value={kpis.eventTrend} />{' '}
            {isCalendarFilter ? 'vs mois préc.' : 'vs période préc.'}
          </span>
        </div>
        <div className={styles.kpiCard}>
          <span className={styles.kpiLabel}>Critiques</span>
          <span className={styles.kpiValue} style={{ color: kpis.critCount > 0 ? '#ef4444' : '#13BA8E' }}>
            {kpis.critCount}
          </span>
          <span className={styles.kpiSub}>{kpis.warnCount} warnings sur la période</span>
        </div>
        <div className={styles.kpiCard}>
          <span className={styles.kpiLabel}>Disponibilité 1 mois</span>
          <span className={styles.kpiValue} style={{ color: '#15D1A0' }}>
            {kpis.avail1m != null ? `${Math.round(kpis.avail1m)}%` : '-'}
          </span>
          <span className={styles.kpiSub}>
            {kpis.availTrend != null ? (
              <>vs 3 mois : {kpis.availTrend >= 0 ? '+' : ''}{kpis.availTrend} pts</>
            ) : (
              '-'
            )}
          </span>
        ...
        <div className={styles.kpiCard}>
          <span className={styles.kpiLabel}>Service le plus actif</span>
          <span className={styles.kpiValue} style={{ fontSize: '0.95rem' }}>
            {kpis.topService ? kpis.topService.count : '-'}
          </span>
          <span className={styles.kpiSub} title={kpis.topService?.name}>
            {kpis.topService?.name ?? 'Aucun événement'}
          </span>
        </div>
        <div className={styles.kpiCard}>
      ...
      <div className={styles.gridLayout}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.toolbar}>
            ...
            <div className={styles.filterPills}>
              ...
              {SERVICE_STATE_META.slice(0, 3).map(({ state, label, color }) => (
                <button
                  key={state}
                  type="button"
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  <th onClick={() => toggleServiceSort('state')}>
                    État {serviceSort.col === 'state' ? (serviceSort.dir === 'asc' ? '▲' : '▼') : ''}
                  </th>
              ...
              <tbody>
                {filteredServices.length === 0 ? (
                  <tr>
                    <td colSpan={4} className={styles.emptyMsg}>Aucun service</td>
                  </tr>
                ) : (
                  filteredServices.map((service, idx) => {
                    ...
                    return (
                      <tr key={idx} className={styles.dataRow} onClick={() => onOpenService(service)}>
                        <td>
        ...
        <div className={styles.section}>
          <div className={styles.sectionHeader}>
            ...
            <span className={styles.sectionCount}>{filteredEvents.length} alerte{filteredEvents.length !== 1 ? 's' : ''}</span>
          </div>

          <div className={styles.periodBar}>
            <span className={styles.periodBarLabel}>Glissant :</span>
            {CHECKMK_DATE_FILTER_OPTIONS.map(({ value, label }) => (
              <button
                key={value}
                type="button"
                className={`${styles.periodPill} ${eventsDateFilter === value ? styles.periodPillActive : ''}`}
                onClick={() => { setEventsDateFilter(value); setEventsPage(1); }}
              >
                {label}
              </button>
            ))}
          ...
          <div className={styles.calendarFilterBar}>
            <div className={styles.calendarFilterRow}>
              <span className={styles.periodBarLabel}>Mois précis :</span>
              <input
                type="month"
                ...
                min="2010-01"
                title="Choisir un mois et une année"
              />
              {isCalendarFilter && (
                <button
                  type="button"
                  className={styles.calendarClearBtn}
                  onClick={() => { setEventsDateFilter('1m'); setEventsPage(1); }}
                  title="Revenir au filtre glissant"
                >
                  <Icon icon="mdi:close" />
                </button>
              )}
              {isCalendarFilter && (
                <span className={styles.calendarActiveLabel}>
            ...
            {calendarMonthOptions.length > 0 && (
              <div className={styles.calendarMonthChips}>
                <span className={styles.calendarChipsHint}>Raccourcis :</span>
                {calendarMonthOptions.map(({ key, count, label }) => (
                  <button
                    ...
                    onClick={() => { setEventsDateFilter(key); setEventsPage(1); }}
                    title={`${count} événement${count > 1 ? 's' : ''}`}
                  >
                    {label}
          ...
          <div className={styles.toolbar}>
            ...
            <div className={styles.filterPills}>
              <button
                ...
                onClick={() => { setEventStateFilter(null); setEventsPage(1); }}
              >
                Warning + Critique ({eventStateCounts.all})
              </button>
              {EVENT_STATE_META.filter((m) => m.state === 1 || m.state === 2).map(({ state, label, color }) => (
                <button
                  ...
                  onClick={() => { setEventStateFilter(eventStateFilter === state ? null : state); setEventsPage(1); }}
                >
            ...
            <div className={styles.toolbar} style={{ paddingTop: 0, borderBottom: 'none' }}>
              <div className={styles.filterPills}>
                <button
                  ...
                  onClick={() => { setEventServiceFilter(null); setEventsPage(1); }}
                >
                  Tous services
                </button>
                {eventServices.slice(0, 8).map((svc) => (
                  <button
                    ...
                    onClick={() => { setEventServiceFilter(eventServiceFilter === svc ? null : svc); setEventsPage(1); }}
                    title={svc}
                  >
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  <th onClick={() => toggleEventsSort('state')}>
                    État {eventsSort.col === 'state' ? (eventsSort.dir === 'asc' ? '▲' : '▼') : ''}
                  </th>
              ...
              <tbody>
                {paginatedEvents.length === 0 ? (
                  <tr>
                    <td colSpan={4} className={styles.emptyMsg}>Aucune alerte Warning ou Critique sur cette période</td>
                  </tr>
                ) : (
                  paginatedEvents.map((event, idx) => {
                    ...
                    return (
                      <tr key={idx} className={styles.dataRow} onClick={() => onOpenEvent(event)}>
                        <td>
                          <span className={styles.statusBadge} style={{ backgroundColor: meta.color }}>
                            {meta.label}
                          </span>

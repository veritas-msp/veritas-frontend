...
import {
  buildDiskDonutSegments,
  buildMetricSnapshot,
  buildUsageDonutSegments,
  formatDiskVolumeFooter,
  formatDiskVolumeTitle,
  formatPct,
  formatStorageGB,
  formatTemp,
  resolveInstantDiskDrives,
  toneClassForPct,
} from "./rmmMetricDashboardUtils";
import styles from "./RmmMetricHistoryPanel.module.css";

function DonutTooltipPct({ active, payload, unit = "%" }) {
...
function DonutCard({
  ...
  return (
    <article className={`${styles.donutCard} ${styles[`donutCard_${tone}`] || ""}`}>
      ...
      {!hasData ? (
        <p className={styles.donutEmpty}>{emptyHint}</p>
      ) : (
        <>
...
function KpiGauge({ label, value, hint, tone, icon }) {
  return (
    <div className={`${styles.kpiGauge} ${styles[`kpiGauge_${tone}`] || ""}`}>
...
export default function RmmInstantMetrics({ agent }) {
  ...
  return (
    <>
      <section className={styles.kpiRow} aria-label={instant.aria}>
        <KpiGauge
          label={instant.cpuCurrent}
          value={formatPct(snapshot.cpuPct)}
          tone={toneClassForPct(snapshot.cpuPct)}
          icon="mdi:chip"
        />
        <KpiGauge
          label={instant.ramCurrent}
          value={formatPct(snapshot.ramPct)}
          hint={
          ...
          tone={toneClassForPct(snapshot.ramPct, { warn: 70, critical: 90 })}
          icon="mdi:memory"
        />
        <KpiGauge
          label={interpolate(instant.disk, { drive: diskDrive })}
          value={formatPct(snapshot.diskPct)}
          tone={toneClassForPct(snapshot.diskPct, { warn: 75, critical: 90 })}
          icon="mdi:harddisk"
        />
        <KpiGauge
          label={instant.temperature}
          value={formatTemp(snapshot.tempC)}
          tone={snapshot.tempC >= 80 ? "critical" : snapshot.tempC >= 65 ? "warn" : "good"}
      ...
      <section className={styles.dashboardSection}>
        ...
        <div className={styles.donutGrid}>
          <DonutCard
            title={instant.cpuLoad}
            icon="mdi:chip"
            segments={cpuSegments}
            centerValue={formatPct(snapshot.cpuPct)}
            centerLabel={instant.used}
            tone={toneClassForPct(snapshot.cpuPct)}
            emptyHint={instant.cpuMissing}
          />
          <DonutCard
            title={instant.ramMemory}
            icon="mdi:memory"
            segments={ramSegments}
            centerValue={formatPct(snapshot.ramPct)}
            centerLabel={instant.usedF}
            footer={
              snapshot.ramUsedGB != null && snapshot.ramTotalGB != null
                ? `${formatStorageGB(snapshot.ramUsedGB)} / ${formatStorageGB(snapshot.ramTotalGB)} Go`
                : null
          ...
          />
          {instantDisks.map((disk) => (
            <DonutCard
              key={disk.label}
              title={formatDiskVolumeTitle(disk)}
              icon="mdi:harddisk"
              segments={buildDiskDonutSegments(disk)}
              centerValue={formatPct(disk.pct)}
              centerLabel={disk.pct != null ? instant.occupied : instant.capacity}
              footer={formatDiskVolumeFooter(disk)}
              tone={toneClassForPct(disk.pct, { warn: 75, critical: 90 })}
              emptyHint={interpolate(instant.diskCapacityMissing, {
                title: formatDiskVolumeTitle(disk),
              })}
            />
          ))}
          {instantDisks.length === 0 ? (
            <DonutCard
              title={interpolate(instant.disk, { drive: diskDrive })}
              icon="mdi:harddisk"
              segments={buildDiskDonutSegments({ pct: snapshot.diskPct })}
              centerValue={formatPct(snapshot.diskPct)}
              centerLabel={instant.occupied}
              tone={toneClassForPct(snapshot.diskPct, { warn: 75, critical: 90 })}
              emptyHint={instant.diskMissing}
            />
          ) : null}
        </div>
      </section>

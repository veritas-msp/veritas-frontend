...
import {
  readInternetConnectionFields,
  buildInternetSectionNavMeta,
} from "./internetConnectionUtils";
...
export const ROUTEUR_TYPE_OPTIONS = [
  { value: "Routeur", label: "Routeur", icon: "mdi:router-wireless", description: "Routeur edge, box opérateur ou appliance" },
  { value: "SD-WAN", label: "SD-WAN", icon: "mdi:wan", description: "Overlay SD-WAN, orchestrateur ou service cloud" },
];
...
const ALIMENTATION_DEPLOYMENT_TYPE_VALUES = new Set(
  ALIMENTATION_TYPE_OPTIONS.map((option) => option.value)
);
...
export const INTERNET_TYPE_OPTIONS = [
  "Fibre", "ADSL", "SDSL", "VDSL", "4G", "5G", "Satellite", "Câble", "Radio", "SD-WAN", "Autre",
];
export const INTERNET_CATEGORIE_OPTIONS = ["Principale", "Backup"];
...
export const RAID_OPTIONS = ["RAID 0", "RAID 1", "RAID 5", "RAID 6", "RAID 10", "Aucun", "Autre"];
export const SERVER_TYPE_OPTIONS = [
  { value: "physique", label: "Physique", icon: "mdi:server", description: "Bare-metal, blade ou tour" },
  { value: "virtuel", label: "Virtuel", icon: "mdi:cloud-outline", description: "Machine virtuelle on-prem ou cloud" },
];

export const SERVER_HYPERVISOR_OPTIONS = [
  "VMware ESXi",
  "Microsoft Hyper-V",
  "Proxmox VE",
  "Nutanix AHV",
...
export const FIREWALL_TYPE_OPTIONS = [
  { value: "materiel", label: "Matériel", icon: "mdi:shield-outline", description: "Appliance physique (FortiGate, Stormshield…)" },
  { value: "virtuel", label: "Virtuel", icon: "mdi:cloud-outline", description: "Machine virtuelle (FortiGate VM, vSN…)" },
  { value: "cloud", label: "Cloud", icon: "mdi:cloud-lock-outline", description: "FWaaS / pare-feu managé cloud" },
  { value: "logiciel", label: "Logiciel", icon: "mdi:application-outline", description: "Pare-feu logiciel (pfSense, OPNsense, DynFi…)" },
  { value: "autre", label: "Autre", icon: "mdi:dots-horizontal", description: "Autre mode de déploiement" },
];

export function normalizeFirewallType(value) {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return "";
  if (["materiel", "matériel", "physique", "hardware", "appliance"].includes(normalized)) return "materiel";
...
export function applyFirewallTypeChange(prev, nextType) {
  ...
  if (!profile.showSerial) next = { ...next, serial: "" };
  if (!profile.showFirmware) next = { ...next, firmware: "" };
  if (!profile.showWarranty) next = { ...next, expirationGarantie: "" };
  if (!profile.showHa) {
    next = {
      ...next,
      modeHA: false,
...
export function applyRouteurTypeChange(prev, nextType) {
  ...
  if (!profile.showFirmware) next = { ...next, firmware: "" };
  if (!profile.showWarranty) next = { ...next, expirationGarantie: "" };
  if (!profile.showMac) next = { ...next, adresseMac: "" };

  return next;
}

export function getAlimentationFormProfile(alimentationType) {
  const isPdu = String(alimentationType || "Onduleur").trim() === "PDU";
  return {
    isPdu,
    isUps: !isPdu,
    showCapaciteVA: true,
    ...
    managementLabel: isPdu ? "PDU manageable" : "Onduleur manageable",
    managementHint: isPdu
      ? "PDU avec interface web ou SNMP pour supervision des prises et consommation."
      : "Onduleur avec carte réseau ou interface web de supervision.",
    adminUrlPlaceholder: isPdu
      ? "https://192.168.1.50 ou https://pdu.example.com"
      : "https://192.168.1.51 ou interface NMC/APC",
  };
}
...
export function isToipVoipSectionVisible(toipType) {
  ...
  if (!type || type === "Téléphone IP") return false;
...
export function getToipFormProfile(toipType) {
  ...
  const isPhone = type === "Téléphone IP";
  ...
  return {
    ...
    managementLabel: isPbx
      ? "IP-PBX manageable"
      : isSbc
        ? "SBC manageable"
        : "Équipement manageable",
    managementHint: "Console web ou interface d'administration de la solution VoIP.",
    adminUrlPlaceholder: isPbx
      ? "https://pbx.exemple.fr:5001 ou https://3cx.example.com"
      : "https://192.168.1.60",
...
export function applyServerTypeChange(prev, nextType) {
  ...
  let next = { ...prev, typeServer: type };

  if (!profile.showHardware) {
    next = { ...next, manufacturer: "", model: "", serial: "", expirationGarantie: "" };
  }
  if (!profile.showHypervisor) {
    next = { ...next, hypervisor: "" };
  }
...
export const STORAGE_TYPE_OPTIONS = [
  { value: "nas", label: "NAS", icon: "mdi:nas", description: "Appliance NAS (Synology, QNAP…)" },
  { value: "san", label: "SAN", icon: "mdi:server-network-outline", description: "Baie SAN ou appliance block storage" },
  { value: "virtuel", label: "Virtuel", icon: "mdi:cloud-outline", description: "Stockage virtuel, vSAN ou VM dédiée" },
  { value: "cloud", label: "Cloud", icon: "mdi:cloud-lock-outline", description: "Stockage managé cloud (Azure, AWS…)" },
  { value: "robot", label: "Robot sauvegarde", icon: "mdi:backup-restore", description: "Robot de bandes ou librairie RDX" },
  { value: "externe", label: "Disque externe", icon: "mdi:harddisk", description: "Disque USB ou rotation externe" },
];
...
export function normalizeStorageType(value) {
  ...
  if (raw === "Stockage virtuel") return "virtuel";
  if (raw === "Stockage cloud") return "cloud";
  return lower;
}
...
const STORAGE_FORM_PROFILES = {
  ...
  externe: {
    ...
    hardwareLabel: "Matériel",
    hardwareDescription: "Marque et modèle du périphérique",
...
export function applyStorageTypeChange(prev, nextType) {
  ...
  if (!profile.showQuickConnect) {
    next = { ...next, quickConnect: "" };
  }
  if (!profile.showNumeroDisque) {
    next = { ...next, numeroDisque: "" };
  }
...
const SECTION_CONNECTION = {
  ...
  label: "Connexion",
  icon: "mdi:wan",
  description: "Fournisseur, débits, IP et contrat",
};
...
export const INTERNET_DRAFT_FORM_SECTIONS = [
  SECTION_INTERNET_TYPE,
  SECTION_INTERNET_LINK,
  SECTION_INTERNET_NETWORK,
  SECTION_INTERNET_CONTRACT,
  SECTION_INTERNET_NOTES,
];
const SECTION_HARDWARE = {
  id: "hardware",
  label: "Matériel",
  icon: "mdi:server-outline",
  description: "Marque, modèle et numéro de série",
};
const SECTION_SYSTEM = {
  id: "system",
  label: "Système",
  icon: "mdi:microsoft-windows",
  description: "OS, ressources et rôles",
};
const SECTION_STORAGE = {
  id: "storage",
  label: "Stockage",
  icon: "mdi:database-outline",
  description: "RAID, capacité et disques",
};
const SECTION_HA = {
  id: "ha",
  label: "Haute dispo",
  icon: "mdi:shield-sync-outline",
  description: "Cluster firewall et pair HA",
};
const SECTION_LICENCES = {
  id: "licences",
  label: "Licences",
  icon: "mdi:license",
  description: "Licences et dates d'expiration",
};
const SECTION_MAINTENANCE = {
  id: "maintenance",
  label: "Maintenance",
  icon: "mdi:calendar-clock-outline",
  description: "Garantie et licences",
};
...
const SECTION_MANAGEMENT = {
  ...
  description: "Manageabilité et interface d'administration",
};
...
const SECTION_WIFI = {
  ...
  label: "WiFi",
  icon: "mdi:wifi",
  description: "SSID diffusés et paramètres radio",
};
...
const SECTION_POWER = {
  ...
  label: "Puissance",
  icon: "mdi:flash",
  description: "Capacité, prises et batterie",
};
...
export const EQUIPMENT_FORM_SECTIONS_BY_MODULE = {
  ...
  Firewalls: [
    { ...SECTION_IDENTITY, description: "Nom, lieux et type de déploiement" },
    { ...SECTION_HARDWARE, description: "Marque, modèle, série et garantie" },
    { ...SECTION_NETWORK, description: "IP, VLAN et URL d'administration" },
    SECTION_HA,
    SECTION_LICENCES,
    SECTION_NOTES,
  ],
  Serveurs: [
    { ...SECTION_IDENTITY, description: "Nom, lieux et type (Physique / Virtuel)" },
    { ...SECTION_HARDWARE, description: "Marque, modèle, n° de série et garantie" },
    { ...SECTION_NETWORK, description: "Adresse IP et VLAN" },
    { ...SECTION_SYSTEM, description: "OS, ressources et rôles" },
    SECTION_REMOTE,
    SECTION_NOTES,
  ],
  Stockage: [
    { ...SECTION_IDENTITY, description: "Nom, lieux et type de stockage" },
    { ...SECTION_HARDWARE, description: "Marque, modèle et n° de série" },
    { ...SECTION_NETWORK, description: "Adresse IP et VLAN" },
    { ...SECTION_STORAGE, description: "Rôle, RAID, capacité et disques" },
    SECTION_NOTES,
  ],
  Switch: [
    { ...SECTION_IDENTITY, description: "Nom et lieux" },
    { ...SECTION_HARDWARE, description: "Marque, modèle, firmware et caractéristiques" },
    { ...SECTION_NETWORK, description: "Adresse IP, VLAN et MAC" },
    { ...SECTION_MANAGEMENT, description: "Manageabilité et interface d'administration" },
    SECTION_NOTES,
  ],
  BorneWifi: [
    { ...SECTION_IDENTITY, description: "Nom et lieux" },
    { ...SECTION_HARDWARE, description: "Marque, modèle, n° de série et firmware" },
    { ...SECTION_NETWORK, description: "Adresse IP, VLAN et MAC" },
    { ...SECTION_WIFI, description: "SSID diffusés par la borne" },
    SECTION_NOTES,
  ],
  Alimentation: [
    { ...SECTION_IDENTITY, description: "Nom, lieux et type de déploiement (Onduleur / PDU)" },
    { ...SECTION_HARDWARE, description: "Marque, modèle, n° de série, firmware et garantie" },
    { ...SECTION_POWER, description: "Capacité, prises et batterie" },
    { ...SECTION_NETWORK, description: "Adresse IP, VLAN et MAC" },
    { ...SECTION_MANAGEMENT, description: "Gestion réseau et interface web" },
    SECTION_NOTES,
  ],
  Routeur: [
    { ...SECTION_IDENTITY, description: "Nom, lieux et type (Routeur / SD-WAN)" },
    { ...SECTION_HARDWARE, description: "Marque, modèle et caractéristiques" },
    { ...SECTION_NETWORK, description: "IP, VLAN et URL d'administration" },
    SECTION_NOTES,
  ],
  TOIP: [
    { ...SECTION_IDENTITY, description: "Nom, lieux et type de déploiement VoIP" },
    { ...SECTION_HARDWARE, description: "Marque, modèle, n° de série et firmware" },
    { ...SECTION_VOIP, description: "Extensions, domaine SIP et version" },
    { ...SECTION_NETWORK, description: "Adresse IP, VLAN et MAC" },
    { ...SECTION_MANAGEMENT, description: "Console d'administration et supervision" },
    SECTION_NOTES,
  ],
  Ordinateurs: [
    { ...SECTION_IDENTITY, description: "Nom Veritas et lieu du poste" },
    { ...SECTION_HARDWARE, description: "Marque, modèle et n° de série (recommandé pour lier l'agent RMM)" },
    { ...SECTION_NETWORK, description: "Adresse IP, VLAN et MAC (recommandé pour lier l'agent RMM)" },
    { ...SECTION_SYSTEM, description: "Système d'exploitation, domaine et ressources" },
    SECTION_NOTES,
  ],
};
...
export function getEquipmentNamePlaceholder(apiType, { routeurType, serverType, storageType } = {}) {
  if (apiType === "Routeur" && normalizeRouteurType(routeurType) === "SD-WAN") {
    return "SDWAN-Siège";
  }
  if (apiType === "Serveurs") {
    return getServerFormProfile(serverType).namePlaceholder || EQUIPMENT_NAME_PLACEHOLDERS.Serveurs;
  }
  if (apiType === "NAS") {
    return getStorageFormProfile(storageType).namePlaceholder || EQUIPMENT_NAME_PLACEHOLDERS.NAS;
  }
  return EQUIPMENT_NAME_PLACEHOLDERS[apiType] || "Nom de l'équipement";
}

export function getEquipmentFormSections(moduleKey, { firewallType, routeurType, serverType, storageType, toipType } = {}) {
  ...
  if (moduleKey === "Firewalls") {
    ...
    return base
      .filter((section) => profile.sectionIds.has(section.id))
      .map((section) => {
...
export function extractEquipmentSite(equipment) {
  ...
  const layers = [
    equipment,
    equipment.data && typeof equipment.data === "object" ? equipment.data : null,
    equipment.rawData && typeof equipment.rawData === "object" ? equipment.rawData : null,
    equipment.rawData?.data && typeof equipment.rawData.data === "object"
...
function readFirewallHaPartnerName(fw) {
  return String(
    fw?.firewallHAName ||
      fw?.data?.firewallHAName ||
      fw?.rawData?.firewallHAName ||
      fw?.rawData?.data?.firewallHAName ||
      ""
  ).trim();
}
...
function findFirewallByName(catalog, partnerName, { exclude } = {}) {
...
export function resolveFirewallHaPeer(equipment, peers = []) {
  ...
  if (partnerName) {
    const forward = findFirewallByName(catalog, partnerName, { exclude: equipment });
...
function isFirewallEngagedInHa(fw) {
  const layers = [fw, fw?.data, fw?.rawData, fw?.rawData?.data].filter(
...
export function getFirewallPartnerOptions(
  client,
  currentEquipment,
  { currentPartnerName = "", peerFirewalls = [] } = {}
) {
  const currentId = currentEquipment?.id != null ? String(currentEquipment.id) : null;
  const currentRawId =
    currentEquipment?.rawData?.id != null ? String(currentEquipment.rawData.id) : null;
  const currentName = getFirewallDisplayName(currentEquipment);
  const keepPartnerName = String(currentPartnerName || "").trim();
  const firewalls = mergeFirewallPeerSources(
    Array.isArray(client?.equipements?.Firewalls) ? client.equipements.Firewalls : [],
    peerFirewalls
  );
...
export function buildInitialFormData(equipment, moduleKey, { client } = {}) {
  ...
  if (moduleKey === "Firewalls" || moduleKey === "Firewall") {
    const licences = Array.isArray(raw.licences)
      ? raw.licences
      : Array.isArray(equipment?.licences)
        ? equipment.licences
        : [];
    return {
      ...base,
  ...
  if (moduleKey === "Serveurs") {
    ...
    const remoteAccess = readServerRemoteAccess({ ...raw, ...equipment });
    return {
      ...base,
      vlan: d("vlan", ""),
  ...
  if (moduleKey === "BorneWifi") {
    const { clientSsids, assignedSsidIds } = buildBorneWifiSsidFormState(equipment, client);
    ...
    return {
      ...
      toipType: (() => {
        ...
        const hasPersistedIdentity = Boolean(
          equipment?.id || d("nom", d("name", "")).trim() || d("marque", d("fabricant", "")).trim()
        );
  ...
  if (moduleKey === "Stockage") {
    ...
    return {
      ...
      disques: Array.isArray(raw.disques)
        ? raw.disques
        : Array.isArray(d("disques"))
          ? d("disques")
          : [],
...
export function getApiType(moduleKey, equipment) {
  ...
  if (moduleKey === "Stockage") return equipment?.type === "SAN" ? "NAS" : "NAS";
  return moduleKey;
}
...
export function buildEquipmentForUpdate(clientId, moduleKey, equipment) {
  ...
  const raw = equipment?.data || equipment?.rawData || equipment;
  const row =
    equipment?.rawData?.data && equipment?.rawData?.id != null
      ? equipment.rawData
      : null;
  return {
    clientId,
...
export function buildEquipmentSectionMeta(form, moduleKey, { isPhysicalServer = false, isAddMode = false } = {}) {
  if (moduleKey === "Internet") {
    return buildInternetSectionNavMeta(form, { isAddMode, includeIdentity: true }).complete;
  }
  ...
  const meta = {
    ...
    maintenance: Boolean(
      form?.expirationGarantie?.trim() ||
      form?.stormshieldWanUrl?.trim() ||
      form?.dateBatterie?.trim() ||
      form?.capaciteVA?.trim()
    ),
  };
  ...
  if (moduleKey === "Firewalls") {
    ...
    meta.hardware = Boolean(
      form?.manufacturer?.trim() ||
      (profile.showModel && form?.model?.trim()) ||
      (profile.showSerial && form?.serial?.trim()) ||
      (profile.showFirmware && form?.firmware?.trim()) ||
      (profile.showWarranty && form?.expirationGarantie?.trim())
    );
    if (!profile.showHa) meta.ha = false;
    meta.licences = Boolean(
      Array.isArray(form?.licences) &&
      form.licences.some(
        (lic) => String(lic?.nom || "").trim() || String(lic?.expiration || "").trim()
      )
    );
    meta.network = Boolean(
      form?.ip?.trim() || form?.vlan?.trim() || form?.stormshieldWanUrl?.trim()
    );
  ...
  if (moduleKey === "Serveurs") {
    ...
    meta.system = Boolean(
      ...
      form?.memoire?.trim() ||
      form?.stockage?.trim() ||
      (profile.showHypervisor && form?.hypervisor?.trim()) ||
      (Array.isArray(form?.role) && form.role.length > 0)
    );
    meta.remote = Boolean(
      form?.remoteAccessSolution?.trim() && form?.remoteAccessId?.trim()
    );
  ...
  if (moduleKey === "Alimentation") {
    ...
    meta.power = Boolean(
      (profile.showCapaciteVA && form?.capaciteVA?.trim()) ||
      (profile.showCapaciteW && form?.capaciteW?.trim()) ||
      (profile.showNbPrises && form?.nbPrises?.trim()) ||
      (profile.showBatteryDate && form?.dateBatterie?.trim())
    );
    meta.network = Boolean(
      form?.ip?.trim() || form?.vlan?.trim() || form?.adresseMac?.trim()
    );
  ...
  if (moduleKey === "TOIP") {
    ...
    meta.voip = isToipVoipSectionVisible(form?.toipType) && Boolean(
      (profile.showExtensions && form?.nombreExtensions?.trim()) ||
      (profile.showDomainSip && form?.domaineSip?.trim())
    );
    meta.network = Boolean(
      form?.ip?.trim() || form?.vlan?.trim() || form?.adresseMac?.trim()
    );
  ...
  if (moduleKey === "BorneWifi") {
    meta.hardware = Boolean(
      ...
      form?.serial?.trim() ||
      form?.firmware?.trim() ||
      form?.alimentationPoE
    );
    meta.network = Boolean(
      form?.ip?.trim() || form?.vlan?.trim() || form?.adresseMac?.trim()
    );
    meta.wifi = Boolean(
      (Array.isArray(form?.assignedSsidIds) && form.assignedSsidIds.length > 0) ||
      (Array.isArray(form?.clientSsids) &&
        form.clientSsids.some((entry) => String(entry?.nom || "").trim()))
    );
    meta.notes = Boolean(form?.commentaire?.trim());
  }
  ...
  if (moduleKey === "Stockage") {
    ...
    meta.storage = Boolean(
      (profile.showRole && form?.role?.trim()) ||
      (profile.showRaid && form?.raid?.trim()) ||
      (profile.showCapacite && form?.capacite?.trim()) ||
      (profile.showDisques && (form?.nbDisquesActuels?.trim() || form?.nbDisquesMax?.trim())) ||
      (profile.showQuickConnect && form?.quickConnect?.trim()) ||
      (profile.showNumeroDisque && form?.numeroDisque?.trim())
    );
    meta.maintenance = false;
    meta.notes = Boolean(form?.commentaire?.trim());
  }
...
export function isEquipmentRequiredSectionIncomplete(
  form,
  moduleKey,
  sectionId,
  { isAddMode = false } = {}
  ...
  if (moduleKey === "Internet") {
    const { requiredIncomplete } = buildInternetSectionNavMeta(form, {
      isAddMode,
  ...
  if (moduleKey === "Serveurs" && sectionId === "hardware" && isAddMode) {
    const profile = getServerFormProfile(form?.typeServer);
    return profile.showHardware && !form?.manufacturer?.trim();
  }

  if (moduleKey === "Stockage" && sectionId === "hardware" && isAddMode) {
    const profile = getStorageFormProfile(form?.storageType);
    return profile.showHardware && !form?.manufacturer?.trim();
  }
...
export function validateEquipmentForm(form, moduleKey, { setActiveSection, isAddMode }) {
  ...
  if (moduleKey === "Serveurs" && isAddMode && !form?.typeServer?.trim()) {
    setActiveSection("identity");
    return "Le type de serveur est obligatoire";
  }
  ...
  if (moduleKey === "Stockage" && isAddMode && !form?.storageType?.trim()) {
    setActiveSection("identity");
    return "Le type de stockage est obligatoire";
  }
  ...
  if (moduleKey === "TOIP" && isAddMode && !form?.toipType?.trim()) {
    setActiveSection("identity");
    return "Le type d'équipement est obligatoire";
  }
  ...
  if (moduleKey === "Internet") {
    if (!form?.internetType?.trim()) {
      ...
      return "Le type de connexion est obligatoire";
    }
    ...
    if (!form?.ipNonFixe && !form?.ip?.trim()) {
      ...
      return "Renseignez une adresse IP ou indiquez « IP non fixe »";
    }
  }
  return null;
}

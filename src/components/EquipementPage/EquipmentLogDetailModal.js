...
import {
  ...
  formatLogDateTime,
  getLogActionDetails,
  getLogDetailsFallbackText,
  parseLogDetails,
} from "./equipmentLogUtils";

function LogDetailField({ label, children, className = "" }) {
  return (
    <div className={`${styles.logDetailField} ${className}`.trim()}>
...
export default function EquipmentLogDetailModal({ log, onClose }) {
  if (!log) return null;
  ...
  const userLabel = log.user_name || log.user || "Système";
  const actionLabel = log.action || meta.label;

  return (
    <div className={styles.modalOverlay} onClick={onClose}>
      <div
        className={`${styles.modalContent} ${styles.logDetailModal}`}
        onClick={(event) => event.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-labelledby="equipment-log-detail-title"
      >
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle} id="equipment-log-detail-title">
            <Icon icon={meta.icon} className={styles.modalIcon} style={{ color: meta.color }} />
            Détails du log
          </h2>
          <SmartTooltip
            as="button"
        ...
        <div className={styles.modalBody}>
          <div className={styles.logDetailActionRow}>
            <LogDetailField label="Action">
              <span
                className={styles.logDetailBadge}
          ...
          <div className={styles.logDetailSummaryGrid}>
            <LogDetailField label="Utilisateur">{userLabel}</LogDetailField>
            <LogDetailField label="Date et heure">
          ...
          {log.details ? (
            <section className={styles.logDetailSection}>
              <h3 className={styles.logDetailSectionTitle}>Détails</h3>
              {detailRows && detailRows.length > 0 ? (
                <dl className={styles.logDetailKvGrid}>
                  {detailRows.map((row) => (
                    <div
                      key={`${row.label}-${row.value}`}
                      className={`${styles.logDetailKvItem} ${row.fullWidth ? styles.logDetailKvItemFull : ""}`.trim()}
                    >
                ...
                <pre className={styles.logDetailPre}>{fallbackText}</pre>
              ) : (
                <p className={styles.logDetailEmpty}>Aucun détail supplémentaire.</p>
              )}
            </section>
          ) : null}
        </div>

...
const StepStockage = ({ form, setForm, showTypeModal, setShowTypeModal: setShowTypeModalProp }) => {
  ...
  const updateStorageBrand = (index, brand) => {
    ...
    updated[index] = {
      ...
      fabricant: brand,
      modele: "",
      ...(!isSynologyBrand(brand) ? { quickConnect: "" } : {}),
    };
    setForm((prev) => ({
      ...prev,
      equipements: { ...prev.equipements, NAS: updated },
            ...
            >
              ...
              {expandedItems.has(i) && (
                <div style={{
                  ...
                  {/* Ligne 2 : Marque / Modèle / N° Série / Garantie */}
                  {equipment.type !== 'Disque dur externe' && (
                    <div className={styles.formGrid} style={{ gridTemplateColumns: '1fr 1fr 1fr 1fr' }}>
                      <BrandModelFields
                        catalog={STORAGE_CATALOG}
                        manufacturer={equipment.fabricant}
                        model={equipment.modele}
                        manufacturerId={`nas-fabricant-${i}`}
                        modelId={`nas-modele-${i}`}
                        fieldClassName={styles.formField}
                        onManufacturerChange={(value) => updateStorageBrand(i, value)}
                      ...
                      <div className={styles.formField}>
                        <label htmlFor={`nas-garantie-${i}`}>Garantie (fin)</label>
                        <input
                          id={`nas-garantie-${i}`}
                          type="date"
                  ...
                  {equipment.type !== 'Disque dur externe' && isSynologyBrand(equipment.fabricant) && (
                    <div className={styles.formGrid} style={{ gridTemplateColumns: '1fr' }}>
                      <div className={styles.formField}>
                        <label htmlFor={`nas-quickconnect-${i}`}>QuickConnect</label>
                        <input
                          id={`nas-quickconnect-${i}`}
                          value={equipment.quickConnect || ""}
                          onChange={(e) => update(i, "quickConnect", e.target.value)}
                          placeholder="monnas.quickconnect.to"
                        />
                      </div>
                    </div>
                  )}

                  {/* Ligne 3 : Adresse IP / VLAN */}
                  {equipment.type !== 'Disque dur externe' && (
                    <div className={styles.formGrid} style={{ gridTemplateColumns: '1fr 1fr' }}>
                      <div className={styles.formField}>
                  ...
                  {equipment.type !== 'Robot de sauvegarde' && equipment.type !== 'Disque dur externe' && (
                    <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      <StorageDiskBayPicker
                        idPrefix={`nas-disk-bay-${i}`}
                        nbDisquesActuels={equipment.nbDisquesActuels}
                        nbDisquesMax={equipment.nbDisquesMax}
                        disques={equipment.disques}
                        capacite={equipment.capacite}
                        onChange={(value) => update(i, "diskBays", value)}
                      />
                      <div className={styles.formGrid} style={{ gridTemplateColumns: '1fr 1fr' }}>
                        <div className={styles.formField}>
                          <CapacityInput
                            ...
                            onChange={(value) => update(i, "capacite", value)}
                            placeholder="16000"
                          />
                        </div>
                        <div className={styles.formField}>
                          <label htmlFor={`nas-capacite-utilisee-${i}`}>Espace utilisé (GB)</label>
                          <input
                            id={`nas-capacite-utilisee-${i}`}
                            type="text"
                            inputMode="numeric"
                            pattern="[0-9]*"
                            value={equipment.capaciteUtilisee || ""}
                            onChange={(e) => update(i, "capaciteUtilisee", e.target.value)}
                            onKeyPress={(e) => {
                              if (!/[0-9]/.test(e.key)) {
                                e.preventDefault();
                              }
                  ...
                  {equipment.type !== 'Robot de sauvegarde' && equipment.type !== 'Disque dur externe' && (
                    <div className={styles.formGrid} style={{ gridTemplateColumns: '1fr 1fr' }}>
                      ...
                      <div className={styles.formField}>
                        <label htmlFor={`nas-etat-disques-${i}`}>État de santé des disques</label>
                        <input
                          id={`nas-etat-disques-${i}`}
                          placeholder="OK / Dégradé / HS (ou détail par disque)"
                          value={equipment.etatDisques || ""}
                          onChange={(e) => update(i, "etatDisques", e.target.value)}
                        />
                      </div>
                    </div>
                  )}
          ...
          >
            <div className={equipmentModalStyles.modalHeader}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <Icon icon="mdi:harddisk" className={equipmentModalStyles.modalIcon} style={{ color: '#3b82f6' }} />
            ...
            <div className={equipmentModalStyles.modalBody} style={{ padding: '1.5rem' }}>
              <div
                style={{
                  display: 'grid',

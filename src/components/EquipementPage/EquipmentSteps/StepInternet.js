...
import {
  ...
  formatInternetDebitDisplay,
  syncInternetLegacyDebit,
  buildInternetSectionNavMeta,
  getInternetConnectionTypeDef,
} from "../internetConnectionUtils";
import { showError } from "../../utils/toast";

const StepInternet = ({ form, setForm, showTypeModal, setShowTypeModal: setShowTypeModalProp }) => {
  ...
  const draftSectionNav = useMemo(
    () => buildInternetSectionNavMeta(draftConnection, { isAddMode: true, includeIdentity: false }),
    [draftConnection]
  );
  ...
  const removeConnection = (index) => {
    ...
    setForm((prev) => ({
      ...prev,
      equipements: { ...prev.equipements, Internet: updated },
    }));
  };

  const submitDraftConnection = () => {
    if (!draftConnection.type) {
      showError("Sélectionnez un type de connexion.");
      return;
    }
    if (!draftConnection.fournisseur?.trim()) {
      showError("Le fournisseur est obligatoire.");
      return;
    }
  ...
  const connectionSites = connections
    .map(conn => conn.site)
  ...
  const groupedBySite = connections.reduce((acc, conn, index) => {
    const site = conn.site || "Sans site";
    if (!acc[site]) {
      acc[site] = [];
    }
    acc[site].push({ ...conn, originalIndex: index });
    return acc;
  }, {});
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", flex: 1, minHeight: 0 }}>
        {sortedSites.length === 0 ? (
          <div style={{
            ...
            <Icon icon="mdi:web-off" width={48} height={48} color="#9ca3af" style={{ marginBottom: '1rem' }} />
            <p style={{ margin: 0, fontSize: '1rem', fontWeight: 600, color: '#1a1a1a' }}>
              Aucune connexion configurée
            </p>
            <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.875rem' }}>
              Ajoutez une connexion Internet pour commencer.
            </p>
          </div>
        ) : (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <span style={{
                      ...
                      {siteConnections.length} connexion{siteConnections.length > 1 ? 's' : ''}
                    </span>
                  </div>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {siteConnections.length === 0 ? (
                    <div style={{
                      padding: '2rem',
                      textAlign: 'center',
                      background: '#f9fafb',
                      ...
                      Aucune connexion dans ce site. Glissez-déposez une connexion ici.
                    </div>
                  ) : (
                    siteConnections.map((connection, siteIndex) => {
                            ...
                            <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                              ...
                              <div style={{ flex: 1 }}>
                                <div style={{
                                  ...
                                  {connection.type} - {connection.fournisseur || `Connexion ${i + 1}`}
                                </div>
                                <div style={{
                                  fontSize: '0.8rem',
                                  color: '#6b7280',
                                  ...
                                  {formatInternetDebitDisplay(connection) && (
                                    <span>• {formatInternetDebitDisplay(connection)}</span>
                                  )}
                          ...
                          {isExpanded && (
                            <div style={{
                              ...
                              <InternetConnectionFields
                                ...
                                idPrefix={`internet-edit-${i}`}
                                showSite
                                sites={allAvailableSites.filter((site) => site !== "Sans site")}
                              />
                            </div>
                          )}
                        </motion.div>
      ...
      {/* Modal de sélection du type de connexion */}
      {showTypeModalState && (
        <div className={equipmentModalStyles.modalOverlay} onClick={() => setShowTypeModalState(false)}>
          <div
            className={internetFormStyles.internetFormShell}
            onClick={(e) => e.stopPropagation()}
          >
            <div className={equipmentModalStyles.modalHeader}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                <Icon icon="mdi:web" className={equipmentModalStyles.modalIcon} style={{ color: "#3b82f6" }} />
                <h3 style={{ margin: 0 }}>Ajouter une connexion Internet</h3>
              </div>
              <button
            ...
            <div className={internetFormStyles.internetFormBody}>
              <nav className={internetFormStyles.internetFormNav} aria-label="Sections du formulaire">
                {INTERNET_DRAFT_FORM_SECTIONS.map((section) => (
                  <button
                    key={section.id}
                    type="button"
                    className={`${formStyles.navItem} ${
                      activeDraftSection === section.id ? formStyles.navItemActive : ""
                    }`}
                    onClick={() => setActiveDraftSection(section.id)}
                    aria-current={activeDraftSection === section.id ? "step" : undefined}
                  >
                    ...
                    <span className={formStyles.navItemText}>
                      <span
                        className={`${formStyles.navItemLabel} ${
                          draftSectionNav.requiredIncomplete[section.id]
                    ...
                    {draftSectionNav.complete[section.id] ? (
              ...
              <div className={internetFormStyles.internetFormContent}>
                {INTERNET_DRAFT_FORM_SECTIONS.filter((s) => s.id === activeDraftSection).map((section) => (
                  <div key={section.id}>
                    ...
                    {section.id === "internetType" ? (
                      <InternetTypePicker
                        value={draftConnection.type ?? ""}
                        onChange={(nextType) =>
                          setDraftConnection((prev) => ({ ...prev, type: nextType }))
                        }
                      />
                    ) : (
                      <InternetConnectionFields
                        values={draftConnection}
                        onChange={setDraftConnection}
                        idPrefix="internet-draft"
                        section={section.id}
                        showSite={section.id === "internetLink"}
                        sites={Array.isArray(form.sites) ? form.sites : []}
                      />
                    )}
                  </div>
                ))}
            ...
            <div className={equipmentModalStyles.modalFooter}>
              <button
                type="button"
                className={equipmentModalStyles.secondaryButton}
                onClick={() => setShowTypeModalState(false)}
              >
                Annuler
              </button>
              <button
                type="button"
                className={equipmentModalStyles.primaryButton}
                onClick={submitDraftConnection}
              >

/** Marques et modèles prédéfinis pour la création / édition d'équipements. */

export const FIREWALL_MODEL_OTHER = "__other__";

export const FIREWALL_BRAND_META = {
  Stormshield: { label: "Stormshield" },
  Fortinet: { label: "Fortinet" },
  PaloAltoNetworks: { label: "Palo Alto" },
  CheckPoint: { label: "Check Point" },
  Sophos: { label: "Sophos" },
  WatchGuard: { label: "WatchGuard" },
  Cisco: { label: "Cisco" },
  SonicWall: { label: "SonicWall" },
...
export const FIREWALL_CATALOG_VIRTUEL = {
  ...
  CheckPoint: [
    "Security Gateway VM",
    "VSX",
    "Quantum Spark 1600 Virtual",
    "Quantum Spark 1800 Virtual",
  ],
  Sophos: [
    "XGS Virtual",
    "SFOS Virtual",
...
/** @deprecated Préférer getFirewallCatalogByDeploymentType · alias matériel */
export const FIREWALL_CATALOG = FIREWALL_CATALOG_MATERIEL;

const ALL_FIREWALL_CATALOG_BRANDS = new Set([
  ...Object.keys(FIREWALL_CATALOG_MATERIEL),
  ...Object.keys(FIREWALL_CATALOG_VIRTUEL),
  ...Object.keys(FIREWALL_CATALOG_CLOUD),
  ...Object.keys(FIREWALL_CATALOG_LOGICIEL),
]);

export function normalizeFirewallDeploymentType(value) {
  const normalized = String(value || "materiel").trim().toLowerCase();
  if (["materiel", "matériel", "physique", "hardware", "appliance"].includes(normalized)) {
    return "materiel";
  }
...
export const ROUTER_BRAND_META = {
  ...
  Sophos: { label: "Sophos" },
  WatchGuard: { label: "WatchGuard" },
  Peplink: { label: "Peplink" },
  VMware: { label: "VMware" },
  Zscaler: { label: "Zscaler" },
  MicrosoftAzure: { label: "Azure" },
  AWS: { label: "AWS" },
  Cloudflare: { label: "Cloudflare" },
  Cradlepoint: { label: "Cradlepoint" },
  Huawei: { label: "Huawei" },
};
...
const ALL_ROUTER_CATALOG_BRANDS = new Set([
  ...Object.keys(ROUTER_CATALOG_ROUTEUR),
  ...Object.keys(ROUTER_CATALOG_SDWAN),
]);
...
export function getRouterCatalogByType(routeurType) {
  return normalizeRouteurType(routeurType) === "SD-WAN"
    ? ROUTER_CATALOG_SDWAN
    : ROUTER_CATALOG_ROUTEUR;
}
...
export function applyRouterTypeChange(prev, nextType) {
  ...
  if (!currentBrand) {
    return { ...prev, routeurType: nextType };
  }

  if (!Object.keys(nextCatalog).includes(currentBrand)) {
    if (ALL_ROUTER_CATALOG_BRANDS.has(currentBrand)) {
      return { ...prev, routeurType: nextType, manufacturer: "", model: "" };
    }
    return { ...prev, routeurType: nextType, model: "" };
  }
...
export function applyFirewallDeploymentTypeChange(prev, nextType) {
  ...
  if (!currentBrand) {
    return { ...prev, firewallType: nextType };
  }

  if (!Object.keys(nextCatalog).includes(currentBrand)) {
    if (ALL_FIREWALL_CATALOG_BRANDS.has(currentBrand)) {
      return { ...prev, firewallType: nextType, manufacturer: "", model: "" };
    }
    return { ...prev, firewallType: nextType, model: "" };
  }
...
export const STORAGE_BRAND_META = {
  ...
  QNAP: { label: "QNAP" },
  Asustor: { label: "Asustor" },
  WesternDigital: { label: "Western Digital" },
  Netgear: { label: "Netgear" },
  FUJITSU: { label: "Fujitsu" },
  Dell: { label: "Dell EMC" },
  HPE: { label: "HPE" },
  NetApp: { label: "NetApp" },
  PureStorage: { label: "Pure Storage" },
  IBM: { label: "IBM" },
  VMware: { label: "VMware" },
  MicrosoftAzure: { label: "Azure" },
  AWS: { label: "AWS" },
  ...
  OverlandTandberg: { label: "Overland-Tandberg" },
  Wasabi: { label: "Wasabi" },
  OVHcloud: { label: "OVHcloud" },
};
...
/** @deprecated Préférer getStorageCatalogByType */
...
/** Modèles Ubiquiti éligibles à l'API locale UniFi (bouton actions) */
...
export const ALIMENTATION_BRAND_META = {
  APC: { label: "APC" },
  Eaton: { label: "Eaton" },
  CyberPower: { label: "CyberPower" },
  Schneider: { label: "Schneider Electric" },
  Legrand: { label: "Legrand" },
  Vertiv: { label: "Vertiv" },
  Raritan: { label: "Raritan" },
};
...
export function getAlimentationCatalogByType(alimentationType) {
  return String(alimentationType || "Onduleur").trim() === "PDU"
    ? ALIMENTATION_PDU_CATALOG
    : ALIMENTATION_UPS_CATALOG;
}

export const TOIP_BRAND_META = {
  "3CX": { label: "3CX" },
  Yeastar: { label: "Yeastar" },
  Avaya: { label: "Avaya" },
  Cisco: { label: "Cisco" },
  Grandstream: { label: "Grandstream" },
  Mitel: { label: "Mitel" },
  AudioCodes: { label: "AudioCodes" },
  Patton: { label: "Patton" },
  Ribbon: { label: "Ribbon" },
  Yealink: { label: "Yealink" },
  Poly: { label: "Poly" },
  Fanvil: { label: "Fanvil" },
};
...
export function getToipCatalogByType(toipType) {
  ...
  if (type === "Téléphone IP") return TOIP_CATALOG_PHONE;

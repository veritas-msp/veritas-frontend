...
export const SERVER_REMOTE_ACCESS_SOLUTIONS = [
  ...
  },
  {
    ...
    placeholder: "Référence ou identifiant",
    idLabel: "Identifiant",
    supportsLaunch: false,
  },
];
...
export function buildServerRemoteAccessLaunchTargets(solution, rawId) {
  ...
  switch (key) {
    ...
    case "teamviewer":
      return [{ url: `teamviewer10://control?device=${encodeURIComponent(compact)}` }];
    case "rustdesk":
      return [{ url: `rustdesk://${encodeURIComponent(compact)}` }];
    case "rdp": {
...
export function getServerRemoteAccessTooltip(equipment) {
  const { solution, id } = readServerRemoteAccess(equipment);
  if (!id) return "Prise en main non configurée";
...
export function buildServerRemoteAccessApiFields(formData) {
  const { solution, id } = readServerRemoteAccessFromForm(formData);
  return {
...
export function getServerRemoteAccessIcon(equipment) {
  const { solution } = readServerRemoteAccess(equipment);

...
export function openQuickConnectUrl(equipment) {
  ...
  if (!url) {
    void logEquipmentQuickConnectAttempt(equipment, { url: null, ok: false, reason: "missing" });
    return { ok: false, reason: "missing" };
  }
  try {
    window.open(url, "_blank", "noopener,noreferrer");
    void logEquipmentQuickConnectAttempt(equipment, { url, ok: true, reason: "opened" });
    return { ok: true, reason: "opened" };
  } catch {
    void logEquipmentQuickConnectAttempt(equipment, { url, ok: false, reason: "failed" });

...
import {
  WIFI_SSID_BAND_OPTIONS,
  WIFI_SSID_TYPE_OPTIONS,
  createWifiSsidEntry,
  getWifiSsidById,
...
export default function WifiApSsidEditor({
  ...
  const updateCatalogRow = (id, patch) => {
    updateCatalog(
      catalog.map((row) => (row.id === id ? { ...row, ...patch } : row))
    );
  };
  ...
  const selectedNames = assignedSsidIds
    .map((id) => getWifiSsidById(catalog, id)?.nom)
    .filter(Boolean);
                ...
                <div key={row.id} className={styles.card}>
                  <div className={styles.cardHead}>
                    <span className={styles.cardIndex}>
                      {interpolate(copy.ssidIndex, { index: index + 1 })}
                    </span>
                    <button
                      type="button"
                      className={styles.removeButton}
                      onClick={() => removeCatalogRow(row.id)}
                      aria-label={interpolate(copy.removeSsidAria, { index: index + 1 })}
                    >
                  ...
                  <div className={styles.grid}>
                    <div className={`${formStyles.field} ${styles.fieldWide}`}>
                      ...
                      <input
                        ...
                        value={row.nom}
                        onChange={(e) => updateCatalogRow(row.id, { nom: e.target.value })}
                    ...
                    <div className={formStyles.field}>
                      ...
                      <input
                        ...
                        className={formStyles.input}
                        value={row.vlan}
                        onChange={(e) => updateCatalogRow(row.id, { vlan: e.target.value })}
                    ...
                    <div className={formStyles.field}>
                      ...
                      <select
                        ...
                        className={formStyles.input}
                        value={row.bande}
                        onChange={(e) => updateCatalogRow(row.id, { bande: e.target.value })}
                      >
                        {WIFI_SSID_BAND_OPTIONS.map(({ value }) => (
                    ...
                    <div className={formStyles.field}>
                      ...
                      >
                        {WIFI_SSID_TYPE_OPTIONS.map(({ value }) => (
                          <option key={value} value={value}>
                        ...
                        <label className={formStyles.slaToggle} htmlFor={`${rowId}-portail`}>
                          ...
                          <span className={formStyles.switchWrap}>
                            <input
                              ...
                              checked={!!row.portailCaptif}
                              onChange={(e) =>
                                updateCatalogRow(row.id, { portailCaptif: e.target.checked })
                              }
                            />
          ...
          <div className={styles.assignList}>
            {broadcastable.map((entry) => {
              ...
              return (
                <label
                  key={entry.id}
                  className={`${styles.assignItem} ${checked ? styles.assignItemActive : ""}`}
                >
                  <input
                    type="checkbox"
                    className={styles.assignCheckbox}
                    checked={checked}
                    onChange={() => toggleAssigned(entry.id)}
                  />
                  <span className={styles.assignContent}>
                    ...
                    <span className={styles.assignMeta}>
                      {[
                        entry.vlan
                          ? interpolate(copy.vlanMeta, { vlan: entry.vlan })
                          : null,
                        getWifiBandLabel(locale, entry.bande),
                        entry.type === "public" ? copy.typePublic : copy.typePrivate,
                      ]
                        .filter(Boolean)
                        .join(" · ")}
                    </span>
                  ...
                  {checked && (
                    <Icon icon="mdi:wifi" className={styles.assignWifiIcon} aria-hidden />
                  )}
                </label>
              );
            })}

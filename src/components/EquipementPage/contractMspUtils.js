const STATUS_RANK = { expired: 0, suspended: 1, expiring: 2 };

export function buildContractFleetRows(alerts = [], licenseAlerts = [], options = {}) {
  const { licenseModuleSections = [], contractType = {} } = options;
  const rows = [];
  ...
  for (const alert of Array.isArray(alerts) ? alerts : []) {
    rows.push({
      ...
      type: contractType.enterprise || "Contrat entreprise",
      typeKey: "contract",
      categoryId: "contract",
      categoryLabel: contractType.enterprise || "Contrat entreprise",
      status: alert.status,
  ...
  for (const alert of Array.isArray(licenseAlerts) ? licenseAlerts : []) {
    const typeLabel =
      alert.moduleLabel ||
      licenseModuleSections.find((section) => section.module === alert.module)?.label ||
      alert.module ||
...
export function buildContractFleetStats(rows = []) {
  ...
  const statusCounts = { expired: 0, expiring: 0, suspended: 0 };
  ...
  const healthScore =
    list.length === 0
      ...
      : Math.max(
          ...
          Math.round(
            100 -
              ((statusCounts.expired * 3 + statusCounts.suspended * 2 + statusCounts.expiring) /
                Math.max(list.length * 3, 1)) *
                100
          )
        );
...
export function filterContractFleetRows(rows = [], { search = "", statusFilter = "all", categoryFilter = "all" } = {}) {
  const query = String(search || "").trim().toLowerCase();
  return (Array.isArray(rows) ? rows : []).filter((row) => {
    if (statusFilter !== "all" && row.status !== statusFilter) return false;
    if (categoryFilter !== "all" && row.categoryId !== categoryFilter) return false;
    if (!query) return true;
    const haystack = [row.name, row.clientName, row.subtitle, row.type, row.categoryLabel]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
...
export function sortContractFleetRows(rows = [], sortBy = "expiration", sortDirection = "asc") {
  ...
  return [...(Array.isArray(rows) ? rows : [])].sort((a, b) => {
    ...
    return String(va).localeCompare(String(vb), "fr", { numeric: true }) * direction;
  });
}
...
export function groupContractFleetByCategory(rows = []) {
  ...
  return Array.from(groups.values()).sort((a, b) =>
    a.categoryLabel.localeCompare(b.categoryLabel, "fr")
  );
}

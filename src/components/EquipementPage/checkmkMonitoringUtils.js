/** Utilitaires CheckMK · KPIs et filtrage à partir des données persistées */

export const CHECKMK_DATE_FILTER_OPTIONS = [
  { value: '1m', label: '30 jours', shortLabel: '1 mois' },
  { value: '3m', label: '3 mois', shortLabel: '3 mois' },
  { value: '6m', label: '6 mois', shortLabel: '6 mois' },
  { value: '1y', label: '1 an', shortLabel: '1 an' },
  { value: '10y', label: 'Tout', shortLabel: 'Tout' },
];
...
export const SERVICE_STATE_META = [
  { state: 0, label: 'OK', color: '#13BA8E', icon: 'mdi:check-circle' },
  { state: 1, label: 'Warning', color: '#f59e0b', icon: 'mdi:alert' },
  { state: 2, label: 'Critique', color: '#ef4444', icon: 'mdi:alert-circle' },
  { state: 3, label: 'Inconnu', color: '#6b7280', icon: 'mdi:help-circle' },
];

export const EVENT_STATE_META = [
  { state: 0, label: 'OK', color: '#13BA8E' },
  { state: 1, label: 'Warning', color: '#f59e0b' },
  { state: 2, label: 'Critique', color: '#ef4444' },
  { state: 3, label: 'Inconnu', color: '#6b7280' },
];
...
/** Filtre calendaire au format YYYY-MM (ex. 2024-01 = janvier 2024) */
...
export function getCalendarMonthRange(filterKey) {
  ...
  return { startMs, endMs, year, month };
}

export function getPreviousCalendarMonthKey(filterKey) {
  const { year, month } = getCalendarMonthRange(filterKey);
  const prevMonth = month === 1 ? 12 : month - 1;
  const prevYear = month === 1 ? year - 1 : year;
  return `${prevYear}-${String(prevMonth).padStart(2, '0')}`;
}

export function formatCalendarFilterLabel(filterKey, { capitalize = true } = {}) {
...
/** Mois distincts présents dans les alertes warn/crit (pour raccourcis) */
...
export function getPeriodDurationMs(filterKey) {
  if (isCalendarMonthFilter(filterKey)) {
    const { startMs, endMs } = getCalendarMonthRange(filterKey);
...
/** Warning (1) ou Critique (2) uniquement · pas les OK */
...
export function computeCheckMKKpis({ services = [], events = [], availabilityByPeriod = {}, eventsDateFilter = '1m' }) {
  ...
  if (isCalendarMonthFilter(eventsDateFilter)) {
    eventsPrevPeriod = getCheckMKEventsInDateRange(
      alertEvents,
      getPreviousCalendarMonthKey(eventsDateFilter),
      nowMs
    );
  } else {
  ...
  const availTrend =
    avail1m != null && avail3m != null ? Math.round((avail1m - avail3m) * 10) / 10 : null;

  const eventTrend =
    eventsPrevPeriod.length > 0
      ? Math.round(((eventsInPeriod.length - eventsPrevPeriod.length) / eventsPrevPeriod.length) * 100)
      : eventsInPeriod.length > 0
        ? 100
        : 0;

  const serviceEventCounts = eventsInPeriod.reduce((acc, e) => {
    const svc = e.service ?? e.log_service_description ?? 'Hôte';
    if (!svc) return acc;
    acc[svc] = (acc[svc] || 0) + 1;
    return acc;
  }, {});

...
import {
  SERVER_REMOTE_ACCESS_SOLUTIONS,
  getServerRemoteAccessSolutionDef,
  normalizeRemoteAccessSolution,
  openServerRemoteAccess,
  readServerRemoteAccessFromForm,
...
export default function ServerRemoteAccessFields({
  ...
  const activeSolution =
    SERVER_REMOTE_ACCESS_SOLUTIONS.find((item) => item.value === normalizedSolution)?.value || "";
  ...
  const handleTestLaunch = () => {
    openServerRemoteAccess(
      readServerRemoteAccessFromForm({ remoteAccessSolution: activeSolution, remoteAccessId })
    );
  };
  ...
  return (
    <div className={formStyles.fieldFull} style={{ gridColumn: "1 / -1" }}>
      ...
      <div className={formStyles.modulesGrid} style={{ marginTop: "0.45rem" }}>
        {SERVER_REMOTE_ACCESS_SOLUTIONS.map(({ value, label, icon, description, placeholder }) => {
          ...
          return (
            <button
              ...
              className={`${formStyles.moduleTile} ${
                activeSolution === value ? formStyles.moduleTileActive : ""
              }`}
              onClick={() => onSolutionChange(value)}
              aria-pressed={activeSolution === value}
              title={localized.description || description}
            >
      ...
      {activeSolution && (
        <div className={formStyles.field} style={{ marginTop: "0.85rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.55rem", alignItems: "stretch" }}>
            <input
              id="server-remote-access-id"
              type="text"
              className={formStyles.input}
              style={{ flex: 1 }}
              value={remoteAccessId ?? ""}
              onChange={(e) => onIdChange(e.target.value)}
              placeholder={solutionDef?.placeholder || widgetsCopy.identifier || "Identifiant"}
              autoFocus={!remoteAccessId}
            />
            {canTestLaunch && (
              <button
                type="button"
                ...
                title={
                  solutionDef?.supportsLaunch
                    ? widgetsCopy.testLaunch || "Tester la connexion (ouvre le client)"
                    : widgetsCopy.copyId || "Copier l'identifiant"
                }
              >
                {solutionDef?.supportsLaunch
                  ? widgetsCopy.test || "Tester"
                  : widgetsCopy.copy || "Copier"}
              </button>
            )}
          </div>
        </div>

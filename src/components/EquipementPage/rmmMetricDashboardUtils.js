...
export function formatMetricPeriodLabel(days) {
  ...
  return days === 1 ? `${label} · jour en cours` : `${label} · répartition des jours`;
}
...
export function listInventoryDrives(agent) {
  ...
  const pushDrive = (label, { sizeGB, freeGB, volumeName = null } = {}) => {
    ...
    const usedGB =
      rawSize != null && rawFree != null
        ? roundStorageGB(Math.max(0, rawSize - rawFree))
        : null;
    const pct =
      rawSize != null && rawSize > 0 && rawFree != null
        ? Math.round(((rawSize - rawFree) / rawSize) * 100)
        : null;
    drives.push({
      label,
  ...
  if (Array.isArray(mapped)) {
    for (const drive of mapped) {
      ...
      pushDrive(label, {
        ...
        volumeName: remote ? `Réseau · ${remote}` : "Réseau",
      });
    }
  }
...
/** Volumes affichés dans l'instantané : inventaire agent + métrique disque principal si besoin. */
export function resolveInstantDiskDrives(snapshot) {
  ...
  const mergeAgentDrive = (label, pct) => {
    ...
    if (existingIndex >= 0) {
      if (drives[existingIndex].pct == null) {
        drives[existingIndex] = { ...drives[existingIndex], pct: Number(pct) };
      }
      return;
    }
    drives.push({
  ...
  return drives
    .filter((drive) => drive.pct != null || drive.sizeGB != null)
...
export function buildUsageDonutSegments(usedPct, { usedLabel = "Utilisé", freeLabel = "Libre" } = {}) {
  const used = Math.max(0, Math.min(100, Number(usedPct) || 0));
  const free = Math.max(0, 100 - used);
  return [
    { name: usedLabel, value: used, color: ZONE_COLORS.used },
    { name: freeLabel, value: free, color: ZONE_COLORS.free },
  ];
}
...
export function buildDiskDonutSegments(disk) {
  ...
  return [
    { name: "Occupé", value: used, color: ZONE_COLORS.diskUsed },
    { name: "Libre", value: 100 - used, color: ZONE_COLORS.diskFree },
  ];
}

export function resolvePrimaryDiskHistory(disks) {
  if (!disks || typeof disks !== "object") return { drive: null, history: null };
  const keys = Object.keys(disks);
  if (!keys.length) return { drive: null, history: null };
  const preferred = keys.find((key) => /^C:/i.test(key)) || keys[0];
  return { drive: preferred, history: disks[preferred] || null };
}

export function computeLoadZoneDistribution(
  points,
  { warn = 60, critical = 85 } = {}
) {
  ...
  for (const point of rows) {
    ...
    if (value >= critical) criticalCount += 1;
    else if (value >= warn) elevated += 1;
    else normal += 1;
  }

  return [
    { name: "Normal", value: normal, color: ZONE_COLORS.normal },
    { name: "Élevé", value: elevated, color: ZONE_COLORS.elevated },
    { name: "Critique", value: criticalCount, color: ZONE_COLORS.critical },
...
export function computeUpdatesDayDistribution(points) {
  ...
  return [
    { name: "Sans MAJ en attente", value: okDays, color: ZONE_COLORS.updatesOk },
    { name: "MAJ en attente", value: pendingDays, color: ZONE_COLORS.updatesPending },
  ].filter((item) => item.value > 0);
}
...
export function computeTemperatureZoneDistribution(points) {
  ...
  for (const point of rows) {
    ...
    if (value >= 80) hot += 1;
    else if (value >= 65) warm += 1;
    else cool += 1;
  }

  return [
    { name: "< 65 °C", value: cool, color: ZONE_COLORS.tempCool },
    { name: "65–79 °C", value: warm, color: ZONE_COLORS.tempWarm },
    { name: "≥ 80 °C", value: hot, color: ZONE_COLORS.tempHot },
  ].filter((item) => item.value > 0);
}

export function summarizeHistoryPoints(points) {
  const values = (Array.isArray(points) ? points : [])
    .map((point) => Number(point?.last))
...
export function toneClassForPct(pct, { warn = 60, critical = 85 } = {}) {

...
export function normalizeDiskVolumes(disques, activeCount, maxBays) {
  ...
  return Array.from({ length: safeActive }, (_, index) => ({
    capacite: sanitizeDiskCapacity(source[index]?.capacite),
  }));
}

export function buildDiskBayState({ nbDisquesActuels, nbDisquesMax, disques, capacite }) {
  const parsedMax = parseDiskCount(nbDisquesMax);
  const parsedActive = parseDiskCount(nbDisquesActuels);
  const maxBays = Math.min(
    ABSOLUTE_MAX_BAYS,
    Math.max(DEFAULT_MAX_BAYS, parsedMax, parsedActive)
  );
  const activeCount = Math.min(parsedActive, maxBays);
  const normalizedDisques = normalizeDiskVolumes(disques, activeCount, maxBays);
  const summedCapacity = normalizedDisques.reduce(
    (total, disk) => total + (parseInt(disk.capacite, 10) || 0),
    0
  );
...
export function shouldShowStorageDiskBays(formData, { showDisques = false } = {}) {
  ...
  const hasCounts =
    parseDiskCount(formData.nbDisquesActuels) > 0 || parseDiskCount(formData.nbDisquesMax) > 0;
  const hasDiskVolumes =
    Array.isArray(formData.disques) &&
    formData.disques.some((disk) => sanitizeDiskCapacity(disk?.capacite));

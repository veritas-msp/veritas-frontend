...
export default function SiteSuggestInput({
  ...
  const filteredSites = useMemo(() => {
    ...
    return uniqueSites
      .filter((site) => normalizeSiteQuery(site).includes(query))
      .slice(0, 25);
  }, [uniqueSites, value]);
      ...
      >
        {allowEmpty ? (
          <button
            ...
            className={`${formStyles.dropdownOption} ${
              !String(value || "").trim() ? formStyles.dropdownOptionSelected : ""
            }`}
        ...
        {filteredSites.length === 0 ? (
          <div className={formStyles.dropdownEmpty}>
            {String(value || "").trim()
              ? "Aucun lieu correspondant · la saisie libre est conservée"
              : "Aucun lieu enregistré pour ce client"}
          </div>
        ) : (
          filteredSites.map((site) => (
            <button
              ...
              type="button"
              className={`${formStyles.dropdownOption} ${
                String(value || "").trim() === site ? formStyles.dropdownOptionSelected : ""
              }`}
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => {
                onChange(site);

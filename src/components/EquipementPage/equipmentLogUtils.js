...
export function buildLogDetailRows(parsed, rawDetails) {
  ...
  if (kind === "rmm_agent_update") {
    const eventLabel = parsed.event === "install" ? "Installation" : parsed.event === "re_enroll" ? "Re-enrollment" : parsed.event === "update" ? "Update" : parsed.event;
    return [detailRow("Previous version", parsed.previousVersion), detailRow("New version", parsed.newVersion), detailRow("Event", eventLabel), detailRow("Source", parsed.source), detailRow("Poste", parsed.hostname)].filter(Boolean);
  }

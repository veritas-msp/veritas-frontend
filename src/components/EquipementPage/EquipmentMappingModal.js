/**
 * Modal de mapping CheckMK pour un périphérique
 * - Hosts catégorisés (Serveurs, Firewalls, Switch, WiFi, Stockage, Autres)
 * - Infos utiles sur chaque host (alias, IP)
 * - Suggestions basées sur numéro client, nom client, nom équipement
 */
...
const EQUIPMENT_TYPE_TO_FAMILY = {
  'Serveurs': 'servers', 'Stockage': 'stockage', 'NAS': 'stockage',
  'Firewalls': 'firewall', 'Switch': 'switch', 'BorneWifi': 'wifi',
  'Alimentation': 'alimentation', 'Routeur': 'routeur', 'TOIP': 'toip',
  'Sauvegarde': 'save', 'Internet': 'internet'
};

const CATEGORY_ORDER = ['Serveurs', 'Firewalls', 'Routeur', 'Switch', 'BorneWifi', 'Alimentation', 'TOIP', 'Stockage', 'Autres'];
...
/** Score de suggestion : numéro client, nom client, nom équipement */
function scoreSuggestion(host, equipmentName, clientName, clientId) {
  ...
  // Numéro client (ex: clientId 42 → "42", "042", "client-42")
  if (cid) {
  ...
  if (cl) {
    ...
    clientWords.forEach(w => { if (h.includes(w)) score += 15; });
  ...
  if (eq) {
    ...
    eqWords.forEach(w => { if (h.includes(w)) score += 12; });
...
export default function EquipmentMappingModal({ isOpen, onClose, equipment, onMappingSaved, requireService = false }) {
  ...
  const loadHosts = async () => {
    ...
    try {
      ...
      toast.error('Impossible de charger les hôtes CheckMK');
      setHosts([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadServices = async (hostName, serviceToSelect = "") => {
    ...
    try {
      ...
      if (serviceToSelect && serviceToSelect.trim()) {
        const found = serviceNames.find(
          (s) => s.toLowerCase() === serviceToSelect.trim().toLowerCase()
        );
      ...
      console.error("Erreur chargement services CheckMK:", err);
      toast.error("Impossible de charger les services CheckMK");
      setServices([]);
      setSelectedService("");
    } finally {
  ...
  const suggestions = useMemo(() => {
    ...
    return [...hosts]
      .map(h => ({ host: h, score: scoreSuggestion(h, equipmentName, clientName, clientId) }))
      .filter(s => s.score > 0)
  ...
  const handleSave = async () => {
    if (!selectedHost.trim()) {
      toast.error('Veuillez sélectionner un host CheckMK');
      return;
    }
    if (!clientId || !equipmentName) {
      toast.error('Données équipement manquantes (client, nom)');
      return;
    }
    if (requireService && !selectedService.trim()) {
      toast.error('Veuillez sélectionner un service CheckMK');
      return;
    }

    const family =
      EQUIPMENT_TYPE_TO_FAMILY[equipmentType] ||
      EQUIPMENT_TYPE_TO_FAMILY[equipmentType === 'NAS' ? 'Stockage' : equipmentType];
    if (!family) {
      toast.error(`Type non supporté: ${equipmentType}`);
      return;
    }
    ...
    try {
      ...
      toast.success('Mapping CheckMK enregistré');
    ...
    <div className={styles.overlay} onClick={requestClose}>
      ...
      >
        ...
        <div className={`${adminStyles.modalHeader} ${styles.panelHeader}`}>
          <div>
            ...
            <p style={{ margin: '0.25rem 0 0', fontSize: '0.875rem', color: '#6b7280' }}>
              ...
              {clientId && <span style={{ marginLeft: 8, opacity: 0.8 }}>#{clientId}</span>}
            </p>
          </div>
          <button onClick={requestClose} className={adminStyles.closeButton} title="Fermer">
            <FaTimes />
          </button>
        </div>
        ...
        <div className={styles.body}>
          ...
          {suggestions.length > 0 && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, fontSize: '0.8125rem', fontWeight: 600, color: '#0d9488' }}>
                <FaLightbulb size={14} />
                Suggestions (nom client, n° client, équipement)
              </div>
                ...
                {suggestions.map((h, idx) => {
                  ...
                  return (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => setSelectedHost(hostId)}
                      style={{
                        padding: '6px 12px',
                        fontSize: '0.8rem',
                    ...
                    >
                      {hostId}
                      {h.ip && <span style={{ fontSize: '0.7rem', opacity: 0.85 }}>({h.ip})</span>}
                    </button>
                  );
                })}
              </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200, position: 'relative' }}>
              <FaSearch style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af', fontSize: 12 }} />
              <input
                type="text"
          ...
          <div
            className={`${styles.contentGrid} ${
              requireService && selectedHost ? styles.contentGridDouble : styles.contentGridSingle
            }`}
          >
            {/* Liste des hosts par catégorie */}
            <div className={styles.hostsList}>
              {loading ? (
                <div style={{ padding: '3rem', textAlign: 'center', color: '#6b7280' }}>
                  <FaSync style={{ animation: 'spin 1s linear infinite', marginBottom: 8 }} />
                  <p>Chargement des hôtes...</p>
                </div>
              ) : (
                CATEGORY_ORDER.map(cat => {
                        ...
                        return (
                          ...
                          >
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: isSelected ? 600 : 500, fontSize: '0.875rem' }}>{h.id}</div>
                              {(h.alias || h.ip) && (
                                <div style={{ fontSize: '0.75rem', opacity: isSelected ? 0.9 : 0.65, marginTop: 2 }}>
                                  {h.alias && <span>{h.alias}</span>}
                                  {h.alias && h.ip && ' · '}
                                  {h.ip && <span style={{ fontFamily: 'monospace' }}>{h.ip}</span>}
                                </div>
                              )}
                            </div>
              ...
              {!loading && Object.values(hostsByCategory).every(arr => !arr?.length) && (
                <div style={{ padding: '3rem', textAlign: 'center', color: '#6b7280' }}>
            ...
            {requireService && selectedHost && (
              <div className={styles.servicesPanel}>
                <div style={{ padding: '10px 12px', borderBottom: '1px solid #e5e7eb', background: '#f3f4f6', fontSize: '0.8rem', fontWeight: 600 }}>
                  Services CheckMK · {selectedHost}
                </div>
                <div style={{ padding: 10 }}>
                  <div style={{ position: 'relative', marginBottom: 8 }}>
                    <FaSearch style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af', fontSize: 12 }} />
                    <input
                      type="text"
                  ...
                  <div style={{ maxHeight: 300, overflowY: 'auto', border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff' }}>
                    {loadingServices ? (
                      <div style={{ padding: '1rem', textAlign: 'center', color: '#6b7280' }}>
                        <FaSync style={{ animation: 'spin 1s linear infinite' }} /> Chargement...
                      </div>
        ...
        <div className={`${adminStyles.modalActions} ${styles.footer}`}>
          {currentMapping?.checkmk_host_name && (
            <button
          ...
          >
            {saving ? <FaSync style={{ animation: 'spin 1s linear infinite' }} /> : <FaCheck />}
          </button>
        </div>
      </motion.div>
    </div>

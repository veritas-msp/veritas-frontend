...
export default function EquipmentStatsPanel({
  ...
  const deviceFacts = useMemo(() => {
    const rows = [
      { label: copy.stats.deviceType, value: typeLabel || "-" },
      { label: copy.fields.name, value: equipment?.name || equipment?.rawData?.nom || "-" },
    ];
    const ip = equipment?.ip || equipment?.rawData?.ip;
    if (ip) rows.push({ label: copy.fields.ip, value: ip });
    const serial = equipment?.serial || equipment?.rawData?.numeroSerie;
    if (serial) rows.push({ label: copy.fields.serial, value: serial });
    if (equipment?.clientName) {
      rows.push({ label: copy.fields.clientName, value: equipment.clientName });
    }
    ...
    const lastSync =
      equipment?.rmmLastHeartbeat ||
      equipment?.rawData?.rmm_last_heartbeat ||
      equipment?.checkmkLastSyncedAt;
    if (lastSync) {
      rows.push({
        ...
        {loading ? (
          ...
          <>
            <div className={styles.activityKpiGrid}>
              {ticketKpis.map((kpi) => (
                <div key={kpi.key} className={`${styles.activityKpiCard} ${styles[`activityKpi_${kpi.tone}`]}`}>
            ...
            <div className={styles.activityStatsGrid}>
              <div className={styles.activityStatsBlock}>
                ...
                {stats.avgResolutionHours != null ? (
                  <p className={styles.activityStatsHint}>

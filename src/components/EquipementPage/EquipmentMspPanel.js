...
import {
  EQUIPMENT_MONITOR_META,
  buildMonitorStatusCounts,
  buildSupervisionTodoActions,
  computeEquipmentHealthScore,
} from "./equipmentMspUtils";
import {
  buildEquipmentSupervisionSummary,
  equipmentHasSupervisionIssues,
  formatEquipmentLastCollection,
} from "./equipmentSupervisionUtils";
...
function FleetStatusChip({ label, tone = "neutral", title }) {
  if (!label || label === "-") {
  ...
  return (
    <span
      className={`${fleetStyles.chip} ${fleetStyles[`chip_${chipTone}`] || fleetStyles.chip_neutral}`}
      title={title}
    >
      {label}
    </span>
  ...
  return (
    <div className={`${styles.healthGauge} ${styles[`healthGauge_${tone}`]}`}>
      ...
      <div className={styles.healthGaugeCore}>
        ...
        <span className={styles.healthLabel}>Santé parc</span>
      </div>
    </div>
  );
}

function KpiTile({ icon, label, value, sub, tone = "default" }) {
  return (
    <div className={`${styles.kpiTile} ${styles[`kpiTile_${tone}`]}`}>
...
function StatusBadge({ status, compact = false }) {
  ...
  return (
    <span
      className={`${styles.statusBadge} ${compact ? styles.statusBadgeCompact : ""}`}
...
function DeviceFleetTableRow({
  ...
  const summary = buildEquipmentSupervisionSummary(equipment, {
    monitorStatus,
    checkmkEnabled,
    isMkMapped: mapped,
    alertRules,
  ...
  return (
    <tr
      className={fleetStyles.tableRow}
    ...
    >
      <td>
        <span
          className={fleetStyles.statusDot}
          style={{ background: toneToDotColor(dotTone) }}
          aria-hidden
        />
      </td>
      <td>
        <div className={fleetStyles.solutionCell}>
          <Icon
            icon={typeIconMap?.[displayType] || "mdi:devices"}
            className={fleetStyles.solutionLogoIcon}
            aria-hidden
          />
          <div className={fleetStyles.solutionText}>
            ...
            {nameSubtitle ? (
              <span className={fleetStyles.cellSub} title={nameSubtitle}>
                {nameSubtitle}
              </span>
            ) : null}
          </div>
      ...
      <td className={fleetStyles.cellMuted}>
        {lastCollection ? (
          <span title={copy.formatLastCollectionTitle(lastCollection.source)}>
...
function DeviceFleetTable({
  ...
  return (
    <section className={fleetStyles.panel}>
              ...
              return (
                <React.Fragment key={type}>
                  ...
                  {!isCollapsed
                    ? list.map((equipment) => (
                        <DeviceFleetTableRow
                          key={getEquipmentListKey(equipment)}
                          equipment={equipment}
                          displayType={type}
                          showClientName={showClientName}
                          typeIconMap={typeIconMap}
                          resolveMonitorStatus={resolveMonitorStatus}
                          getMkSummary={getMkSummary}
                          isMkMapped={isMkMapped}
...
function DeviceCard({
  ...
  return (
    ...
    >
      <span className={styles.deviceStatusBar} style={{ background: meta.color }} />
      <div className={styles.deviceCardHead}>
        <div className={styles.deviceTitleBlock}>
          <span className={styles.deviceName}>{equipment.name || "Sans nom"}</span>
          {showClient ? (
            <span className={styles.deviceClient}>{equipment.clientName}</span>
          ) : null}
        </div>
      ...
      <div className={styles.deviceCardFoot} onClick={(e) => e.stopPropagation()}>
        {checkmkEnabled && isMkMapped && renderMonitoringBadge ? (
          <div className={styles.deviceMkBadge}>
            {renderMonitoringBadge(equipment, mkSummary)}
          </div>
        ) : null}
        {renderActions ? (
          <div className={styles.deviceActions}>{renderActions(equipment, displayType)}</div>
        ) : null}
      </div>
    </article>
  );
...
function ActionQueue({ actions, onOpen }) {
  if (!actions.length) return null;
  return (
    <section className={styles.actionQueue}>
        ...
        {actions.map(({ equipment, status }) => {
          ...
          return (
            <button
              ...
              onClick={() => onOpen?.(equipment)}
            >
              <span className={styles.actionDot} style={{ background: meta.color }} />
              <div className={styles.actionBody}>
                <span className={styles.actionName}>{equipment.name}</span>
                <span className={styles.actionMeta}>
                  {equipment.clientName ? `${equipment.clientName} · ` : ""}
                  {equipment.type === "NAS" ? "Stockage" : equipment.type}
                  {equipment.ip ? ` · ${equipment.ip}` : ""}
                </span>
              </div>
              <span className={styles.actionVerb} style={{ color: meta.color }}>
...
function MspSkeleton({ embedded }) {
  return (
    <div className={styles.skeleton} data-embedded={embedded || undefined}>
...
export default function EquipmentMspPanel({
  ...
  onBulkMkSync,
  mkBulkSyncing = false,
  mkBulkSyncProgress = { done: 0, total: 0 },
  onTypeSelect,
  onEquipmentOpen,
  ...
  const { rules: alertRules } = useSupervisionAlertRules();
  ...
  const statusCounts = useMemo(
    () => buildMonitorStatusCounts(statsItems, resolveMonitorStatus, { alertRules }),
  ...
  const filterList = (list) => {
    ...
    if (localFilter === "todo") {
      return list.filter((eq) => {
        ...
        const summary = buildEquipmentSupervisionSummary(eq, {
          monitorStatus,
          checkmkEnabled: checkmkIntegrationEnabled,
          isMkMapped: isMkMapped?.(eq),
          alertRules,
        });
        return equipmentHasSupervisionIssues(summary);
      });
    }
  ...
  const supervisionStats = useMemo(() => {
    ...
    (statsItems || []).forEach((eq) => {
      ...
      if (summary.isUpToDate) upToDate += 1;
      else todo += 1;
    });
    return { total: statsItems.length, todo, upToDate };
  ...
  const renderStatusFilters = (compact = false) => (
    <div
      className={`${styles.filterGroup} ${compact ? styles.filterGroupCompact : ""}`}
      role="group"
      aria-label={copy.filters.aria}
    >
      {copy.statusFilters.map((f) => {
        ...
        return (
          <button
            key={f.id}
            type="button"
            className={`${styles.filterChip} ${localFilter === f.id ? styles.filterChipActive : ""}`}
            disabled={disabled}
            aria-pressed={localFilter === f.id}
  ...
  const hasAny = sectionMode
    ? portfolioHasDevices
    : typesToRender.some((t) => (equipmentByType[t] || []).length > 0);
  const hasVisibleAfterFilter = sectionMode
    ? Boolean(
        activeType && filterList(equipmentByType[activeType] || []).length > 0
      )
    : typesToRender.some((t) => filterList(equipmentByType[t] || []).length > 0);

  const fleetGroups = sectionMode
    ? typesToRender
        .map((type) => ({
          type,
  ...
  const renderFamilyRail = () => (
    <aside className={styles.familyRail} role="tablist" aria-label={copy.toolbar.typeTabsAria}>
      {typeOrder.map((type) => {
        ...
        return (
          <button
            ...
            className={`${styles.familyRailBtn} ${isActive ? styles.familyRailBtnActive : ""}`}
            onClick={() => onTypeSelect?.(type)}
            title={copy.formatTypeTitle(type, count)}
            aria-label={copy.formatTypeTitle(type, count)}
          >
            <Icon
              icon={typeIconMap[type] || "mdi:devices"}
              className={styles.familyRailIcon}
              width={20}
              aria-hidden
            />
  ...
  const renderSectionMainContent = () => {
    if (!activeType) {
      return (
        <MspEmptyState
          ...
          title={copy.empty.selectFamilyTitle}
          text={copy.empty.selectFamilyText}
        />
      );
    }
    if (localFilter === "all" && !(equipmentByType[activeType] || []).length) {
      return (
        <MspEmptyState
          icon={typeIconMap[activeType] || "mdi:devices"}
          title={copy.formatTypeTitle(activeType, 0)}
          text={copy.empty.noEquipment}
        />
      );
    }
      ...
      return (
        <>
          <MspEmptyState
            ...
            title={copy.empty.noFilterMatchTitle}
            text={copy.empty.noFilterMatchText}
          />
    ...
    return (
      <DeviceFleetTable
        ...
        onMiddleClick={onEquipmentMiddleClick}
  ...
  return (
    <div
      ...
      data-embedded={hideChrome || undefined}
      data-section-mode={sectionMode || undefined}
    >
      {!hideChrome ? (
      ...
      <header className={styles.hero}>
        <div className={styles.heroLeft}>
          ...
          <div className={styles.heroCopy}>
            <span className={styles.heroEyebrow}>Vue décisionnelle</span>
            <h2 className={styles.heroTitle}>
              Supervision multi-clients
            </h2>
            <p className={styles.heroDesc}>
              Priorisez les alertes, filtrez par type et accédez au détail en un clic.
            </p>
          </div>
        </div>
        <div className={styles.kpiRow}>
          <KpiTile icon="mdi:devices" label="Périphériques" value={statusCounts.total} />
          <KpiTile
            icon="mdi:alert-circle-outline"
            label="En alerte"
            value={statusCounts.issues || 0}
            tone={statusCounts.critical > 0 ? "bad" : statusCounts.warning > 0 ? "warn" : "default"}
          />
          <KpiTile
            icon="mdi:eye-off-outline"
            label="Non mappés"
            value={statusCounts.unmapped || 0}
            tone={(statusCounts.unmapped || 0) > 0 ? "warn" : "default"}
          />
          {checkmkIntegrationEnabled && mkAlertStats.mapped > 0 ? (
            <KpiTile
              icon="mdi:radar"
              label="CheckMK"
              value={mkAlertStats.mapped}
              sub={`${mkAlertStats.issues || 0} alerte${(mkAlertStats.issues || 0) > 1 ? "s" : ""}`}
            />
          ) : (statusCounts.unmapped || 0) > 0 ? (
            <KpiTile
              icon="mdi:eye-off-outline"
              label="À mapper"
              value={statusCounts.unmapped || 0}
      ...
      {checkmkIntegrationEnabled && mkAlertStats.mapped > 0 ? (
        <div className={styles.mkBar}>
          ...
          <div className={styles.mkPills}>
            {[
              { id: "issues", label: copy.mkBar.alerts, count: mkAlertStats.issues },
              { id: "critical", label: copy.mkBar.critical, count: mkAlertStats.critical },
              { id: "warning", label: copy.mkBar.warning, count: mkAlertStats.warning },
            ].map((pill) => (
              <button
                key={pill.id}
                type="button"
                className={`${styles.mkPill} ${mkStatusFilter === pill.id ? styles.mkPillActive : ""}`}
                disabled={!pill.count}
                onClick={() => onMkStatusFilter?.(pill.id)}
              >
                {pill.label} {pill.count}
              </button>
            ))}
            {mkStatusFilter ? (
              <button type="button" className={styles.mkClear} onClick={onClearMkFilter}>
          ...
          <button
            type="button"
            className={styles.mkSync}
            onClick={onBulkMkSync}
      ...
      {!sectionMode ? (
        <ActionQueue actions={priorityActions} onOpen={onEquipmentOpen} />
      ) : null}
      ...
      {sectionMode ? (
        !hasAny ? (
          <MspEmptyState
            icon="mdi:devices"
            title={copy.empty.noDevicesTitle}
            text={copy.empty.noDevicesText}
          />
        ) : (
            ...
            <div className={styles.sectionMain}>
              {activeType ? (
                <div className={styles.sectionToolbar}>
                  ...
                  {supervisionStats ? (
                    <div className={styles.toolbarMetaInline}>
                      <span>{copy.formatDeviceCount(supervisionStats.total)}</span>
                      {supervisionStats.todo > 0 ? (
                        <span className={styles.toolbarMetaWarn}>
                          {copy.formatTodoCount(supervisionStats.todo)}
                        </span>
                      ) : (
                        <span>{copy.formatOkCount(supervisionStats.upToDate)}</span>
                      )}
                    </div>
                  ) : null}
                </div>
            ...
            {typeOrder.map((type) => {
              ...
              return (
                <button
                  ...
                  role="tab"
                  aria-selected={isActive}
                  className={`${styles.typeTab} ${isActive ? styles.typeTabActive : ""}`.trim()}
                  onClick={() => onTypeSelect?.(type)}
                  title={copy.formatTypeTitle(type, count)}
                >
      ...
      {!hasAny ? (
        ...
        typesToRender.map((type) => {
          ...
          return (
            <section key={type} className={styles.pillar}>
              {!activeType && !embedded ? (
              ...
              {list.length === 0 ? (
                <div className={styles.pillarEmpty}>
                  {getEmptyMessage?.(type, embedded) || "Aucun équipement."}
                </div>
              ) : (
                <div className={styles.deviceGrid}>
                  {list.map((equipment) => (
                    <DeviceCard
                      ...
                      equipment={equipment}
                      displayType={type}
                      showClient={showClientName && !embedded}
                      monitorStatus={resolveMonitorStatus(equipment)}
                      mkSummary={getMkSummary?.(equipment)}
                      isMkMapped={isMkMapped?.(equipment)}
                      checkmkEnabled={checkmkIntegrationEnabled}
                      onOpen={onEquipmentOpen}
                      onMiddleClick={onEquipmentMiddleClick}
                      renderActions={renderCardActions}
                      renderMonitoringBadge={renderMonitoringBadge}
                      copy={copy}
                    />
                  ))}
                </div>

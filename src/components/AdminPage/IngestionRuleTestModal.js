...
export default function IngestionRuleTestModal({
  ...
  const patchSample = (patch) => onSampleChange({ ...draft, ...patch });
  ...
  return createPortal(
    <div className={layout.overlay} onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="ingestion-rule-test-title"
      >
        ...
        <div style={{ overflowY: "auto", padding: "1.25rem 1.5rem 1.35rem" }}>
          <div className={layout.field} style={{ marginBottom: "1rem" }}>
            <label className={layout.label} htmlFor="rule-test-collector">
              {irt.collectorLabel}
            </label>
            <select
              id="rule-test-collector"
              className={layout.input}
              value={draft.collectorId || ""}
              onChange={(e) => patchSample({ collectorId: e.target.value })}
            >
          ...
          <div className={layout.fieldGrid2}>
            <div className={layout.field}>
              ...
              <input
                ...
                onChange={(e) => patchSample({ fromAddress: e.target.value })}
            ...
            <div className={layout.field}>
              ...
              <input
                id="rule-test-from-name"
                type="text"
                className={layout.input}
                value={draft.fromName || ""}
                onChange={(e) => patchSample({ fromName: e.target.value })}
            ...
            <div className={`${layout.field} ${layout.fieldFull}`}>
              ...
              <input
                ...
                className={layout.input}
                value={draft.subject || ""}
                onChange={(e) => patchSample({ subject: e.target.value })}
            ...
            <div className={`${layout.field} ${layout.fieldFull}`}>
              ...
              <textarea
                ...
                value={draft.body || ""}
                onChange={(e) => patchSample({ body: e.target.value })}
            ...
            <div className={styles.resultPanel}>
              <div
                className={`${styles.resultHeadline} ${
                  hasMatch ? styles.resultHeadline_match : styles.resultHeadline_none
                }`}
              >
                <Icon
                  icon={hasMatch ? "mdi:check-decagram-outline" : "mdi:close-circle-outline"}
                  className={`${styles.resultIcon} ${
                    hasMatch ? styles.resultIcon_match : styles.resultIcon_none
                  }`}
                  aria-hidden
                />
                <div>
                  <p className={styles.resultTitle}>
                    {hasMatch
                      ? interpolate(irt.matchTitle, { name: result.firstMatchName })
                      : irt.noMatchTitle}
                  </p>
                  <p className={styles.resultSub}>
                    {hasMatch
                      ? interpolate(irt.matchSub, { count: matches.length })
                      : irt.noMatchSub}
                  </p>
                </div>
              </div>
              ...
              {matches.length > 0 && (
                <div className={styles.matchList}>
                  {matches.map((rule) => (
                    <div key={rule.id} className={styles.matchItem}>

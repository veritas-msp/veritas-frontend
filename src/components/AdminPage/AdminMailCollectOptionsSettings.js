...
import {
  DEFAULT_MAIL_COLLECT_SETTINGS,
  normalizeMailCollectSettings,
} from "../../utils/mailCollectSettingsConstants";
import {
  Card,
  Field,
  Btn,
...
export default function AdminMailCollectOptionsSettings() {
  ...
  const orphanSegmentOptions = useMemo(
    () => orphanOptions.map(({ value, label }) => ({ value, label })),
    [orphanOptions]
  );
  ...
  const orphanHint = useMemo(
    () =>
      orphanOptions.find((item) => item.value === form.orphanReplyBehavior)?.subtitle ||
      opt.orphanReply.hintFallback,
    [form.orphanReplyBehavior, orphanOptions, opt.orphanReply.hintFallback]
  );
  ...
  useEffect(() => {
    ...
    fetchTicketAutomationConfig()
      ...
      .catch(() => toast.error(copy.toast.loadError))
          ...
          <section className={styles.group}>
            ...
            <FormGrid cols={2}>
              <FieldRow
                ...
                label={opt.threadReplies.label}
                hint={opt.threadReplies.hint}
                className={styles.compactRow}
              >
                <Switch
                  checked={form.threadRepliesEnabled}
                  onChange={(on) => patch({ threadRepliesEnabled: on })}
                />
              </FieldRow>

              <FieldRow
                icon="mdi:identifier"
                label={opt.deduplicate.label}
                hint={opt.deduplicate.hint}
                className={styles.compactRow}
              >
                <Switch
                  checked={form.deduplicateByMessageId}
                  onChange={(on) => patch({ deduplicateByMessageId: on })}
                />
              </FieldRow>
            </FormGrid>

            {form.threadRepliesEnabled ? (
              <div className={styles.subOption}>
                <Field
                  label={opt.orphanReply.label}
                  hint={orphanHint || opt.orphanReply.hintFallback}
                >
                  <ChoiceGroup
                    variant="segment"
                    ariaLabel={opt.orphanReply.ariaLabel}
                    value={form.orphanReplyBehavior}
                    options={orphanSegmentOptions}
                    onChange={(value) => patch({ orphanReplyBehavior: value })}
                  />
                </Field>
              </div>
            ) : null}
            ...
            <Field label={opt.maxLogEntries.label} hint={opt.maxLogEntries.hint}>
              <div className={styles.stepperWrap}>
                <NumberStepper
                  min={50}
                  max={2000}
                  value={form.maxLogEntriesPerCollector}
                  onChange={(value) => patch({ maxLogEntriesPerCollector: value })}

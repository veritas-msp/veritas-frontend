...
function SubscriptionFaqItem({ href, icon, question, answer, linkLabel }) {
    ...
    <details className={s.faqItem}>
      ...
      <div className={s.faqAnswer}>
        ...
        >
          {linkLabel}
          <Icon
            icon={isMailto ? "mdi:email-outline" : "mdi:open-in-new"}
            className={s.faqLinkIcon}
            aria-hidden
          />
        </a>
      </div>
...
export default function AdminLicense() {
  ...
  const metaLine = useMemo(() => {
    ...
    if (license.agentCount != null) {
      parts.push(
        license.agentCount > 1
          ? interpolate(copy.agentCountPlural, { count: license.agentCount })
          : interpolate(copy.agentCount, { count: license.agentCount })
      );
    }
    ...
    if (checkedAt) parts.push(interpolate(copy.checkedAt, { date: checkedAt }));
    return parts.join(" · ");
  }, [license, copy, appLocale]);

  const statusMessage = useMemo(
    () => resolveLicenseStatusMessage({ isValid, status, copy }),
    [isValid, status, copy]
  );
  ...
  return (
    <Page>
      <Card
        title={copy.title}
        action={
          <Btn variant="secondary" icon="mdi:refresh" onClick={onRefresh} disabled={refreshing || loading}>
        ...
        {loading ? (
          ...
          <div className={s.layout}>
            <div className={s.statusBar}>
              ...
              {info?.keyHint ? <Badge variant="muted">{interpolate(copy.keyHint, { hint: info.keyHint })}</Badge> : null}
            </div>
            ...
            {license?.devBypass ? (
              <p className={s.devHint}>
                {interpolate(copy.devHint, { code: copy.devCode })}
              </p>
            ) : null}

            {!isValid && (
              <form onSubmit={onActivate} className={s.activateForm}>
                <Input
                  value={licenseKey}
      ...
      <Card title={copy.subscriptionTitle}>
        ...
        <div className={s.faqList}>
          {subscriptionFaq.map((item) => (
            <SubscriptionFaqItem key={item.key} {...item} />
          ))}
        </div>
      </Card>
    </Page>

...
function MfaStatusBadge({ user, locale }) {
  ...
  return (
    <span
      className={`${mfaStyles.status} ${mfaStyles[status.className]}`}
      title={`${mfaLabels.titlePrefix} ${status.label}`}
    >
...
export default function AgentFormModal({
          ...
          <>
            ...
            <div className={layout.fieldGrid2}>
              <div className={layout.field}>
                <label
                  className={`${layout.label} ${isEdit ? "" : layout.labelRequired}`}
              ...
              <div className={layout.field}>
                ...
                <input
                  ...
                  onChange={(e) => patchDraft({ password2: e.target.value })}
            ...
            <div className={layout.fieldGrid2}>
              <div className={`${layout.field} ${layout.fieldFull}`}>
                ...
                <select
                  ...
                  className={layout.input}
                  value={draft.profile || ""}
                  onChange={(e) => patchDraft({ profile: e.target.value })}
                >
                  {profiles.map((p) => (
          ...
          <>
            ...
            <div className={mfaStyles.mfaPanel}>
              ...
              <p className={mfaStyles.mfaHint}>
                {draft.mfa_enabled
                  ? modalCopy.mfaHintEnabled
                  : draft.mfa_pending_setup
                  ? modalCopy.mfaHintPending
                  : modalCopy.mfaHintOff}
              </p>
              {canReleaseMfa && (
                <button type="button" className={mfaStyles.mfaReleaseBtn} onClick={onReleaseMfa}>
  ...
  const footerParts = [
    draft.email?.trim() || modalCopy.footerNoEmail,
    draft.profile || modalCopy.footerNoProfile,
  ];
  ...
  return createPortal(
    <div
      className={`${layout.overlay} ${stacked ? layout.overlayStacked : ""}`}
      onClick={onClose}
      role="presentation"
    >
      <div
        ...
        <footer className={layout.footer}>
          ...
          <div className={layout.footerActions}>
            {isEdit && onDelete && (
              <button
                type="button"
                className={layout.dangerBtn}
                onClick={onDelete}
                disabled={deleteDisabled || saving}
                title={deleteDisabled && deleteDescription ? deleteDescription : modalCopy.deleteTitle}
              >

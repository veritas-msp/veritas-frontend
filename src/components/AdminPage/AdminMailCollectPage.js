...
export default function AdminMailCollectPage({ isCommunity = false }) {
  const locale = useAppLocale();
  const copy = useMemo(() => getAdminMailCollectCopy(locale), [locale]);
  const hubViews = useMemo(
    () => HUB_VIEW_KEYS.map((key) => ({ key, label: copy.tabs[key] })),
    [copy.tabs]
  );
  ...
  return (
    <Page>
      ...
      {activeView === "email-ingestion" ? (
        <AdminEmailIngestionSettings isCommunity={isCommunity} />
      ) : null}

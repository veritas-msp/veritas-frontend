...
function describeFormVisibility(form) {
  if (form?.visibility !== "assigned") return "Tous les agents";
  const parts = [];
  if (form.profileNames?.length) parts.push(`${form.profileNames.length} profil(s)`);
  if (form.userIds?.length) parts.push(`${form.userIds.length} agent(s)`);
  if (form.teamIds?.length) parts.push(`${form.teamIds.length} équipe(s)`);
  return parts.length ? parts.join(" · ") : "Restreint (vide)";
}
...
export default function SalesFormsAdmin() {
  ...
  const loadForms = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await fetchSalesForms({ includeDisabled: true });
      setForms(Array.isArray(rows) ? rows : []);
    } catch (error) {
      toast.error(error.message || "Erreur lors du chargement des formulaires");
      setForms([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <Card
        ...
        description="Créez les natures de demande (prestation ou installation) et les champs affichés lors de la création."
        fill
        action={
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {[
              ...
              <button
                key={item.key || "all"}
                type="button"
                className={`${pageLayout.chip} ${kindFilter === item.key ? pageLayout.chipActive : ""}`}
          ...
          <table className={`${styles.userTable} ${styles.clientTable}`}>
            ...
            <tbody>
              ...
              {filteredForms.map((form) => (
                <tr key={form.id} className={styles.userRow}>
                  <td>{form.kind === "installation" ? "Installation" : "Prestation"}</td>
                  <td>
                  ...
                  <td>{form.enabled !== false ? "Oui" : "Non"}</td>
      ...
      <SalesFormModal
        ...
        kindDefault={kindFilter || "prestation"}
        onClose={closeModal}
        onSaved={handleSaved}
      />
    </>

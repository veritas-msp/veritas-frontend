...
function SortableMacroActionRow({
  ...
  return (
    <li
      ref={setNodeRef}
      style={style}
      className={`${styles.actionsListRow} ${isDragging ? styles.actionsListRowDragging : ""}`}
    >
      <button
        type="button"
      ...
      <button
        type="button"
        className={`${styles.actionListItem} ${isSelected ? styles.actionListItemActive : ""}`}
        onClick={onSelect}
      >
      ...
      <div className={styles.actionListTools}>
        <button
          type="button"
          className={`${styles.listToolBtn} ${styles.listToolBtnDanger}`}
          title={deleteTitle}
          onClick={onDelete}
        >
...
export default function MacroFormModal({
  ...
  const handleDragEnd = (event) => {
    ...
    setDraft((prev) => {
      ...
      return { ...prev, actions: arrayMove(currentActions, oldIndex, newIndex) };
    });
  };
  ...
  const stepsHeaderLabel =
    actions.length === 1
      ? interpolate(m.stepsHeader, { count: actions.length })
      : interpolate(m.stepsHeaderPlural, { count: actions.length });
  const footerActionsLabel =
    actionsCount === 1
      ? interpolate(m.footerActionsSingular, { count: actionsCount })
      : interpolate(m.footerActionsPlural, { count: actionsCount });
      ...
      case "actions":
        return (
          <>
            ...
            {actions.length === 0 ? (
              <p className={styles.emptyActions}>{m.emptyActions}</p>
            ) : (
                    ...
                    >
                      <ol className={styles.actionsList}>
                        {actions.map((action, index) => {
                          ...
                          const subtitle =
                            describeActionBrief?.(action, index) ||
                            describeAction?.(action, index) ||
                            ss.common.emptyDash;
                          const stepNumber = index + 1;
                          return (
                            <SortableMacroActionRow
                              key={action.id}
                              action={action}
                              index={index}
                              ...
                              reorderAria={interpolate(m.reorderAria, { step: stepNumber })}
                              stepLabel={interpolate(m.stepLabel, { step: stepNumber })}
                    ...
                    <>
                      <div className={styles.actionDetailHead}>
                        <span className={styles.actionDetailStep}>
                          {interpolate(m.stepLabel, { step: selectedIndex + 1 })}
                        </span>
                        <span className={styles.actionDetailSummary}>
                          {describeActionBrief?.(selectedAction, selectedIndex) ||
                            describeAction?.(selectedAction, selectedIndex)}
                        </span>
                      </div>
                      {renderActionEditor?.({
                        ...
                        onChange: (patch) => {
                          setDraft((prev) => ({
                            ...
                            actions: (prev.actions || []).map((item) =>
                              item.id === selectedAction.id ? { ...item, ...patch } : item
                            ),
                          }));
                        },
                      })}
  ...
  return createPortal(
    <div className={layout.overlay} onClick={onClose} role="presentation">
      <div
        className={`${layout.shell} ${styles.macroShell}`}

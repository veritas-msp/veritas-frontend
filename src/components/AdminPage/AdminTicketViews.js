...
import {
  ...
  getTicketViewVisibilityLabel,
} from "./adminSupportSettingsI18n";
import { interpolate } from "../../i18n/translate";

export default function AdminTicketViews({ profiles = [], users = [], teams = [] }) {
  ...
  const filteredViews = useMemo(() => {
    ...
    return views.filter((view) => {
      ...
      if (!q) return true;
      return (
        String(view.name || "").toLowerCase().includes(q)
        || String(view.description || "").toLowerCase().includes(q)
        || describeViewAssignments(view).toLowerCase().includes(q)
      );
    });
  }, [views, filter, search]);
  ...
  return (
    <>
      <Card
        title={supportViewMeta["ticket-views"].title}
        description={tv.cardDescription}
        fill
        action={
      ...
      >
        ...
        <div className={s.tableSection}>
            ...
            <table className={s.table}>
              ...
              <tbody>
                {loading ? (
                  ...
                  viewsPagination.paginatedItems.map((view) => {
                    const filtersText = describeViewCriteria(view.rules);
                    const assignmentText =
                      view.visibility === "assigned"
                        ? describeViewAssignments(view)
                        : ss.common.emptyDash;
                    return (
                      <tr key={view.id}>
                        <td>
                          <div className={s.viewTableName}>
                            <Icon
                              icon={view.icon || "mdi:view-list"}
                              className={s.viewTableNameIcon}
                              aria-hidden
                            />
                            <div className={s.viewTableNameText}>
                              <div className={s.viewTableNameTitle}>
                                {view.name}
                                {view.isBuiltin && (
                                  <span className={s.viewTableBuiltin}>{tv.builtin}</span>
                                )}
                              </div>
                              {view.description && (
                                <div className={s.viewTableNameDesc}>{view.description}</div>
                              )}
                            </div>
                          </div>
                        </td>
          ...
          {!loading && filteredViews.length > 0 && (
            <Pagination
              page={viewsPagination.page}
              totalPages={viewsPagination.totalPages}
              onPageChange={viewsPagination.setPage}
              pageSize={viewsPagination.pageSize}
              onPageSizeChange={viewsPagination.setPageSize}
              rangeLabel={viewsPagination.rangeLabel}
            />
          )}
        </div>
      </Card>
      ...
      <ConfirmModal
        ...
        title={deleteCopy.ticketViewTitle}
        icon="mdi:delete-alert-outline"
        message={interpolate(deleteCopy.ticketViewMessage, {

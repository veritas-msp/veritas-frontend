...
import {
  formatInAppActiveEventsDescription,
  getAdminInAppNotificationsCopy,
  getLocalizedAdminInAppEventOptions,
  getLocalizedInAppEventGroups,
  getLocalizedInAppTestTypeOptions,
} from "./adminInAppNotificationsI18n";
...
export default function AdminInAppNotificationsSettings() {
  ...
  const eventOptionByKey = useMemo(
    () => Object.fromEntries(eventOptions.map((item) => [item.key, item])),
    [eventOptions]
  );

  const [settings, setSettings] = useState(() =>
    normalizeInAppSettings(getTicketAutomationConfig()?.notificationSettings?.inAppSettings)
  );
  ...
  const renderEventCard = (eventKey) => {
    ...
    return (
      <article key={eventKey} className={`${styles.eventCard} ${isDisabled ? styles.eventCardDisabled : ""}`}>
        <FieldRow
          icon={eventOption.icon}
          label={eventOption.label}
          hint={eventOption.description}
          className={styles.compactRow}
        >
          <Switch
            checked={isEventEnabled}
            disabled={isDisabled}
            onChange={(value) => updateEventField(eventKey, "enabled", value)}
          />
        </FieldRow>

        {isEventEnabled && eventOption.fields.length > 0 && !isDisabled ? (
          <div className={styles.eventOptions}>
            {eventOption.fields.map((field) => (
              <FieldRow
                key={`${eventKey}-${field.key}`}
                label={field.label}
                hint={field.hint}
                className={`${styles.compactRow} ${styles.compactSubRow}`}
              >
                <Switch
                  checked={eventSettings[field.key] === true}
                  onChange={(value) => updateEventField(eventKey, field.key, value)}
                />
              </FieldRow>
            ))}
          </div>
  ...
  return (
    <div className={styles.layout}>
      ...
      <Card
        title={copy.eventsCard.title}
        description={
          settings.enabled
            ? formatInAppActiveEventsDescription(locale, activeEventsCount, eventOptions.length)
            : copy.eventsCard.disabledHint
        }
      >

...
import { deleteRmmClientSettings, fetchRmmClientSettings, fetchRmmClientSettingsList, updateRmmClientSettings } from "../../api/rmm";
...
export default function RmmClientSettingsPanel({
  ...
  const loadClientSettings = useCallback(async clientId => {
    if (!clientId) {
  ...
  const selectedClientLabel = useMemo(() => clientOptions.find(c => c.value === selectedClientId)?.label || "", [clientOptions, selectedClientId]);
  ...
  const formatOverridesSummary = (overrides = {}) => {
    ...
    if (o.collectors && Object.keys(o.collectors).length > 0) {
      parts.push(interpolate(cs.overrideCollectors, {
        count: Object.keys(o.collectors).length
      }));
    }
    if (o.metrics && Object.keys(o.metrics).length > 0) {
      parts.push(interpolate(cs.overrideMetrics, {
  ...
  const handleSave = async () => {
    ...
    try {
      if (!form.useCustom) {
        await updateRmmClientSettings(selectedClientId, {
          useCustom: false
        });
  ...
  return <>
      <Card title={cs.listTitle} description={cs.listDescription} fill>
        ...
        key: "actions",
        label: "",
        render: row => <Btn variant="ghost" onClick={() => setSelectedClientId(String(row.clientId))}>
                  {copy.common.edit}
                </Btn>
      ...
      <Card title={cs.editorTitle} description={cs.editorDescription}>
        <FormGrid cols={2}>
          <Field label={cs.companyField}>
            <select className={styles.clientSelect} value={selectedClientId} onChange={e => setSelectedClientId(e.target.value)}>
          ...
          <Field label={cs.modeField}>
            ...
            ...prev,
            useCustom: checked
          }))} label={form.useCustom ? cs.customConfig : cs.useGlobal} disabled={!selectedClientId || loading} />
          </Field>
        </FormGrid>

        {selectedClientId ? loading ? <p className={styles.collectorsHint}>{copy.common.loadingShort}</p> : <>
              <p className={styles.clientEditorTitle}>
                <Icon icon="mdi:office-building-outline" aria-hidden />
                {selectedClientLabel}
                {form.useCustom ? <Badge variant="warn">{cs.badgeCustom}</Badge> : <Badge variant="muted">{cs.badgeGlobal}</Badge>}
              </p>

              <section className={styles.settingsSection}>
                <h3 className={styles.settingsSectionTitle}>{copy.settings.communicationTitle}</h3>
                <RmmClientTimingFields copy={copy} form={form} disabled={!form.useCustom} onCustomize={(path, value) => patchCustomized(path, value)} onHeartbeatChange={value => patchValue("heartbeatIntervalMinutes", value)} onOfflineChange={value => patchValue("offlineThresholdMinutes", value)} />
              </section>

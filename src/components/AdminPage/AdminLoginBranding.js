...
import {
  deleteLoginBrandingAsset,
  fetchLoginBrandingAdmin,
  updateLoginBranding,
  uploadLoginBrandingAsset,
} from "../../api/loginBranding";
import {
  DEFAULT_SIDE_COLORS,
  LOGIN_SIDES,
  flatToSideForm,
  ...
  return (
    <Field label={label} hint={hint}>
      <div className={s.colorRow}>
        ...
        />
        {value ? (
          <button type="button" className={s.colorReset} onClick={() => onChange("")}>
            ×
          </button>
        ) : null}
      </div>
    ...
    <Field label={label} hint={hint}>
      <div className={s.assetBox}>
        {previewUrl ? (
          ...
          <div className={s.assetPlaceholder}>-</div>
        )}
        <div className={s.assetActions}>
          ...
          {path ? (
            <Btn variant="ghost" disabled={uploading} onClick={onDelete}>
              {removeLabel}
            </Btn>
          ) : null}
        </div>
      ...
      <div className={s.previewFrame}>
        <aside className={s.previewLeft} style={panelStyle}>
          <div className={s.previewBrand}>
            {logoUrl ? (
              <img src={logoUrl} alt="" className={s.previewLogo} />
            ) : (
              <div className={s.previewBrandIcon} style={{ background: accent }}>V</div>
            )}
        ...
        <div
          className={s.previewRight}
          style={{ background: form.rightBgColor || defaults.rightBgColor }}
        >
...
export default function AdminLoginBranding({ isCommunity = false }) {
  ...
  const [forms, setForms] = useState({ agent: { ...EMPTY_SIDE }, client: { ...EMPTY_SIDE } });
    ...
    <Page>
        ...
        <FormGrid cols={2}>
          <Field label={copy.headline1Label}>
            <Input
              ...
              placeholder={copy.previewHeadline1}
            />
          </Field>
          <Field label={copy.headline2Label}>
            <Input
              ...
              placeholder={copy.previewHeadline2}
            />
          </Field>
      ...
      <FormGrid cols={2}>
        <Card title={copy.visualTitle} description={copy.visualDescription}>
          <FormGrid cols={2}>
            <ColorField
              label={copy.bgStartLabel}
              value={form.bgColorStart}
              fallback={defaults.bgColorStart}
              onChange={(value) => setField("bgColorStart", value)}
            />
            <ColorField
              label={copy.bgEndLabel}
              value={form.bgColorEnd}
              fallback={defaults.bgColorEnd}
              onChange={(value) => setField("bgColorEnd", value)}
            />
            <ColorField
            ...
            />
            <ColorField
              label={copy.rightBgLabel}
              value={form.rightBgColor}
              fallback={defaults.rightBgColor}
              onChange={(value) => setField("rightBgColor", value)}
            />
            <AssetUploadField
            ...
            />
            <AssetUploadField
              label={copy.bgImageLabel}
              hint={copy.bgImageHint}
              path={form.bgImagePath}
              uploading={uploading === "background"}
              chooseLabel={copy.chooseFile}
              removeLabel={copy.removeFile}
              onUpload={(file) => handleUpload("background", file)}
              onDelete={() => handleDeleteAsset("background")}
            />
          </FormGrid>
        </Card>

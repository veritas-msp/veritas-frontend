...
} from "../../api/equipmentFamilies";
import EquipmentFamilyFormModal from "./EquipmentFamilyFormModal";
import {
  buildDefaultEquipmentFamilyDraft,
  buildEquipmentFamilyDraftFromFamily,
  getDefaultEquipmentFamilies,
} from "./equipmentFamilyConstants";
import {
  Btn,
  Card,
  ConfirmModal,
...
export default function AdminEquipmentFamilies() {
  ...
  const allFamilies = useMemo(() => {
    const custom = families.map((family) => ({ ...family, isSystem: false }));
  ...
  const saveFamily = async () => {
    ...
    try {
      ...
      if (editing) {
        const { family } = await updateEquipmentFamily(editing.id, payload);
        ...
        const { family } = await createEquipmentFamily(payload);
  ...
  const toggleEnabled = async (family, enabled) => {
    try {
      const { family: updated } = await updateEquipmentFamily(family.id, { enabled });
  ...
  const columns = [
      ...
      render: (row) => (
        row.isSystem ? (
          <Icon
            ...
            title={adminCopy.systemFamily}
            style={{ fontSize: "1.1rem", color: "var(--msp-muted-light, #94a3b8)" }}
          />
        ) : (
    ...
    },
    {
      ...
      width: "72px",
      render: (row) => (
        <Icon icon={row.icon || "mdi:devices"} style={{ fontSize: "1.25rem", color: "var(--msp-accent)" }} />
      ),
    },
    {
      key: "fields",
    ...
    },
    {
      ...
      render: (row) => (
        row.isSystem ? (
          <span className={adminUi.tableSubtext} title={adminCopy.builtInFamily}>-</span>
        ) : (
    ...
    <Page>
      ...
      >
        ...
        {loading ? (
          ...
          <Table columns={columns} rows={allFamilies} emptyMessage={copy.empty} />
        )}
      </Card>

      <EquipmentFamilyFormModal
        open={modalOpen}
      ...
      <ConfirmModal
        ...
        message={
          confirmDelete
            ? interpolate(copy.deleteMessage, { label: confirmDelete.label })
            : ""
        }

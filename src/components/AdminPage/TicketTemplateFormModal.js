          ...
          <>
            ...
            <div className={styles.editorShell}>
                ...
                <div className={styles.toolGroup} role="group" aria-label={m.toolbarSizeAria}>
                  {[25, 50, 75, 100].map((pct) => (
                    <button
                      key={pct}
                      type="button"
                      className={`${styles.toolBtn} ${styles.sizeBtn}`}
                      onClick={() => onResizeImage?.(pct)}
                      title={interpolate(m.widthPct, { pct })}
                    >
                      {pct}%
                    </button>
                  ))}
                ...
                <div className={styles.widthGroup} role="group" aria-label={m.toolbarWidthAria}>
                  ...
                  <div className={styles.numberStepper}>
                    <input
                      ...
                      min="1"
                      step="10"
                      value={selectedImageWidthPx}
                      onChange={(e) => setSelectedImageWidthPx?.(e.target.value)}
                      placeholder="px"
                      aria-label={m.widthPxAria}
                    />
                    <div className={styles.numberStepperBtns}>
                      <button
                        ...
                        title={m.increaseWidth}
                        aria-label={m.increaseWidthAria}
                      >
                      ...
                      <button
                        ...
                        title={m.decreaseWidth}
                        aria-label={m.decreaseWidthAria}
                      >
              ...
              <div
                ...
                onInput={(e) => {
                  const htmlContent = String(e.currentTarget?.innerHTML || "").replace(
                    /\soutline:\s*[^;"']+;?/gi,
                    ""
                  );
                  setDraft((prev) => ({ ...prev, content: htmlContent }));
                }}
            ...
            <button
              type="button"
              className={`${styles.variablesBtn} ${!templateVariablesEnabled ? styles.variablesBtnLocked : ""}`}
              onClick={
                templateVariablesEnabled
                  ? onOpenVariables
                  : () => setShowVariablesProPromo(true)
              }
              title={
                templateVariablesEnabled ? m.variablesBtnTitle : m.variablesBtnLockedTitle
              }
            >
  ...
  const modalNode = (
    <div className={layout.overlay} onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="ticket-template-form-title"
      >
        ...
        <div className={layout.body}>
          <nav className={layout.nav} aria-label={m.sectionsAria}>
            {templateFormSections.map((section) => (
              <button
                key={section.id}
                type="button"
              ...
              >
                ...
                {section.id === "general" && Boolean(draft.name?.trim()) && (
                  <span className={layout.navBadge}>✓</span>
                )}
                {section.id === "content" && Boolean(draft.content?.trim()) && (
                  <span className={layout.navBadge}>✓</span>
                )}
              </button>
            ))}
  ...
  return (
    <>
      ...
      <ProFeaturePromoModal
        open={showVariablesProPromo}
        featureKey="ticketTemplateVariables"
        onClose={() => setShowVariablesProPromo(false)}
      />
    </>
  );
}

...
/** Taille moyenne estimée d'un heartbeat light (JSON upload). */
export const RMM_HEARTBEAT_UPLOAD_KB = 20;
/** Réponse serveur typique. */
export const RMM_HEARTBEAT_RESPONSE_KB = 1;
/** Inventaire JSONB fusionné par poste en base (ordre de grandeur). */
...
export function estimateRmmClientImpact({
  ...
  return {
    ...
    rows: [
      { key: "duration", value: "3–10 s" },
      { key: "cpuPeak", value: "~5–15 %" },
      {
        key: "cpuAvg",
        value: `< ${Math.max(0.05, avgCpuPct).toFixed(2)} %`,
        noteParams: { perDay },
      },
      { key: "ram", value: "50–120 Mo" },
      {
        key: "networkUpload",
      ...
      },
      {
        key: "fullSync",
        value: "15–60 s",
        noteParams: { count: fullSyncPerWeek },
      },
    ],
  };
}
...
export function estimateRmmServerImpact({
  ...
  return {
    ...
    rows: [
      { key: "sqlPerHb", value: "~4–8" },
      {
        key: "sqlPerDay",
        value: formatStorageNumber(totalHeartbeatsDay * sqlPerHeartbeat),
        noteParams: { agents, perDay: perAgentDay },
      },
      { key: "cpuNode", value: "5–30 ms" },
      { key: "inventoryWrite", value: `~${jsonbWriteKb} Ko / hb` },
      { key: "historicalMetrics", value: "~1 / h / poste" },
      {
        key: "avgLoad",
        value: `${Math.round(heartbeatsPerMinute * 10) / 10} hb/min`,
        noteParams: { note: agents > 0 ? "active" : "none" },
      },
    ],
  };
}
  ...
  return {
    ...
    rows: [
      ...
      },
      {
        ...
        noteParams: { perDay, kb: kbPerHeartbeat },
      },
      { key: "perAgentMonth", value: formatStorageBytes(bytesPerAgentDay * 30) },
      {
        key: "fleetDay",
        value: formatStorageBytes(bytesTotalDay),
        noteParams: { agents },
      },
      { key: "fleetMonth", value: formatStorageBytes(bytesTotalMonth) },
      { key: "fullSync", value: "50–200 Ko" },
    ],
  };
}

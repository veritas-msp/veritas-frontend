...
export default function MultiSuggestPicker({
  ...
  const dropdownNode = open ? (
    ...
    >
      {filteredOptions.length === 0 ? (
        <div className={s.contactEmpty}>Aucun résultat</div>
      ) : (
        filteredOptions.map((option, index) => (
          <button
            ...
            role="option"
            aria-selected={false}
            className={`${s.contactOption} ${highlight === index ? s.contactOptionActive : ""}`}
  ...
  return (
    <div className={s.equipmentField}>
      {label && (
        <label className={s.equipmentFieldLabel} id={inputLabelId}>
          {label}
        </label>
      )}
      <div className={s.contactPicker} ref={rootRef}>
        <div
          ref={anchorRef}
          className={`${s.contactInputWrap} ${open ? s.contactInputWrapOpen : ""}`}
        >
      ...
      <div className={s.chipsWrap}>
        {selectedIds.length === 0 ? (
          <span className={s.emptyChipHint}>{emptyHint}</span>
        ) : (
          selectedIds.map((id) => (
            <span key={String(id)} className={s.chip}>
              {labelById.get(String(id)) || String(id)}
              <button type="button" onClick={() => removeOption(id)} aria-label={`Retirer ${labelById.get(String(id)) || id}`}>
                ×
              </button>
            </span>
          ))

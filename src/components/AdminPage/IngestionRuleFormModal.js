...
import {
  describeLocalizedMailCriterionBrief,
  getLocalizedIngestionRuleFormSections,
  getLocalizedMailCriterionFields,
  getLocalizedMailCriterionOperators,
  getLocalizedRuleActionOptions,
  interpolate,
...
export default function IngestionRuleFormModal({
  ...
  const describeRuleBrief = useCallback(
    (criterion) => describeLocalizedMailCriterionBrief(criterion, copy),
    [copy]
  );

  const filterRoot = useMemo(
    () => normalizeExclusionFilterRoot(draft || {}),
    [draft]
  );
  ...
  const patchFilterRoot = useCallback(
    (nextRoot) => {
      setDraft((prev) => ({
        ...prev,
        filterRoot: nextRoot,
  ...
  const collectorOptions = useMemo(
    () =>
      (Array.isArray(mailCollectors) ? mailCollectors : []).map((collector) => ({
  ...
  const addRule = (groupId = filterRoot.id) => {
    const next = addRuleToGroup(filterRoot, groupId, buildDefaultMailFilterRule({ includeConnector: true }));
  ...
  const addGroup = (groupId = filterRoot.id) => {
    const next = addGroupToGroup(filterRoot, groupId, buildDefaultGroup({ includeConnector: true }));
    patchFilterRoot(next);
    const added = findNodeWithParent(next, groupId);
    const lastChild = added?.node?.children?.[added.node.children.length - 1];
    if (lastChild?.id) setSelectedFilterNodeId(lastChild.id);
  ...
  const modalTitle = isCreate
    ? irf.createTitle
    : draft.name
      ? interpolate(irf.editTitle, { name: draft.name })
  ...
  const footerHint =
    ruleCount === 0
      ? irf.footerAllEmails
      : ruleCount === 1
        ? irf.footerCriteriaOne
        : interpolate(irf.footerCriteriaMany, { count: ruleCount });
          ...
          <>
            ...
            <div className={layout.fieldGrid2}>
              <div className={`${layout.field} ${layout.fieldFull}`}>
                ...
                <select
                  id="ingestion-rule-collector"
                  className={layout.input}
                  value={draft.collectorId || ""}
                  onChange={(e) => patchDraft({ collectorId: e.target.value })}
                >
              ...
              <div className={`${layout.field} ${layout.fieldFull}`}>
                ...
                <select
                  ...
                  className={layout.input}
                  value={normalizeIngestionAction(draft.action)}
                ...
                {normalizeIngestionAction(draft.action) === "create_ticket_services" && (
                  <div className={styles.proActionHint}>
      ...
      case "criteria":
        return (
          <>
            ...
            {ruleCount === 0 ? (
              <div className={`${viewStyles.criteriaEmpty} ${viewStyles.filtersBlock}`}>
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleFilterDragEnd}>
                <div className={`${macroModalStyles.actionsLayout} ${viewStyles.filtersBlock}`}>
                  <TicketViewFilterBuilder
                    ...
                    ruleIcons={MAIL_CRITERION_FIELD_ICONS}
                    ruleCount={ruleCount}
                  />
                    ...
                    {selectedRule ? (
                        ...
                        <div className={viewStyles.criterionEditorGrid}>
                          <div className={viewStyles.field}>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                updateFilterRule(selectedRule.id, { field: e.target.value, value: "" })
                              }
                            >
                              {mailCriterionFields.map((opt) => (
                          ...
                          <div className={viewStyles.field}>
                            ...
                            >
                              {mailCriterionOperators.map((opt) => (
                                <option key={opt.value} value={opt.value}>
                                  {opt.label}
                                </option>
                              ))}
                      ...
                      <>
                        ...
                        <div className={viewStyles.groupEditorActions}>
                          <button
                            type="button"
                            className={viewStyles.addRuleBtnSecondary}
                          ...
                          <button
                            type="button"
                            className={viewStyles.addRuleBtnSecondary}
                            onClick={() => addGroup(selectedGroup.id)}
                          >
  ...
  return createPortal(
    <div className={layout.overlay} onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="ingestion-rule-form-title"
      >
          ...
          <nav className={layout.nav} aria-label={irf.sectionsAria}>
            {formSections.map((section) => (
              ...
              >
                ...
                {section.id === "criteria" && ruleCount > 0 && (
                  <span className={layout.navBadge}>{ruleCount}</span>
                )}
                {section.id !== "criteria" && sectionMeta[section.id] && (

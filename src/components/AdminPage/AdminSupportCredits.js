...
import {
  deleteSupportCreditPack,
  fetchAllSupportCreditPacks,
  fetchClientsList,
} from "../../api/clients";
...
export default function AdminSupportCredits() {
  ...
  const loadPacks = useCallback(async () => {
    setLoading(true);
    try {
      const [packRows, clientRows] = await Promise.all([
        fetchAllSupportCreditPacks(),
    ...
    if (q) {
      rows = rows.filter((pack) => {
        const haystack = [
          ...
          pack.label,
          pack.note,
          getPackStatusLabel(locale, pack.status),
        ]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
  ...
  const totals = useMemo(() => {
    ...
    const available = packs
      ...
      .reduce((sum, pack) => sum + Number(pack.remaining_amount || 0), 0);
    return {
  ...
  const columns = [
    {
      ...
      render: (row) => <div style={{ fontWeight: 600 }}>{getEnterpriseLabel(row)}</div>,
    },
    {
      ...
      render: (row) => (
        <div>
          ...
          {row.note ? (
            <div className={adminUi.tableSubtext}>{row.note}</div>
          ) : null}
        </div>
      ),
    },
    {
      ...
      render: (row) => (
        <span style={{ fontVariantNumeric: "tabular-nums", fontWeight: 600 }}>{row.remaining_amount}</span>
      ),
    },
    {
      ...sortCol("initial_amount", copy.columns.initial, "88px"),
      render: (row) => (
        <span style={{ fontVariantNumeric: "tabular-nums", fontWeight: 600 }}>{row.initial_amount}</span>
      ),
    },
    {
  ...
  return (
    <div className={styles.wrap}>
      ...
      <Card
        ...
        fill
        action={
          <Btn icon="mdi:plus" onClick={openCreateModal}>
            {copy.page.newPack}
          </Btn>
        }
      >
        <Toolbar
          ...
          meta={formatPackMeta(locale, filtered.length, totals.available)}
          action={
            <Select
              value={statusFilter}
        ...
        {loading ? (
          <p className={adminUi.adminMutedText}>{copy.emptyLoading}</p>
        ) : (
          <>
            <Table
              columns={columns}
            ...
            <Pagination
              ...
              rangeLabel={interpolate(copy.range, {
                start: rangeStart,
                end: rangeEnd,
      ...
      <ConfirmModal
        ...
        title={deleteCopy.supportCreditTitle}
        message={
          deletePack
            ? interpolate(deleteCopy.supportCreditMessage, {

...
export default function SolutionCatalogEntryModal({
  ...
  const categoryLabel =
    draft.category === "action" ? m.categoryAction : m.categoryIntervention;
  ...
  const modalTitle = isCreate
    ? draft.category === "action"
      ? m.createActionTitle
      : m.createInterventionTitle
    : interpolate(m.editTitle, { name: draft.label || categoryLabel.toLowerCase() });
  ...
  return createPortal(
    <div className={layout.overlay} onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="solution-catalog-modal-title"
      >
          ...
          <div className={layout.content}>
            <div className={layout.fieldStack}>
              ...
              <div className={layout.field}>
                ...
                <input
                  id="solution-catalog-order"
                  type="number"
                  min="0"
                  className={layout.input}
                  value={Number(draft.displayOrder) || 0}
                  onChange={(e) => patchDraft({ displayOrder: Number(e.target.value) || 0 })}
                  disabled={saving}
                />
              </div>
              ...
              <div className={`${shared.statusRow} ${styles.statusRow}`}>
                ...
                <Switch
                  ...
                  label={draft.isActive !== false ? m.activeOn : m.activeOff}
                  disabled={saving}
                />
              </div>
            </div>
          ...
          <footer className={layout.footer}>
            ...
            <div className={layout.footerActions}>
              ...
              <button
                ...
                disabled={saving || !String(draft.label || "").trim()}
              >

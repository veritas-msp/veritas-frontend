...
import { fetchUsers, fetchUser, createUser, updateUser, deleteUser, releaseUserMfa, resetPassword, buildUserUpdatePayload } from "../../api/users";
...
const PROTECTED_PROFILE_NAMES = ["super admin", "super-admin", "super_admin", "superadmin", "super administrateur", "administratif", "direction", "responsable", "superviseur", "supervisor", "agent", "collaborateur", "collaborator", "lecture", "lecteur", "read-only", "read only"];
...
export default function AdminUsers({
  ...
  const filteredProfiles = profiles.filter(p => (p.name || "").toLowerCase().includes(profilesSearchTerm.toLowerCase()) || (p.label || "").toLowerCase().includes(profilesSearchTerm.toLowerCase()));
  ...
  const isProfileProtected = name => {
    const key = String(name || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
    return PROTECTED_PROFILE_NAMES.some(n => n === key);
  };
  ...
  return <Page>
              ...
              return <div key={p.name} className={s.profileCard}>
                      <div className={s.profileCardHead}>
                        ...
                        <div className={s.profileCardMeta}>
                          ...
                          {p.label && <p className={s.profileCardDesc}>{p.label}</p>}
                      ...
                      <td colSpan={accessColumns.length + 1} className={s.empty}>
                        ...
                        <td>
                          <div className={s.accessProfileCell}>
                            ...
                            {p.label && <span className={s.accessProfileDesc}>{p.label}</span>}
      ...
      setConfirmDeleteProfile(false);
        ...
        {profileModalMode === "edit" && <>
            <ModalEntityHeader icon="mdi:account-cog-outline" title={profileNameInput} subtitle={editingProfileIsProtected ? copy.profiles.systemSubtitle : copy.profiles.customSubtitle} badge={editingProfileIsProtected ? <Badge variant="warn">{copy.profiles.systemTag}</Badge> : <Badge variant="success">{copy.profiles.customBadge}</Badge>} />
            <ModalForm>

...
import {
  createRmmEnrollmentToken,
  downloadRmmWindowsAgentSetup,
  downloadRmmWindowsAgentSetupMsi,
  downloadRmmWindowsAgentSetupZip,
  fetchRmmInstallerInfo,
  fetchRmmMetricsStorage,
  fetchRmmAgents,
  fetchRmmEnrollmentTokens,
  fetchRmmSettings,
  revokeRmmEnrollmentToken,
  restoreRmmEnrollmentToken,
  deleteRmmEnrollmentTokenPermanently,
  updateRmmAgentStatus,
  updateRmmSettings,
} from "../../api/rmm";
import {
  Badge,
...
const RMM_TAB_KEYS = [
  { key: "deploy", icon: "mdi:download-outline" },
  { key: "settings", icon: "mdi:cog-outline" },
  { key: "clientSettings", icon: "mdi:office-building-outline", proOnly: true, promoKey: "adminRmmClientSettings" },
  { key: "tokens", icon: "mdi:key-outline" },
  { key: "agents", icon: "mdi:laptop" },
  { key: "consumptions", icon: "mdi:chart-box-outline" },
];

const DEPLOY_PLATFORM_META = {
  windows: { icon: "mdi:microsoft-windows", available: true },
  macos: { icon: "mdi:apple", available: false },
  linux: { icon: "mdi:linux", available: false },
  ios: { icon: "mdi:apple-ios", available: false },
  android: { icon: "mdi:android", available: false },
  chromeos: { icon: "mdi:google-chrome", available: false },
};
...
function DeployPlatformInstructions({
  ...
  return (
    <>
      <strong>{interpolate(copy.deploy.soon.title, { platform: title })}</strong>
      <p className={styles.instructionsSoon}>
        {interpolate(copy.deploy.soon.message, { platform: title })}
      </p>
    </>
  );
}
...
export default function AdminRmm({ isCommunity = false }) {
  ...
  const rmmTabs = useMemo(
    () =>
      RMM_TAB_KEYS.map((item) => ({
        ...item,
  ...
  const deployPlatforms = useMemo(
    () =>
      DEPLOY_PLATFORM_KEYS.map((key) => ({
        key,
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [settingsData, activeTokensData, trashedTokensData, agentsData, clientsData, installerData, storageData] =
        await Promise.all([
        fetchRmmSettings(),
        fetchRmmEnrollmentTokens(undefined, { status: "active" }),
        fetchRmmEnrollmentTokens(undefined, { status: "revoked" }),
        fetchRmmAgents(),
        fetchClientsList(),
        fetchRmmInstallerInfo().catch(() => null),
        fetchRmmMetricsStorage().catch(() => null),
      ]);
  ...
  const clientOptions = useMemo(
    () =>
      [...clients]
        ...
        .map((c) => ({ value: String(c.id), label: c.name || formatRmmClientLabel(c.id, copy) })),
    [clients, locale, copy]
  );
  ...
  const patchCollector = (key, value) => {
    setSettings((prev) => ({
      ...prev,
      collectors: { ...prev.collectors, [key]: value },
    }));
  };
  ...
  const metricsAgentCount =
    metricsStorage?.stats?.activeAgents ??
    agents.length ??
    metricsStorage?.stats?.agentCountWithData ??
    0;
  ...
  const handleDownloadWindows = async (format = "zip") => {
    ...
    try {
      ...
      toast.success(
        version
          ? interpolate(copy.toast.downloadWithVersion, {
  ...
  return (
    <Page>
      ...
      {tab === "deploy" && (
        <Card title={copy.deploy.cardTitle} description={copy.deploy.cardDescription}>
            ...
            {deployPlatforms.map((platform) => {
              ...
              return (
                <button
                  ...
                  } ${!platform.available ? styles.platformCardSoon : ""}`}
                  aria-pressed={isSelected}
                  onClick={() => setSelectedDeployPlatform(platform.key)}
                >
          ...
          <div className={styles.instructions}>
            <DeployPlatformInstructions
              platformKey={selectedDeployPlatform}
              copy={copy}
              installerInfo={installerInfo}
              downloading={downloading}
              onDownloadWindows={handleDownloadWindows}
              onOpenTokensTab={() => setTab("tokens")}
            />
          </div>
        </Card>
      )}
      ...
      {tab === "settings" && settings && (
        <Card
          ...
          fill
          action={
            <Btn icon="mdi:content-save-outline" onClick={handleSaveSettings} disabled={saving}>
        ...
        >
          <div className={styles.settingsLayout}>
            <section className={styles.settingsSection}>
              ...
              <RmmTimingFields
                copy={copy}
                heartbeatMinutes={settings.heartbeatIntervalMinutes}
                offlineThresholdMinutes={settings.offlineThresholdMinutes}
                onHeartbeatChange={(value) =>
                  setSettings((prev) => ({ ...prev, heartbeatIntervalMinutes: value }))
                }
                onOfflineChange={(value) =>
                  setSettings((prev) => ({ ...prev, offlineThresholdMinutes: value }))
                }
              />
            </section>
            ...
            <RmmMetricsStorageSection
              copy={copy}
              locale={locale}
              metrics={settings.metrics || DEFAULT_METRICS}
              collectors={settings.collectors}
              onMetricChange={patchMetric}
              storageStats={metricsStorage?.stats}
              agentCount={metricsAgentCount}
              avgDisksPerAgent={metricsAvgDisks}
            />

            <RmmCollectorsSection
              copy={copy}
              isCommunity={isCommunity}
              onProClick={() => setProPromoFeature("adminRmmCollectors")}
              getChecked={(key) => !!settings.collectors?.[key]}
              onToggle={(key, checked) => patchCollector(key, checked)}
            />
          </div>
        </Card>
      )}

      {tab === "clientSettings" && settings && !isCommunity && (
        <RmmClientSettingsPanel
          copy={copy}
          locale={locale}
          globalSettings={settings}
          clientOptions={clientOptions}
          metricsStorageStats={metricsStorage?.stats}
          activeAgentCount={metricsAgentCount}
          onProClick={() => setProPromoFeature("adminRmmCollectors")}
        />
      )}
      ...
      {tab === "tokens" && (
        <Card
          ...
          fill
          action={
            tokenListView === "active" ? (
              <Btn icon="mdi:plus" onClick={openCreateTokenModal}>
        ...
        >
          <div className={styles.tokensToolbar}>
            <div className={styles.tokenViewToggle} role="tablist" aria-label={copy.tokens.viewAria}>
              <button
                type="button"
                role="tab"
                aria-selected={tokenListView === "active"}
                className={`${styles.tokenViewBtn} ${
                  tokenListView === "active" ? styles.tokenViewBtnActive : ""
                }`}
                onClick={() => setTokenListView("active")}
              >
                {copy.tokens.viewActive}
                {activeTokens.length > 0 ? (
                  <span className={styles.tokenViewCount}>{activeTokens.length}</span>
                ) : null}
              </button>
              <button
                type="button"
                role="tab"
                aria-selected={tokenListView === "trash"}
                className={`${styles.tokenViewBtn} ${
                  tokenListView === "trash" ? styles.tokenViewBtnActive : ""
                }`}
                onClick={() => setTokenListView("trash")}
              >
                <Icon icon="mdi:trash-can-outline" aria-hidden />
                {copy.tokens.viewTrash}
                {trashedTokens.length > 0 ? (
                  <span className={styles.tokenViewCount}>{trashedTokens.length}</span>
                ) : null}
              </button>
            </div>
          </div>
          ...
          {loading ? (
            <p className={adminUi.adminMutedText}>{copy.common.loadingShort}</p>
          ) : (
            <>
              <Table
                columns={
                  tokenListView === "trash"
                    ? [
                        { key: "client_name", label: copy.tokens.colCompany },
                        {
                          key: "token",
                          label: copy.tokens.colToken,
                          render: (row) => <RmmEnrollmentTokenValue token={row.token} compact />,
                        },
                        { key: "label", label: copy.tokens.colLabel },
                        { key: "uses_count", label: copy.tokens.colUses },
                        { key: "revoked_at", label: copy.tokens.colRevoked },
                        {
                          key: "actions",
                          label: "",
                          render: (row) => (
                            <div className={styles.tokenRowActions}>
                              ...
                              <BtnIcon
                                ...
                                variant="danger"
                                onClick={() => setConfirmPermanentDelete(row.id)}
                              />
                            </div>
                          ),
                        },
                    ...
                    : [
                        ...
                        { key: "label", label: copy.tokens.colLabel },
                        { key: "uses_count", label: copy.tokens.colUses },
                        { key: "expires_at", label: copy.tokens.colExpires },
                        { key: "created_at", label: copy.tokens.colCreated },
                        {
                          key: "actions",
                          label: "",
                          render: (row) => (
                ...
                rows={tokensPagination.paginatedItems.map((t) => mapTokenTableRow(t, locale))}
                emptyMessage={
                  tokenListView === "trash" ? copy.tokens.emptyTrash : copy.tokens.emptyActive
                }
              />
      ...
      {tab === "agents" && (
        <Card title={copy.agents.title} description={copy.agents.description} fill>
          {loading ? (
            <p className={adminUi.adminMutedText}>{copy.common.loadingShort}</p>
          ) : (
            <Table
              columns={[
                { key: "client_name", label: copy.agents.colCompany },
                { key: "hostname", label: copy.agents.colHostname },
                { key: "agent_version", label: copy.agents.colVersion },
                {
                  key: "online",
                  label: copy.agents.colStatus,
                  render: (row) =>
                    row.online ? (
                      <Badge variant="success">{copy.agents.statusOnline}</Badge>
                    ) : (
                      <Badge variant="warn">{copy.agents.statusOffline}</Badge>
                    ),
                },
                { key: "last_seen_at", label: copy.agents.colLastSeen },
                {
                  key: "actions",
                  label: "",
                  render: (row) => (
                    <Btn variant="ghost" onClick={() => handleRevokeAgent(row.id)}>
                      {copy.common.revoke}
                    </Btn>
                  ),
                },
      ...
      {tab === "consumptions" && settings && (
        <Card title={copy.consumptions.cardTitle} description={copy.consumptions.cardDescription} fill>
          <RmmConsumptionsPanel
            copy={copy}
            locale={locale}
            settings={settings}
      ...
      <RmmEnrollmentTokenFormModal
        open={tokenModalOpen}
        copy={copy}
        draft={tokenDraft}
        setDraft={setTokenDraft}
        saving={creatingToken}
        clientOptions={clientOptions}
        onClose={closeCreateTokenModal}
        onSave={handleCreateToken}
      />

      <RmmEnrollmentTokenCreatedModal
        open={Boolean(createdToken)}
        copy={copy}
        token={createdToken}
        onClose={() => setCreatedToken(null)}
        onCopy={handleCopyCreatedToken}
      />

      <ConfirmModal
        open={!!confirmRevoke}
        title={rmmCopy.revokeTitle}
        message={rmmCopy.revokeMessage}
        confirmLabel={rmmCopy.revokeConfirm}
      ...
      <ConfirmModal
        open={!!confirmRestore}
        title={rmmCopy.restoreTitle}
        message={rmmCopy.restoreMessage}
        confirmLabel={rmmCopy.restoreConfirm}
        onConfirm={handleRestoreToken}
        onClose={() => setConfirmRestore(null)}
      />

      <ConfirmModal
        open={!!confirmPermanentDelete}
        title={rmmCopy.purgeTitle}
        icon="mdi:delete-forever-outline"
        message={rmmCopy.purgeMessage}
        confirmLabel={rmmCopy.purgeConfirm}
        confirmVariant="dangerSolid"
        onConfirm={handlePermanentDeleteToken}

    ...
    <Page>
      <div className={s.teamsLayout}>
        <Card
          ...
          fill
          action={
            <Btn icon="mdi:plus" onClick={openCreateTeam}>
              {copy.newTeam}
            </Btn>
          }
        >
            ...
            <div className={ui.toolLeft}>
              ...
              <span className={ui.count}>
                {filteredTeams.length > 1
                  ? interpolate(copy.teamCountPlural, { count: filteredTeams.length })
                  : interpolate(copy.teamCount, { count: filteredTeams.length })}
              </span>
            </div>
          </div>
          ...
          <div className={s.teamsListPane}>
            {loading ? (
              <div className={s.empty}>{adminCopy.loading}</div>
            ) : filteredTeams.length === 0 ? (
              <div className={s.empty}>{copy.noTeams}</div>
            ) : (
              filteredTeams.map((team) => {
                ...
                return (
                  <button
                    key={team.id}
                    type="button"
                    className={`${s.teamListItem} ${active ? s.teamListItemActive : ""}`}
                  ...
                  >
                    ...
                    <span className={s.teamListMain}>
                      ...
                      {team.description && (
                        <span className={s.teamListDesc}>{team.description}</span>
                      )}
                    </span>
                    <span className={s.teamListMeta}>
                      {(team.memberCount || 0) > 1
                        ? interpolate(copy.memberCountPlural, { count: team.memberCount || 0 })
                        : interpolate(copy.memberCount, { count: team.memberCount || 0 })}
                    </span>
                  </button>
                );
              })
        ...
        <Card
          title={selectedTeam ? selectedTeam.name : copy.detailTitle}
          description={
            selectedTeam
              ? interpolate(copy.detailDescription, {
                  members: selectedTeam.memberCount || 0,
                  leaders: selectedTeam.leaderCount || 0,
                })
              : copy.detailDescriptionEmpty
          }
          fill
          action={
            selectedTeam ? (
              <div className={s.teamDetailActions}>
        ...
        >
          {!selectedTeamId ? (
            <div className={s.empty}>{copy.selectTeamHint}</div>
          ) : detailLoading ? (
            <div className={s.empty}>{adminCopy.loading}</div>
          ) : (
            <>
              {selectedTeam && !selectedTeam.isActive && (
                <div className={s.teamInactiveBanner}>
                        ...
                        selectedTeam.members.map((member) => (
                          <tr key={member.userId}>
                            <td>
                              <div className={s.userCell}>
                                ...
                                <div className={s.userInfo}>
                                  ...
                                  {member.username && (
                                    <div className={s.userName}>{member.username}</div>
                                  )}
                                </div>
                              </div>
                            </td>
                            ...
                            <td>
                              <button
                                type="button"
                                className={`${s.leaderToggle} ${member.isLeader ? s.leaderToggleOn : ""}`}
                                title={member.isLeader ? copy.removeLeaderTitle : copy.setLeaderTitle}
                                disabled={busy}
                            ...
                            <td>
                              <button
                                ...
                                title={copy.removeFromTeamTitle}
                                disabled={busy}
              ...
              {selectedTeam && (
                <div className={s.teamDangerZone}>
                  <button
                    type="button"
                    className={s.teamDeleteBtn}
      ...
      <Modal
        ...
        eyebrow={adminCopy.organizationEyebrow}
        title={copy.addMemberTitle}
        subtitle={copy.addMemberSubtitle}
        ...
        footer={
          <ModalFooter
            ...
            confirmLabel={adminCopy.add}
            confirmDisabled={busy || !addMemberUserId}
          />
        }
      >
        <ModalForm>
          <ModalFormSection title={adminCopy.agent} icon="mdi:account-outline">
            ...
            <label className={s.leaderCheckLabel}>
              <input
                type="checkbox"
                checked={addMemberLeader}
                onChange={(e) => setAddMemberLeader(e.target.checked)}
              />
              {copy.setTeamLeader}
            </label>
          </ModalFormSection>
      ...
      <ConfirmModal
        ...
        title={copy.deleteTeamTitle}
        icon="mdi:delete-alert-outline"
        message={interpolate(copy.deleteTeamMessage, { name: confirmDeleteTeam?.name || "" })}

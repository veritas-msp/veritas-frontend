...
import {
  formatCountLabel,
  getBitdefenderIntegrationModalCopy,
  interpolate,
...
function BitdefenderTestResultModal({ result, error, onClose, copy, locale }) {
      ...
      {isSuccess ? (
        <>
          <div className={styles.kpiGrid}>
            ...
            <div className={styles.kpiCard}>
              <div className={styles.kpiValue}>
                {tenant.license?.used != null && tenant.license?.total != null
                  ? `${tenant.license.used}/${tenant.license.total}`
                  : "-"}
              </div>
          ...
          <dl className={styles.metaList}>
            ...
            {tenant.license?.companyName ? (
              <div className={styles.metaRow}>
                <dt>{copy.licenseLabel} · {tenant.license.companyName}</dt>
                <dd>
                  {tenant.license.expirationDate
                    ? interpolate(copy.licenseExpiration, {
    ...
    <>
      <div className={formStyles.sectionHead}>
        <h3 className={formStyles.sectionTitle}>
          {interpolate(copy.companiesDetected, { count: tenant.companies.length })}
        </h3>
    ...
    <>
      <div className={formStyles.sectionHead}>
        <h3 className={formStyles.sectionTitle}>
          ...
          {tenant.accountsCount > tenant.accounts.length
            ? interpolate(copy.on, { count: tenant.accountsCount })
            : ""}
          )
        </h3>
      ...
      <div className={styles.tableScroll}>
        <table className={styles.dataTable}>
          ...
          <tbody>
            {tenant.accounts.map((account, index) => (
              <tr key={`${account.email || "acc"}-${index}`}>
                <td>{account.email || "-"}</td>
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="bitdefender-test-result-title"
      >
...
export default function BitdefenderIntegrationModal({
  ...
  const handleTest = async () => {
    ...
    setTestError(null);
    try {
      const data = await testGlobalBitdefenderIntegration({ apiUrl: url, apiKey: key });
      setTestResult(data);
    ...
    <>
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="bitdefender-integration-modal-title"
        >
          <div className={styles.accentBarBitdefender} aria-hidden />
          <header className={formStyles.header}>
            <div className={formStyles.headerMain}>
              <div
                className={`${formStyles.headerIconWrap} ${styles.headerIconBitdefender}`}
                aria-hidden
              >
                <Icon icon="simple-icons:bitdefender" />
              </div>
            ...
            <div className={formStyles.footerActions}>
              ...
              >
                <Icon
                  ...
                  className={testing ? formStyles.spinning : ""}
                  aria-hidden
                />

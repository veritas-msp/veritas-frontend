...
// 🧩 Composant principal
// ──────────────────────────────
export default function MonitoringClientSkeleton({ isLoading, searchTerm: externalSearchTerm = "", setSearchTerm: externalSetSearchTerm }) {
  ...
  const getModuleColor = (moduleKey) => {
    ...
    const infraColor = "#2563eb";     // Bleu
    const cyberColor = "#ef4444";     // Rouge
    const serviceColor = "#8b5cf6";   // Violet
  ...
  // Fonction pour obtenir le nombre de périphériques pour un module
  const getDeviceCount = (client, moduleKey) => {
    ...
    switch (key) {
      ...
      case "antivirus":
        return null; // Ne pas afficher de nombre
      case "antispam":
        return null; // Ne pas afficher de nombre
      case "ndd":
  ...
  // Liste légère puis filtre Monitoring (pas de fetchClients / N+1 modules)
  useEffect(() => {
    ...
    loadAdminClientsListCached({
      ...
      .then((allClients) => {
        ...
        const monitoringClients = allClients.filter(
          (client) =>
            client.modules &&
            (client.modules.Monitoring === true || client.modules.monitoring === true)
        );
  ...
  return (
    <div>
      ...
      {/* Tableau des clients avec monitoring */}
          ...
          <tbody>
            {isLoading
              ? Array.from({ length: 6 }).map((_, i) => (
                  <tr key={`skeleton-${i}`} className={styles.userRow} style={{ opacity: 0.9 }}>
                    {['40%', '25%', '35%'].map((width, idx) => (
                      <td key={idx}>
                        <ContentLoader
                          speed={1.6}
                          width="100%"
              ...
              : paginatedClients.map((client) => (
                  ...
                  >
                    <td style={{ fontSize: '0.9rem', fontWeight: 'normal' }}>{client.name}</td>
                    <td>
                      <span style={{
                        ...
                        {client.report_frequency || "Non définie"}
                      </span>
                    </td>
                    <td>
                      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                        {(() => {
                          ...
                          return moduleOrder
                            .filter(key => client.modules_monitoring?.[key])
                            .map(key => (
                              <span
                                key={key}
                                alt={key}
                        ...
                        {Object.values(client.modules_monitoring || {}).every(v => !v) && (
                          <span>-</span>
                        )}
                      </div>
                    </td>
      ...
      {/* Modal édition client (portail body pour éviter le clipping dans la carte) */}
      {showModal &&
        createPortal(
          <>
            ...
            <MonitoringClientModal
              ...
              onClose={async () => {
                ...
                const monitoringClients = refreshed.filter(
                  (client) =>
                    client.modules &&
                    (client.modules.Monitoring === true || client.modules.monitoring === true)
                );

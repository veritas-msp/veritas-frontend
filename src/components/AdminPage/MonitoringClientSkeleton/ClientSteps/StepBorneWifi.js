...
const StepBorneWifi = ({
      ...
      try {
        ...
        if (response.ok) {
          ...
          (form.equipements?.BorneWifi || []).forEach((ap, idx) => {
            const matchingMapping = mappings.find(m => m.equipment_type === 'BorneWifi' && m.equipment_id === ap.id);
            if (matchingMapping) {
  ...
  const cleanBorneData = borne => {
    ...
    if (!cleaned.expirationWarranty || cleaned.expirationWarranty.trim() === '') delete cleaned.expirationWarranty;
      ...
      <div className={styles.formSection}>
        <div className={styles.scrollable}>
          {visibleSites.length === 0 ? <div className={styles.emptyState}>
              <p className={styles.emptyStateTitle}>No access points configured</p>
          ...
          return <div key={siteName} className={styles.siteGroup}>
                  <div className={`${styles.siteHeader} ${dragOverSite === siteName ? styles.siteDragOver : ''}`} onDragOver={e => handleSiteDragOver(e, siteName)} onDragLeave={handleSiteDragLeave} onDrop={e => handleSiteDrop(e, siteName)}>
                    ...
                    <span className={styles.siteCount}>{siteAps.length} access point{siteAps.length > 1 ? 's' : ''}</span>
                  </div>
              ...
              if (draggedIndex !== null) handleSiteDrop(e, siteName);
                    ...
                    {siteAps.length === 0 && <div className={styles.emptySiteMessage}>No access points in this site. Drag and drop an access point here to assign it.</div>}
                  ...
                  boxShadow: draggedIndex === i ? '0 8px 20px rgba(0,0,0,0.04)' : 'none'
                    ...
                    transition: 'background 0.2s ease'
                      ...
                      gap: '0.75rem'
                        ...
                        e.target.style.boxShadow = checkmkMappings[i] ? '0 4px 8px rgba(21, 209, 160, 0.35)' : '0 4px 8px rgba(156, 163, 175, 0.35)';
                      }} onMouseLeave={e => {
                        e.target.style.transform = 'scale(1)';
                        e.target.style.boxShadow = checkmkMappings[i] ? '0 2px 4px rgba(21, 209, 160, 0.3)' : '0 2px 4px rgba(156, 163, 175, 0.3)';
                      }}>
                                <FaLink size={14} />
                              </button>
                              <button onClick={e => {
                        e.stopPropagation();
                        remove(i);
                      }} title="Delete this access point" style={{
                        background: 'transparent',
                        border: 'none',
                        color: '#ef4444',
                        cursor: 'pointer',
                    ...
                    duration: 0.3
                      ...
                      gridTemplateColumns: '1fr 1fr 1fr'
                                ...
                                <div className={styles.formField}>
                                  <label htmlFor={`ap-fabricant-${i}`}>Brand</label>
                                  <input id={`ap-fabricant-${i}`} value={ap.fabricant} onChange={e => update(i, "fabricant", e.target.value)} />
                                </div>
                                <div className={styles.formField}>
                                  <label htmlFor={`ap-modele-${i}`}>Model</label>
                                  <input id={`ap-modele-${i}`} value={ap.modele} onChange={e => update(i, "modele", e.target.value)} />
                                </div>
                                <div className={styles.formField}>
                                  <label htmlFor={`ap-serie-${i}`}>Serial No.</label>
      ...
      setForm(prev => ({
        ...
        equipements: {
          ...
          BorneWifi: [...bornes, ...importedDevices]
              ...
              {}
              {checkmkMappingModal.isOpen && checkmkMappingModal.borneIndex !== null && bornes[checkmkMappingModal.borneIndex] && <CheckMKMappingModal isOpen={checkmkMappingModal.isOpen} onClose={() => setCheckmkMappingModal({
      isOpen: false,
      borneIndex: null
    })} equipmentName={bornes[checkmkMappingModal.borneIndex].nom || `WiFi AP #${checkmkMappingModal.borneIndex + 1}`} equipmentType="BorneWifi" equipmentIndex={checkmkMappingModal.borneIndex} equipmentId={bornes[checkmkMappingModal.borneIndex].id} clientId={form.id} requireService={false} onMappingSaved={mapping => {
      if (mapping) {
        setCheckmkMappings(prev => ({
          ...prev,

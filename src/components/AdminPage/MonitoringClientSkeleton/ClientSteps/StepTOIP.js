...
const StepTOIP = ({ form, setForm, onAdd, currentStepData }) => {
  ...
  // Fonction pour obtenir les solutions TOIP avec initialisation sécurisée
  const getTOIPData = () => {
    const toip = form?.equipements?.TOIP;
    
    // Si c'est l'ancien format (tableau avec un seul objet), on le convertit
  ...
  const handleDrop = (e, dropIndex) => {
    ...
    // Mettre à jour les expandedSolutions pour conserver l'état d'expansion
  ...
  const addSolution = (solutionName) => {
    ...
    // Compter combien de solutions avec le même nom existent déjà
    const existingCount = currentTOIP.solutions.filter(s => 
      s.logiciel && s.logiciel.startsWith(solutionName)
    ).length;
    
    // Si aucune solution avec ce nom n'existe, utiliser le nom tel quel
    // Sinon, ajouter un numéro (#2, #3, etc.)
    const logicielName = existingCount === 0 
      ? solutionName 
      : `${solutionName} #${existingCount + 1}`;
  ...
  const getSolutionIcon = (solutionName) => {
    if (solutionName && solutionName.startsWith("3CX")) {
      return <img src={getIconPath('3cx.png')} alt="3CX" style={{ width: '20px', height: '20px', display: 'inline-block', verticalAlign: 'middle', borderRadius: '4px' }} />;
    }
    return null;
  };
    ...
    >
        ...
        <div className={styles.scrollable}>
          {solutions.length === 0 ? (
            <div className={styles.emptyState}>
              <p className={styles.emptyStateTitle}>Aucune solution TOIP configurée</p>
                    ...
                    <div className={styles.formGrid}>
                      ...
                      <div className={styles.formField}>
                        ...
                        <input
                          ...
                          onChange={(e) => updateSolution(i, "version", e.target.value)}
                          placeholder="Ex: 18.0, 2023"
                        />
                      </div>
                      <div className={styles.formField}>
                        <label htmlFor={`toip-expiration-${i}`}>Expiration de licence</label>
                        <input
                          id={`toip-expiration-${i}`}
                          type="date"
      ...
      {showSolutionModal && createPortal(
        <div className={adminStyles.modalOverlay} onClick={() => setShowSolutionModal(false)}>
          <div className={adminStyles.modalContent} onClick={(e) => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            <div className={adminStyles.modalHeader}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <Icon icon="mdi:phone" className={adminStyles.modalIcon} />
                <h3>Sélectionner une solution TOIP</h3>
              </div>
              <button className={adminStyles.closeButton} onClick={() => setShowSolutionModal(false)} title="Fermer">
                <Icon icon="mdi:close" />
              </button>
            </div>
            <div className={adminStyles.modalBody}>
              <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', gap: '1.5rem', justifyContent: 'center', alignItems: 'center' }}>
                <button
                  onClick={() => addSolution("3CX")}
                  style={{
                    width: '125px',
                    height: '125px',
                    display: 'flex',
                ...
                >
                  <img src={getIconPath('3cx.png')} alt="3CX" style={{ width: '48px', height: '48px', borderRadius: '8px' }} />
                  <div style={{ fontSize: '0.875rem', fontWeight: '600', color: '#374151', textAlign: 'center' }}>3CX</div>
                </button>
              </div>
            </div>
          </div>

...
const StepSwitch = ({
      ...
      try {
        ...
        if (response.ok) {
          ...
          (form.equipements?.Switch || []).forEach((sw, idx) => {
            const matchingMapping = mappings.find(m => m.equipment_type === 'Switch' && m.equipment_id === sw.id);
            if (matchingMapping) {
  ...
  return <motion.div className={styles.stepContainer} initial={{
    ...
    ease: "circOut"
      ...
      <div className={styles.formSection}>
        ...
        <div className={styles.scrollable}>
          {visibleSites.length === 0 ? <div className={styles.emptyState}>
              <p className={styles.emptyStateTitle}>No switches configured</p>
              ...
              if (draggedIndex !== null) {
                handleSiteDrop(e, siteName);
                    ...
                    {siteSwitches.length === 0 && <div className={styles.emptySiteMessage}>
                        No switches in this site. Drag and drop a switch here to assign it to this site.
                      </div>}
                  ...
                  <div className={styles.serverActions}>
                    <button onClick={e => {
                        ...
                        if (!sw.id || sw.id.startsWith('switch-')) {
                          alert('Please save the switch first before mapping it with CheckMK.');
                          return;
                        }
                    ...
                    duration: 0.3
                      ...
                      gridTemplateColumns: '1fr 1fr 1fr'
                                ...
                                <div className={styles.formField}>
                                  <label htmlFor={`sw-fabricant-${i}`}>Brand</label>
                                  <input id={`sw-fabricant-${i}`} value={sw.fabricant} onChange={e => update(i, "fabricant", e.target.value)} />
                                </div>
                                <div className={styles.formField}>
                                  <label htmlFor={`sw-modele-${i}`}>Model</label>
                                  <input id={`sw-modele-${i}`} value={sw.modele} onChange={e => update(i, "modele", e.target.value)} />
                                </div>
                                <div className={styles.formField}>
                                  <label htmlFor={`sw-serie-${i}`}>Serial No.</label>
      ...
      {}
      {checkmkMappingModal.isOpen && checkmkMappingModal.switchIndex !== null && switches[checkmkMappingModal.switchIndex] && switches[checkmkMappingModal.switchIndex].id && !switches[checkmkMappingModal.switchIndex].id.startsWith('switch-') && <CheckMKMappingModal isOpen={checkmkMappingModal.isOpen} onClose={() => setCheckmkMappingModal({
      isOpen: false,
      switchIndex: null
    })} equipmentName={switches[checkmkMappingModal.switchIndex].nom || `Switch #${checkmkMappingModal.switchIndex + 1}`} equipmentType="Switch" equipmentIndex={checkmkMappingModal.switchIndex} equipmentId={switches[checkmkMappingModal.switchIndex].id} clientId={form.id} requireService={false} onMappingSaved={mapping => {
      if (mapping) {
        setCheckmkMappings(prev => ({
          ...prev,

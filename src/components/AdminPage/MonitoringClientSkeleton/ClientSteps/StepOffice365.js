...
const StepOffice365 = ({ form, setForm, initialClient }) => {
  ...
  useEffect(() => {
    if (typeof setForm === "function") {
      // Si des credentials existent, considérer comme configuré même si le test a échoué
      const effectiveStatus = existingCredentials ? "success" : (connectionStatus || null);

      // Préparer les données Office365 pour la sauvegarde globale
      const office365Data = {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      // Si des credentials existent déjà en base, tester avec ceux de la base
      if (existingCredentials) {
        console.log('Test avec credentials existants en base');
        result = await testClientOffice365Connection(initialClient.id);
      } else {
        // Sinon, tester avec les credentials du formulaire
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ fontWeight: 700, color: '#111827' }}>Connexion API Microsoft Entra</span>
            {connectionStatus === "success" && (
              <span style={{ padding: '0.15rem 0.5rem', background: '#ecfdf3', color: '#027a48', borderRadius: 999, fontSize: '0.75rem' }}>
                Connecté {applicationDisplayName ? `(${applicationDisplayName})` : ""}
              </span>
            )}
            {connectionStatus === "error" && (
              <span style={{ padding: '0.15rem 0.5rem', background: '#fef2f2', color: '#b91c1c', borderRadius: 999, fontSize: '0.75rem' }}>
                Connexion en erreur
              </span>
            )}
          </div>
        ...
        {!form.modules_monitoring?.Office365 ? (
          <div style={{ 
            padding: '1rem', 
            ...
            Activez d'abord le module Microsoft Entra dans l'assistant de configuration.
          </div>
        ) : (
          <div style={{ 
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', marginBottom: '0.35rem', color: '#111827' }}>
                <span style={{ fontWeight: 700 }}>Droits Graph API à accorder</span>
                <span
            ...
            <div style={{ 
              ...
              <div>
                <label style={{ display: 'block', marginBottom: 6, fontWeight: 600, color: '#1a1a1a', fontSize: '0.85rem' }}>
                  Tenant ID *
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCredentials(prev => ({ ...prev, clientIdAzure: e.target.value }))}
                  placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
                  disabled={loadingCredentials}
                  style={inputStyle}
                />
              ...
              <div>
                ...
                <input
                  type="text"
                  value={existingCredentials && !credentials.secretKeyId ? "****************" : credentials.secretKeyId || ""}
                  onChange={(e) => {
                    if (e.target.value === "****************") return;
                    setCredentials(prev => ({ ...prev, secretKeyId: e.target.value }));
                  }}
                  onFocus={(e) => {
                    if (e.target.value === "****************") {
                      e.target.value = "";
                      setCredentials(prev => ({ ...prev, secretKeyId: "" }));
                    }
                  }}
              ...
              <div>
                <label style={{ display: 'block', marginBottom: 6, fontWeight: 600, color: '#1a1a1a', fontSize: '0.85rem' }}>
                  Valeur du secret (client secret) *
                </label>
                <input
                  type="password"
                  value={existingCredentials && !credentials.clientSecret ? "****************" : credentials.clientSecret}
                  onChange={(e) => {
                    if (e.target.value === "****************") return;
                  ...
                  placeholder={existingCredentials ? "Laisser vide pour conserver" : "Client secret"}
                  disabled={loadingCredentials}
                  style={inputStyle}
                />
              </div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
              ...
              >
                {testingConnection ? (
                  <IconifyIcon icon="mdi:loading" rotate={1} style={{ fontSize: '1.1rem' }} />
                ) : (
                  <IconifyIcon icon="mdi:play-circle-outline" style={{ fontSize: '1.2rem' }} />
                )}
              </button>
            </div>
          </div>
...
// Fonctions utilitaires pour le masquage RGPD
const maskName = (name) => {
  if (!name) return '-';

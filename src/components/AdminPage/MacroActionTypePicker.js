...
function isMacroActionOptionDisabled(opt, { isCommunity, isTeamsIntegrationActive, macroWebhookOptionsCount }) {
  if (isCommunity && opt.proOnly) return true;
  if (
    opt.value === "teams_message" &&
    !isCommunity &&
...
export default function MacroActionTypePicker({
  ...
  const updateMenuPosition = useCallback(() => {
    ...
    const menuHeight = openUp
      ? Math.min(maxMenuHeight, spaceAbove)
  ...
  const menuNode = open ? (
    <ul
      ref={menuRef}
      className={styles.actionTypeMenu}
      style={
        menuStyle || {
          position: "fixed",
          top: 0,
        ...
        return (
          <li key={opt.value} role="presentation">
            <button
              ...
              disabled={disabled}
              className={`${styles.actionTypeOption} ${isSelected ? styles.actionTypeOptionActive : ""} ${
                disabled ? styles.actionTypeOptionDisabled : ""
              }`}
              onClick={() => {
                if (disabled) return;
                onChange(opt.value);
            ...
            >
              ...
              <span className={styles.actionTypeOptionText}>
                ...
                {showWebhookHint ? (
                  <span className={styles.actionTypeOptionHint}>Webhook requis</span>
                ) : null}
              </span>
              {opt.proOnly ? (
                <ProFeatureBadge variant="inline" className={styles.actionTypeProBadge} />
              ) : null}
            </button>
          </li>
        );
  ...
  return (
    <div className={styles.actionTypePicker} ref={rootRef}>
      <button
        ref={triggerRef}
        type="button"
        className={`${styles.actionTypeTrigger} ${triggerClassName}`.trim()}
      ...
      >
        ...
        <Icon
          ...
          className={styles.actionTypeChevron}
          aria-hidden
        />
      </button>

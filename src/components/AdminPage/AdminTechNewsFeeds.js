...
import { Badge, Btn, BtnIcon, Card, ConfirmModal, Page, SubTabs, Switch, Table, Toolbar } from "./AdminUi";
...
export default function AdminTechNewsFeeds() {
  ...
  const localeTabs = useMemo(() => APP_LOCALES.map(loc => ({
    key: loc.code,
  ...
  return <Page>
      ...
      <Card title={copy.title} description={interpolate(copy.description, {
      locale: activeLocale?.label || feedLocale
    })} fill action={<Btn icon="mdi:plus" onClick={openCreate}>
            {copy.addFeed}
          </Btn>}>
      ...
      <TechNewsFeedFormModal open={modalOpen} mode={editing ? "edit" : "create"} draft={form} setDraft={setForm} saving={saving} categoryOptions={meta.categories} categoryLabel={categoryLabel} onClose={closeModal} onSave={saveFeed} />

      <ConfirmModal open={confirmReset} onClose={() => !saving && setConfirmReset(false)} onConfirm={handleReset} title={copy.resetTitle} message={interpolate(copy.resetMessage, {
      locale: activeLocale?.label || feedLocale
    })} confirmLabel={adminCopy.reset} icon="mdi:restore" confirmVariant="primary" />

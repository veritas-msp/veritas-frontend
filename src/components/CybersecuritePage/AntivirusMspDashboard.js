...
import {
  buildAntivirusFleetFromClients,
  buildAntivirusFleetStats,
  filterAntivirusFleetRows,
  groupAntivirusFleetByProvider,
  sortAntivirusFleetRows,
...
function UsageCell({ used, total, percent, formatLicensesTitle }) {
  ...
  const fillClass =
    pct != null && pct >= 95 ? styles.usageFill_bad : pct != null && pct >= 80 ? styles.usageFill_warn : "";
  return (
    <div className={styles.usageBar} title={total != null ? formatLicensesTitle(used, total) : undefined}>
      {pct != null ? (
        <div className={styles.usageTrack}>
          <div className={`${styles.usageFill} ${fillClass}`} style={{ width: `${Math.min(100, pct)}%` }} />
        </div>
      ) : null}
...
function FleetTableRow({ row, onOpen, onOpenClient, onMiddleClick, getStatusMeta, formatDate, formatDateTime, formatLicensesTitle }) {
    ...
    >
      ...
      <td>
        <SolutionCell
          ...
          subtitle={row.solutionSubtitle || row.mappingMode}
      ...
      <td>
        <UsageCell
          used={row.usedLicenses}
          total={row.totalLicenses}
          percent={row.usagePercent}
          formatLicensesTitle={formatLicensesTitle}
        />
      </td>
...
export default function AntivirusMspDashboard({
  ...
  const filteredRows = useMemo(
    () => filterAntivirusFleetRows(fleetRows, { search, statusFilter, providerFilter }),
    [fleetRows, search, statusFilter, providerFilter]
  );

  const sortedRows = useMemo(
    () => sortAntivirusFleetRows(filteredRows, sortBy, sortDirection),
    ...
    <div className={styles.dashboard}>
        ...
        <div className={styles.heroLeft}>
          ...
          <div className={styles.heroCopy}>
            ...
            <p className={styles.heroDesc}>
              {stats.issues > 0
                ? copy.formatHeroIssues("antivirus", stats.issues)
                : av.heroDescOk}
            </p>
          </div>
        </div>
                ...
                >
                  ...
                  <span className={styles.priorityBody}>
                    ...
                    <span className={styles.priorityMeta}>
                      ...
                      {row.expirationDate
                        ? ` · ${msp.expPrefix} ${formatDisplayDate(row.expirationDate)}`
                        : ""}
                    </span>
                  </span>
      ...
      <div className={styles.toolbar}>
        <label className={styles.searchBox}>
          ...
          <input
            type="search"
            placeholder={av.searchPlaceholder}
            value={search}
      ...
      {loading ? (
        ...
        <MspEmptyState
          ...
          title={fleetRows.length === 0 ? av.emptyTitle : av.noResultsTitle}
          text={fleetRows.length === 0 ? av.emptyText : av.noResultsText}
        />
      ) : (
                          ...
                          <td colSpan={colCount}>
                            ...
                            >
                              {group.providerImage ? (
                                ...
                                <Icon icon={group.providerIcon || "mdi:shield-bug-outline"} width={16} />
                              )}
                      ...
                      {!collapsed
                        ? group.list.map((row) => (
                            <FleetTableRow
                              ...
                              formatLicensesTitle={copy.formatLicensesTitle}

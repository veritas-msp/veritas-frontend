...
function KpiCard({ icon, label, value, tone = "neutral", active, disabled, onClick }) {
  const toneKey =
    tone === "red" ? "bad" : tone === "amber" || tone === "orange" ? "warn" : tone === "violet" ? "neutral" : tone === "blue" ? "good" : tone;
  return (
    <button
      type="button"
...
function buildSecurityActions({ antivirusData, antispamData, campaigns, isCommunity, actionsCopy }) {
  const actions = [];
  ...
  const toneOrder = { bad: 0, warn: 1, good: 2 };
...
export default function SecurityOverviewPanel({
  ...
  const stats = useMemo(() => {
    const avIssues =
      antivirusData.filter((r) => r.status === "inactif" || r.status === "expire_bientot").length;
    const antispamIssues =
      antispamData.filter((r) => r.status === "inactif" || r.status === "expire_bientot").length;
    const campaignIssues = isCommunity
      ? 0
      : campaigns.filter((c) => c.status === "suspendue").length;
    ...
    const healthScore =
      monitored > 0 ? Math.round((healthy / monitored) * 100) : total === 0 ? 100 : 40;
    return { avIssues, antispamIssues, campaignIssues, total, healthScore };
  ...
  const securityPriorityItems = useMemo(
    () =>
  ...
  return (
    <div className={`${dashStyles.dashboard} ${panelStyles.dashboard} ${panelStyles.overviewStack}`}>
      <MspOverviewHexPanel
        title={overview.hexTitle}
        hint={stats.total > 0 ? copy.formatPortfolioAlerts(stats.total) : undefined}
        items={hexItems}
      />

      <MspPriorityPanel
        title={overview.priorityTitle}
        countLabel={
          priorityActions.length > 0 ? copy.formatPortfolioAlerts(stats.total) : undefined
        }
        items={securityPriorityItems}
        onItemClick={(item) => handleOpenAction(item.action)}
        emptyIcon="mdi:shield-check-outline"
        emptyTitle={overview.emptyTitle}
        emptyText={overview.emptyText}
        limit={15}
      />

      <SolutionStatsBand
        eyebrow={overview.av.eyebrow}
        title={overview.av.title}
        description={copy.formatAvFleetDescription(avStats)}
        icon="mdi:shield-search"
        viewFleetLabel={overview.viewFleet}
        onOpen={() => onGoTab?.("antivirus")}
      >
        <KpiCard
          icon="mdi:shield-bug-outline"
          label={overview.kpi.solutions}
          value={avStats.total}
          tone="blue"
          onClick={() => onGoTab?.("antivirus")}
        />
        <KpiCard
          ...
          label={overview.kpi.enterprises}
          value={avStats.clients}
          tone="neutral"
          onClick={() => onGoTab?.("antivirus")}
        />
        <KpiCard
          icon="mdi:desktop-classic"
          label={overview.kpi.endpoints}
          value={avStats.endpoints}
          tone="neutral"
          disabled={avStats.endpoints === 0}
          onClick={() => onGoTab?.("antivirus")}
        />
        <KpiCard
          icon="mdi:license"
          label={overview.kpi.licenses}
          value={avStats.licenses}
          tone="neutral"
          disabled={avStats.licenses === 0}
          onClick={() => onGoTab?.("antivirus")}
        />
        <KpiCard
          icon="mdi:alert-circle-outline"
          label={overview.kpi.todo}
          value={avStats.issues}
          tone={avStats.issues > 0 ? "red" : "good"}
          disabled={avStats.issues === 0}
          onClick={() => onGoTab?.("antivirus")}
        />
        <KpiCard
          icon="mdi:heart-pulse"
          label={overview.kpi.health}
          value={avStats.healthScore ?? "-"}
          tone={
            avStats.healthScore == null
              ? "neutral"
              : avStats.healthScore >= 80
                ? "good"
                : avStats.healthScore >= 50
                  ? "warn"
                  : "bad"
          }
          onClick={() => onGoTab?.("antivirus")}
      ...
      <SolutionStatsBand
        eyebrow={overview.as.eyebrow}
        title={overview.as.title}
        description={copy.formatAsFleetDescription(asStats)}
        icon="mdi:email-secure-outline"
        viewFleetLabel={overview.viewFleet}
        onOpen={() => onGoTab?.("antispam")}
      >
        <KpiCard
          icon="mdi:email-secure-outline"
          label={overview.kpi.solutions}
        ...
        <KpiCard
          icon="mdi:office-building-outline"
          label={overview.kpi.enterprises}
          value={asStats.clients}
          tone="neutral"
          onClick={() => onGoTab?.("antispam")}
        />
        <KpiCard
          icon="mdi:account-group-outline"
          label={overview.kpi.users}
          value={asStats.users}
          tone="neutral"
          disabled={asStats.users === 0}
          onClick={() => onGoTab?.("antispam")}
        />
        <KpiCard
          icon="mdi:web"
          label={overview.kpi.domains}
          value={asStats.domains}
          tone="neutral"
          disabled={asStats.domains === 0}
          onClick={() => onGoTab?.("antispam")}
        />
        <KpiCard
          icon="mdi:alert-circle-outline"
          label={overview.kpi.todo}
          value={asStats.issues}
          tone={asStats.issues > 0 ? "amber" : "good"}
          disabled={asStats.issues === 0}
          onClick={() => onGoTab?.("antispam")}
        />
        <KpiCard
          icon="mdi:heart-pulse"
          label={overview.kpi.health}
          value={asStats.healthScore ?? "-"}
          tone={
            asStats.healthScore == null
              ? "neutral"
              : asStats.healthScore >= 80
                ? "good"
                : asStats.healthScore >= 50
                  ? "warn"
                  : "bad"
          }
          onClick={() => onGoTab?.("antispam")}
...
export function computeSecurityOverviewStats(antivirusData, antispamData, campaigns, isCommunity) {
  ...
  const campaignIssues = isCommunity
    ? 0
    : (campaigns || []).filter((c) => c.status === "suspendue").length;
  return { total: avIssues + antispamIssues + campaignIssues, avIssues, antispamIssues, campaignIssues };
}

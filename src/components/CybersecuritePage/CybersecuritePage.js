...
// ──────────────────────────────
// 🧩 Composant : CybersecuritePage
// ──────────────────────────────
const COMMUNITY_DEFAULT_TAB = "overview";

export default function CybersecuritePage({ onNavigate, cybersecuriteParams }) {
  ...
  const [allCampaigns, setAllCampaigns] = useState([]); // Pour les statistiques globales
  ...
  const [selectedStatuses, setSelectedStatuses] = useState(new Set()); // Pour antivirus
  const [selectedPaymentPlans, setSelectedPaymentPlans] = useState(new Set()); // Pour antivirus

  // Tri des colonnes (antivirus legacy)
  const [antivirusSortBy, setAntivirusSortBy] = useState('expirationDate');
  const [antivirusSortOrder, setAntivirusSortOrder] = useState('asc'); // par défaut : date d'expiration du plus ancien au plus récent

  const [antivirusCurrentPage, setAntivirusCurrentPage] = useState(1);
  const [antivirusPageSize, setAntivirusPageSize] = useState(10);

  // Appliquer les paramètres de navigation (onglet actif)
  useEffect(() => {
  ...
  // Fonction pour obtenir la date dans un mois au format YYYY-MM-DD
  ...
  // Bootstrap : GET /clients/cyber-page-data (campagnes + m_antivirus / m_antispam / m_save)
  useEffect(() => {
    (async () => {
      loadUsers();
      await loadClients({ withModules: true });
    })();
    return () => {
  ...
  // Fetch par onglet (même logique "simple" que ServicePage):
  // chaque onglet charge uniquement ses données nécessaires.
  useEffect(() => {
  ...
  const loadCampaignsDataset = async ({ skipCache = false } = {}) => {
    if (!skipCache) {
    ...
    const request = getAllCampaigns({}, { signal: controller.signal })
      .then((data) => {
  ...
  const loadAllCampaignsForStats = async () => {
    try {
      ...
      console.error('Erreur lors du chargement des campagnes pour stats:', error);
      setAllCampaigns([]);
    }
  };
  ...
  const loadClients = async ({ withModules = true, force = false, skipCache = false } = {}) => {
      ...
      if (withModules) {
        ...
        try {
          sessionStorage.setItem(
            ...
            JSON.stringify({
              savedAt: Date.now(),
              data: { clients: clientsData, campaigns: campaignsPayload },
            })
          );
        } catch {
  ...
  const saveBitdefenderSync = async ({ clientId, solutionName, companyId, companyName, syncData }) => {
    try {
      ...
      console.error('❌ Erreur sauvegarde Bitdefender:', error);
      return false;
    }
  };

  // Fonction pour synchroniser toutes les licences Bitdefender
  const syncAllBitdefenderLicenses = async () => {
    ...
    try {
      ...
      // Parcourir tous les clients qui ont des licences Bitdefender
      let processed = 0;
      for (const target of targets) {
        const { client, solution } = target;
        try {
          setSyncStatus(pageCopy.formatSyncProgress(processed + 1, targets.length, client.name));
          console.log(`Synchronisation Bitdefender pour ${client.name} (Company ID: ${solution.companyId})`);
          ...
          if (saved) {
            successCount++;
            console.log(`✅ Synchronisation + sauvegarde réussie pour ${client.name}`);
          } else {
            throw new Error('Erreur lors de la sauvegarde');
          }
        } catch (error) {
          console.error(`❌ Erreur synchronisation pour ${client.name}:`, error);
          errorCount++;
        } finally {
          processed += 1;
      ...
      // Recharger les clients après la synchronisation
      await loadClients({ withModules: true, force: true, skipCache: true });
  ...
  // Parc antivirus / antispam : toutes les solutions enregistrées sur les fiches entreprise
  ...
  const filteredAntivirusData = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(item =>
        ...
        (item.licenseKey && item.licenseKey.toLowerCase().includes(query)) ||
        ...
        (item.paymentPlan && item.paymentPlan.toLowerCase().includes(query))
      );
    }
  ...
  // Comptages par statut antivirus (pour les KPI cliquables)
  const antivirusStatusCounts = useMemo(() => {
    const counts = { actif: 0, expire_bientot: 0, inactif: 0 };
  ...
  // Clic sur une carte KPI antivirus : appliquer le filtre par statut
  const handleAntivirusStatCardClick = (status) => {
    setSelectedStatuses(new Set([status]));
  };

  // Clic sur la carte "Total" antivirus : réinitialiser le filtre statut
  ...
  // Fonctions pour les modals de campagne
  const resetForm = () => {
    setFormData({
      client_id: '',
      name: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCampaign) {
        ...
        // Recharger les campagnes filtrées (pour l'onglet campagnes)
        invalidateCampaignsCache();
        await loadCampaigns({ skipCache: true });
        // Recharger toutes les campagnes (pour les statistiques)
        await loadAllCampaignsForStats();
        setShowModal(false);
        setEditingCampaign(null);
        resetForm();
      } else {
        // Créer la campagne et récupérer les données
        const newCampaign = await createClientCampaign(formData.client_id, formData);
        ...
        // Recharger les campagnes pour mettre à jour la liste
        invalidateCampaignsCache();
        await loadCampaigns({ skipCache: true });
        await loadAllCampaignsForStats();
        
        // Naviguer vers la page de détail de la campagne créée
        if (onNavigate && newCampaign) {
          // S'assurer que la campagne a toutes les données nécessaires
          const campaignData = {
            ...newCampaign,
  ...
  const handleOpenCampaignClient = ({ clientId, name }) => {
    if (!onNavigate || !clientId) return;
    onNavigate("ContratDetail", { clientId, name });
  };
  ...
  // Calculer les statistiques des licences Bitdefender
  const antivirusStats = useMemo(() => {
    ...
    const itemsWithUsage = antivirusData.filter(item =>
      item.usedLicenses !== null &&
      item.totalLicenses !== null &&
      item.totalLicenses > 0
    );

    const avgUsagePercent = itemsWithUsage.length > 0
      ? itemsWithUsage.reduce((sum, item) =>
          sum + (item.usedLicenses / item.totalLicenses), 0) / itemsWithUsage.length
      : 0;
  ...
  const securityOverviewStats = useMemo(
    () => computeSecurityOverviewStats(antivirusData, antispamData, campaigns, isCommunity),
  ...
  return (
    <div className={`${styles.mspPage} ${styles.mspPageOrbital}`}>
      ...
      <div className={styles.mspMain}>
          ...
          <div className={styles.mspTabBar} role="tablist" aria-label={pageCopy.tabSectionsAria}>
            {pageCopy.tabs.map((tab) => {
              ...
              const showBadge =
                tab.key === "overview"
                  ? badge > 0
                  : tab.key === "antivirus" || tab.key === "antispam" || tab.key === "campaigns";
              return (
              <button
                key={tab.key}
                type="button"
                className={[
                  styles.mspTab,
                  activeTab === tab.key ? styles.mspTabActive : "",
                  tab.proOnly && isCommunity ? styles.mspTabProLocked : "",
                ].filter(Boolean).join(" ")}
                onClick={() => handleTabSelect(tab)}
                title={
                  tab.proOnly && isCommunity
                    ? `${tab.label}${pageCopy.proTabSuffix}`
                    : tab.label
                }
                aria-disabled={tab.proOnly && isCommunity ? true : undefined}
              >
                ...
                <span className={styles.mspTabLabelRow}>
                  ...
                  {tab.proOnly && isCommunity ? (
                    <ProFeatureBadge variant="inline" className={styles.mspTabProBadge} />
                  ) : null}
                </span>
              </button>
            );
          ...
          <div className={styles.tabContent}>
            {activeTab === "overview" && (
              <SecurityOverviewPanel
                ...
                antispamData={antispamData}
                campaigns={campaigns}
                loading={loadingCyberData}
                isCommunity={isCommunity}
                onGoTab={goToTab}
                onOpenAntivirus={handleViewAntivirusSolution}
                onOpenAntispam={handleViewAntispamSolution}
                onOpenClient={handleOpenAntivirusClient}
              />
            )}

            {activeTab === "antivirus" && (
              <AntivirusMspDashboard
                copy={pageCopy}
                clients={clients}
                loading={loadingCyberData}
                onOpenSolution={handleViewAntivirusSolution}
                onOpenClient={handleOpenAntivirusClient}
              />
            )}

            {activeTab === "antispam" && (
              <AntispamMspDashboard
                copy={pageCopy}
                clients={clients}
                loading={loadingCyberData}
                onOpenSolution={handleViewAntispamSolution}
                onOpenClient={handleOpenAntivirusClient}
              />
            )}

            {activeTab === "campaigns" && (
              <CampaignsMspDashboard
                copy={pageCopy}
                campaigns={campaigns}
                loading={loadingCampaigns}
                onViewCampaign={handleViewCampaign}
                onOpenClient={handleOpenCampaignClient}
                onAddCampaign={() => {
                  setEditingCampaign(null);
                  resetForm();
                  setShowModal(true);
        ...
        <CampaignFormModal
          ...
          copy={campaignsCopy}
          getCampaignTypeLabel={pageCopy.getCampaignTypeLabel}
        />
            ...
            <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
              ...
              <div className={styles.modalBody}>
                ...
                <div style={{ marginTop: '1rem' }}>
                  ...
                  <input
                    ...
                    placeholder={campaignsCopy.delete.confirmPlaceholder}
                    style={{
                      width: '100%',
                      padding: '0.5rem',
              ...
              <div className={styles.modalActions}>
                ...
                <button
                  ...
                  disabled={confirmText !== campaignsCopy.delete.confirmWord}
                  style={{
      ...
      <ProFeaturePromoModal
        open={campaignProPromoOpen}
        featureKey={pageCopy.proFeatureKey}
        onClose={() => setCampaignProPromoOpen(false)}
      />
    </div>
  );
}

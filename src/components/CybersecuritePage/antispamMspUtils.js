import {
  ...
  computeAntispamExpirationStatus,
...
export function filterAntispamFleetRows(
  rows,
  { search = "", statusFilter = "all", providerFilter = "all", clientFilter = null } = {}
) {
  ...
  if (query) {
    filtered = filtered.filter(
      (row) =>
        ...
        row.solutionSubtitle?.toLowerCase().includes(query) ||

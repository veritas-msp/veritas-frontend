...
import { FaWifi, FaSync, FaExclamationCircle } from "react-icons/fa";
...
const Wifi = ({ config, setConfig, data, setData, onSyncAllCheckMKReady }) => {
    ...
    // Fonction helper pour obtenir le mapping CheckMK d'un wifi (par nom ou id)
    ...
    // Charger les bornes WiFi depuis la base (v_b_clients_m_wifi) au montage
    useEffect(() => {
        ...
        const loadWifiFromDb = async () => {
            try {
                ...
                console.error("Erreur chargement bornes WiFi:", err);
            }
        };
    ...
    // Fonction pour formater les informations de la borne WiFi
    ...
    // Fonction pour sauvegarder une borne WiFi
    const saveWifi = async (wifiId, wifiData) => {
        ...
        const { id, __fromDb, __index, ...dataForDb } = wifiData;

        const body = {
            item_key: wifiData.nom || `wifi-${wifiId}`,
            name: wifiData.nom || `Borne WiFi`,
            data: dataForDb,
            is_active: true
        };
        ...
        const url = wifiId
            ? `${API_BASE_URL}/clients/modules/${clientId}/bornes-wifi/${wifiId}`
            : `${API_BASE_URL}/clients/modules/${clientId}/bornes-wifi`;
    ...
    const handleSaveWifi = async () => {
        ...
        try {
            ...
            toast.success("Borne WiFi mise à jour", toastOptions);
            setEditingWifi(null);
            setEditForm(null);
        } catch (error) {
        ...
        wifiList.forEach(ap => {
            ...
            if (wifiData) {
                ...
                if (wifiData.checkmkData) {
                    setCheckmkData(prev => ({ ...prev, [ap.nom]: wifiData.checkmkData }));
                }
                
                // Restaurer les événements
                if (wifiData.eventsCount !== undefined) {
                    setEventsCount(prev => ({ ...prev, [ap.nom]: wifiData.eventsCount }));
                }
                if (wifiData.eventsData) {
                    setEventsData(prev => ({ ...prev, [ap.nom]: wifiData.eventsData }));
                }
                
                // Restaurer la disponibilité
                if (wifiData.availabilityData) {
                    setAvailabilityData(prev => ({ ...prev, [ap.nom]: wifiData.availabilityData }));
                }
            }
        });
    }, [wifiList, data]);
    ...
    const loadEventsCount = async (wifiName, checkmkHostName) => {
        ...
        if (!period.start_time || !period.end_time) {
            setEventsCount(prev => ({ ...prev, [wifiName]: 0 }));
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                setEventsCount(prev => ({ ...prev, [wifiName]: eventsCountValue }));
                setEventsData(prev => ({ ...prev, [wifiName]: eventsDataValue }));
            ...
            console.error(`Erreur récupération événements pour ${wifiName}:`, error);
            setEventsCount(prev => ({ ...prev, [wifiName]: 0 }));
        }
    };
    ...
    const loadAvailabilityData = async (wifiName, checkmkHostName) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setAvailabilityData(prev => ({ ...prev, [wifiName]: availabilityValue }));
            ...
            console.error(`Erreur récupération disponibilité pour ${wifiName}:`, error);
        }
    };
    ...
    const loadCheckMKData = async (wifiName, checkmkHostName, showToasts = true) => {
        ...
        setAnimatedScore(prev => ({ ...prev, [wifiName]: 0 }));
        setScoreAnimationComplete(prev => ({ ...prev, [wifiName]: false }));

        setLoadingCheckMK(prev => ({ ...prev, [wifiName]: true }));
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCheckmkData(prev => ({ ...prev, [wifiName]: checkmkDataValue }));
                ...
                if (periodReport?.start_time && periodReport?.end_time) {
                    try {
                        ...
                        setEventsCount(prev => ({ ...prev, [wifiName]: ev.events_count || 0 }));
                        setEventsData(prev => ({ ...prev, [wifiName]: ev.events || [] }));
                        if (av.availability) {
                            setAvailabilityData(prev => ({ ...prev, [wifiName]: av.availability }));
                        } else {
                            await loadAvailabilityData(wifiName, checkmkHostName);
                        }
                    } catch (e) {
                        await Promise.all([
                            loadEventsCount(wifiName, checkmkHostName),
                            loadAvailabilityData(wifiName, checkmkHostName)
                        ]);
                    }
                } else {
                    await Promise.all([
                ...
                setTimeout(() => {
                    ...
                    if (finalScore !== null) {
                        ...
                        const scoreTimer = setInterval(() => {
                            ...
                            setAnimatedScore(prev => ({ ...prev, [wifiName]: Math.round(newValue) }));
                            
                            if (currentStep >= steps) {
                                clearInterval(scoreTimer);
                                setAnimatedScore(prev => ({ ...prev, [wifiName]: finalScore }));
                                setTimeout(() => {
                                    setScoreAnimationComplete(prev => ({ ...prev, [wifiName]: true }));
                                }, 100);
                            }
                        }, duration / steps);
                    }
                    ...
                    if (finalServices > 0) {
                        setAnimatedServices(prev => ({ ...prev, [wifiName]: 0 }));
                        const duration = 2000;
                        const steps = 100;
                        ...
                        const servicesTimer = setInterval(() => {
                            ...
                            setAnimatedServices(prev => ({ ...prev, [wifiName]: Math.round(newValue) }));
                            
                            if (currentStep >= steps) {
                                clearInterval(servicesTimer);
                                setAnimatedServices(prev => ({ ...prev, [wifiName]: finalServices }));
                            }
                        }, duration / steps);
                    }
                    ...
                    if (finalEvents !== undefined) {
                        setAnimatedEvents(prev => ({ ...prev, [wifiName]: 0 }));
                        const duration = 2000;
                        const steps = 100;
                        ...
                        const eventsTimer = setInterval(() => {
                            ...
                            setAnimatedEvents(prev => ({ ...prev, [wifiName]: Math.round(newValue) }));
                            
                            if (currentStep >= steps) {
                                clearInterval(eventsTimer);
                                setAnimatedEvents(prev => ({ ...prev, [wifiName]: finalEvents }));
                            }
                        }, duration / steps);
                    }
                    ...
                    if (availability && availability.up !== undefined) {
                        ...
                        setAnimatedAvailability(prev => ({ ...prev, [wifiName]: 0 }));
                        const duration = 2000;
                        const steps = 100;
                        ...
                        const availabilityTimer = setInterval(() => {
                            ...
                            setAnimatedAvailability(prev => ({ ...prev, [wifiName]: Math.round(newValue) }));
                            
                            if (currentStep >= steps) {
                                clearInterval(availabilityTimer);
                                setAnimatedAvailability(prev => ({ ...prev, [wifiName]: finalAvailability }));
                            }
                        }, duration / steps);
                    }
                }, 100);
            ...
            setLoadingCheckMK(prev => ({ ...prev, [wifiName]: false }));
        }
    };

    // Synchroniser toutes les bornes WiFi mappées
    const syncAllCheckMK = useCallback(async () => {
        ...
        if (mappedWifis.length === 0) {
            toast.warning('Aucune borne WiFi mappée avec Check MK', toastOptions);
            return;
        }
        
        const syncPromises = mappedWifis.map(ap => 
            loadCheckMKData(ap.nom, checkmkMappings[ap.id].checkmk_host_name, false)
        );
    ...
    useEffect(() => {
        if (onSyncAllCheckMKReadyRef.current && syncAllCheckMKRef.current) {
            ...
            // Stabiliser l'état de synchro global pour éviter les clignotements
            if (
    ...
    // Fonction pour calculer le score de santé de la borne WiFi
    const getWifiHealthScore = (wifiName) => {
        ...
        // Pour les périphériques mappés, calculer uniquement avec événements + disponibilité
        let score = 0;
        let weightSum = 0;
        ...
        // 1. Score de disponibilité (50% du score total)
        ...
        // 2. Score événements (50% du score total)
        let eventsScore = 100;
    ...
    const groupedBySite = wifiList.reduce((acc, ap) => {
        const siteName = ap.site || "Sans site";
        if (!acc[siteName]) {
            acc[siteName] = [];
        }
        ...
        <div className={styles.container}>
            ...
            <div className={styles.wifiGrid}>
                    ...
                    .map((ap, i) => {
                        const needsSyncAttention = Boolean(
                            ap.id && checkmkMappings[ap.id] &&
                            !data?.[ap.nom]?.lastSyncDate &&
                            !loadingCheckMK[ap.nom]
                        );
                        return (
                            <div key={i} className={`${styles.wifiCard} ${styles.withComment}`}>
                                        ...
                                        <div className={styles.buttonGroup}>
                                            {ap.id && checkmkMappings[ap.id] && (
                                                <button
                                                    ...
                                                    title={`Mappé vers Check MK: ${checkmkMappings[ap.id].checkmk_host_name}. Cliquer pour synchroniser.`}
                                                    disabled={loadingCheckMK[ap.nom]}
                                                    style={{
                                                        display: 'inline-flex',
                                                        alignItems: 'center',
                                                ...
                                                >
                                                    <IconifyIcon
                                                        ...
                                                        width={14}
                                                        height={14}
                                                        className={loadingCheckMK[ap.nom] ? styles.loadingIcon : ''}
                                                    />
                                                </button>
                                            )}
                                            <button
                                ...
                                {(() => {
                                    const { info } = getWifiInfo(ap);
                                    return info.length > 0 && (
                                        <div className={`${commonStyles.moduleMeta} ${styles.wifiMetaFlex}`}>
                                            <span style={{ flex: 1 }}>
                                                {info.join(" • ")}
                                            </span>
                                        </div>
                                ...
                                {/* Container pour Santé + Stats sur la même ligne - masqué si non mappé */}
                                {ap.id && getCheckMKMapping(ap.id) && (
                                <div style={{
                                    display: 'flex',
                                    flexDirection: 'row',
                                    gap: '0.5rem',
                                                ...
                                                <div style={{
                                                    ...
                                                    {isEditing ? (
                                                        <input
                                                            ...
                                                            min="0"
                                                            max="100"
                                                            value={editingScoreValue[ap.nom] !== undefined ? editingScoreValue[ap.nom] : displayScore}
                                                            onChange={(e) => setEditingScoreValue(prev => ({ ...prev, [ap.nom]: e.target.value }))}
                                                            onBlur={() => saveEditScore(ap.nom)}
                                                            onKeyDown={(e) => {
                                                                if (e.key === 'Enter') {
                                                        ...
                                                        <div
                                                            ...
                                                            onKeyDown={(e) => {
                                                                if (displayScore !== null && !isLoading && (e.key === 'Enter' || e.key === ' ')) {
                                                                    e.preventDefault();
                                                                    setEditingScore(prev => ({ ...prev, [ap.nom]: true }));
                                                                    setEditingScoreValue(prev => ({ ...prev, [ap.nom]: displayScore }));
                                                                }
                                                            }}
                                                        ...
                                                        >
                                                            <LetterScale 
                                                                ...
                                                                letters={["F", "E", "D", "C", "B", "A"]}
                                                                size="compact"
                                                                onSelect={!isLoading ? (letter) => handleManualLetterSelect(ap.nom, letter) : undefined}
                                ...
                                <div className={styles.statsGrid}>
                                    ...
                                    <div 
                                        className={`${styles.statCard} ${loadingCheckMK[ap.nom] ? styles.statCardDisabled : (checkmkData[ap.nom] ? styles.statCardEnabled : styles.statCardDefault)}`}
                                        onMouseEnter={(e) => {
                                            const mapping = ap.id ? getCheckMKMapping(ap.id) : null;
                                    ...
                                    >
                                        ...
                                        {!(ap.id && getCheckMKMapping(ap.id)) ? (
                                            ...
                                            <div className={`${styles.statValue} ${checkmkData[ap.nom] ? styles.statValuePrimary : styles.statValueSecondary}`}>
                                                {checkmkData[ap.nom] 
                                                    ? (animatedServices[ap.nom] !== undefined ? animatedServices[ap.nom] : (checkmkData[ap.nom].services?.length || 0))
                                                    : 'N/A'}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div 
                                        className={`${styles.statCard} ${loadingCheckMK[ap.nom] ? styles.statCardDisabled : (eventsCount[ap.nom] !== undefined ? styles.statCardEnabled : styles.statCardDefault)}`}
                                        onMouseEnter={(e) => {
                                            const mapping = ap.id ? getCheckMKMapping(ap.id) : null;
                                    ...
                                    >
                                        ...
                                        {!(ap.id && getCheckMKMapping(ap.id)) ? (
                                            ...
                                            <div className={`${styles.statValue} ${eventsCount[ap.nom] !== undefined ? (eventsCount[ap.nom] > 0 ? styles.statValueEventsWarning : styles.statValueEventsNormal) : styles.statValueSecondary}`}>
                                                {eventsCount[ap.nom] !== undefined 
                                                    ? (animatedEvents[ap.nom] !== undefined ? animatedEvents[ap.nom] : eventsCount[ap.nom])
                                                    : 'N/A'}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {/* Carte: Disponibilité */}
                                    <div 
                                        className={`${styles.statCard} ${loadingCheckMK[ap.nom] ? styles.statCardDisabled : (availabilityData[ap.nom] ? styles.statCardEnabled : styles.statCardDefault)}`}
                                        onMouseEnter={(e) => {
                                            const mapping = ap.id ? getCheckMKMapping(ap.id) : null;
                                            ...
                                            <div 
                                                ...
                                                style={{ 
                                                    color: availabilityData[ap.nom] && availabilityData[ap.nom].up !== undefined
                                                        ? (() => {
                                                            const displayValue = animatedAvailability[ap.nom] !== undefined 
                                                                ? animatedAvailability[ap.nom] 
                                                                : parseFloat(availabilityData[ap.nom].up || 0);
                                                            return getProgressiveColor(displayValue);
                                                        })()
                                                        : 'var(--text-secondary)'
                                                }}
                                            >
                                                {availabilityData[ap.nom] && availabilityData[ap.nom].up !== undefined 
                                                    ? `${animatedAvailability[ap.nom] !== undefined 
                                                        ? animatedAvailability[ap.nom] 
                                                        : Math.round(parseFloat(availabilityData[ap.nom].up || 0))}%`
                                                    : 'N/A'
                                                }
                                            </div>
                                        )}
                                ...
                                <textarea
                                    id={`comment-${ap.nom}`}
                                    value={data?.[ap.nom]?.comment || ""}
                                    onChange={(e) => updateComment(ap.nom, e.target.value)}
            ...
            {hoveredTooltip && (
                <div style={{
                    ...
                    {hoveredTooltip.type === 'availability' && availabilityData[hoveredTooltip.wifiName] && (
                        <div>
                            <div style={{
                                fontSize: '0.9rem',
                                fontWeight: '700',
                            ...
                            <div style={{
                                ...
                                {availabilityData[hoveredTooltip.wifiName].up !== undefined && (
                                    <div style={{
                                        display: 'flex',
                                        justifyContent: 'space-between',
                                        alignItems: 'center',
                                ...
                                {availabilityData[hoveredTooltip.wifiName].down !== undefined && (
                                    <div style={{
                                        display: 'flex',
                                        justifyContent: 'space-between',
                                        alignItems: 'center',
                    ...
                    {hoveredTooltip.type === 'services' && checkmkData[hoveredTooltip.wifiName] && (
                        <div>
                            <div style={{
                                fontSize: '0.9rem',
                                fontWeight: '700',
                            ...
                            <div style={{
                                ...
                                {(() => {
                                    const services = checkmkData[hoveredTooltip.wifiName].serviceInfo?.services || 
                                                   checkmkData[hoveredTooltip.wifiName].services || 
                                                   [];
                    ...
                    {hoveredTooltip.type === 'events' && eventsData[hoveredTooltip.wifiName] && eventsData[hoveredTooltip.wifiName].length > 0 && (
                        <div>
                            <div style={{
                                fontSize: '0.9rem',
                                fontWeight: '700',
                            ...
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                                {eventsCount[hoveredTooltip.wifiName] || 0} événement(s) sur la période.
                            </div>
                            <div style={{
                                display: 'flex',
                                flexDirection: 'column',
                                ...
                                {eventsData[hoveredTooltip.wifiName].length > 10 && (
            ...
            {/* Modal d'édition pour une borne WiFi */}
            {editingWifi && editForm && (
                    ...
                    <div className={styles.editModalBody}>
                        {isSaving ? (
                            ...
                            <div className={styles.editModalForm}>
                                ...
                                {/* Ligne 4 : Numéro de série + Adresse MAC */}
                                <div className={styles.editModalFormRow}>
                                    <label className={styles.editModalLabel}>
                                        Numéro de série
                                        <input
                                    ...
                                    if (globalSSIDs.length > 0) {
                                        return (
                                            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #e5e7eb' }}>
                                                <label style={{ fontWeight: 600, fontSize: '0.85rem', color: '#1a1a1a', marginBottom: '0.75rem', display: 'block' }}>
                                                    ...
                                                    {globalSSIDs.map((ssid) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                />
                                                                <span style={{ flex: 1, fontSize: '0.85rem', color: '#1a1a1a' }}>
                                                                    {ssidNom}
                                                                </span>
                                                            </label>
                                                        );

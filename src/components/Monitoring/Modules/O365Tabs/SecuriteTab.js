...
export default function SecuriteTab({ 
    ...
    // Calcul des recommandations triées
    ...
    const renderSortIcon = (column) => {
        const iconStyle = { marginLeft: '0.35rem' };
    ...
    return (
        <div className={styles.securitySection} style={{ position: 'relative' }}>
            <h4 className={styles.sectionTitle} style={{ marginBottom: '0.5rem' }}>Sécurité</h4>
            <div style={{
                width: '100%',
                height: '1px',
            ...
            {securityData ? (
                <>
                    {/* Section d'édition du score de sécurité (spécifique au module O365) */}
                    {identityScoreCurrent !== null && (
                        <div style={{
                            ...
                            <div style={{ 
                                ...
                                Note de sécurité globale (éditable)
                            </div>
                            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                {editingSecurityScore ? (
                                    <input
                                        type="number"
                                        min="0"
                                        max={identityScoreMax || 100}
                                        value={editingSecurityScoreValue !== null ? editingSecurityScoreValue : (identityScoreCurrent ?? '')}
                                        onChange={(e) => setEditingSecurityScoreValue(e.target.value)}
                                        onBlur={() => {
                                            if (editingSecurityScoreValue !== null) {
                                                ...
                                                update({ manualSecurityScore: percentage });
                                            }
                                    ...
                                    <div
                                        role="button"
                                        tabIndex={identityScoreCurrent !== null ? 0 : -1}
                                        onKeyDown={(e) => {
                                        ...
                                        title={identityScoreCurrent !== null ? "Cliquer pour éditer le score" : ""}
                                        style={{ 
                    ...
                    {/* Statistiques MFA : Total, Admin, Utilisateurs, Méthodes */}
                    <div style={{
                        display: 'flex',
                        gap: '1.5rem',
                        marginBottom: '1.5rem',
                        ...
                        <div style={{
                            ...
                            <div style={{ fontSize: '2rem', fontWeight: '700', color: theme === 'dark' ? '#f9fafb' : '#111827', marginBottom: '0.5rem' }}>
                                {users?.length || 0}
                            </div>
                            <div style={{ fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', marginBottom: '0.5rem' }}>
                                MFA: <strong style={{ color: theme === 'dark' ? '#f9fafb' : '#111827' }}>{securityData.mfa?.usersWithMFA || 0}</strong> | Taux: <strong style={{ color: theme === 'dark' ? '#f9fafb' : '#111827' }}>{(() => {
                    ...
                    {(() => {
                        ...
                        const getMethodDisplayInfo = (methodType) => {
                            switch (methodType) {
                                ...
                                default:
                                    return { name: methodType, icon: 'mdi:help-circle' };
                            }
                        };
                        ...
                        if (mfaMethodStats && Object.keys(mfaMethodStats).some(key => mfaMethodStats[key] > 0)) {
                            return (
                                <div style={{ marginBottom: '1.5rem' }}>
                                    <h5 style={{ margin: 0, fontSize: '0.9rem', fontWeight: '600', marginBottom: '0.75rem' }}>
                        ...
                        <div style={{ 
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                                <div style={{ minWidth: '160px', flex: '1' }}>
                                    <div style={{ fontSize: '0.85rem', fontWeight: '600', color: theme === 'dark' ? '#f9fafb' : '#111827', marginBottom: '0.75rem' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem' }}>
                                        ...
                                        <div style={{ fontSize: '0.9rem', color: theme === 'dark' ? '#d1d5db' : '#6b7280' }}>
                                            / {securityData.defenderSecureScore.maxScore || 0}
                                        </div>
                                    </div>
                                    {securityData.defenderSecureScore.percentage !== null && (
                                        <div style={{ fontSize: '0.85rem', color: theme === 'dark' ? '#d1d5db' : '#374151', marginTop: '0.25rem', fontWeight: '600' }}>
                                ...
                                {securityData.defenderSecureScore.averageComparativeScores && securityData.defenderSecureScore.averageComparativeScores.length > 0 && (
                                    <div style={{ flex: '1', minWidth: '200px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
                                        {securityData.defenderSecureScore.averageComparativeScores.map((comparison, idx) => (
                                            <div key={idx} style={{ fontSize: '0.8rem', color: theme === 'dark' ? '#d1d5db' : '#374151' }}>
                            ...
                            {securityData?.secureScoreHistory && securityData.secureScoreHistory.length > 0 && (
                                <div style={{ marginTop: '1rem' }}>
                                    <div style={{ fontSize: '0.8rem', fontWeight: '600', marginBottom: '0.5rem', color: theme === 'dark' ? '#d1d5db' : '#374151' }}>
                    ...
                    {/* Recommandations Secure Score avec tri personnalisé */}

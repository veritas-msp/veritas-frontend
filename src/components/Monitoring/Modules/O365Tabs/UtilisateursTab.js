...
import UtilisateursTabBase from '../../../ServicePage/TenantDetailTabs/UtilisateursTab';
...
export default function UtilisateursTab({ 
    ...
    // Réinitialiser la pagination quand on change de tri (doit être avant tout return)
    useEffect(() => {
    ...
    if (!users || users.length === 0) {
        return (
            <div className={styles.usersSection} style={{ position: 'relative' }}>
                <h4 className={styles.sectionTitle} style={{ marginBottom: '0.5rem' }}>Utilisateurs</h4>
                <div style={{
                    width: '100%',
                    height: '1px',
                ...
                {renderCommentSection && renderCommentSection('utilisateurs', 'Utilisateurs')}
            </div>
        );
    }
        ...
        <div className={styles.usersSection} style={{ position: 'relative' }}>
                        ...
                        {paginatedUsers.map((user, idx) => {
                            ...
                            return (
                                <tr key={startIndex + idx}>
                                    <td style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '150px', maxWidth: '150px' }}>
                                    ...
                                    <td>{lastLogin ? formatDate(lastLogin) : 'Jamais'}</td>
                                    <td>
                                        <span style={{
                                            padding: '0.25rem 0.5rem',
                                            borderRadius: '4px',
                                            fontSize: '0.75rem',
                                            backgroundColor: user.accountEnabled === false ? '#fee2e2' :
                                                           isActive30 ? '#d1fae5' :
                                                           isInactive ? '#fef3c7' : '#e0e7ff',
                                            color: user.accountEnabled === false ? '#dc2626' :
                                                   isActive30 ? '#059669' :
                                                   isInactive ? '#d97706' : '#4338ca',
                                            fontWeight: '500'
                                        }}>
                                            {user.accountEnabled === false ? 'Bloqué' :
                                             isActive30 ? 'Actif' :
                                             isInactive ? 'Inactif (>90j)' : 'Inactif'}
                                        </span>
                                    </td>
                                    <td style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        {user.licenses 
            ...
            {totalPages > 1 && (
                <div style={{
                    ...
                    <button
                        onClick={() => setUsersPagination(prev => Math.max(1, prev - 1))}
                        disabled={usersPagination === 1}
                        style={{
                            padding: '0.5rem 1rem',
                            background: usersPagination === 1 
                                ? (theme === 'dark' ? '#2a2a4a' : '#f3f4f6')
                                : (theme === 'dark' ? '#3b82f6' : '#3b82f6'),
                            color: usersPagination === 1 
                    ...
                    <button
                        onClick={() => setUsersPagination(prev => Math.min(totalPages, prev + 1))}
                        disabled={usersPagination === totalPages}
                        style={{
                            padding: '0.5rem 1rem',
                            background: usersPagination === totalPages 
                                ? (theme === 'dark' ? '#2a2a4a' : '#f3f4f6')
                                : (theme === 'dark' ? '#3b82f6' : '#3b82f6'),
                            color: usersPagination === totalPages 
                                ? (theme === 'dark' ? '#6b7280' : '#9ca3af')
                                : '#ffffff',
                            border: 'none',
                            borderRadius: '6px',

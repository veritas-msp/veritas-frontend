...
import DashboardTabBase from '../../../ServicePage/TenantDetailTabs/DashboardTab';
...
export default function DashboardTab({ 
    ...
    // Calcul du score de sécurité pour l'affichage
    ...
    const displayedScoreValue = isEditing
        ? (editingSecurityScoreValue !== null ? editingSecurityScoreValue : baseScoreValue)
        : (animatedSecurityScore !== null ? animatedSecurityScore : baseScoreValue);
    const normalizedDisplayedScore = displayedScoreValue !== null && displayedScoreValue !== undefined
        ? Math.max(0, Math.min(100, parseInt(displayedScoreValue, 10) || 0))
        : null;
    ...
    return (
        <div className={styles.dashboardSection} style={{ position: 'relative' }}>
                ...
                <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            {editingSecurityScore ? (
                                <input
                                    ...
                                    min="0"
                                    max="100"
                                    value={editingSecurityScoreValue !== null ? editingSecurityScoreValue : baseScoreValue ?? ''}
                                    ...
                                    onBlur={() => {
                                        if (editingSecurityScoreValue !== null) {
                                            ...
                                            update({ manualSecurityScore: score });
                                        }
                                ...
                                <div
                                    title={hasScore ? "Cliquer sur une lettre pour sélectionner une note" : ""}
                                    style={{ 
                                ...
                                >
                                    <LetterScale 
                                        ...
                                        letters={["F", "E", "D", "C", "B", "A"]}
                                        size="normal"
                                        onSelect={hasScore ? (letter) => {
                                            const score = letterToScore(letter);
                                            if (score !== null) {
                                                // Marquer comme changement manuel pour éviter l'animation
                                        ...
                                        highlightLetter={calculatedScore !== null && baseScoreValue !== null && baseScoreValue !== calculatedScore ? scoreToLetter(calculatedScore) : null}
                                    />
                                </div>
                            )}
                                ...
                                <FaInfoCircle 
                                    ...
                                    onMouseEnter={(e) => {
                                        const scoreBreakdown = securityScore?.factors?.map(factor => ({
                                            label: factor.name,
                                            description: factor.isPenalty 
                                                ? `Pénalité: -${(factor.weight - factor.earnedPoints).toFixed(0)} pts` 
                                                : `${factor.earnedPoints} pts`,
                                            weight: factor.isPenalty 
                                                ? `-${(factor.weight - factor.earnedPoints).toFixed(0)} pts` 
                                                : `${factor.earnedPoints} pts`,
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: '0.75rem', color: theme === 'dark' ? '#d1d5db' : '#374151', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                            ...
                            <strong>MFA Utilisateurs</strong>
                        </div>
                        <div style={{ fontSize: '0.75rem', color: theme === 'dark' ? '#d1d5db' : '#374151', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            ...
            {/* Score d'adoption Microsoft 365 */}
            {adoptionScore && (
                <div style={{
                    marginBottom: '1.5rem',
                    padding: '1rem',
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', marginBottom: '0.5rem', fontWeight: '600' }}>
                            Score d'adoption
                        </div>
                        <div style={{
                            fontSize: '2.5rem',
                        ...
                        {adoptionScore.totalScore && adoptionScore.maxScore && (
                            <div style={{
                                fontSize: '0.7rem',
                                color: theme === 'dark' ? '#9ca3af' : '#6b7280',
                                marginTop: '0.5rem',
                    ...
                    <div style={{
                        ...
                        {adoptionScore.peopleExperiences !== undefined && (
                            <div style={{ fontSize: '0.75rem', color: theme === 'dark' ? '#d1d5db' : '#374151', whiteSpace: 'nowrap' }}>
                                <strong>Expériences personnes:</strong> {adoptionScore.peopleExperiences} / 400
                            </div>
                        )}
                        {adoptionScore.technologyExperiences !== undefined && (
                            <div style={{ fontSize: '0.75rem', color: theme === 'dark' ? '#d1d5db' : '#374151', whiteSpace: 'nowrap' }}>
                                <strong>Expériences technologiques:</strong> {adoptionScore.technologyExperiences} / 400
                            </div>
                        )}
                    </div>
                </div>
            ...
            {dashboardMetrics && (
                <>
                    ...
                    <div className={styles.metricsRow} style={{ marginTop: '1rem' }}>
                        <div className={styles.metricItem}>
                            <div className={styles.metricLabel}>Licences totales</div>
                            <div className={styles.metricValue}>
                                {animatedDashboardMetrics?.totalLicenses ?? dashboardMetrics.totalLicenses}
                            </div>
                        </div>
                        <div className={styles.metricItem}>
                            <div className={styles.metricLabel}>Licences utilisées</div>
                        ...
                        <div className={styles.metricItem}>
                            ...
                            <div className={styles.metricValue}>
                                {(animatedDashboardMetrics?.totalLicenses ?? dashboardMetrics.totalLicenses) > 0 
                                    ? Math.round(((animatedDashboardMetrics?.usedLicenses ?? dashboardMetrics.usedLicenses) / (animatedDashboardMetrics?.totalLicenses ?? dashboardMetrics.totalLicenses)) * 100)
                                    : 0}%
                            </div>
                        </div>
                    </div>

...
// Services par défaut utilisés dans Firewalls.js
const defaultServices = ["CPU", "MEMOIRE", "TRAFIC", "UPTIME"];

// Modules disponibles dans Firewalls.js
...
const getAvailabilityValue = (firewallData = {}) => {
  const raw =
    firewallData?.availabilityData?.up ??
    firewallData?.availability?.up ??
    firewallData?.availability;
...
const getModuleStats = (firewallData = {}) => {
  if (!firewallData?.modules) {
    return { active: 0, inactive: 0, disabled: 0, total: 0 };
  }
...
const getFirewallStatus = (firewallName, firewallData = {}) => {
  if (!firewallData) return { status: "unknown", color: "#6b7280" };
...
// Fonction pour calculer le score de santé du firewall (identique à Firewalls.js)
export const computeFirewallHealthScore = (
  firewallData = {},
  {
    eventsCount = null,
    availabilityValue = null,
    moduleStats = { active: 0, inactive: 0, disabled: 0, total: 0 },
    isMapped = false
  }
) => {
...
const FirewallsSummary = ({ data, config, selectedSites = [] }) => {
  ...
  if (Object.keys(groupedBySite).length === 0) {
    return (
      <div className={`${styles.container} ${theme === "dark" ? styles.dark : ""}`}>
        <div className={styles.emptyState}>
          <p>Aucun firewall ne correspond aux filtres sélectionnés.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`${styles.container} ${theme === "dark" ? styles.dark : ""}`}>
      ...
      >
        {siteEntries.map(([siteName, siteFirewalls]) => (
            ...
            <div className={`${styles.siteColumn} ${siteCount === 1 ? styles.siteColumnGrid2x2 : ''}`}>
              {[...siteFirewalls].sort((a, b) => {
                ...
                const healthLetter = resolvedScore !== undefined && resolvedScore !== null
                  ...
                  : getFirewallLetterFromStatus(statusFallback.status);
                ...
                const identityPartsLeft = [
                  firewall.fabricant || firewall.marque,
                  firewall.modele,
                  firewall.numeroSerie ? `S/N ${firewall.numeroSerie}` : null
                ].filter(Boolean);

                const identityPartsRight = [
                  firewall.vlan ? `VLAN ${firewall.vlan}` : null,
                  firewall.ip
                ].filter(Boolean);
                ...
                // Partie VLAN/IP à afficher à droite du firmware sur la ligne 2
                const vlanIpLine = identityPartsRight.length > 0 ? identityPartsRight.join(" - ") : null;

                // Obtenir les informations HA
                const haInfo = firewall.modeHA ? { role: firewall.roleHA } : null;
                ...
                return (
                  <article key={idx} className={styles.serverHealthCard}>
                    ...
                    <div className={styles.cardContent}>
                      ...
                      {/* Sous-titre : identité / Firmware (avec VLAN/IP) */}
                      {(identityLine || firewall.firmware || vlanIpLine) && (
                        <div className={styles.serverMeta}>
                          ...
                          {/* Ligne 2 : Firmware + VLAN/IP à droite */}
                          {(firewall.firmware || vlanIpLine) && (
                      ...
                      {/* Note + stats compactes + grille 2x2 comme les serveurs */}
                      <div className={styles.healthScoreAndInfos}>
                        ...
                        {/* Colonne de droite : stats + grille CPU/RAM/TRAFIC/UPTIME + modules */}
                        <div className={styles.healthInfos}>
                          <div className={styles.statsAndServicesContainer}>
                            ...
                            {/* Grid 2x2 : CPU / RAM / TRAFIC / UPTIME (camemberts) */}
                            <div className={styles.modulesSummary}>
                              <div className={styles.modulesSummaryList}>
                                {serviceMetrics.map((service) => {
                                  ...
                                  return (
                                    <div key={`${firewall.nom}-${service.label}`} className={styles.serviceStatCard}>
                                      <ServiceIcon 
                                        size="1.6rem" 
                                        color={theme === "dark" ? "#9ca3af" : "#6b7280"}

...
import { FaServer, FaCube, FaCheckCircle, FaExclamationCircle, FaTimesCircle, FaQuestionCircle, FaInfoCircle, FaSync } from "react-icons/fa";
...
const SauvegardeSummary = ({ data, config }) => {
  ...
  const getJobInfo = (job) => {
    ...
    if (job.type && job.regularite && job.horaire) {
      // Ne pas afficher le type s'il est "job" (marqueur)
      const backupType = job.type.toLowerCase() !== 'job' ? job.type : null;
      if (backupType) {
        parts.push(`Sauvegarde ${backupType} ${job.regularite} à ${job.horaire}`);
      } else {
        parts.push(`Sauvegarde ${job.regularite} à ${job.horaire}`);
      }
    } else {
      // Ne pas afficher le type s'il est "job" (marqueur)
        ...
        <div style={{ marginTop: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {instances.map((instance, instanceIndex) => {
              ...
              return (
                <div key={`instance-${instanceIndex}`} className={styles.instanceCard}>
                    ...
                    <div className={styles.headerLeft}>
                      ...
                      <div className={styles.instanceInfo}>
                        ...
                        {instanceInfo.length > 0 && (
                          <p className={styles.instanceDetails}>
                            {instanceInfo.join(" • ")}
                          </p>
                        )}
                      </div>
                  ...
                  {/* Flux dynamique pour HyperBackup - Source vers Destination */}
                  {instance.logiciel === "HyperBackup" && instance.hyperbackupSource && instance.hyperbackupDestination && (
                    <div className={styles.flowInfo} style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                      <div className={styles.flowInline} style={{ gap: '2rem' }}>
                        <div className={styles.flowSource} style={{ minWidth: '180px', maxWidth: '220px', padding: '1.5rem' }}>
                          <span className={styles.flowIcon}>
                            {getStorageIcon(instance.hyperbackupSource, "2.5rem") || <Icon path={mdiNas} size="2.5rem" style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle', display: 'inline-block' }} />}
                          </span>
                        ...
                        <div className={styles.flowDestination} style={{ minWidth: '180px', maxWidth: '220px', padding: '1.5rem' }}>
                          <span className={styles.flowIcon}>
                            {getStorageIcon(instance.hyperbackupDestination, "2.5rem") || <Icon path={mdiNas} size="2.5rem" style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle', display: 'inline-block' }} />}
                          </span>
                          <span className={styles.flowText} style={{ fontSize: '1rem' }}>
                  ...
                  {/* Représentation visuelle Active Backup - Design moderne épuré */}
                  {instance.logiciel === "Active Backup for Microsoft 365" && instance.activeBackupModules && instance.activeBackupStorage && (
                    <div style={{ 
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', flexWrap: 'wrap' }}>
                              ...
                              return (
                                <div 
                                  ...
                                  style={{
                                    ...
                                    border: isActive 
                                      ? `1.5px solid #10b981`
                                      : `1px solid ${theme === 'dark' ? '#3a3a5a' : '#e2e8f0'}`,
                        ...
                        <div className={styles.flowDestination} style={{ minWidth: '180px', maxWidth: '220px', padding: '1.5rem' }}>
                          <span className={styles.flowIcon}>
                            {getStorageIcon(instance.activeBackupStorage, "2.5rem") || <Icon path={mdiNas} size="2.5rem" style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle', display: 'inline-block' }} />}
                          </span>
                          <span className={styles.flowText} style={{ fontSize: '1rem' }}>
                  ...
                  {(instance.logiciel === "Veeam" || instance.logiciel === "HYCU Backup") && (instance.jobs || []).length > 0 && (
                    <>
                      {/* Table des jobs */}
                      <div className={styles.jobsListSection} style={{ marginTop: '1rem' }}>
                        <div className={styles.jobsListTable}>
                          <div className={styles.jobsListHeader}>
                            <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Nom</div>
                            {(instance.logiciel === "HYCU Backup" || instance.logiciel === "Veeam") && (
                              <>
                                <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Cible</div>
                                <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Destination</div>
                              </>
                            )}
                            <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Type</div>
                            <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Régularité</div>
                            <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Horaire</div>
                            <div className={styles.jobsListCell} style={{ fontWeight: '600', fontSize: '0.75rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Rétention</div>
                          </div>
                          <div className={styles.jobsListTableContent}>
                            {(instance.jobs || []).map((job, jobIndex) => (
                              <div key={`job-list-${instanceIndex}-${jobIndex}`} className={styles.jobsListRow}>
                                <div className={styles.jobsListCell} style={{ fontWeight: '500', color: theme === 'dark' ? '#f9fafb' : '#111827' }}>
                                  {job.nom || `Job #${jobIndex + 1}`}
                                </div>
                                {instance.logiciel === "HYCU Backup" && (
                                  <>
                                    <div className={styles.jobsListCell} style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280' }}>
                                      {job.serveurLie || 'N/A'}
                                    </div>
                                    <div className={styles.jobsListCell} style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280' }}>
                      ...
                      {/* Cartes détaillées des jobs - Design moderne épuré */}
                      <div style={{ marginTop: '2rem' }}>
                        <h5 style={{
                          fontSize: '1rem',
                          fontWeight: '600',
                          ...
                          Détails des jobs ({(instance.jobs || []).length})
                        </h5>
                        <div style={{ 
                          display: 'grid',
                        ...
                        {(instance.jobs || []).map((job, jobIndex) => {
                          ...
                          return (
                            <div key={`job-${instanceIndex}-${jobIndex}`} style={{
                              padding: '1.5rem',
                                ...
                                <div className={styles.flowInfo} style={{ marginTop: '0.75rem', marginBottom: '0.75rem' }}>
                                  <div className={styles.flowInline}>
                                    <div className={styles.flowSource}>
                                      <span className={styles.flowIcon}>
                                        {getServerIcon(job.serveurLie) || <FaServer style={{ fontSize: '1.25rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle' }} />}
                                      </span>
                                    ...
                                    <div className={styles.flowDestination}>
                                      <span className={styles.flowIcon}>
                                        {getStorageIcon(job.stockageLie || job.destination, "1.25rem") || <Icon path={mdiNas} size="1.25rem" style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle', display: 'inline-block' }} />}
                                      </span>
                                    ...
                                    {job.stockageLie && job.replicationVers && (
                                      <>
                                        ...
                                        <div className={styles.flowDestination}>
                                          <span className={styles.flowIcon}>
                                            {getStorageIcon(job.replicationVers, "1.25rem") || <Icon path={mdiNas} size="1.25rem" style={{ color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle', display: 'inline-block' }} />}
                                          </span>
                                          <span className={styles.flowText}>
                              ...
                              {instance.logiciel === "HYCU Backup" && job.serveurLie && (
                                <div className={styles.flowInfo} style={{ marginTop: '0.75rem', marginBottom: '0.75rem', maxWidth: '550px', marginLeft: 'auto', marginRight: 'auto' }}>
                                  <div className={styles.flowInline}>
                                    ...
                                    <div className={styles.flowDestination}>
                                      <span className={styles.flowIcon}>
                                        <FaServer style={{ fontSize: '1.25rem', color: theme === 'dark' ? '#9ca3af' : '#6b7280', verticalAlign: 'middle' }} />
                                      </span>
                                      <span className={styles.flowText}>
                                        DC PSI Bouillac
                                      </span>
                              ...
                              {/* Statistiques CheckMK - uniquement pour les jobs Veeam */}
                              {instance.logiciel === "Veeam" && job.nom && (() => {
                                ...
                                return (
                                  <div style={{
                                    ...
                                    {/* Carte: Événements */}
                                    {(() => {
                                      // Récupérer les événements depuis les données - chercher dans plusieurs emplacements possibles
                                      let eventsCount = null;
                                      
                                      // 1. Chercher directement dans jobData
                                      ...
                                      // 3. Chercher dans checkmkEventsDetails (array)
                                      ...
                                      // Normaliser eventsCount (0 est valide)
                                      ...
                                      return (
                                        <div style={{
                                          ...
                                          <div style={{ 
                                            ...
                                            fontWeight: '700', 
                                            color: hasEventsData
                                              ? (eventsCount === 0 ? '#10b981' : eventsCount <= 3 ? '#f59e0b' : '#ef4444')
                                              : (theme === 'dark' ? '#9ca3af' : '#6b7280'),
                                            marginBottom: hasEventsData ? '0' : '0.25rem'
                                          }}>
                                            {hasEventsData ? eventsCount : 'N/A'}
                                          </div>
                                          {!hasEventsData && hasCheckMKData && (
                                            <div style={{ 
                                              fontSize: '0.625rem', 
                                              color: theme === 'dark' ? '#6b7280' : '#9ca3af',
                                              fontStyle: 'italic'
                  ...
                  {/* Liste des jobs pour les autres types d'instances (sous forme de cartes) */}
                  {instance.logiciel !== "Veeam" && instance.logiciel !== "HYCU Backup" && (instance.jobs || []).length > 0 && (
                    <div style={{ marginTop: '1rem' }}>
                      ...
                      <div style={{ 
                        ...
                        {(instance.jobs || []).map((job, jobIndex) => {
                          ...
                          return (
                            <div key={`job-${instanceIndex}-${jobIndex}`} className={styles.jobCard}>
                              <div className={styles.jobHeader}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                  {getInstanceIcon(instance.logiciel) && (
                                    <img 
                                      src={instance.logiciel === "Veeam" ? getIconPath('veeam.png') :
                                            instance.logiciel === "HYCU Backup" ? getIconPath('hycu.png') :
                                            instance.logiciel === "HyperBackup" ? getIconPath('hyperbackup.png') :
                                            instance.logiciel === "Active Backup for Microsoft 365" ? getIconPath('active-backup.png') : ""}
                                      alt={instance.logiciel} 
                                      style={{ width: '1rem', height: '1rem', borderRadius: '4px' }} 
                                    />
                                  )}
                                  {!getInstanceIcon(instance.logiciel) && (
                                    <StatusIcon style={{ fontSize: '0.875rem', color: jobStatus.color }} />
                                  )}
                                  <h6 style={{ 
                                    margin: 0, 
                                    fontSize: '0.875rem', 
                              ...
                              {/* Contenu pour Active Backup for Microsoft 365 (si pas de modules/stockage) */}
                              {instance.logiciel === "Active Backup for Microsoft 365" && (!instance.activeBackupModules || !instance.activeBackupStorage) && (
                                ...
                                <div style={{ 
                                  ...
                                  {job.retention && (
                                    <div style={{ 
                                      ...
                                      Rétention: {job.retention}
                              </div>
                                  )}
                                </div>
                              )}

                              {/* Contenu pour HyperBackup */}
...
export default SauvegardeSummary;

...
// Mapping entre les noms utilisés dans SwitchesSummaryCards et ceux dans Switch.js
...
const buildServiceMetrics = (switchData = {}) => {
  return defaultServices.map((service) => {
    // Mapper le nom du service vers celui utilisé dans Switch.js
...
// Fonction de calcul de score identique à getSwitchHealthScore de Switch.js
export const computeSwitchHealthScore = (
  serverData = {},
  {
    eventsCount = null,
  ...
  // Pour les switchs, le calcul est simplifié : disponibilité (50%) + événements (50%)
  // Pas de score SLA basé sur les services comme pour les serveurs
...
const getSwitchStatus = (switchName, switchData = {}) => {
  if (!switchData) return { status: "unknown", color: "#6b7280" };
...
const SwitchesSummaryCards = ({ config, data, selectedSites = [] }) => {
  const { theme } = useTheme();
  const switches = config?.client?.equipements?.Switch || [];
  // Les données sont dans data directement (data[switchName])
  // data correspond à data.switches passé depuis MonitoringSummary ou data directement
  ...
  return (
    <div className={`${styles.container} ${theme === "dark" ? styles.dark : ""}`}>
      ...
      >
        {siteEntries.map(([siteName, siteSwitches]) => (
            ...
            <div className={styles.switchesSiteColumn}>
              {siteSwitches.map((sw, idx) => {
                ...
                const healthLetter = resolvedScore !== undefined && resolvedScore !== null
                  ...
                  : getSwitchLetterFromStatus(statusFallback.status);
                ...
                const identityPartsLeft = [
                  sw.marque || sw.fabricant,
                  sw.modele,
                  sw.sn || sw.numeroSerie ? `S/N ${sw.sn || sw.numeroSerie}` : null
                ].filter(Boolean);
                      ...
                      <div className={styles.healthTitleRow}>
                        <div className={styles.serverTitleMain}>
                          <h3 className={styles.serverName}>
                            <FaNetworkWired style={{ marginRight: "0.5rem", fontSize: "1rem", verticalAlign: "middle", position: "relative", top: "-2px" }} />
                            {switchName}
                          </h3>
                        </div>
                        {sw.garantie && (
                              ...
                              <div className={styles.modulesSummary}>
                                <div className={styles.modulesSummaryList}>
                                  {serviceMetrics.map((service) => {
                                    ...
                                    return (
                                      <div key={`${switchName}-${service.label}`} className={styles.serviceStatCard}>
                                        <ServiceIcon 
                                          size="1.6rem" 
                                          color={theme === "dark" ? "#9ca3af" : "#6b7280"}

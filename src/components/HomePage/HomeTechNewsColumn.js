...
export default function HomeTechNewsColumn({
  ...
  return <aside className={styles.column} aria-label={t.title}>
      <div className={styles.header}>
        ...
        <button type="button" className={styles.refreshBtn} onClick={() => setRefreshKey(k => k + 1)} disabled={loading} title={t.refresh} aria-label={t.refresh}>
          <Icon icon="mdi:refresh" className={loading ? styles.spinning : ""} />
        </button>
      </div>

      {partial && !loading && !error && <p className={styles.partialNote}>{t.partial}</p>}
      ...
      <div className={styles.feed}>
        ...
        {!loading && !error && items.length > 0 && <ul className={styles.list}>
            {items.map(item => {
          ...
          return <li key={item.id} className={styles.listItem}>
                  <a href={item.link} target="_blank" rel="noopener noreferrer" className={styles.article}>

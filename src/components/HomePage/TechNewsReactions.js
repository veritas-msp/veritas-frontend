...
function computePickerPosition(anchorEl) {
  ...
  top = Math.max(
    PICKER_MARGIN,
    Math.min(top, window.innerHeight - PICKER_HEIGHT - PICKER_MARGIN)
  );
      ...
      <div className={styles.reactionAddWrap}>
        {showTextReactBtn ? (
          ...
          <button
            ref={anchorRef}
            type="button"
            className={`${styles.reactionAddBtn} ${styles.reactionAddBtnIconOnly} ${myReaction ? styles.reactionAddBtnActive : ""}`}
            onClick={togglePicker}
            disabled={pending}
            aria-label={myReaction ? t.changeReaction : t.reactAction}
            aria-expanded={pickerOpen}
            title={myReaction ? `${t.changeReaction} (${myReaction})` : t.reactAction}
          >
            {myReaction ? (
              <span className={styles.reactionAddMine}>{myReaction}</span>
            ) : (

...
import {
  ...
  isMonitoringDocumentNameTaken,
  saveMonitoringDocument,
  updateMonitoringDocument,
} from "../../../api/monitoringDocuments";
...
import {
  buildDefaultInterventionData,
  buildDefaultSaveName,
  buildInterventionReportPeriod,
  createTodoItem,
  isDuplicateMonitoringSaveResult,
  mergeInterventionData,
  getInterventionStepValidationAlert,
  serializeInterventionSavePayload,
  validateInterventionStep,
} from "./interventionReportModel";
import {
  buildInterventionPdfLabels,
  exportInterventionPdf,
} from "./exportInterventionPdf";
import {
  ModernToggle,
  MovementTypePicker,
  NumberStepper,
} from "./InterventionFormControls";
...
export default function RapportInterventionBuilder({
  copy: pageCopy,
  reportType,
  client,
  onBack,
  ...
  const interventionCopy = useMemo(
    () => getInterventionReportCopy(pageCopy?.localeCode || locale),
    [pageCopy?.localeCode, locale]
  );
  ...
  const clientLabel =
    client?.name ||
    client?.nom ||
    (client?.id ? pageCopy.create.getClientLabel(client.id) : "-");
    ...
    fetchClientGeneral(clientId)
      .then((generalData) => {
        ...
        setFormData((prev) => {
          ...
          const firstSite = Array.isArray(generalData?.sites)
            ? generalData.sites[0]
            : null;
          if (!String(next.contactSite || "").trim() && firstSite) {
            next.contactSite =
              firstSite.contact || firstSite.contact_name || next.contactSite;
          }
  ...
  useEffect(() => {
    ...
    if (stepIndex === 3 && !saveNameInitializedRef.current && !documentId) {
      saveNameInitializedRef.current = true;
      setSaveName(buildDefaultSaveName(clientLabel, interventionCopy, { withRandom: true }));
    }
  ...
  const handleMouvementChange = (index, field, value) => {
    setFormData((prev) => {
      ...
      mouvements[index] = { ...mouvements[index], [field]: value };
      return { ...prev, mouvements };
    });
  };
  ...
  const addMouvement = () => {
    setFormData((prev) => ({
      ...
      mouvements: [
        ...(prev.mouvements || []),
        { designation: "", quantite: "", type: "déposé", commentaire: "" },
      ],
    }));
  };
  ...
  const captureSignatures = () => ({
    ...
    signaturePrestataire:
      getSignatureDataUrl(prestataireRef) || formData.signaturePrestataire,
  ...
  const syncSignatureToForm = () => {
    setFormData((prev) => ({
      ...prev,
      signaturePrestataire:
        getSignatureDataUrl(prestataireRef) || prev.signaturePrestataire,
      signatureClient: getSignatureDataUrl(clientSignRef) || prev.signatureClient,
    }));
  };

  const getStepAlert = (index, data = formData) =>
    getInterventionStepValidationAlert(index, data, interventionCopy.alerts) ||
    interventionCopy.alerts.stepReport;
  ...
  const pdfLabels = useMemo(
    () => buildInterventionPdfLabels(interventionCopy),
    [interventionCopy]
  );
  ...
  const archiveInterventionToClientVault = async ({
    ...
    try {
      ...
      if (!blob) throw new Error("Échec de la génération du PDF");
  ...
  const handleSave = async (forceOverwrite = false) => {
    ...
    setSaving(true);
    try {
      const { config, data } = serializeInterventionSavePayload(
        client,
        payload,
        REPORT_TYPE_IDS.INTERVENTION
      );
      const clientName = client?.name || client?.nom || "CLIENT";
      const reportPeriod = buildInterventionReportPeriod(payload);

      const result = documentId
        ? await updateMonitoringDocument(documentId, {
            name: saveName.trim(),
      ...
      if (result?.success) {
        ...
        if (vaultResult.success) {
          toast.success(
            saveVisibleToClient
              ? interventionCopy.alerts.saveSuccessVisible
              : interventionCopy.alerts.saveSuccessInternal
          );
      ...
      if (
        !documentId &&
        !forceOverwrite &&
        isDuplicateMonitoringSaveResult(result)
      ) {
  ...
  const movementOptions = useMemo(
    () => [
      { value: "déposé", label: inter.depose, icon: "mdi:package-down", tone: "deposit" },
      { value: "enlevé", label: inter.enleve, icon: "mdi:package-up", tone: "remove" },
      { value: "réparé", label: inter.repare, icon: "mdi:wrench-outline", tone: "repair" },
    ],
    ...
    <div className={styles.formGridSingle}>
      <div className={styles.formGrid}>
        ...
        <label className={styles.field}>
          ...
          <NumberStepper
            ...
            onChange={(value) => updateField({ dureeIntervention: value })}
            min={0}
            step={0.5}
            decimals={1}
            ariaLabel={inter.duree}
          />
        </label>
        <label className={styles.field}>
          ...
          <input
            ...
            value={formData.numeroIntervention || ""}
            onChange={handleChange}
            placeholder="456789"
          />
        </label>
      ...
      <label className={styles.field}>
        ...
        <textarea
          ...
          onChange={handleChange}
          placeholder={inter.descriptionDemande}
        />
      </label>
      ...
      <div className={styles.materialBlock}>
        ...
        {(formData.mouvements || []).length === 0 ? (
          <p className={styles.emptyMovements}>{inter.emptyMovements}</p>
        ) : (
          <div className={styles.movementTableWrap}>
            <div className={styles.movementTable} role="table">
              ...
              {(formData.mouvements || []).map((row, index) => (
                <div key={index} className={styles.movementRow} role="row">
                  <span className={styles.movementRowIndex} role="cell">
                    {index + 1}
                  </span>
                  <input
                    role="cell"
                    className={styles.movementCellInput}
                    value={row.designation || ""}
                    onChange={(e) =>
                      handleMouvementChange(index, "designation", e.target.value)
                    }
                    placeholder={inter.designation}
                    aria-label={`${inter.designation} ${index + 1}`}
                  />
                  <div role="cell">
                    <NumberStepper
                      compact
                      value={row.quantite || ""}
                      onChange={(value) => handleMouvementChange(index, "quantite", value)}
                      min={0}
                      step={1}
                      ariaLabel={`${inter.quantite} ${index + 1}`}
                    />
                  </div>
                  <div role="cell">
                    <MovementTypePicker
                      compact
                      value={row.type || "déposé"}
                      options={movementOptions}
                      onChange={(value) => handleMouvementChange(index, "type", value)}
                    />
                  </div>
                  <input
                    role="cell"
                    ...
                    onChange={(e) =>
                      handleMouvementChange(index, "commentaire", e.target.value)
                    }
                    placeholder={inter.commentaire}
                    aria-label={`${inter.commentaire} ${index + 1}`}
                  />
                  <button
                    type="button"
                    role="cell"
                    className={styles.movementRowRemove}
                    onClick={() => removeMouvement(index)}
                    aria-label={inter.removeLine}
                  >
    ...
    <div className={styles.formGridSingle}>
      ...
      <section className={styles.todosBlock}>
        ...
        {(formData.todos || []).map((todo, index) => (
          <article
            key={todo.id || index}
            className={`${styles.todoCard} ${todo.done ? styles.todoCardDone : ""}`}
          >
            <label className={styles.todoCheck}>
              <input
                ...
                onChange={(event) =>
                  handleTodoChange(index, "done", event.target.checked)
                }
              />
            ...
            <div className={styles.todoFields}>
              <input
                className={styles.fieldInput}
                value={todo.text || ""}
                onChange={(event) => handleTodoChange(index, "text", event.target.value)}
                placeholder={todosCopy.placeholder}
              />
              {!todo.done ? (
                <label className={styles.todoPlanned}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      handleTodoChange(index, "plannedFor", event.target.value)
                    }
                  />
                </label>
              ) : null}
  ...
  const renderSignatureForm = () => (
    <div className={styles.signatureForm}>
      <div className={styles.signatureToggleRow}>
        ...
        <ModernToggle
          checked={Boolean(formData.requireSignature)}
          onChange={(value) =>
            updateField({
      ...
      {!formData.requireSignature ? null : (
        <>
          ...
          <label className={styles.field}>
            ...
            <textarea
              className={styles.fieldTextarea}
              name="signatureReserve"
              value={formData.signatureReserve || ""}
              onChange={handleChange}
            />
          </label>
          <label className={styles.checkboxRow}>
            <input
              ...
              checked={Boolean(formData.signatureAccord)}
              onChange={handleChange}
            />
            ...
            <div className={styles.signaturePad}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  updateField({ signaturePrestataire: "" });
                }}
              >
                {rep.clear}
              </button>
  ...
  const renderValidationStep = () => {
    ...
    const visibleTodos = (previewData.todos || []).filter((item) =>
      ...
      <div className={styles.validationLayout}>
        <div className={styles.previewPanel}>
          <article className={styles.previewDoc}>
            ...
            {String(previewData.compteRendu || "").trim() ? (
              <PreviewBlock title={val.compteRendu}>
                <p className={styles.previewText}>{previewData.compteRendu}</p>
              </PreviewBlock>
            ) : null}

            {visibleTodos.length > 0 ? (
              <PreviewBlock title={val.todos}>
                <ul className={styles.previewTodoList}>
                  {visibleTodos.map((todo, index) => (
                    <li
                      key={todo.id || index}
                      className={todo.done ? styles.previewTodoDone : styles.previewTodoPending}
                    >
                      <Icon
                        icon={todo.done ? "mdi:checkbox-marked-circle" : "mdi:checkbox-blank-circle-outline"}
                        aria-hidden
                      />
                      <span>{todo.text}</span>
                      {!todo.done && todo.plannedFor ? (
                        <span className={styles.previewTodoPlanned}>
            ...
            {(previewData.mouvements || []).length > 0 ? (
              <PreviewBlock title={val.materiel}>
                <table className={styles.previewTable}>
                  ...
                  <tbody>
                    {previewData.mouvements.map((row, index) => (
                      <tr key={index}>
                ...
                <>
                  ...
                  <div className={styles.previewSignatureGrid}>
                    <div className={styles.previewSignatureBox}>
                      <span className={styles.fieldLabel}>{rep.prestataire}</span>
                      {previewData.signaturePrestataire ? (
                        <img
                          src={previewData.signaturePrestataire}
                          alt={rep.prestataire}
                          className={styles.previewSignatureImage}
                        />
                      ) : (
                        <div className={styles.previewSignatureEmpty}>-</div>
                      )}
                    </div>
                    <div className={styles.previewSignatureBox}>
                      <span className={styles.fieldLabel}>{rep.clientSign}</span>
                      {previewData.signatureClient ? (
                        <img
                          src={previewData.signatureClient}
                          alt={rep.clientSign}
                          className={styles.previewSignatureImage}
                        />
                      ) : (
        ...
        <aside className={styles.actionsPanel}>
          ...
          <label className={styles.saveField}>
            ...
            <input
              className={`${styles.fieldInput} ${nameConflict ? styles.fieldInputConflict : ""}`}
              value={saveName}
              onChange={(e) => {
                saveNameTouchedRef.current = true;
            ...
            />
            {nameConflict ? (
              <span className={styles.saveNameWarning} role="alert">
  ...
  return (
    <div className={shellStyles.shell}>
            ...
            const canJump =
              ...
              (index > stepIndex &&
                Array.from({ length: index }).every((_, i) =>
                  validateInterventionStep(i, formData)
                ));
            return (
              <button
                key={label}
                type="button"
                className={`${shellStyles.stepNavItem} ${
                  isActive ? shellStyles.stepNavItemActive : ""
                } ${isDone ? shellStyles.stepNavItemDone : ""}`}
                onClick={() => {
      ...
      <ConfirmModal
        ...
        title={interventionCopy.alerts.overwriteTitle}
        message={interventionCopy.alerts.overwriteMessage}
        confirmLabel={interventionCopy.alerts.overwriteConfirm}
        cancelLabel={interventionCopy.alerts.overwriteCancel}
        variant="danger"
        loading={saving}
        onConfirm={() => {
      ...
      <InterventionSaveModal
        open={showSaveModal}
        copy={interventionCopy.saveModal}

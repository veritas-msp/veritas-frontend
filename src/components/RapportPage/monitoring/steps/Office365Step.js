...
function isFreeLicense(lic) {
  ...
  if (!raw) return false;
  return FREE_LICENSE_PATTERNS.some((pattern) =>
    raw.includes(pattern.toUpperCase())
  );
}
...
function getAdminRoleIcons(adminRoleString) {
  ...
  const roles = String(adminRoleString)
    .split(",")
    .map((r) => r.trim())
    .filter(Boolean);
  const result = [];
...
function getMfaUserForUser(user, mfaDetails) {
  ...
  return (
    (Array.isArray(mfaDetails) &&
      mfaDetails.find((m) => {
        const mUpn = (m.userPrincipalName || m.user_principal_name || "")
          .toLowerCase()
          .trim();
  ...
  switch (filter) {
    ...
    default:
      ...
      if (filter.startsWith(USER_FILTER_METHOD_PREFIX)) {
        ...
        return users.filter((u) =>
          userHasMethod(getMfaUserForUser(u, mfaDetails), methodKey)
        );
      }
      return users;
  }
...
function filterExchangeDataByPeriod(exchange, reportPeriod) {
  ...
  const daily = Array.isArray(exchange.emailActivity.dailyActivity)
    ? exchange.emailActivity.dailyActivity
    : null;
  if (!daily || daily.length === 0) return exchange;

  const periodStart =
    reportPeriod?.startTime || reportPeriod?.start || reportPeriod?.from;
  const periodEnd =
    reportPeriod?.endTime || reportPeriod?.end || reportPeriod?.to;
  ...
  const sent = filteredDaily.reduce(
    (sum, d) => sum + (d.sent || 0),
    0
  );
  const received = filteredDaily.reduce(
    (sum, d) => sum + (d.received || 0),
    0
  );
  const read = filteredDaily.reduce(
    (sum, d) => sum + (d.read || 0),
    0
  );
  ...
  const readRate =
    received > 0 ? (read / received) * 100 : 0;
...
function filterTeamsDataByPeriod(teams, reportPeriod) {
  if (
    ...
    !Array.isArray(teams.licensedActivity.dailyActivity)
  ) {
    return teams;
  }
...
export default function Office365Step({
  ...
  useEffect(() => {
    if (!highlightedEquipmentKey || typeof highlightedEquipmentKey !== "string")
      return;
    const key = highlightedEquipmentKey;
    if (key === "Office365:exchange") setActiveTab("exchange");
    else if (key === "Office365:teams") setActiveTab("teams");
    else if (key === "Office365:onedrive") setActiveTab("onedrive");
    else if (key === "Office365:sharepoint") setActiveTab("sharepoint");
    else if (key === "Office365:security") setActiveTab("securite");
    else if (key.startsWith("Office365:licence:")) setActiveTab("licences");
    else if (key.startsWith("Office365:user:")) setActiveTab("utilisateurs");
  }, [highlightedEquipmentKey]);

  const [licencesSort, setLicencesSort] = useState({
    column: null,
    direction: "asc",
  });
  ...
  const reloadSnapshotFromDb = async () => {
    ...
    if (!resp.ok) {
      throw new Error("Impossible de recharger le snapshot Office 365");
    }
  ...
  const handleSyncOffice365 = async () => {
    ...
    try {
      if (
        ...
        typeof window.__office365SyncTrigger === "function"
      ) {
        try {
          window.__office365SyncTrigger();
        } catch (e) {
          console.error("Erreur trigger sync Office365:", e);
        }
      }
      ...
      if (Number.isNaN(startDate.getTime()) || Number.isNaN(endDate.getTime())) {
        throw new Error("Période de rapport invalide pour la synchronisation");
      }
      endDate.setHours(23, 59, 59, 999);
      ...
      let period = "D30";
      if (diffDays <= 10) period = "D7";
      else if (diffDays <= 45) period = "D30";
      else if (diffDays <= 120) period = "D90";
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || "Erreur lors de la synchronisation Office 365");
      }
      ...
      console.error("Erreur synchronisation Office365 step:", error);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // On prend le snapshot le plus récent
  ...
  const {
    ...
    const totalLicenses = licencesArr.reduce(
      (sum, lic) => sum + (lic.total || 0),
      0
    );
    const usedLicenses = licencesArr.reduce(
      (sum, lic) => sum + (lic.utilisees || 0),
      0
    );

    const adoptionScore = snapshot.adoptionScore || null;
    const secureScore =
      snapshot.securityData?.secureScore || snapshot.security?.secureScore || null;
  ...
  const o365Subtitle = useMemo(() => {
    if (reportPeriod?.start && reportPeriod?.end) {
      try {
        const startFmt = new Date(reportPeriod.start).toLocaleDateString("fr-FR", {
          day: "2-digit",
          month: "short",
          year: "numeric",
        });
        const endFmt = new Date(reportPeriod.end).toLocaleDateString("fr-FR", {
          day: "2-digit",
          month: "short",
          year: "numeric",
        });
        return `Période : ${startFmt} – ${endFmt}`;
      } catch {
    ...
    if (metrics) {
      ...
      if (u > 0 || total > 0) {
        return total > 0
          ? `${u.toLocaleString()} utilisateurs • ${lic.toLocaleString()} / ${total.toLocaleString()} licences`
          : `${u.toLocaleString()} utilisateurs`;
      }
    }
    return "Microsoft 365";
  ...
  const enrichedUsers = useMemo(() => {
    ...
    return users.map((u) => {
      ...
      const mfa =
        mfaDetails.find(
          (m) =>
            (m.userPrincipalName || m.user_principal_name || "")
              .toLowerCase()
              .trim() === upn
        ) || null;
      if (!mfa) return u;
      ...
      const hasMfa =
        mfa.has_mfa === true ||
        (methods.length > 0 &&
          methods.some((m) => getMfaMethodIcon(m) !== null));
      return {
        ...u,
        admin_role: mfa.admin_role ?? mfa.adminRole ?? u.admin_role,
        is_admin:
          typeof mfa.is_admin === "boolean" ? mfa.is_admin : u.is_admin ?? false,
        mfa_active: hasMfa,
  ...
  const displayUsers = useMemo(() => {
    ...
    return enrichedUsers.filter((u) => {
      const isService =
        ...
        (u.isServiceAccount !== false && isLikelyServiceAccountFromUser(u));
      return !isService;
    });
  ...
  const filteredUsers = useMemo(() => {
    ...
    return activeFilters.reduce(
      (acc, filterKey) => getFilteredUsers(acc, filterKey, mfaDetails),
      usersForKpi
    );
  }, [usersForKpi, activeFilters, mfaDetails]);

  const blockedUsers = filteredUsers.filter(
    (u) => u.accountEnabled === false
  ).length;
  ...
  const mfaActifCount = filteredUsers.filter((u) =>
    userHasMfa(getMfaUserForUser(u, mfaDetails))
  ).length;
  ...
  const methodCounts = useMemo(() => {
    ...
    [
      ...
      counts[key] = filteredUsers.filter((u) =>
        userHasMethod(getMfaUserForUser(u, mfaDetails), key)
      ).length;
    });
    return counts;
  ...
  const domainCounts = useMemo(() => {
    ...
    return Array.from(map.entries())
      ...
      .sort(
        (a, b) =>
          b.count - a.count ||
          (a.domain === NO_DOMAIN_KEY
            ? 1
            : b.domain === NO_DOMAIN_KEY
            ? -1
            : a.domain.localeCompare(b.domain))
      );
  }, [displayUsers]);

  const sortedLicences = useMemo(() => {
  ...
  const sortedUsers = useMemo(() => {
    ...
    return [...filteredUsers].sort((a, b) => {
      ...
      const statusRank = (u, lastLogin) => {
        if (u.accountEnabled === false) return 0; // bloqué
        if (isActive30(lastLogin)) return 3; // actif
        if (isInactive(lastLogin)) return 1; // inactif >90j
        return 2; // inactif
      };
      ...
      switch (usersSort.column) {
        ...
        case "license": {
          const formatUserLicenses = (u) => {
            ...
            return raw
              ...
              .map((lic) => lic.trim())
              .filter(Boolean)
              .map((lic) => getLicenseDisplayName(lic))
              .join(", ")
              .toLowerCase();
          };
        ...
        case "mfa":
          ...
          break;
        case "methods":
          va = Array.isArray(a.mfa_methods)
            ? a.mfa_methods.length
            : String(a.mfa_methods || "").split(",").filter((s) => s.trim()).length;
          vb = Array.isArray(b.mfa_methods)
            ? b.mfa_methods.length
            : String(b.mfa_methods || "").split(",").filter((s) => s.trim()).length;
          break;
        default:
          va = 0;
          vb = 0;
  ...
  return (
    ...
    return (
      <Icon
        icon={direction === "asc" ? "mdi:chevron-up" : "mdi:chevron-down"}
        width={14}
        height={14}
      />
    );
  ...
  const tabs = [
    { id: "licences", label: "Licences", icon: "mdi:license" },
    { id: "utilisateurs", label: "Utilisateurs", icon: "mdi:account-multiple" },
    {
      id: "exchange",
      label: "Exchange",
    ...
    { id: "securite", label: "Sécurité", icon: "mdi:shield-check" },
  ];
  ...
  return (
    <MonitoringStepShell>
      <MonitoringStepHeader
        title="Office 365"
        countLabel={o365Subtitle}
        headerActions={
          <MonitoringStepSyncButton
            onClick={handleSyncOffice365}
            disabled={loading}
            loading={syncing}
            title="Synchroniser les données Office 365"
          />
        }
        footer={
          <div className={styles.o365TabsContainer}>
            {tabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                className={`${styles.o365TabButton} ${
                  activeTab === tab.id ? styles.o365TabButtonActive : ""
                }`}
      ...
      <div className={styles.o365DetailBlock}>
        {syncing && (
          <div className={styles.o365SyncSkeleton}>
            <div className={styles.o365SyncSkeletonStats}>
              {[0, 1, 2, 3].map((idx) => (
                <div key={idx} className={styles.o365SyncSkeletonCard}>
            ...
            <div className={styles.o365SyncSkeletonTable}>
              <div className={styles.o365SyncSkeletonTableHeader}>
                {[0, 1, 2, 3, 4].map((idx) => (
                  <div key={idx} className={styles.o365SyncSkeletonTableCell} />
                ))}
              </div>
              {[0, 1, 2, 3, 4, 5].map((row) => (
                <div key={row} className={styles.o365SyncSkeletonTableRow}>
                  {[0, 1, 2, 3, 4].map((col) => (
                    <div key={col} className={styles.o365SyncSkeletonTableCell} />
                  ))}
                </div>
              ))}
            </div>
        ...
        {!syncing && loading && (
          <div className={styles.antivirusModalEnrichedLoading}>
            <Icon
              icon="mdi:loading"
              width={20}
              height={20}
              className={styles.antivirusModalLoadingIcon}
            />
            Chargement des données Office 365...
          </div>
        )}
          ...
          <>
                ...
                <div className={styles.antivirusModalEndpointsScroll}>
                  <table className={styles.antivirusModalTable}>
                    <thead>
                      <tr>
                        <th
                          className={styles.antivirusStickyTh}
                          onClick={() =>
                            setLicencesSort((prev) => ({
                              column: "type",
                              direction:
                                prev.column === "type" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        >
                          <span style={{ display: "inline-flex", gap: 4, alignItems: "center" }}>
                            Type de licence
                            {getSortIcon(
                              licencesSort.column === "type",
                              licencesSort.direction
                            )}
                          </span>
                        </th>
                        <th
                          ...
                          onClick={() =>
                            setLicencesSort((prev) => ({
                              column: "used",
                              direction:
                                prev.column === "used" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        <th
                          ...
                          onClick={() =>
                            setLicencesSort((prev) => ({
                              column: "total",
                              direction:
                                prev.column === "total" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        <th
                          ...
                          onClick={() =>
                            setLicencesSort((prev) => ({
                              column: "rate",
                              direction:
                                prev.column === "rate" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                    ...
                    <tbody>
                      {licences.length === 0 ? (
                        <tr>
                          <td colSpan={6}>Aucune licence trouvée.</td>
                        </tr>
                      ) : (
                        sortedLicences.map((lic, idx) => {
                          ...
                          const displayName = getLicenseDisplayName(
                            lic.nom || lic.displayName || lic.skuId || "-"
                          );
                          ...
                          const equipmentKey =
                            ...
                            lic.skuId ||
                            lic.nom ||
                            `Office365:licence:${displayName}`;
                          const commentCount =
                            commentCounts && commentCounts[equipmentKey]
                              ? commentCounts[equipmentKey]
                              : 0;
                          const ticketCount =
                            ticketCounts && ticketCounts[equipmentKey]
                              ? ticketCounts[equipmentKey]
                              : 0;
                          const isHighlighted =
                            highlightedEquipmentKey != null &&
                            String(highlightedEquipmentKey) ===
                              String(equipmentKey);
                          const item = { nom: displayName, name: displayName };

                          return (
                            <tr
                              key={equipmentKey}
                              className={`${styles.infraTableRow} ${
                                isHighlighted
                                  ? styles.infraTableRowHighlight
                                  : ""
                              }`}
                            >
                              <td>
                              ...
                              <td className={styles.infraTableCellActions}>
                                {typeof onOpenComments === "function" && (
                                  ...
                                  >
                                    ...
                                    {commentCount > 0 && (
                                      <span
                                        className={styles.infraCommentBadge}
                                      >
                                        {commentCount}
                                      </span>
                                    )}
                                ...
                                {typeof onTicketCreatedForEquipment ===
                                  "function" &&
                                (client?.id || client?.uuid) ? (
                                  <span
                                    className={styles.infraIconButtonWrapper}
                                  >
                                    
                                    {ticketCount > 0 && (
                                      <span
                                        className={styles.infraTicketBadge}
                                      >
                                        {ticketCount}
                                      </span>
                                    )}
              ...
              <div>
                <div className={styles.antivirusModalEndpointsHeader}>
                  ...
                  <div className={styles.antivirusModalEndpointsSearch}>
                    <Icon
                      icon="mdi:magnify"
                      className={styles.antivirusModalSearchIcon}
                    />
                    <input
                      type="text"
                      className={styles.antivirusModalSearchInput}
                ...
                {domainCounts.length > 0 && (
                  <div style={{ marginTop: "0.75rem", marginBottom: "0.75rem" }}>
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => setDomainAndResetFilters(DOMAIN_FILTER_ALL)}
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                          gap: "0.5rem",
                          ...
                          borderRadius: 999,
                          border:
                            domainFilter === DOMAIN_FILTER_ALL
                              ? "1px solid #2563eb"
                              : "1px solid #e5e7eb",
                          backgroundColor:
                            domainFilter === DOMAIN_FILTER_ALL ? "#eff6ff" : "#ffffff",
                          fontSize: "0.8rem",
                          cursor: "pointer",
                        }}
                      >
                      ...
                      {domainCounts.map(({ domain, count }) => {
                        ...
                        const label =
                          domain === NO_DOMAIN_KEY ? "Sans domaine" : domain;
                        return (
                          <button
                            key={domain}
                            type="button"
                            onClick={() => setDomainAndResetFilters(domain)}
                            style={{
                              display: "inline-flex",
                              alignItems: "center",
                              gap: "0.5rem",
                  ...
                  >
                      ...
                      style={{
                        ...
                        border:
                          activeFilters.length === 0
                            ...
                            : "1px solid #e5e7eb",
                        backgroundColor:
                          activeFilters.length === 0 ? "#eff6ff" : "#ffffff",
                        fontSize: "0.8rem",
                        cursor: "pointer",
                      }}
                    >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_ADMIN)
                          ? "1px solid #3b82f6"
                          : "1px solid #e5e7eb",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_NON_ADMIN)
                          ? "1px solid #4b5563"
                          : "1px solid #e5e7eb",
                        backgroundColor: "#ffffff",
                        fontSize: "0.8rem",
                    ...
                    >
                      ...
                      <span>Non admin</span>
                      <span style={{ fontWeight: 600 }}>{nonAdminCount}</span>
                    </button>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_ACTIVE_30)
                          ? "1px solid #10b981"
                          : "1px solid #e5e7eb",
                        backgroundColor: "#ffffff",
                        fontSize: "0.8rem",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_INACTIVE_90)
                          ? "1px solid #f59e0b"
                          : "1px solid #e5e7eb",
                        backgroundColor: "#ffffff",
                        fontSize: "0.8rem",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_BLOCKED)
                          ? "1px solid #ef4444"
                          : "1px solid #e5e7eb",
                        backgroundColor: "#ffffff",
                        fontSize: "0.8rem",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_MFA_ACTIF)
                          ? "1px solid #10b981"
                          : "1px solid #e5e7eb",
                        backgroundColor: "#ffffff",
                        fontSize: "0.8rem",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: "0.45rem 0.7rem",
                        borderRadius: 8,
                        border: activeFilters.includes(USER_FILTER_MFA_INACTIF)
                          ? "1px solid #ef4444"
                          : "1px solid #e5e7eb",
                        backgroundColor: "#ffffff",
                        fontSize: "0.8rem",
                    ...
                    {[
                      ...
                      { key: "emailauthenticationmethod", label: "Email" },
                      {
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          toggleFilter(USER_FILTER_METHOD_PREFIX + key)
                        }
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                          ...
                          padding: "0.45rem 0.7rem",
                          borderRadius: 8,
                          border: activeFilters.includes(
                            USER_FILTER_METHOD_PREFIX + key
                          )
                            ? "1px solid #6b7280"
                            : "1px solid #e5e7eb",
                          backgroundColor: "#ffffff",
                  ...
                  <table className={styles.antivirusModalTable}>
                    <thead>
                      <tr>
                        <th
                          className={styles.antivirusStickyTh}
                          style={{ whiteSpace: "nowrap", maxWidth: 180, cursor: "pointer" }}
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "name",
                              direction:
                        ...
                        <th
                          className={styles.antivirusStickyTh}
                          style={{ whiteSpace: "nowrap", maxWidth: 220, cursor: "pointer" }}
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "email",
                              direction:
                        ...
                        <th
                          ...
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "created",
                              direction:
                                prev.column === "created" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        <th
                          ...
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "status",
                              direction:
                                prev.column === "status" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        <th
                          ...
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "license",
                              direction:
                                prev.column === "license" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        <th
                          ...
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "admin",
                              direction:
                                prev.column === "admin" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                        ...
                        <th
                          ...
                          onClick={() =>
                            setUsersSort((prev) => ({
                              column: "mfa",
                              direction:
                                prev.column === "mfa" && prev.direction === "asc"
                                  ? "desc"
                                  : "asc",
                            }))
                          }
                      ...
                      {sortedUsers.length === 0 ? (
                        ...
                        sortedUsers.map((u, idx) => {
                          ...
                          const lastLogin = u.lastLoginDate
                            ? new Date(u.lastLoginDate)
                            : null;
                          const period90 = new Date(
                            Date.now() - 90 * 24 * 60 * 60 * 1000
                          );
                          const period30 = new Date(
                            Date.now() - 30 * 24 * 60 * 60 * 1000
                          );
                          const isInactive =
                            !lastLogin || lastLogin < period90;
                          const isActive30 =
                            lastLogin && lastLogin >= period30;
                          ...
                          if (u.accountEnabled === false) {
                            ...
                            statusLabel = "Inactif (>90j)";
                          }

                          const licenseNamesRaw = u.licenses || u.licences || "";
                          const licenseNames = licenseNamesRaw
                            ? licenseNamesRaw
                                .split(",")
                                .map((lic) => lic.trim())
                                .filter(Boolean)
                          ...
                          const displayName =
                            u.displayName || u.name || u.userPrincipalName || "-";
                          const moduleKey = "Office365";
                          const equipmentKey =
                            u.commentKey ||
                            u.id ||
                            u.userPrincipalName ||
                            `Office365:user:${displayName}`;
                          const commentCount =
                            commentCounts && commentCounts[equipmentKey]
                              ? commentCounts[equipmentKey]
                              : 0;
                          ...
                          return (
                            ...
                            >
                              ...
                              <td style={{ whiteSpace: "nowrap" }}>{statusLabel}</td>
                              <td style={{ maxWidth: 200, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>
                                {licenseNames || "Aucune"}
                              </td>
                              <td>
                                    ...
                                    return (
                                      <div className={styles.mfaMethodsList}>
                                        {roleIcons.map(({ icon, label }, i) => (
                                          <span
                                            key={`${icon}-${i}`}
                                            className={styles.mfaMethodIcon}
                                            title={label}
                                          >
                                            <Icon icon={icon} />
                                          </span>
                              ...
                              <td>
                                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                                  {u.is_admin ? (
                                    <Icon icon="mdi:check-bold" width={16} height={16} style={{ color: "#10b981" }} />
                                  ) : (
                                    <Icon icon="mdi:close-thick" width={16} height={16} style={{ color: "#9ca3af" }} />
                                  )}
                                </span>
                              </td>
                              <td>
                                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                                  {u.mfa_active ? (
                                    ...
                                    <Icon icon="mdi:close-thick" width={16} height={16} style={{ color: "#ef4444" }} />
                                  )}
                                </span>
                              </td>
                              <td>
                                {Array.isArray(u.mfa_methods) ||
                                typeof u.mfa_methods === "string" ? (
                                  <div
                                    style={{
                                      display: "flex",
                                      gap: "0.25rem",
            ...
            {activeTab === "exchange" && (
              <div>
                <div className={styles.infraTableHeaderInline}>
                  ...
                  <div className={styles.infraTableHeaderInlineActions}>
                    {typeof onOpenComments === "function" && (
                      ...
                      >
                        <Icon icon="mdi:comment-text-outline" />
                        {commentCounts?.["Office365:exchange"] > 0 && (
                    ...
                    {typeof onTicketCreatedForEquipment === "function" &&
                      (client?.id || client?.uuid) && (
                        <span className={styles.infraIconButtonWrapper}>
                          
                          {ticketCounts?.["Office365:exchange"] > 0 && (
                ...
                {(!exchangeData ||
                  (!exchangeData.emailActivity &&
                    !exchangeData.mailboxes &&
                    !exchangeData.topUsers)) && (
                  <div className={styles.antivirusModalNoData}>
                    Aucune donnée Exchange disponible. Veuillez synchroniser les
                    données.
                  </div>
                )}
                {exchangeData && (exchangeData.emailActivity || exchangeData.mailboxes) && (
                  <>
                      ...
                      >
                        {[
                          ...
                          },
                          {
                            ...
                            total:
                              typeof exchangeData.emailActivity.readRate === "number"
                                ? `${exchangeData.emailActivity.readRate.toFixed(1)}%`
                                : "N/A",
                            extra:
                              exchangeData.emailActivity.read != null &&
                              exchangeData.emailActivity.received != null
                          ...
                          >
                            ...
                            >
                              ...
                              {card.avg != null && (
                                ...
                                >
                                  Moyenne: {card.avg.toLocaleString()}/jour
                                </div>
                              )}
                              {card.extra && (
                                <div
                    ...
                    {exchangeData.emailActivity?.dailyActivity &&
                      exchangeData.emailActivity.dailyActivity.length > 0 && (
                        <div style={{ marginTop: "1rem" }}>
                          ...
                          >
                            <ResponsiveContainer width="100%" height="100%">
                              <LineChart
                                data={exchangeData.emailActivity.dailyActivity.map(
                                  (day) => ({
                          ...
                          {[
                            {
                              ...
                              label: "Total boîtes",
                              value:
                                exchangeData.mailboxes.total != null
                                  ? exchangeData.mailboxes.total.toLocaleString()
                                  : "N/A",
                              subLabel: "Nombre",
                            },
                            {
                            ...
                            },
                            {
                              ...
                              label: "Total emails",
                              value:
                                exchangeData.mailboxes.totalItems != null
                                  ? exchangeData.mailboxes.totalItems.toLocaleString()
                                  : "N/A",
                              subLabel: "Volume",
                            },
                          ].map((card, idx) => (
                    ...
                    {Array.isArray(exchangeData.topUsers) &&
                      exchangeData.topUsers.length > 0 && (
                        <div style={{ marginTop: "1rem" }}>
                          <h5 className={styles.antivirusModalSubtitle}>
                            Top 5 utilisateurs (activité email)
                          </h5>
                          <div className={styles.antivirusModalEndpointsScroll}>
                            <table className={styles.antivirusModalTable}>
                              <thead>
                              ...
                              <tbody>
                                {exchangeData.topUsers.map((u, idx) => (
                                  <tr key={idx}>
                                    <td>
            ...
            {activeTab === "teams" && (
              <div>
                <div className={styles.infraTableHeaderInline}>
                  ...
                  <div className={styles.infraTableHeaderInlineActions}>
                    {typeof onOpenComments === "function" && (
                      ...
                      >
                        <Icon icon="mdi:comment-text-outline" />
                        {commentCounts?.["Office365:teams"] > 0 && (
                          <span className={styles.infraCommentBadge}>
                    ...
                    {typeof onTicketCreatedForEquipment === "function" &&
                      (client?.id || client?.uuid) && (
                        <span className={styles.infraIconButtonWrapper}>
                          ...
                          {ticketCounts?.["Office365:teams"] > 0 && (
                            <span className={styles.infraTicketBadge}>
                ...
                {!teamsData && (
                  <div className={styles.antivirusModalNoData}>
                    Aucune donnée Teams disponible. Veuillez synchroniser les données.
                  </div>
                )}
                {teamsData && (
                  <>
                    {(() => {
                      ...
                      const activeUsers =
                        ...
                        teamsData.teams?.activeUsers ??
                        0;
                      const inactiveUsers = Math.max(
                        0,
                        licensedUsers - activeUsers
                      );
                      const adoptionRate =
                        licensedUsers > 0
                          ? (activeUsers / licensedUsers) * 100
                          : null;
                      ...
                      const meetingsStats =
                        ...
                        !Array.isArray(rawMeetings)
                          ...
                          : {
                              total:
                                typeof rawMeetings === "number"
                                  ? rawMeetings
                                  : 0,
                            };
                      const rawCalls =
                        teamsData.activity?.calls || teamsData.calls;
                      const callsStats =
                        rawCalls &&
                        ...
                        !Array.isArray(rawCalls)
                          ...
                          : {
                              total:
                                typeof rawCalls === "number"
                                  ? rawCalls
                                  : 0,
                            };

                      const dailyActivity =
                        teamsData.licensedActivity?.dailyActivity;
                      ...
                      return (
                        <>
                          ...
                          {/* Deux colonnes : KPI activité + graphique */}
                          <div
                            style={{
                              display: "flex",
                              gap: "1.5rem",
                          ...
                          >
                            {/* Colonne gauche : messages / réunions / appels */}
                            <div
                              style={{
                                flex: "0 0 360px",
                                minWidth: 0,
                              ...
                              >
                                ...
                                >
                                  {[
                                    ...
                                    {
                                      label:
                                        "Messages en chats privés",
                                      value:
                                        messageStats.privateChat || 0,
                                    },
                                    {
                                      label: "Messages de canal",
                                      value:
                                        messageStats.teamChat || 0,
                                    },
                                  ].map((kpi, idx) => (
                                    <div
                                      key={idx}
                                ...
                                >
                                  {[
                                    ...
                                    },
                                    {
                                      label: "Durée totale",
                                      value:
                                        callsStats.totalDuration ||
                                        "0h 0m",
                                    },
                                    {
                                      label: "Durée moyenne",
                                      value:
                                        callsStats.averageDuration ||
                                        "0h 0m",
                                    },
                                  ].map((kpi, idx) => (
                                    <div
                            ...
                            {Array.isArray(dailyActivity) &&
                              dailyActivity.length > 0 && (
                                ...
                                >
                                  <h5
                                    className={styles.antivirusModalSubtitle}
                                  >
                    ...
                    {/* Table simple des équipes (optionnelle) */}
                    <div className={styles.antivirusModalEndpointsScroll}>
                      <table className={styles.antivirusModalTable}>
                        <thead>
                          <tr>
            ...
            {activeTab === "onedrive" && (
              <div>
                <div className={styles.infraTableHeaderInline}>
                  ...
                  <div className={styles.infraTableHeaderInlineActions}>
                    {typeof onOpenComments === "function" && (
                      ...
                      >
                        <Icon icon="mdi:comment-text-outline" />
                        {commentCounts?.["Office365:onedrive"] > 0 && (
                          <span className={styles.infraCommentBadge}>
                    ...
                    {typeof onTicketCreatedForEquipment === "function" &&
                      (client?.id || client?.uuid) && (
                        <span className={styles.infraIconButtonWrapper}>
                          ...
                          {ticketCounts?.["Office365:onedrive"] > 0 && (
                            <span className={styles.infraTicketBadge}>
                ...
                {!onedriveData && (
                  <div className={styles.antivirusModalNoData}>
                    Aucune donnée OneDrive disponible. Veuillez synchroniser les données.
                  </div>
                )}
                {onedriveData && onedriveData.success === false && (
                  <div className={styles.antivirusModalNoData}>
                    <div style={{ color: "#ef4444", fontWeight: 500 }}>
                      ❌ Erreur lors du chargement des données OneDrive
                    </div>
                ...
                {onedriveData && onedriveData.success !== false && (
                  <>
                    ...
                    >
                      {[
                        ...
                        },
                        {
                          ...
                          value:
                            onedriveData.storage?.totalFiles !== undefined
                              ? onedriveData.storage.totalFiles.toLocaleString()
                              : "0",
                        },
                        {
                          icon: "mdi:account",
                    ...
                    {onedriveData.topUsers &&
                      Array.isArray(onedriveData.topUsers) &&
                      onedriveData.topUsers.length > 0 && (
                                ...
                                {onedriveData.topUsers
                                  ...
                                  .map((user, idx) => (
                                    <tr key={idx}>
                                      ...
                                      <td>
                                        {user.sharedCount?.toLocaleString() ||
                                          "0"}
                                      </td>
                                    </tr>
                                  ))}
                    ...
                    {onedriveData.usersNearQuota &&
                      Array.isArray(onedriveData.usersNearQuota) &&
                      onedriveData.usersNearQuota.length > 0 && (
                        <div style={{ marginBottom: "1rem" }}>
                          <h5 className={styles.antivirusModalSubtitle}>
                            Utilisateurs proches du quota (&gt;90%)
                          </h5>
                          <div className={styles.antivirusModalEndpointsScroll}>
                            <table className={styles.antivirusModalTable}>
                              <thead>
                              ...
                              <tbody>
                                {onedriveData.usersNearQuota.map(
                                  (user, idx) => (
                                    <tr key={idx}>
                                      <td>{user.name || "N/A"}</td>
            ...
            {activeTab === "sharepoint" && (
              <div>
                <div className={styles.infraTableHeaderInline}>
                  ...
                  <div className={styles.infraTableHeaderInlineActions}>
                    {typeof onOpenComments === "function" && (
                      ...
                      >
                        <Icon icon="mdi:comment-text-outline" />
                        {commentCounts?.["Office365:sharepoint"] > 0 && (
                          <span className={styles.infraCommentBadge}>
                    ...
                    {typeof onTicketCreatedForEquipment === "function" &&
                      (client?.id || client?.uuid) && (
                        <span className={styles.infraIconButtonWrapper}>
                          ...
                          {ticketCounts?.["Office365:sharepoint"] > 0 && (
                            <span className={styles.infraTicketBadge}>
                ...
                {!sharepointData && (
                  <div className={styles.antivirusModalNoData}>
                    Aucune donnée SharePoint disponible. Veuillez synchroniser les données.
                  </div>
                )}
                ...
                {sharepointData && sharepointData.success === false && (
                  <div className={styles.antivirusModalNoData}>
                    <div style={{ color: "#ef4444", fontWeight: 500 }}>
                      ❌ Erreur lors du chargement des données SharePoint
                    </div>
                    <div style={{ marginTop: 4, color: "#6b7280" }}>
                ...
                {sharepointData && sharepointData.success !== false && (
                  <>
                    {/* KPIs principaux SharePoint (même origine que SharePointTab) */}
                    <div
                      style={{
                        display: "flex",
                        flexWrap: "wrap",
                      ...
                      {[
                        {
                          ...
                          value:
                            sharepointData.stats?.totalSites !== undefined
                              ? sharepointData.stats.totalSites
                              : Array.isArray(sharepointData.sites)
                              ? sharepointData.sites.length
                              : 0,
                        },
                        {
                          icon: "mdi:check-circle",
                          ...
                          value:
                            sharepointData.stats?.activeSites !== undefined
                              ? sharepointData.stats.activeSites
                              : Array.isArray(sharepointData.sites)
                              ? sharepointData.sites.filter(
                                  (s) => s.isActive !== false
                                ).length
                              : 0,
                        },
                    ...
                    {/* Table des sites SharePoint (même origine que SharePointTab) */}
                    {Array.isArray(sharepointData.sites) &&
                      sharepointData.sites.length > 0 && (
                        <div className={styles.antivirusModalEndpointsScroll}>
                          <table className={styles.antivirusModalTable}>
                            <thead>
                              <tr>
                            ...
                            <tbody>
                              {sharepointData.sites.map((site, idx) => (
                                <tr key={site.id || idx}>
                                  <td>
                                    <span style={{ fontWeight: 600 }}>
                                      {site.name ||
                    ...
                    {(!sharepointData.sites ||
                      !Array.isArray(sharepointData.sites) ||
                      sharepointData.sites.length === 0) && (
                      <div className={styles.antivirusModalNoData}>
                        Aucun site SharePoint disponible.
                      </div>
                    )}
                  </>
                )}
            ...
            {activeTab === "securite" && (
              <div>
                <div className={styles.infraTableHeaderInline}>
                  <div className={styles.infraTableHeaderInlineInfo}>
                    <h4 className={styles.antivirusDetailBlockTitle}>Sécurité</h4>
                  </div>
                  <div className={styles.infraTableHeaderInlineActions}>
                    {typeof onOpenComments === "function" && (
                      <button
                      ...
                      >
                        <Icon icon="mdi:comment-text-outline" />
                        {commentCounts?.["Office365:security"] > 0 && (
                          <span className={styles.infraCommentBadge}>
                    ...
                    {typeof onTicketCreatedForEquipment === "function" &&
                      (client?.id || client?.uuid) && (
                        <span className={styles.infraIconButtonWrapper}>
                          ...
                          {ticketCounts?.["Office365:security"] > 0 && (
                            <span className={styles.infraTicketBadge}>
                ...
                {!securityData && (
                  <div className={styles.antivirusModalNoData}>
                    Aucune donnée de sécurité disponible. Veuillez synchroniser les données.
                  </div>
                )}
                {securityData && securityData.success === false && (
                  <div className={styles.antivirusModalNoData}>
                    ...
                    <div style={{ marginTop: 4, color: "#6b7280" }}>
                      {securityData.error || "Erreur inconnue"}
                    </div>
                  </div>
                )}
                {securityData && securityData.success !== false && (
                  <>
                    {/* Bloc 1 : Carte Score mis en avant (comme SecuriteTab) */}
                    {metrics.secureScore?.currentScore != null && (
                      <div
                        style={{
                          marginTop: "0.5rem",
                          marginBottom: "1rem",
                    ...
                    {/* Bloc 2 : Grille 3 cartes (Global, Admins, Non-admins) */}
                    <div
                      style={{
                        display: "grid",
                        gridTemplateColumns:
                    ...
                    >
                      {[
                        ...
                        {
                          title: "Utilisateurs non administrateurs",
                          icon: "mdi:account-group-outline",
                          total:
                            securityKpiStats.nonAdminWithMFA +
                            securityKpiStats.nonAdminWithoutMFA,
                        ...
                        const rate =
                          section.withMfa + section.withoutMfa > 0
                            ? Math.round(
                                (section.withMfa /
                                  (section.withMfa + section.withoutMfa)) *
                                  100
                              )
                            : 0;
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                Top 3 méthodes préférées
                              </div>
                              {section.top3.length > 0 ? (
                                <div
                                  style={{
                                    display: "flex",
                                    flexWrap: "wrap",

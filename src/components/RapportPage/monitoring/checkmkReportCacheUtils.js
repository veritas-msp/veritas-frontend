export function parseCheckMKResponses(servicesResp, eventsResp, availabilityResp) {
  ...
  const eventsCount =
    typeof eventsResp?.events_count === "number" ? eventsResp.events_count : events.length;

  return { services, events, availability, eventsCount };
}
...
export function computeCheckMKEquipmentStatus({ services, events, availability, eventsCount }) {
  const criticalCount = eventsCount ?? (Array.isArray(events) ? events.length : 0);
  const up =
    availability != null && typeof availability.up === "number" ? availability.up : null;
...
export function isCheckMKCacheValidForPeriod(cachedPeriod, reportStartDate, reportEndDate) {
  ...
  return (
    cachedPeriod.reportStartDate === reportStartDate &&
    cachedPeriod.reportEndDate === reportEndDate
  );
}

export function buildCheckMKReportSnapshot(
  equipmentData,
  equipmentStatus,
  reportStartDate,
  reportEndDate
...
/**
 * Exclut les notifications techniques CheckMK (SNMP, piggyback…) sans valeur métier.
 */
...
export function filterCheckMKEventsForReportPeriod(events, reportPeriod, { criticalOnly = false } = {}) {
  ...
  if (criticalOnly) {
    return filterCheckMKNoiseEvents(
      filtered.filter((event) => Number(event?.state) === 2)
    );
  }
...
export function filterCheckMKDisplayData(preLoadedData, reportPeriod) {
  ...
  const periodEvents = filterCheckMKEventsForReportPeriod(
    preLoadedData.events,
    reportPeriod
  );

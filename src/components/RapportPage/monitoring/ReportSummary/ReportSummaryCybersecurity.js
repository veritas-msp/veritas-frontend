...
import {
  ReportCategoryKpisBlock,
  ReportTableBlock,
} from "./ReportSummaryBlocks";
import {
  buildAntivirusEndpointRowsForClient,
  buildAntivirusPolicyRowsForClient,
...
function getProtectionStatusMeta(rawStatus) {
  ...
  if (!s) {
    return { icon: "mdi:help-circle-outline", color: "#9ca3af", label: "-" };
  }
  const protectedHit =
    ...
    s.includes("enabled") ||
    s.includes("ok");
  const unprotectedHit =
    s.includes("unprotected") ||
    s.includes("non prot") ||
    s.includes("not protected") ||
    s.includes("inactive") ||
    s.includes("disabled") ||
    s.includes("off");

  if (protectedHit && !unprotectedHit) {
    return { icon: "mdi:shield-check", color: "#16a34a", label: rawStatus };
  }
  if (unprotectedHit) {
    return { icon: "mdi:shield-off-outline", color: "#ef4444", label: rawStatus };
  }
  return { icon: "mdi:shield-outline", color: "#6b7280", label: rawStatus };
}
...
function getEndpointLastSeenSummary(endpoint) {
  const raw =
    endpoint.lastSeen ||
    endpoint.lastCheckIn ||
    endpoint.lastConnectedAt ||
    endpoint.lastSeenDate ||
    null;

  if (!raw) {
    return { label: "N/A", isOver24h: false };
  }

  const d = new Date(raw);
  if (Number.isNaN(d.getTime())) {
    return { label: String(raw), isOver24h: false };
  }
...
function getEndpointTypeIcon(endpoint) {
  ...
  // 1 = Physical, 2 = Virtual (même logique que dans AntivirusDetailPage)
  if (machineType === 2) {
...
function isEndpointInfected(endpoint) {
  return (
    endpoint.isInfected === true ||
    endpoint.malwareDetected === true ||
    endpoint.hasThreat === true
  );
}

function isEndpointInactive(endpoint) {
  const managed =
    endpoint.isManaged === true || endpoint.managed === true;
...
function BackupStatusBadge({ status }) {
  ...
  const toneClass =
    tone === "success"
      ? cyberStyles.statusSuccess
      : tone === "warn"
      ? cyberStyles.statusWarn
      : tone === "error"
      ? cyberStyles.statusError
      : tone === "info"
      ? cyberStyles.statusInfo
      : cyberStyles.statusNeutral;
  return (
    <span className={`${cyberStyles.statusBadge} ${toneClass}`}>
...
function CyberNotificationLegend({ commentTotal = 0, ticketTotal = 0 }) {
  return (
...
export default function ReportSummaryCybersecurity({
  ...
  const sauvegardeStandardInstances = useMemo(
    () =>
      sauvegardeInstances.filter(
        (inst) =>
          inst.logiciel !== "Active Backup for Microsoft 365" && inst.logiciel !== "HyperBackup"
      ),
    [sauvegardeInstances]
  );
  const sauvegardeActiveBackupInstances = useMemo(
    () => sauvegardeInstances.filter((inst) => inst.logiciel === "Active Backup for Microsoft 365"),
    [sauvegardeInstances]
  );
  const sauvegardeHyperBackupInstances = useMemo(
    () => sauvegardeInstances.filter((inst) => inst.logiciel === "HyperBackup"),
    [sauvegardeInstances]
  );
  ...
  const antivirusStats = useMemo(() => {
    ...
    antivirusSolutions.forEach((sol) => {
      const list =
        ...
        sol.endpoints ??
        (Array.isArray(sol.data?.endpoints)
          ? sol.data.endpoints
          : sol.data?.endpoints?.list ?? []);
      ...
      arr.forEach((ep) => {
        ...
        const osKey = osRaw ? String(osRaw).trim() : "Inconnu";
    ...
    const connectedRate =
      totalEndpoints > 0
        ? Math.round((connected / totalEndpoints) * 1000) / 10
        : null;
    const managementRate =
      totalEndpoints > 0
        ? Math.round((managed / totalEndpoints) * 1000) / 10
        : null;
  ...
  const antispamDetail = useMemo(() => {
    ...
    antispamSolutions.forEach((sol) => {
      const data =
        sol && typeof sol.data === "object" && sol.data !== null
          ? sol.data
          : sol || {};
      const solutionName =
        sol.nom || sol.logiciel || sol.solution || sol.name || "Antispam";
      const protectedUsersFromLicense =
    ...
    const totalProtectedUsers = hasProtectedUsersFromLicenses
      ? totalProtectedUsersFromLicenses
      : users.length;
  ...
  const isBackupActive = !!modules.Sauvegarde;
  const storageIpByName = useMemo(() => {
    const sources = [
      ...(Array.isArray(client?.equipements?.NAS) ? client.equipements.NAS : []),
      ...(Array.isArray(client?.equipements?.SAN) ? client.equipements.SAN : []),
      ...(Array.isArray(client?.equipements?.Stockage) ? client.equipements.Stockage : []),
    ];
    const map = {};
  ...
  const cyberCommentTotal = useMemo(
    () =>
      sumEquipmentCountsForModules(
        equipmentCommentCounts,
        REPORT_CYBER_MODULES,
  ...
  const cyberTicketTotal = useMemo(
    () =>
      sumEquipmentCountsForModules(
        equipmentTicketCounts,
        REPORT_CYBER_MODULES,
  ...
  const backupKpis = useMemo(() => {
    const totalJobs = sauvegardeInstances.reduce(
      (acc, inst) => acc + (Array.isArray(inst.jobs) ? inst.jobs.length : 0),
      0
    );
    return [
      { label: "Active Backup", value: sauvegardeActiveBackupInstances.length, icon: "mdi:microsoft-office" },
      { label: "HyperBackup", value: sauvegardeHyperBackupInstances.length, icon: "mdi:nas" },
      { label: "Instances de sauvegarde", value: sauvegardeInstances.length, icon: "mdi:database-sync" },
      { label: "Jobs de sauvegarde", value: totalJobs, icon: "mdi:briefcase-clock-outline" },
    ];
  }, [
    sauvegardeInstances,
    sauvegardeActiveBackupInstances,
    sauvegardeHyperBackupInstances,
  ]);

  const antivirusKpis = useMemo(
    () => [
      { label: "Endpoints inventoriés", value: antivirusStats.totalEndpoints || 0, icon: "mdi:laptop" },
      { label: "Endpoints gérés", value: antivirusStats.managed || 0, icon: "mdi:check-circle-outline" },
      { label: "Endpoints non gérés", value: antivirusStats.unmanaged || 0, icon: "mdi:alert-circle-outline" },
      {
        label: "Taux de gestion AV",
        value:
          antivirusStats.managementRate != null
            ? `${Number(antivirusStats.managementRate).toFixed(1)} %`
            : "N/A",
        icon: "mdi:percent",
      },
    ],
  ...
  const antivirusConnectionKpis = useMemo(
    () => [
      { label: "Déconnectés > 24h", value: antivirusStats.disconnected || 0, icon: "mdi:lan-disconnect" },
      {
        label: "Taux vus < 24h",
        value:
          antivirusStats.connectedRate != null
            ? `${Number(antivirusStats.connectedRate).toFixed(1)} %`
            : "N/A",
  ...
  const antivirusOsKpis = useMemo(() => {
    const entries = Object.entries(antivirusStats.osCounts || {})
  ...
  const antispamKpis = useMemo(
    () => [
      { label: "Total emails", value: antispamDetail.totals.total || 0, icon: "mdi:email" },
      { label: "Emails valides", value: antispamDetail.totals.valid || 0, icon: "mdi:check-circle" },
      { label: "Spam", value: antispamDetail.totals.spam || 0, icon: "mdi:email-alert" },
      {
        label: "Bloqués",
        value:
          (antispamDetail.totals.infected || 0) +
          (antispamDetail.totals.banned || 0) +
          (antispamDetail.totals.spearphishing || 0),
        icon: "mdi:shield-lock",
      },
      { label: "Spearphishing", value: antispamDetail.totals.spearphishing || 0, icon: "mdi:target-account" },
      { label: "En attente", value: antispamDetail.totals.pending || 0, icon: "mdi:timer-sand" },
    ],
    [antispamDetail]
  );
  ...
  const backupJobRows = useMemo(
    () =>
      sauvegardeInstances.flatMap((inst, idx) => {
        ...
        return jobs.map((job, jobIdx) => {
          ...
          const lastBackupSync =
            job.last_backup_date ?? job.lastBackupDate ?? job.lastSyncDate ?? "";
          return {
            _rowKey: `${instanceName}-${job.nom || job.jobName || jobIdx}-${jobIdx}`,
            instanceName,
            name: job.nom || job.jobName || "",
            type: job.type || job.typeSauvegarde || "",
            serveurLie: job.serveurLie || job.source || "",
            destination:
              inst.logiciel === "HYCU Backup"
                ? "DataCenter PSI"
                : job.destination || job.serveurLie || job.source || "",
  ...
  const antivirusPolicyRows = useMemo(
    () => buildAntivirusPolicyRowsForClient(antivirusSolutions),
    [antivirusSolutions]
  );

  const antivirusEndpointRows = useMemo(
    () => buildAntivirusEndpointRowsForClient(antivirusSolutions),
    [antivirusSolutions]
  );
  ...
  return (
    <div className={infraStyles.root}>
      <div className={infraStyles.overviewContainer}>
        <CyberNotificationLegend
          commentTotal={cyberCommentTotal}
          ticketTotal={cyberTicketTotal}
        />
      </div>
          ...
          <div className={infraStyles.sectionTitleWrapper}>
            ...
            <div>
              ...
              <div className={infraStyles.sectionSubtitle}>
                Synthèse des solutions et jobs de sauvegarde identifiés
              </div>
            </div>
          </div>
        </div>
        <div className={infraStyles.sectionTitleSeparator} />
        {sauvegardeInstances.length === 0 ? (
          <div className={infraStyles.sectionHelperMuted}>
            Aucune solution de sauvegarde renseignée pour ce client.
          </div>
        ) : (
          <>
            {/* Active Backup + HyperBackup regroupés dans le même bloc visuel */}
            {(sauvegardeActiveBackupInstances.length > 0 ||
              sauvegardeHyperBackupInstances.length > 0) &&
              (() => {
                ...
                <div style={{ marginTop: "1.1rem" }}>
                  <div className={infraStyles.backupFlowRow}>
                    {sauvegardeActiveBackupInstances.map((inst, idx) => {
                      ...
                      const explicitStorageIp =
                        inst.activeBackupStorageIp ||
                        inst.activeBackupStorageIP ||
                        inst.storageIp ||
                        inst.storageIP ||
                        inst.ip ||
                        ...
                        "";
                      const storageIp =
                        extractIpv4(explicitStorageIp) ||
                        extractIpv4(storage) ||
                        storageIpByName[normalizeStorageKey(storage)] ||
                        "-";
                      const storageIcon = /nas|synology|qnap|san|diskstation|disque/i.test(
                        String(storage || "")
                      )
                        ? "mdi:nas"
                        : "mdi:database";
                      return (
                        <article
                          key={`active-${inst.id || idx}`}
                          className={`${infraStyles.topologyStorageChip} ${infraStyles.backupFlowCard}`}
                          title={name}
                        >
                          ...
                          <div className={infraStyles.topologyStorageText}>
                            <div
                              className={infraStyles.topologyStorageHeaderRow}
                            ...
                            <div className={infraStyles.backupFlowMain}>
                                ...
                                {ACTIVE_BACKUP_MODULES.map((m) => {
                                  ...
                                  return (
                                    <span
                                      ...
                                      className={`${infraStyles.backupFlowModuleIcon} ${
                                        isActive ? "" : infraStyles.backupFlowModuleInactive
                                      }`}
                                    >
                              ...
                              <span className={infraStyles.backupFlowArrow} title="Flux vers le stockage">
                                <IconifyIcon icon="mdi:arrow-right" width={16} height={16} />
                              </span>
                              <span className={infraStyles.backupFlowNasBlock} title={`Stockage : ${storage}`}>
                    ...
                    {sauvegardeHyperBackupInstances.map((inst, idx) => {
                      ...
                      const sourceIp =
                        extractIpv4(inst.hyperbackupSourceIp || inst.hyperbackupSourceIP || "") ||
                        extractIpv4(inst.hyperbackupSource || "") ||
                        storageIpByName[normalizeStorageKey(source)] ||
                        "-";
                      const destIp =
                        extractIpv4(
                          inst.hyperbackupDestinationIp || inst.hyperbackupDestinationIP || ""
                        ) ||
                        extractIpv4(inst.hyperbackupDestination || "") ||
                        storageIpByName[normalizeStorageKey(dest)] ||
                        "-";
                      return (
                        <article
                          key={`hyper-${inst.id || idx}`}
                          className={`${infraStyles.topologyStorageChip} ${infraStyles.backupFlowCard}`}
                          title={name}
                        >
                          <span className={infraStyles.topologyStorageTypeIcon}>
                          ...
                          <div className={infraStyles.topologyStorageText}>
                            ...
                            <div className={infraStyles.backupFlowMain}>
                              ...
                              <span className={infraStyles.backupFlowArrow} title="Flux HyperBackup">
                                <IconifyIcon icon="mdi:arrow-right" width={16} height={16} />
                              </span>
            ...
            {/* Cartes des instances (Veeam, HYCU, etc.) */}
            {sauvegardeStandardInstances.length > 0 && (
              <div style={{ marginTop: "1.1rem" }}>
                <h5
                  style={{
                    margin: "0 0 0.35rem",
                ...
                <div className={infraStyles.topologyServersRow} style={{ maxWidth: "none" }}>
                  {sauvegardeStandardInstances.map((inst, idx) => {
                    ...
                    return (
                      <article
                        key={inst.id || idx}
                        className={infraStyles.topologyStorageChip}
                        title={name}
                      >
                        <span className={infraStyles.topologyStorageTypeIcon}>
              ...
              columns={[
                {
                  ...
                  label: "Instance",
                  render: (row) => (
                    <span className={cyberStyles.cellBold}>{row.instanceName || "-"}</span>
                  ),
                },
                { id: "name", label: "Nom", render: (row) => row.name || "-" },
                { id: "type", label: "Type", render: (row) => row.type || "-" },
                { id: "serveurLie", label: "Serveur", render: (row) => row.serveurLie || "-" },
                { id: "destination", label: "Destination", render: (row) => row.destination || "-" },
                { id: "regularite", label: "Régularité", render: (row) => row.regularite || "-" },
                { id: "horaire", label: "Horaire", render: (row) => row.horaire || "-" },
                { id: "retention", label: "Rétention", render: (row) => row.retention || "-" },
                {
                  id: "lastBackupStart",
      ...
      <section className={infraStyles.section}>
        <div className={infraStyles.sectionHeader}>
          <div className={infraStyles.sectionTitleWrapper}>
            ...
            <div>
              ...
              <div className={infraStyles.sectionSubtitle}>
                Solutions déployées et couverture des endpoints
              </div>
            </div>
          </div>
        </div>
        ...
        <CyberReportTable
          ...
          count={antivirusSolutions.length}
          rows={antivirusSolutions.map((sol, idx) => ({
            ...sol,
          ...
          columns={[
            {
              ...
              label: "Solution",
              render: (row, idx) => (
                <span className={cyberStyles.cellBold}>
        ...
        <CyberReportTable
          title="Politiques de sécurité (client)"
          count={antivirusPolicyRows.length}
          rows={antivirusPolicyRows}
          columns={[
            {
              id: "solutionName",
              label: "Solution",
              render: (row) => (
                <span className={cyberStyles.cellBold}>{row.solutionName || "-"}</span>
              ),
            },
            {
              id: "name",
            ...
            },
            {
              ...
              label: "Appliquée",
              render: (row) =>
                row.applied != null && row.totalForApplied
        ...
        <CyberReportTable
          title="Endpoints antivirus"
          count={antivirusEndpointRows.length}
          rows={antivirusEndpointRows}
          columns={[
            {
              id: "solution",
              label: "Solution",
              render: (row) => (
                <span className={cyberStyles.cellBold}>{row._solutionName || "-"}</span>
              ),
            },
            {
              id: "name",
              ...
              render: (row) => {
                ...
                return (
                  <span className={cyberStyles.endpointNameCell}>
            ...
            },
            {
              ...
              label: "FQDN",
              render: (row) =>
                row.fqdn || row.domainName || row.dnsName || row.hostFqdn || "-",
            },
            { id: "ip", label: "IP", render: (row) => row.ip || row.address || "-" },
            {
              id: "os",
              label: "OS",
              render: (row) => {
                ...
                return (
                  <span className={cyberStyles.osCell}>
            ...
            },
            {
              ...
              render: (row) => {
                ...
                return (
                  <span className={infected ? cyberStyles.infectedYes : cyberStyles.infectedNo}>
                    {infected ? "Oui" : "Non"}
                  </span>
                );
              },
            },
            {
              ...
              render: (row) => {
                ...
                const statusLabel =
                  endpointState === 1 || row.isManaged ? "Actif" : inactive ? "Inactif" : "-";
                const tone =
                  statusLabel === "Actif"
                    ? cyberStyles.statusSuccess
                    : statusLabel === "Inactif"
                    ...
                    : cyberStyles.statusNeutral;
                return (
                  <span className={`${cyberStyles.statusBadge} ${tone}`}>{statusLabel}</span>
                );
              },
            },
          ]}
      ...
      <section className={infraStyles.section}>
        <div className={infraStyles.sectionHeader}>
          <div className={infraStyles.sectionTitleWrapper}>
            ...
            <div>
              ...
              <div className={infraStyles.sectionSubtitle}>
                Solutions antispam et volumétrie des menaces filtrées
              </div>
            </div>
          </div>
        </div>
        ...
        <CyberReportTable
          title="Licences antispam"
          count={antispamSolutions.length}
          rows={antispamSolutions.map((sol, idx) => ({
            ...sol,
        ...
        <CyberReportTable
          title="Statistiques par période"
          count={antispamStatsByWeek.length}
          rows={antispamStatsByWeek.map((s, idx) => ({
            ...s,
          ...
          columns={[
            ...
            { id: "period", label: "Période", render: (row) => row.period || "-" },
            {
              id: "total",
              label: "Total",
        ...
        <CyberReportTable
          title="Utilisateurs couverts par la protection"
          count={antispamDetail.users.length}
          rows={antispamDetail.users.map((u, idx) => ({
            ...u,

...
/** Politiques réellement utilisées par les endpoints du client (pas tout le catalogue Bitdefender). */
...
export function buildAntivirusPolicyRowsForClient(antivirusSolutions) {
  ...
  return antivirusSolutions.flatMap((sol, solIdx) => {
    ...
    const policies =
      sol.syncData?.policies || sol.data?.policies || sol.policies || [];

...
export default function SuggestionAutocomplete({
  ...
  return (
    <div className={styles.field} ref={rootRef}>
      ...
      <div
        ref={anchorRef}
        className={`${styles.inputWrap} ${open ? styles.inputWrapOpen : ""} ${disabled ? styles.inputWrapDisabled : ""}`}
      >
      ...
      {open && menuStyle
        ? createPortal(
            <div ref={menuRef} className={styles.dropdown} style={menuStyle} role="listbox" aria-labelledby={labelId}>
              {filteredOptions.length === 0 ? (
                <div className={styles.empty}>{emptyMessage}</div>
              ) : (
                filteredOptions.map((labelText, idx) => (
                  <button
                    key={labelText}
                    type="button"
                    role="option"
                    aria-selected={value === labelText}
                    className={`${styles.option} ${highlight === idx ? styles.optionActive : ""} ${
                      value === labelText ? styles.optionSelected : ""
                    }`.trim()}
                    onMouseEnter={() => setHighlight(idx)}
                    onClick={() => pickOption(labelText)}
                  >
                    {labelText}
                  </button>
                ))

...
import {
  ONBOARDING_SAVE_STEP_KEYS,
  ONBOARDING_WIDE_CARD_KEYS,
  countActiveMspAgents,
  filterActiveMspAgents,
} from "./onboardingSetupShared";
...
export default function OnboardingWizard({ step, onStepChange, onComplete }) {
  ...
  const { layout, ui, steps, forms } = content;

  const setup = useOnboardingSetup(forms, locale);
  const { draft, loading, saving, patchGeneral, setWeekSchedule } = setup;
  ...
  const employeeRangeOptions = useMemo(
    () => getEmployeeRangeOptions(locale),
    [locale]
  );
  ...
  const renderStepForm = () => {
    ...
    if (current.key === "identity") {
      return (
        <OnboardingIdentityForm
          labels={forms.identity}
          general={draft.general}
          employeeRangeOptions={employeeRangeOptions}
    ...
    if (current.key === "support") {
      return (
        <OnboardingSupportForm
          labels={forms.support}
          general={draft.general}
          onChange={patchGeneral}
          disabled={saving}
        />
    ...
    if (current.key === "hours") {
      return (
        <OnboardingHoursForm
          ...
          onPatchDay={patchDay}
          onApplyWeekdaysTemplate={applyWeekdaysTemplate}
          disabled={saving}
        />
      );
    }
    ...
    if (current.key === "agents") {
      return (
        <OnboardingAgentsStep
          ...
          maxMspAgents={maxMspAgents}
          agentAtLimit={agentAtLimit}
    ...
    if (current.key === "license") {
      ...
      return (
        <OnboardingLicenseForm
          labels={forms.license}
          licenseKey={licenseKey}
          onChange={setLicenseKey}
          disabled={isBusy || licenseValid}
          alreadyActive={licenseValid}
        />
      );
    }
  ...
  return (
    <motion.div
      className={`${styles.overlay} ${styles[`overlay${wizardTheme === "dark" ? "Dark" : "Light"}`]}`}
      data-wizard-theme={wizardTheme}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      ...
      <div className={styles.wizardShell}>
        <SetupWizardLayout
          ...
          locale={locale}
          onLocaleChange={setUserLocaleOverride}
          theme={wizardTheme}
          onThemeChange={handleThemeChange}
          themeAriaLabel={wizardTheme === "dark" ? ui.themeUseLight : ui.themeUseDark}
          wideCard={ONBOARDING_WIDE_CARD_KEYS.has(layoutCurrent?.key)}
          layoutText={layout}
        >
          <div
            ref={stepPanelRef}
          ...
          >
            <AnimatePresence mode="wait" onExitComplete={handleExitComplete}>
              {contentVisible ? (
                <motion.div
                  key={`${locale}-${current.key}`}
                  className={`${styles.stepBody} ${current.key === "agents" ? styles.stepBodyAgents : ""}`}
              ...
              <p
                className={`${setupStyles.subtitle} ${
                  current.key === "agents" ? styles.stepLeadCompact : ""
                } ${current.leadBefore ? styles.stepSubtitleWithBadge : ""}`}
              >
                {current.leadBefore ? (
                  <>
                    {current.leadBefore}
              ...
              {current.pillars?.length > 0 && (
                <div className={styles.pillarGrid}>
                  {current.pillars.map((pillar, index) => (
                    <article key={`${current.key}-pillar-${index}`} className={styles.pillarCard}>
              ...
              {current.paragraphs?.length > 0 && (
                <div className={styles.stepNarrative}>
                  {current.paragraphs.map((paragraph, index) => (
                    <p key={`${current.key}-p-${index}`}>{paragraph}</p>
                  ))}
                </div>
              )}
              ...
              {current.bullets?.length > 0 && (
                <ul
                  className={
                    current.key === "agents" ? styles.bulletListCompact : styles.bulletList
                  }
                >
                  {current.bullets.map((item, index) => (
                    <li key={`${current.key}-${index}`} className={styles.bulletItem}>
              ...
              {current.key === "agents" ? (
                <div className={styles.agentsStepPanel}>{renderStepForm()}</div>
              ) : (
                renderStepForm()
              )}
              ...
              <div
                className={`${setupStyles.actions} ${styles.stepActions} ${
                  current.key === "agents" ? styles.stepActionsAgents : ""
                }`}
              >
                {displayStep > 1 && (
                  <button
                    type="button"
                    className={setupStyles.btnSecondary}
                    onClick={() => requestStepChange(displayStep - 1)}
                    disabled={isBusy}
                  >
                    {ui.previous}
                  </button>
                ...
                {isLast ? (
                  <button
                    type="button"
                    className={setupStyles.btnPrimary}
      ...
      <button
        type="button"
        className={styles.skipGuideFixed}
        onClick={handleSkip}
        disabled={isBusy}
      >

...
export function useOnboardingSetup(labels, locale = "fr") {
  ...
  useEffect(() => {
    let cancelled = false;
    loadOnboardingSetupData()
      .then(({ draft: nextDraft, hadErrors, generalFailed }) => {
        if (cancelled) return;
        setDraft(nextDraft);
        if (hadErrors) {
          if (generalFailed) {
  ...
  const setWeekSchedule = useCallback((updater) => {
    setDraft((prev) => {
      if (!prev) return prev;
      const next =
        typeof updater === "function" ? updater(prev.weekSchedule) : updater;
      return { ...prev, weekSchedule: next };
    });
  }, []);
  ...
  const saveIdentity = useCallback(async () => {
    ...
    try {
      ...
      const next = { ...ONBOARDING_SETUP_EMPTY, ...(settings || {}) };
      patchGeneral({
        ...
        app_organization_employee_range:
          next.app_organization_employee_range || draft.general.app_organization_employee_range || "",
  ...
  const saveSupport = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { settings: generalSettings } = await updateGeneralSettings(nextGeneral);

      const saved = { ...ONBOARDING_SETUP_EMPTY, ...(generalSettings || nextGeneral) };
      patchGeneral({
      ...
      window.dispatchEvent(new CustomEvent("appGeneralSettingsUpdated", { detail: saved }));
      return true;
    } catch (err) {
      showError(err.message || labels.shared.saveError);
      return false;
  ...
  const saveHours = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { settings } = await updateSlaSettings({

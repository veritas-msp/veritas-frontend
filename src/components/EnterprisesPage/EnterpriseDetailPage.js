...
import { exportReversibilityDossier } from "./exportReversibilityDossier";
...
import {
  getEnterpriseDetailCopy,
  getContractStatusDetail,
  getContractTypeLabel,
  getPrestationCategoryLabel,
  getTicketStatusLabel,
  getEventTypeLabels,
  getCampaignTypeLabel,
  getCampaignStatusLabel,
  interpolate,
} from "./enterpriseDetailI18n";
...
import MonitoringClientModal from "../AdminPage/MonitoringClientSkeleton/MonitoringClientModal";
...
const isPrestationTicket = (ticket) =>
...
function SidebarExpandToggle({ expanded, onClick, panelStyles, copy }) {
  return (
    <div className={panelStyles.sidebarShowMoreWrap}>
      <button
        type="button"
        className={panelStyles.sidebarShowMoreBtn}
        onClick={onClick}
        aria-expanded={expanded}
        aria-label={expanded ? copy.sidebar.collapseSection : copy.sidebar.expandSection}
      >
        <Icon
          icon={expanded ? "mdi:chevron-up" : "mdi:chevron-down"}
          className={panelStyles.sidebarShowMoreIcon}
          aria-hidden
        />
...
function extractAntivirusFromModules(modulesData) {
  ...
  return Object.entries(monitoring)
    .filter(([key, value]) => key.toLowerCase().includes("antivirus") && value)
    .map(([key]) => ({
      name: key,
      nom: key,
      solution: key,
...
function extractAntispamFromModules(modulesData) {
  ...
  const monitoring = modulesData?.modules_monitoring || {};
  return Object.entries(monitoring)
    .filter(([key, value]) => key.toLowerCase().includes("antispam") && value)
    .map(([key]) => ({
      name: key,
      nom: key,
      solution: key,
...
function extractLicencesFromModules(modulesData) {
  const licences = modulesData?.equipements?.LicencesAbonnements;
...
function parseBackupDataFromModules(modulesData, mappingsMap = {}) {
  ...
  if (!sauvegarde) return { instances: [], jobs: [] };
  ...
  return { instances: allInstances, jobs: allJobs };
}

export default function ClientDetailPage({ onNavigate, clientData }) {
  const { clientId: urlClientId } = useParams(); // ID depuis l'URL
  ...
  // États pour les totaux d'équipements du client
  const [equipmentTotals, setEquipmentTotals] = useState({ byType: {}, total: 0 });
  const [loadingTotalEquipment, setLoadingTotalEquipment] = useState(true);

  // États pour les données CheckMK
  const [checkmkStats, setCheckmkStats] = useState({ stats: [], total: 0 });
  ...
  const navigateToAntivirusDetail = useCallback(
    (item, options = {}) => {
  ...
  const navigateToAntispamDetail = useCallback(
    (item, options = {}) => {
      if (!onNavigate || !client?.id) return;
  ...
  const navigateToMicrosoftTenantDetail = useCallback(
    (credentials = null, options = {}) => {
      if (!onNavigate || !client?.id) return;
  ...
  const modulesRequestRef = useRef({ clientId: null, promise: null, data: null });
  ...
  // Données dédupliquées pour éviter les modules en double (ex: GravityZone)
  const uniqueAntivirusData = useMemo(() => {
    const seen = new Set();
    return (antivirusData || [])
      .map((item) => normalizeAntivirusItem(item))
      .filter((item) => {
  ...
  const configuredAntivirusSolutions = useMemo(
    () => listConfiguredAntivirusSolutions(client, uniqueAntivirusData),
    [client, uniqueAntivirusData]
  );

  const uniqueAntispamData = useMemo(() => {
    const seen = new Set();
    return (antispamData || [])
      .map((item) => normalizeAntispamItem(item))
      .filter((item) => {
        const key =
          (item?.id ?? item?.item_key ?? item?.nom ?? item?.name ?? item?.solution ?? "") + "";
  ...
  const configuredAntispamSolutions = useMemo(
    ...
    [client, uniqueAntispamData, mailinblackTenants]
  );
  ...
  useEffect(() => {
    ...
    listClientMailinblackTenants(client.id)
      .then((result) => setMailinblackTenants(result?.tenants || []))
      .catch(() => setMailinblackTenants([]));
  }, [client?.id]);
  ...
  const configuredDomains = useMemo(
    ...
    [client, domainsData, equipmentRevision]
  );
  ...
  useEffect(() => {
    ...
    getGlobalOvhStatus()
      .then((status) => setGlobalOvhConfigured(Boolean(status?.configured)))
      .catch(() => setGlobalOvhConfigured(false));
  }, [client?.id]);

  const getContractModules = useCallback(
    (source = {}) => {
  ...
  const clientDisplayAddress = useMemo(
    () => buildClientAddress(formData),
    [formData.address, formData.addressStreet, formData.addressPostalCode, formData.addressCity]
  );

  const normalizeMonitoringModules = (source = {}) => {
    let raw =
      source?.modules_monitoring ||
      source?.modules ||
      source ||
      {};
    ...
    return {
      Serveurs: Boolean(raw.Serveurs ?? raw.Server ?? raw.Servers),
      Stockage: Boolean(raw.Stockage ?? raw.Storage),
      Firewall: Boolean(raw.Firewall ?? raw.Firewalls),
      Sauvegarde: Boolean(raw.Sauvegarde),
  ...
  const cloneFormSnapshot = (data) => ({
    ...
    primaryContact: { ...(data.primaryContact || emptyPrimaryContact()) },
  });
  ...
  const detectInfoChanges = (currentData, initialData) => {
    ...
    if (
      normalizeLegalIdentifier(currentData.siret) !== normalizeLegalIdentifier(initialData.siret)
    ) {
      return true;
    }
    ...
    const newContrat = getContratData({ contrat: currentData.contrat });
    ...
    const newModules = getContractModules({ modules: currentData.modules });
    for (const key of Object.keys({ ...oldModules, ...newModules })) {
      if (oldModules[key] !== newModules[key]) return true;
    }

    if (
      JSON.stringify(normalizePrimaryContact(currentData.primaryContact)) !==
      JSON.stringify(normalizePrimaryContact(initialData.primaryContact))
    ) {
      return true;
    }
  ...
  useEffect(() => {
    ...
    const stillExists = sites.some(
      (site) => getSiteLocationValue(site) === activeSiteFilter
    );
    if (!stillExists) setActiveSiteFilter(null);
  }, [formData.sites, activeSiteFilter]);

  const filteredCustomFamilyMap = useMemo(
    () => filterCustomFamilyMap(customFamilyMap, activeSiteFilter),
    [customFamilyMap, activeSiteFilter]
  );

  const heroEquipmentTotalCount = useMemo(() => {
    const customCount = (Array.isArray(customFamilyMap) ? customFamilyMap : []).reduce(
      (sum, family) => sum + (Array.isArray(family?.items) ? family.items.length : 0),
      0
    );
  ...
  const refreshClientEquipment = useCallback(async () => {
    ...
    const [modulesData, customMap] = await Promise.all([
      fetchClientModules(client.id),
      fetchClientCustomEquipmentMap(client.id),
    ]);
    ...
    return { modulesData, customFamilies };
  }, [client?.id]);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e) => {
      if (
        clientActionsMenuRef.current &&
        !clientActionsMenuRef.current.contains(e.target)
      ) {
        setClientActionsMenuOpen(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e) => {
      if (
        equipmentExportMenuRef.current &&
        !equipmentExportMenuRef.current.contains(e.target)
      ) {
        setEquipmentExportMenuOpen(false);
      }
    };
  ...
  const getClientModulesOnce = async (clientId, signal) => {
    ...
    const promise = fetchClientModules(clientId, { signal })
      .then((data) => {
        modulesRequestRef.current = { clientId, promise: null, data };
        return data;
      })
      .catch((error) => {
        modulesRequestRef.current = { clientId, promise: null, data: null };
        throw error;
      });
    modulesRequestRef.current = { clientId, promise, data: null };
    return promise;
  };
  ...
  const hydrateServicesFromModules = useCallback((modulesData) => {
    ...
    setLicencesData(extractLicencesFromModules(modulesData));
    const { instances, jobs } = parseBackupDataFromModules(modulesData);
  ...
  const openAntispamConfigEntry = useCallback(async () => {
    if (!client?.id) return;
    try {
      const [modulesData, tenantsResult] = await Promise.all([
        fetchClientModules(client.id),
        ...
        setClient((prev) =>
          prev
            ? {
                ...
                equipements: modulesData?.equipements ?? prev.equipements,
                modules_monitoring:
                  modulesData?.modules_monitoring ?? prev.modules_monitoring,
              }
            : prev
        );
  ...
  const refreshAntispamState = useCallback(async () => {
    if (!client?.id) return { modulesData: null, tenants: [] };
    const [modulesData, tenantsResult] = await Promise.all([
      fetchClientModules(client.id),
      listClientMailinblackTenants(client.id).catch(() => ({ tenants: [] })),
    ]);
    ...
    setEquipmentRevision((revision) => revision + 1);
    return { modulesData, tenants };
  }, [client?.id, hydrateServicesFromModules]);

  const refreshDomainsState = useCallback(async () => {
    if (!client?.id) return { modulesData: null, ovhConfigured: false };
    const [modulesData, ovhStatus] = await Promise.all([
      fetchClientModules(client.id),
      getGlobalOvhStatus().catch(() => ({ configured: false })),
    ]);
    ...
    setEquipmentRevision((revision) => revision + 1);
    return { modulesData, ovhConfigured, domains: monitored };
  }, [client?.id, hydrateServicesFromModules]);
  ...
  const refreshMicrosoftTenantState = useCallback(async () => {
    ...
    try {
      ...
      const hasCredentials = Boolean(
        creds && (creds.id || creds.tenantId || creds.clientIdAzure || creds.hasSecret)
      );
      setClient((prev) =>
        prev
          ? {
  ...
  const handleSslBrickClick = useCallback(async () => {
    if (!client?.id) return;
    try {
      const items = await fetchClientSslCertificates(client.id, { autoCheck: true });
      setSslData(items);
      setEquipmentRevision((revision) => revision + 1);
  ...
  const handleLicencesBrickClick = useCallback(async () => {
    if (!client?.id) return;
    try {
      const items = await fetchClientLicences(client.id);
      setLicencesData(items);
      setEquipmentRevision((revision) => revision + 1);
      setLicencesModalOpen(true);
    } catch (error) {
      console.error(error);
      setLicencesModalOpen(true);
    }
  ...
  const enrichAzureCredentials = async (clientInfo, signal, isCurrentRequest) => {
    let hasAzureCredentials = Boolean(
      clientInfo?.has_azure_credentials ||
      clientInfo?.hasAzureCredentials ||
      clientInfo?.azureHasCredentials
    );
  ...
  const loadClientData = async (signal) => {
    ...
    const isCurrentRequest = () =>
      ...
      loadControllerRef.current?.signal === signal;

    const prefetchClientId =
      urlClientId || clientData?.client?.id || clientData?.clientId || null;
    const optimisticClient =
      clientData?.client &&
      prefetchClientId &&
      String(clientData.client.id) === String(prefetchClientId)
        ? clientData.client
        : null;

    setError(null);
    setLoading(!optimisticClient);
    modulesRequestRef.current = { clientId: null, promise: null, data: null };
    setMonitoringModulesState(null);
    setInitialMonitoringModules(null);
    setCampaigns([]);
    setLoadingBackups(false);
    setEquipmentTotals({ byType: {}, total: 0 });
    setHardwareEquipmentTotalCount(0);
    setLoadingTotalEquipment(true);
    setCheckmkStats({ stats: [], total: 0 });
    ...
    if (optimisticClient && isMountedRef.current) {
      ...
      hydrateServicesFromModules(
        optimisticClient.equipements ? { equipements: optimisticClient.equipements } : null
      );
      setLoading(false);
    } else {
      setClient(null);
    ...
    try {
      ...
      const modulesPromise = prefetchClientId
        ? getClientModulesOnce(prefetchClientId, signal).catch(() => null)
        : Promise.resolve(null);

      const [clientInfo, modulesData] = await Promise.all([
        clientFetchPromise,
        modulesPromise,
      ]);
      ...
      if (!modulesData?.equipements && clientInfo.equipements) {
        hydrateServicesFromModules({ equipements: clientInfo.equipements });
      }
      setLoading(false);

      enrichAzureCredentials(clientInfo, signal, isCurrentRequest)
        .then((enriched) => {
        ...
        .catch((e) => {
          if (e?.name !== "AbortError") console.error("Erreur credentials Azure:", e);
        });
      ...
      void fetchUsers({ signal })
        ...
        .catch((err) => {
          if (err?.name !== "AbortError") console.error("Erreur chargement utilisateurs:", err);
        })
        .finally(() => {
      ...
      const parallelLoads = [
        loadClientTagsAndNotes(clientInfo.id, signal),
        loadClientActivity(clientInfo.id, signal),
        loadTotalEquipment(clientInfo.id, clientInfo, modulesData, signal),
        loadBackupData(clientInfo.id, signal, { silent: true }),
      ];
      if (!isCommunity) {
        parallelLoads.unshift(
          loadCampaigns(clientInfo.id, signal),
          loadCybersecurityData(clientInfo.id, signal)
        );
      } else {
        parallelLoads.unshift(loadSslLicencesData(clientInfo.id, signal));
      }
  ...
  const loadClientModules = async (clientId = null, modulesData = null, clientSnapshot = null) => {
    ...
    try {
      // Source de vérité pour les toggles monitoring : snapshot client (v_b_clients.modules).
      // L'API /clients/:id/modules peut refléter la structure équipements, qui peut diverger.
      const normalizedMonitoring = normalizeMonitoringModules(
        clientSnapshot || modulesData?.modules_monitoring || client
      );
  ...
  const loadContacts = async (clientId = null, signal) => {
    ...
    try {
      const contactsData = await fetchContacts(targetClientId, { signal: requestSignal });
      ...
      if (isMountedRef.current && String(targetClientId) === String(client?.id)) {
        ...
        setFormData((prev) => ({
          ...
          primaryContact: primary
            ? mapContactToPrimary(primary)
            : prev.primaryContact || emptyPrimaryContact(),
        }));
      }
    } catch (err) {
  ...
  const loadLogs = async (page = 1, clientId = null, signal) => {
    ...
    setLoadingLogs(true);
    try {
      const data = await getClientLogs(targetClientId, page, logsLimit, { signal: requestSignal });
      if (requestSignal?.aborted || !isMountedRef.current) return;
      setLogs(data.logs || []);
  ...
  const loadCheckMKStats = async (clientId = null, signal) => {
    ...
    try {
      const stats = await getClientCheckMKStats(targetClientId, { signal });
      if (signal?.aborted) return;
      setCheckmkStats(stats || { stats: [], total: 0 });
    } catch (error) {
      if (error?.name === "AbortError") return;
  ...
  const loadCampaigns = async (clientId = null, signal) => {
    ...
    try {
      const campaignsData = await getClientCampaigns(targetClientId, {}, { signal });
      if (signal?.aborted) return;
  ...
  const loadClientTagsAndNotes = async (clientId = null, signal) => {
    ...
    try {
      const [tags, notesData] = await Promise.all([
        fetchClientTags(targetClientId, { signal }),
        fetchClientNotes(targetClientId, { signal }),
      ]);
      ...
      if (error?.name === "AbortError") return;
      console.error("Erreur lors du chargement des étiquettes/notes:", error);
  ...
  const loadClientActivity = async (clientId = null, signal) => {
    ...
    try {
      const ticketRows = await fetchTickets({ clientId: targetClientId, limit: 100 }).catch(() => []);
      let upcomingRows = [];
      let recentRows = [];
      let creditSummary = null;
      if (!isCommunity) {
        [upcomingRows, recentRows, creditSummary] = await Promise.all([
          fetchEvents({ clientId: targetClientId, upcoming: true, limit: 50, signal }).catch(() => []),
          fetchEvents({ clientId: targetClientId, recent: true, limit: 50, signal }).catch(() => []),
          fetchClientSupportCredits(targetClientId, { signal }).catch(() => null),
        ]);
      }
      if (signal?.aborted || !isMountedRef.current) return;
      ...
      if (error?.name === "AbortError") return;
      console.error("Erreur lors du chargement de l'activité client:", error);
      if (isMountedRef.current) {
        setSupportTickets([]);
        setPrestationTickets([]);
        setUpcomingEvents([]);
  ...
  const reloadSupportCredits = useCallback(async () => {
    ...
    try {
      ...
      console.error("Erreur lors du rechargement des crédits support:", error);
    }
  }, [client?.id, isCommunity]);
  ...
  const handleOpenSupportCreditsAdmin = () => {
    try {
      sessionStorage.setItem(
        "veritas_admin_nav",
        JSON.stringify({ tab: "support-credits" })
      );
    } catch {
  ...
  const openComputerFleetStats = useCallback(
    (options = {}) => {
      if (!client?.id) return;
      const fromEquipmentPage = equipmentPageRef.current?.getComputersForStats?.();
      const computers =
        Array.isArray(fromEquipmentPage) && fromEquipmentPage.length > 0
          ? fromEquipmentPage
          : getEnterpriseComputers();
      if (!computers.length) {
  ...
  const equipmentStatsAction = useMemo(() => {
    const tableType =
      activeEquipmentTableType || equipmentPageRef.current?.getEmbeddedActiveType?.() || null;
  ...
  const loadTotalEquipment = async (clientId = null, preloadedClientData = null, modulesData = null, signal) => {
    ...
    try {
      ...
      if (signal?.aborted) return;
      setEquipmentTotals(totalsData || { byType: {}, total: 0 });
    } catch (error) {
      if (error?.name === "AbortError") return;
      console.error("Erreur lors du chargement des totaux d'équipements:", error);
      setEquipmentTotals({ byType: {}, total: 0 });
    } finally {
  ...
  const formatContactCommunicationLines = (contact) =>
    normalizeContactCommunications(contact).map((entry) => {
  ...
  const copyContactCard = async (contact) => {
    ...
    const lines = [
      `${copy.table.name}: ${fullName}`,
      `${copy.share.role}: ${contact?.poste || copy.defaultContactName}`,
      ...(commLines.length > 0
        ? commLines
        : [`${copy.share.email}: -`, `${copy.share.phone}: -`]),
    ];
    ...
    try {
      if (navigator?.share) {
        await navigator.share({
          title: interpolate(copy.share.title, { name: fullName }),
          text: payload,
        });
        return;
      }
  ...
  const resolveContractStatus = (expirationDate, isSuspended = false) =>
    getContractStatusDetail(expirationDate, isSuspended, locale);
  const handleBack = () => {
    // Utiliser la navigation interne si disponible, sinon fallback vers l'URL directe
    if (onNavigate) {
      onNavigate("Contrat");
    } else {
  ...
  // Fonction pour détecter les changements entre deux objets
  const detectChanges = (oldData, newData) => {
    const changes = [];
    const oldContrat = getContratData(oldData);
    const newContrat = newData.contrat ? getContratData({ contrat: newData.contrat }) : null;
    ...
    const simpleFields = [
      ...
      { key: 'siret', label: 'identifiant légal' },
      { key: 'secteur', label: "secteur d'activité" },
      { key: 'commercialId', label: 'commercial' }
    ];

    // Comparer les sites seulement s'ils existent dans newData
    ...
    simpleFields.forEach(({ key, label }) => {
      ...
      const oldValue = key === 'commercialId'
        ? normalizeId(oldData.commercialId || oldData.commercial_id)
        : (oldData[key] || '');
      const newValue = key === 'commercialId'
        ? normalizeId(newData[key])
        : (newData[key] || '');

      // Ne détecter un changement que si les valeurs normalisées sont différentes
    ...
    // Comparer les contrats seulement s'ils sont fournis
    if (newContrat) {
      ...
      if (oldContrat.suspendu !== newContrat.suspendu) {
        changes.push({
          ...
          label: 'suspension du contrat',
          oldValue: oldContrat.suspendu ? 'suspendu' : 'actif',
          newValue: newContrat.suspendu ? 'suspendu' : 'actif'
        });
      }
      ...
      if (JSON.stringify(oldSla) !== JSON.stringify(newSla)) {
        changes.push({
          field: 'contrat.sla',
          label: 'SLA support',
          oldValue: oldSla.enabled ? 'activé' : 'désactivé',
          newValue: newSla.enabled ? 'activé' : 'désactivé',
        });
      }
    }

    // Comparer les modules seulement s'ils sont fournis
    if (newData.modules) {
      const oldModules = getContractModules(oldData);
      const newModules = getContractModules({ modules: newData.modules });

      Object.keys({ ...oldModules, ...newModules }).forEach(moduleKey => {
        if (oldModules[moduleKey] !== newModules[moduleKey]) {
          const moduleLabel = moduleKey === 'Hebergement' ? 'hébergement' : moduleKey.toLowerCase();
          changes.push({
            field: `modules.${moduleKey}`,
            label: `module ${moduleLabel}`,
            oldValue: oldModules[moduleKey] ? 'activé' : 'désactivé',
            newValue: newModules[moduleKey] ? 'activé' : 'désactivé'
          });
        }
      });
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveClientModules(client.id, { modules: formData.modules });
      ...
      toast.error(interpolate(copy.toast.saveError, { message: error.message }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const loadDeletionCheck = useCallback(async (clientId) => {
    ...
    try {
      ...
      console.error("Vérification suppression entreprise:", error);
      setDeletionCheckClient(null);
    } finally {
  ...
  const enterpriseDeletionBlocked =
    Boolean(deletionCheckClient) &&
    !loadingDeletionCheck &&
    !isClientDeletable(deletionCheckClient);

  const deleteBlockedTooltip = useMemo(() => {
    if (!enterpriseDeletionBlocked) return "";
    const snapshot = deletionCheckClient || client;
    const { total } = getLinkedElementsSummary(snapshot);
    ...
    return interpolate(copy.deleteBlocked.withDetails, { total, details });
  ...
  const handleExportReversibility = async () => {
    ...
    try {
      ...
      await exportReversibilityDossier({
        client,
        formData,
        contacts,
        supportTickets,
        ...
        clientTags,
        commercialLabel:
          users.find((u) => u.id === formData.commercialId)?.username ||
          users.find((u) => u.id === formData.commercialId)?.email ||
          "",
        contractModules: getContractModules({ modules: formData.modules }),
        modulesData,
      });
      toast.success(copy.toast.reversibilityDownloaded);
    } catch (exportError) {
      console.error("Export réversibilité:", exportError);
      toast.error(
        exportError?.message || copy.toast.reversibilityError
      );
    } finally {
  ...
  const loadSslLicencesData = async (clientId = null, signal) => {
    const targetClientId = clientId || client?.id;
    if (!targetClientId) return;

    try {
      const [sslCerts, licences] = await Promise.all([
        fetchClientSslCertificates(targetClientId, { signal }).catch((e) =>
          e?.name === "AbortError" ? Promise.reject(e) : []
        ),
        fetchClientLicences(targetClientId, { signal }).catch((e) =>
          e?.name === "AbortError" ? Promise.reject(e) : []
        ),
      ]);
      if (signal?.aborted) return;
      ...
      setLicencesData(Array.isArray(licences) ? licences : []);
    } catch (error) {
      if (error?.name === "AbortError") return;
      console.error("Erreur lors du chargement SSL / licences:", error);
    }
  };
  ...
  const loadCybersecurityData = async (clientId = null, signal) => {
    ...
    try {
      const [antivirus, antispam, domains, sslCerts, licences] = await Promise.all([
      ...
      if (antivirus.length === 0 && antispam.length === 0) {
        ...
        const modulesData = await getClientModulesOnce(targetClientId, signal).catch((e) => (e?.name === "AbortError" ? Promise.reject(e) : null));
        setAntivirusData(
          modulesData ? mergeAntivirusSources(antivirus, modulesData) : antivirus
        );
        setAntispamData(
          modulesData ? mergeAntispamSources(antispam, modulesData) : antispam
        );
      }
      ...
      if (licences.length > 0) {
        setLicencesData(licences);
      } else {
        const modulesData = await getClientModulesOnce(targetClientId, signal).catch((e) => (e?.name === "AbortError" ? Promise.reject(e) : null));
        setLicencesData(modulesData ? extractLicencesFromModules(modulesData) : []);
      }

      const customMap = await fetchClientCustomEquipmentMap(targetClientId, { signal }).catch((e) =>
        e?.name === "AbortError" ? Promise.reject(e) : { families: [] }
      );
      if (!signal?.aborted) {
      ...
      if (error?.name === "AbortError") return;
      console.error('Erreur lors du chargement des données cybersécurité:', error);
    }
  };
  ...
  const loadBackupData = async (clientId = null, signal, options = {}) => {
    ...
    try {
      const [modulesData, mappingsData] = await Promise.all([
        getClientModulesOnce(targetClientId, signal).catch((e) => (e?.name === "AbortError" ? Promise.reject(e) : null)),
        fetch(`${API_BASE_URL}/checkmk/mapping/${targetClientId}`, { credentials: 'include', signal })
      ...
      if (mappingsData && Array.isArray(mappingsData)) {
        mappingsData.forEach(m => {
          if (m.equipment_type === 'Sauvegarde' && m.equipment_id && m.is_active !== false) {
            mappingsMap[m.equipment_id] = m;
          }
        });
      }

      const { instances, jobs } = parseBackupDataFromModules(modulesData, mappingsMap);
      setBackupInstances(instances);
      setBackupJobs(jobs);
    } catch (error) {
      if (error?.name === "AbortError") return;
  ...
  // Charger les détails antivirus depuis l'API (statistiques, endpoints, etc.)
  const loadAntivirusDetails = async (antivirusItem, isSyncRefresh = false) => {
    ...
    try {
      ...
      const statsResponse = await fetch(
        `${API_BASE_URL}/bitdefender/statistics/${antivirusItem.companyId}${qs}`,
        { credentials: "include" }
      ).catch(() => ({ ok: false }));
      ...
      const endpointsResponse = await fetch(
        `${API_BASE_URL}/bitdefender/endpoints/${antivirusItem.companyId}/enriched${qs}`,
        { credentials: "include" }
      ).catch(() => ({ ok: false }));
      ...
      const policiesResponse = await fetch(
        `${API_BASE_URL}/bitdefender/policies/${antivirusItem.companyId}${qs}`,
        { credentials: "include" }
      ).catch(() => ({ ok: false }));
      ...
      console.error('Erreur lors du chargement des détails antivirus:', error);
  ...
  const handleMonitoringModuleToggle = (moduleKey) => {
    ...
    // Ne pas autoriser l'activation / désactivation du module TOIP depuis cette page
    ...
    setMonitoringModulesState((prev) => {
      ...
      if (initialMonitoringModules) {
        ...
        const hasDiff = allKeys.some(
          (key) =>
            Boolean(initialMonitoringModules[key]) !== Boolean(next[key])
        );
        if (hasDiff) {
          setHasChanges(true);
        }
  ...
  const getModuleIcon = (moduleName, isActive = true, iconClass = styles.moduleIcon) => {
    ...
    // - Cybersécurité : rouge
    // - Services : violet
    let baseColor = "#3b82f6"; // bleu par défaut (infra)
    ...
    const iconColor = isActive ? baseColor : "#6b7280"; // inactif en gris
  ...
  const handleAddClientTag = async ({ label, color }) => {
    const trimmed = String(label || "").trim();
    if (!trimmed || !client?.id || addingTag) return;

    setAddingTag(true);
    try {
      const tag = await addClientTag(client.id, { label: trimmed, color });
      setClientTags((prev) => {
      ...
      console.error("Erreur ajout étiquette:", error);
  ...
  const visibleSupportPacks = useMemo(
    () => supportCreditPacks.filter((pack) => ["active", "upcoming"].includes(pack.status)),
    [supportCreditPacks]
  );
  ...
  const activeContacts = useMemo(
    () =>
      contacts.filter(
        (c) =>
          String(c.statut || "").toLowerCase().includes("actif") &&
          !String(c.statut || "").toLowerCase().includes("inactif")
      ),
    [contacts]
  );
  ...
  const clientSites = useMemo(
    () => normalizeClientSites(formData.sites),
    [formData.sites]
  );
  const enterpriseGuideSteps = useMemo(
    () => {
      ...
      return steps.filter(
        (step) => step.target !== '[data-guide="enterprise-equipment-stats"]'
      );
    },
  ...
  const handleSitesSave = async (newSites) => {
    ...
    try {
      await updateClient(client.id, { sites });
      setFormData((prev) => ({ ...prev, sites }));
      setClient((prev) => ({ ...prev, sites }));
      notifyEnterprisesListRefresh();
      toast.success(copy.toast.sitesUpdated);
    } catch (error) {
      console.error("Erreur lors de la mise à jour des lieux:", error);
      toast.error(interpolate(copy.toast.sitesUpdateError, { message: error.message }));
      throw error;
    }
  };
  ...
  if (error || !client) {
    return (
      <div className={`${styles.contratDetailPage} msp-page-grid`}>
  ...
  const commercialLabel =
    commercialUser?.username || commercialUser?.email || null;

  return (
    <div className={`${styles.contratDetailPage} ${styles.enterpriseDetailPage} msp-page-grid`}>
      <header
        className={`${styles.pageHero} ${isCommunity ? styles.pageHeroProTeaser : ""}`}
        ref={headerRef}
        data-guide="enterprise-hero"
      >
          ...
          <div className={styles.heroMain}>
            ...
            <div className={styles.heroText}>
              ...
              <div className={styles.heroMeta} aria-label={copy.heroMetaAria}>
                <span
                  className={`${styles.contractBadge} ${styles[`contractBadge_${contractStatus.status}`] || styles.contractBadge_unknown}`}
                >
                ...
                <span className={styles.heroMetaItem}>
                  ...
                  {interpolate(
                    heroEquipmentTotalCount > 1 ? copy.equipmentCountPlural : copy.equipmentCount,
                ...
                {activeContactCount > 0 && (
                  <span className={styles.heroMetaItem}>
                    ...
                    {interpolate(
                      activeContactCount > 1 ? copy.contactCountPlural : copy.contactCount,
                      { count: activeContactCount }
                    )}
                  </span>
                )}
                <EnterpriseRmmEnrollmentHero
                  clientId={client?.id}
                  isAdmin={userRole === "admin"}
                  compact
                  itemClassName={`${styles.heroMetaItem} ${styles.heroMetaItemRmm}`}
                  tokenWrapClassName={styles.heroRmmToken}
                />
                {loadingTags ? (
                  <span className={styles.heroTagsLoading}>{copy.loadingTags}</span>
                ) : (
                  <>
                    {clientTags.map((tag) => (
                      <span
                        key={tag.id}
                        className={styles.heroTagChip}
                        style={{
                      ...
                      >
                        ...
                        <button
                          type="button"
                          className={styles.heroTagRemove}
                          onClick={() => handleRemoveClientTag(tag.id)}
                          aria-label={interpolate(copy.removeTagAria, { label: tag.label })}
                        >
                          <FaTimes />
                        </button>
                      </span>
                    ...
                    <div className={styles.heroTagAddWrap}>
                      <SmartTooltip content={copy.addTag}>
                        <button
                          type="button"
                          className={styles.heroTagAddTrigger}
          ...
          <div className={styles.heroActions} ref={clientActionsMenuRef} data-guide="enterprise-hero-actions">
            <SmartTooltip content={copy.actionsMenuTooltip}>
              <button
                ...
                onClick={() => setClientActionsMenuOpen((open) => !open)}
                aria-expanded={clientActionsMenuOpen}
                aria-haspopup="menu"
            ...
            {clientActionsMenuOpen && (
              <div className={styles.heroClientMenu} role="menu">
                ...
                >
                  {generatingReversibility && !isCommunity ? (
                    <Icon icon="mdi:loading" className={styles.spinning} aria-hidden />
                  ) : (
                    <Icon icon="mdi:folder-download-outline" aria-hidden />
                  )}
                  <span className={styles.heroMenuItemLabel}>
                    {generatingReversibility && !isCommunity
                      ? copy.generatingDossier
                      : copy.reversibilityDossier}
                    {isCommunity ? (
        ...
        <UpcomingEventBookmarks
          upcomingEvents={upcomingEvents}
          recentEvents={recentEvents}
          loading={loadingClientActivity}
          typeLabels={eventTypeLabels}
          labels={copy.eventBookmarks}
          menuLabels={copy.eventActionMenu}
          locale={locale}
          users={users}
          proFeatureLabel={copy.proFeatures.planning}
          proFeatureKey="planning"
          proLocked={isCommunity}
          inPageHero
          defaultCollapsed
          onEditEvent={(event) => {
      ...
      <div className={styles.pageBody}>
        <div className={styles.pageGrid}>
          <main className={styles.mainColumn}>
            ...
            <section className={styles.panel} data-guide="enterprise-infra-map">
              <div className={styles.panelHeader}>
                <div className={styles.panelHeaderMain}>
                  ...
                  {activeSiteFilter ? (
                    <span className={styles.panelFilterBadge}>
                      {interpolate(copy.filterBadge, { value: activeSiteFilter })}
                    </span>
                  ) : null}
                </div>
              </div>
              <div className={styles.panelBody}>
                <InfrastructureMap
                  ...
                  antivirusItems={configuredAntivirusSolutions}
                  antispamItems={configuredAntispamSolutions}
                  domainItems={configuredDomains}
                  domainIntegrationReady={globalOvhConfigured}
                  sslItems={sslData}
                  licenceItems={licencesData}
                  customFamilyMap={customFamilyMap}
                  siteFilter={activeSiteFilter}
                  campaignItems={campaigns}
                  tenantInfo={{
                    configured: Boolean(
                      client?.has_azure_credentials ||
                      client?.hasAzureCredentials ||
                      client?.azureHasCredentials
                    ),
                  ...
                  googleWorkspaceInfo={{
                    configured: Boolean(
                      client?.googleWorkspace?.configured ||
                      client?.hasGoogleWorkspaceCredentials
                    ),
                  ...
                  onNodeClick={handleInfraNodeClick}
                  onBrickClick={(brick) => {
                    ...
                    if (familyKey) {
                      ...
                      if (family) {
                        setCustomEquipmentModal({ family, item: null });
                        return;
                      }
                    }
                    if (brick.type === "Sauvegarde") {
                    ...
                    if (brick.type === "LicencesAbonnements") {
                      handleLicencesBrickClick();
                      return;
                    }
                    ...
                    if (brick.type === "Campagne") {
                      ...
                      onNavigate?.("Cybersecurite", { activeTab: "campaigns" });
                    }
                  }}
                />
              </div>
              ...
              <div className={styles.panelHeader}>
                ...
                <div className={styles.panelToolbar}>
                  <span className={styles.equipmentResultCount}>
                    {interpolate(
                      equipmentResultCount > 1 ? copy.resultCountPlural : copy.resultCount,
                      { count: equipmentResultCount }
                    )}
                  </span>
                  {equipmentStatsAction ? (
                    <SmartTooltip as="span" content={equipmentStatsAction.label}>
                      <button
                        type="button"
                        className={styles.exportHeaderButton}
                        data-guide="enterprise-equipment-stats"
                        onClick={() => openComputerFleetStats()}
                        onMouseDown={(e) => {
                          if (e.button === 1) {
                            e.preventDefault();
                            openComputerFleetStats({ background: true });
                          }
                        }}
                  ...
                  <div className={styles.exportMenuWrap} ref={equipmentExportMenuRef}>
                    <SmartTooltip as="span" content={copy.exportCsv}>
                      <button
                        type="button"
                        className={styles.exportHeaderButton}
                        onClick={() => setEquipmentExportMenuOpen((open) => !open)}
                        aria-expanded={equipmentExportMenuOpen}
                        aria-haspopup="menu"
                      >
                        <FaFileExport />
                      </button>
                  ...
                  <SmartTooltip as="span" content={copy.addEquipment}>
                    <button
                      ...
                      onClick={() => equipmentPageRef.current?.openAddEquipmentModal()}
                    >
                      <FaPlus />
                    </button>
                  </SmartTooltip>
              ...
              <div className={styles.panelBody}>
                <div className={styles.equipmentSection}>
                  <EquipmentPage
                    ...
                    embeddedClient={
                      client
                        ? { ...client, sites: formData.sites ?? client.sites ?? [] }
                        : null
                    }
                    onNavigate={onNavigate}
                    searchQuery={equipmentSearchQuery}
                    onSearchQueryChange={setEquipmentSearchQuery}
                    onFilteredCountChange={setEquipmentResultCount}
                    onTotalCountChange={setHardwareEquipmentTotalCount}
                    onEquipmentChanged={refreshClientEquipment}
                    onClientSsidsUpdated={(ssids) => {
                      setClient((prev) => (prev ? { ...prev, ssids } : prev));
                    }}
                    customFamilyMap={filteredCustomFamilyMap}
                    backupInstances={backupInstances}
                    siteFilter={activeSiteFilter}
                    onCustomFamilyManage={(family, item) => {
                      setCustomEquipmentModal({ family, item: item || null });
                    }}
              ...
              <div className={styles.panelBody}>
                {loadingClientActivity ? (
                  <div className={styles.activityLayout}>
                              ...
                              {supportTickets.length === 0 ? (
                                ...
                                supportTickets.slice(0, 8).map((ticket) => {
                                  ...
                                  return (
                                    <tr
                                      key={ticket.id}
                                      className={styles.dataTableRowClickable}
                    ...
                    <ProFeatureLock
                      locked={isCommunity}
                      featureLabel={copy.proFeatures.prestations}
                      featureKey="prestations"
                      className={styles.activityBlockProLock}
                      badgePosition="none"
                    >
                      <div className={styles.activityBlock}>
                        <div className={styles.activityBlockHeader}>
                        ...
                        <div className={styles.dataTableWrapper}>
                          <table className={styles.dataTable}>
                            ...
                            <tbody>
                              {(isCommunity ? DEMO_PRESTATION_TICKETS : prestationTickets).length === 0 ? (
                                <tr>
                                ...
                                (isCommunity ? DEMO_PRESTATION_TICKETS : prestationTickets)
                                  .slice(0, 8)
                                  .map((ticket) => {
                                    const status = normalizeTicketStatus(ticket.status);
                                    return (
                                      <tr
                                        key={ticket.id}
                                        className={
                                          isCommunity ? undefined : styles.dataTableRowClickable
                                        }
                                        onClick={
                                          isCommunity ? undefined : () => handleOpenTicket(ticket)
                                        }
                                        onAuxClick={
                                          isCommunity
                                            ? undefined
                    ...
                    <ProFeatureLock
                      locked={isCommunity}
                      featureLabel={copy.proFeatures.campaigns}
                      featureKey="cyberCampaigns"
                      className={`${styles.activityBlockProLock} ${styles.activityCampaignsRow}`}
                      badgePosition="none"
                    >
                      <div className={styles.activityBlock}>
                        <div className={styles.activityBlockHeader}>
                          ...
                          <span className={styles.activityBlockCount}>
                            {interpolate(copy.campaignsOngoing, { count: ongoingCampaigns.length })}
                          </span>
                        </div>
                        <div className={styles.dataTableWrapper}>
                          <table className={styles.dataTable}>
                            ...
                            <tbody>
                              {ongoingCampaigns.length === 0 ? (
                                ...
                                ongoingCampaigns.slice(0, 8).map((campaign) => (
                                  <tr
                                    key={campaign.id}
                                    className={
                                      isCommunity ? undefined : styles.dataTableRowClickable
                                    ...
                                    onClick={
                                      isCommunity ? undefined : () => handleOpenCampaign(campaign)
                                    }
                                  >
                                    ...
                                    <td className={styles.activityProgressCell}>
                                      {typeof campaign.global_progress === "number"
            ...
            <ProFeatureLock
              locked={isCommunity}
              featureLabel={copy.proFeatures.vault}
              featureKey="vault"
            >
                    ...
                    <div className={styles.panelToolbar}>
                      <SmartTooltip as="span" content={vaultCopy.panel.addToVault}>
                        <button
                          type="button"
                          className={styles.addEquipmentButton}
                          onClick={() => vaultPanelRef.current?.openUploadModal()}
                          aria-label={vaultCopy.panel.addToVault}
                        >
                          <FaPlus />
                        </button>
                      </SmartTooltip>
          ...
          <aside className={styles.asidePanel}>
                  ...
                  <>
                    <div className={styles.sidebarSummaryItem}>
                      ...
                      {formData.siret ? (
                        ...
                        <span className={styles.sidebarSummaryValueEmpty}>-</span>
                      )}
                    </div>
                    <div className={styles.sidebarSummaryItem}>
                      <span className={styles.sidebarSummaryLabel}>{copy.sectorLabel}</span>
                      {formData.secteur?.trim() ? (
                        <span className={styles.sidebarSummaryValue}>{formData.secteur.trim()}</span>
                      ) : (
                        <span className={styles.sidebarSummaryValueEmpty}>-</span>
                      )}
                    </div>
                    <div className={styles.sidebarSummaryItem}>
                      ...
                      {clientDisplayAddress ? (
                        <span className={styles.sidebarSummaryValue}>{clientDisplayAddress}</span>
                      ) : (
                        <span className={styles.sidebarSummaryValueEmpty}>-</span>
                      )}
                    </div>
                ...
                <div className={styles.sidebarSummaryItem}>
                  ...
                  <div className={styles.sidebarOptionsIconsContract}>
                    {enabledModules.map((mod) => {
                      ...
                      return (
                        <SmartTooltip key={mod.moduleKey} content={mod.label}>
                          <span
                            className={`${styles.sidebarOptionIcon} ${
                              active
                                ? styles.sidebarOptionIconActive
                                : styles.sidebarOptionIconInactive
                            }`}
                          >
                            <Icon
                              icon={mod.icon || "mdi:puzzle-outline"}
                              className={styles.sidebarOptionIconInner}
                            />
                          </span>
                        </SmartTooltip>
                      );
            ...
            {(isCommunity || supportCreditBalance !== null) && (
              <section className={`${styles.sidebarSection} ${styles.sidebarCreditsSection}`}>
                <button
                  type="button"
                  className={`${styles.sidebarCollapseHeader} ${
                    isCommunity ? styles.sidebarCollapseHeaderLocked : ""
                  }`}
                  onClick={() => {
                    if (isCommunity) {
                  ...
                  aria-expanded={isCommunity ? false : creditsExpanded}
                  aria-controls="enterprise-sidebar-credits"
                >
                ...
                {!isCommunity && creditsExpanded && (
                  <div className={styles.sidebarBody} id="enterprise-sidebar-credits">
                      ...
                      <div className={styles.sidebarBodyActions}>
                        ...
                        <SmartTooltip content={copy.openCreditsAdmin}>
                          <button
                            type="button"
                            className={styles.editInfoButton}
                            onClick={handleOpenSupportCreditsAdmin}
                            aria-label={copy.openCreditsAdmin}
                          >
                    ...
                    <ProFeatureLock
                      locked={isCommunity}
                      featureLabel={copy.proFeatures.creditPacks}
                      featureKey="credits"
                      badgePosition="none"
                      softLocked={isCommunity}
                    >
                      {visibleSupportPacks.length > 0 && !isCommunity ? (
                        <ul className={styles.activityCreditList}>
                          {visibleSupportPacks.map((pack) => {
                            const remaining = Number(pack.remaining_amount) || 0;
                            const initial = Number(pack.initial_amount) || 0;
                            const until = pack.valid_until
                              ? new Date(pack.valid_until).toLocaleDateString("fr-FR")
                              : null;
                            return (
                              <li key={pack.id} className={styles.activityCreditCard}>
                                ...
                                <div className={styles.activityCreditBody}>
                                  <div className={styles.activityCreditTop}>
                                    ...
                                    <span
                                      className={`${styles.activityCreditValue} ${
                                        remaining <= 0 ? styles.activityCreditValueEmpty : ""
                                      }`}
                                    >
                                      {remaining}/{initial}
                                    </span>
                                  ...
                                  <div className={styles.activityCreditMeta}>
                                    <Icon
                                      ...
                                      className={styles.activityCreditMetaIcon}
                                      aria-hidden
                                    />
                                    <span>
                                      {until ? interpolate(copy.creditValidUntil, { date: until }) : copy.creditNoExpiry}
                                    </span>
                                  </div>
                                </div>
                              </li>
                        ...
                        <span className={styles.sidebarSummaryValue}>{copy.noCreditPack}</span>
                      ) : userRole === "admin" ? (
                        <div className={styles.clientNotesEmpty}>
                          <button
                            type="button"
                            className={styles.clientNotesAddButton}
                            onClick={() => setSupportCreditModalOpen(true)}
                          >
                            {copy.addCredit}
                          </button>
                        ...
                        <span className={styles.sidebarSummaryValue}>{copy.noActiveCreditPack}</span>
                      )}
                    </ProFeatureLock>
                  </div>
                )}
            ...
            <section className={`${styles.sidebarSection} ${styles.sidebarSlaSection}`}>
              <button
                ...
                aria-expanded={isCommunity ? false : slaExpanded}
                aria-controls="enterprise-sidebar-sla"
              >
                <span className={styles.sidebarInfoTitle}>
                  {copy.slaTitle}
                  {isCommunity ? (
                ...
                <Icon
                  icon={!isCommunity && slaExpanded ? "mdi:chevron-up" : "mdi:chevron-down"}
                  className={styles.sidebarCollapseChevron}
                  aria-hidden
                />
              </button>
              {!isCommunity && slaExpanded && (
                <div className={styles.sidebarBody} id="enterprise-sidebar-sla">
                  <ProFeatureLock
                    locked={isCommunity}
                    featureLabel={copy.proFeatures.sla}
                    featureKey="sla"
                    badgePosition="none"
                    softLocked={isCommunity}
                  >
                    {parseClientSla(client?.contrat).enabled ? (
                      <table className={styles.sidebarSlaTable}>
                        <thead>
                          <tr>
                            <th scope="col" />
                        ...
                        <tbody>
                          {formatClientSlaRows(client?.contrat).map((row) => (
                            <tr key={row.key}>
                      ...
                      <span className={styles.sidebarSummaryValue}>{copy.noActiveSla}</span>
                    )}
                  </ProFeatureLock>
                </div>
              )}
            ...
            <section className={styles.sidebarSection} data-guide="enterprise-sidebar-contacts">
              <button
                type="button"
                className={styles.sidebarCollapseHeader}
                onClick={() => setContactsSectionExpanded((prev) => !prev)}
                aria-expanded={contactsSectionExpanded}
                aria-controls="enterprise-sidebar-contacts"
              >
                <span className={styles.sidebarInfoTitle}>
                  {copy.contactsTitle}
                  {!loadingContacts && contacts.length > 0 ? (
                    <span className={styles.sidebarSectionCount}>{activeContactCount}</span>
                  ) : null}
                </span>
                <Icon
                  icon={contactsSectionExpanded ? "mdi:chevron-up" : "mdi:chevron-down"}
                  className={styles.sidebarCollapseChevron}
                  aria-hidden
                />
              </button>
              ...
              {loadingContacts ? (
                <div className={styles.loadingState}>{copy.loadingContacts}</div>
                ...
                <ul className={styles.sidebarContactsList}>
                  {visibleContacts.map((contact) => (
                    <SmartTooltip
                      ...
                      onClick={() => {
                        if (onNavigate) {
                          onNavigate("ContactDetail", { contactId: contact.id });
                        }
                      }}
                      ...
                      <div className={styles.sidebarContactBody}>
                        <div className={styles.sidebarContactTop}>
                          <div className={styles.sidebarContactIdentity}>
                            ...
                            {contact.poste && (
                              <span className={styles.sidebarContactRole}>
                                {contact.poste}
                              </span>
                            )}
                          </div>
                          <div className={styles.sidebarContactActions}>
                            <button
                              type="button"
                              className={styles.sidebarCopyButton}
                              title={copy.copyContactCard}
                              aria-label={copy.copyContactCard}
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            <button
                              ...
                              title={copy.shareContactCard}
                              aria-label={copy.shareContactCard}
                              onClick={(e) => {
                                e.stopPropagation();
                        ...
                        {(() => {
                          ...
                          return (
                            <div className={styles.sidebarContactMeta}>
                              {communications.map((entry) => {
                                ...
                                return (
                                  <RowTag
                                    key={entry.id}
                                    href={href || undefined}
                                    className={styles.sidebarContactMetaRow}
                                    title={
                                      entry.isPrimary
                                        ? interpolate(copy.coordFavoriteTitle, { label: typeDef.label })
                                        : typeDef.label
                                    }
                                  ...
                                  >
                                    <Icon
                                      icon={typeDef.icon}
                                      className={styles.sidebarContactMetaIcon}
                                      aria-hidden
                                    />
                                    ...
                                    {entry.isPrimary ? (
                                      <Icon
                                        icon="mdi:star"
                                        className={styles.sidebarContactFavoriteStar}
                                        title={copy.coordFavorite}
                                        aria-label={copy.coordFavorite}
                                      />
                                    ) : null}
                                  </RowTag>
                                );
            ...
            <section className={styles.sidebarSection}>
              <button
                type="button"
                className={styles.sidebarCollapseHeader}
                onClick={() => setSitesSectionExpanded((prev) => !prev)}
                aria-expanded={sitesSectionExpanded}
                aria-controls="enterprise-sidebar-sites"
              >
                <span className={styles.sidebarInfoTitle}>
                  {copy.sitesTitle}
                  {(formData.sites || []).length > 0 ? (
                    <span className={styles.sidebarSectionCount}>{clientSites.length}</span>
                  ) : null}
                </span>
                <Icon
                  icon={sitesSectionExpanded ? "mdi:chevron-up" : "mdi:chevron-down"}
                  className={styles.sidebarCollapseChevron}
                  aria-hidden
                />
              </button>
              {sitesSectionExpanded && (
                    ...
                    {visibleSites.map((site) => {
                        ...
                        return (
                          <button
                            type="button"
                            key={getSiteId(site)}
                            className={`${styles.sitePreviewCard} ${styles.siteFilterButton} ${
                              isActive ? styles.siteFilterButtonActive : ""
                            }`}
                            onClick={() =>
                              setActiveSiteFilter(isActive ? null : siteValue)
                            }
                            title={interpolate(copy.filterSiteTitle, { name: getSiteDisplayName(site) })}
                          >
                            <div className={styles.sitePreviewMain}>
                              <strong className={styles.sitePreviewName}>
                                {getSiteDisplayName(site)}
                                {site.isPrimary ? (
                                  <span className={styles.sitePreviewPrimary}>{copy.sitePrimary}</span>
                                ) : null}
                              </strong>
                              {address ? (
                                <span className={styles.sitePreviewAddress}>{address}</span>
                              ) : null}
                            </div>
                            <div
                              className={styles.sitePreviewMap}
            ...
            <section className={styles.sidebarSection} data-guide="enterprise-sidebar-notes">
              <button
                type="button"
                className={styles.sidebarCollapseHeader}
                onClick={() => setNotesSectionExpanded((prev) => !prev)}
                aria-expanded={notesSectionExpanded}
                aria-controls="enterprise-sidebar-notes"
              >
                <span className={styles.sidebarInfoTitle}>
                  {copy.notesTitle}
                  {!loadingNotes && notes.length > 0 ? (
                    <span className={styles.sidebarSectionCount}>{notes.length}</span>
                  ) : null}
                </span>
                <Icon
                  icon={notesSectionExpanded ? "mdi:chevron-up" : "mdi:chevron-down"}
                  className={styles.sidebarCollapseChevron}
                  aria-hidden
                />
              </button>
              ...
              {loadingNotes ? (
                <div className={styles.loadingState}>{copy.loadingNotes}</div>
              ) : notes.length === 0 ? (
                <div className={styles.clientNotesEmpty}>
                  <button
                    type="button"
                ...
                <ul className={styles.clientNotesList}>
                  {notes.map((note) => (
                    <li key={note.id} className={styles.clientNoteItem}>
                        ...
                        <div className={styles.clientNoteMeta}>
                          ...
                          <span className={styles.clientNoteDate}>
                            ...
                            {note.updated_at && note.updated_at !== note.created_at
                              ? interpolate(copy.noteEdited, { date: formatNoteDate(note.updated_at) })
                              : ""}
                          </span>
                        </div>
                        {canEditNote(note) ? (
                          <div className={styles.clientNoteActions}>
                            <SmartTooltip content={copy.editNote}>
                              <button
                                type="button"
                                className={`${styles.editInfoButton} ${styles.clientNoteActionBtn}`}
                                onClick={() => openEditNoteModal(note)}
                                aria-label={copy.editNoteAria}
                              >
                                <FaPencilAlt />
                              </button>
                            </SmartTooltip>
                            <SmartTooltip content={copy.deleteNote}>
                              <button
                                type="button"
                                className={`${styles.cancelInfoButton} ${styles.clientNoteActionBtn}`}
                                onClick={() => handleDeleteNote(note.id)}
                                aria-label={copy.deleteNoteAria}
                              >
      ...
      <EnterpriseEditModal
        ...
        onClose={closeEnterpriseEditModal}
        onSave={handleSave}
        updateFormData={updateFormData}
        updateContratField={updateContratField}
        handleModuleToggle={handleModuleToggle}
        onOpenSitesModal={() => setSitesModalOpen(true)}
        onUserCreated={(newUser) =>
          setUsers((prev) => [...prev.filter((user) => user.id !== newUser.id), newUser])
        }
        enabledModules={enabledModules}
        isCommunity={isCommunity}
        onDeleteEnterprise={requestDeleteEnterpriseFromEdit}
        deleteBlockedTooltip={deleteBlockedTooltip}
        deleteDisabled={saving || deletingClient || loadingDeletionCheck || !deletionCheckClient}
        deleteLoading={deletingClient}
        deletionBlocked={enterpriseDeletionBlocked}
        loadingDeletionCheck={loadingDeletionCheck}
      />

      <EnterpriseBlockersModal
        open={deletionBlockersModalOpen}
        client={deletionClientSnapshot}
        onClose={() => setDeletionBlockersModalOpen(false)}
      />

      <EnterpriseDeleteModal
        open={deleteClientModalOpen}
        clientName={formData?.name || client?.name || copy.defaultEnterpriseName}
        saving={deletingClient}
      ...
      <SupportCreditPackModal
        open={supportCreditModalOpen}
        mode="create"
        pack={client?.id ? { client_id: client.id } : null}
        clients={client ? [client] : []}
        lockClient
        onClose={() => setSupportCreditModalOpen(false)}
        onSaved={reloadSupportCredits}
      />
            ...
            <div className={styles.modalBody}>
              <div className={styles.comingSoonMessage}>
                ...
                <ul className={styles.comingSoonList}>
                  {copy.modals.comingSoonBullets.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
      ...
      <ClientTagModal
        open={tagModalOpen}
        entityName={formData?.name || client?.name || ""}
      ...
      <PlanningEventModalBridge
        ...
        initialClientName={formData?.name || client?.name || ""}
        onClose={() => {
      ...
      {domainsConfigModalOpen && client?.id && (
        <DomainsConfigModal
          client={client}
          initialSection={domainsConfigInitialSection}
          initialProviderId={domainsConfigInitialProviderId}
          isCommunity={isCommunity}
          onClose={() => {
      ...
      {domainPickerOpen && client?.id && (
        <DomainSolutionPickerModal
          open={domainPickerOpen}
          client={client}
          domains={domainPickerDomains}
          onClose={() => {
      ...
      {domainOverviewOpen && client?.id && domainOverviewItem && (
        <DomainOverviewModal
          open={domainOverviewOpen}
          client={client}
      ...
      <SslCertificatesModal
        isOpen={sslModalOpen}
        onClose={() => setSslModalOpen(false)}
        certificates={sslData}
        clientId={client?.id}
        onRefresh={async () => {
          if (!client?.id) return;
          const items = await fetchClientSslCertificates(client.id, { autoCheck: true });
      ...
      <LicencesAbonnementsModal
        isOpen={licencesModalOpen}
        onClose={() => setLicencesModalOpen(false)}
      ...
      {antivirusConfigModalOpen && client?.id && (
        <AntivirusConfigModal
          client={client}
          initialSection={antivirusConfigInitialSection}
          initialEditingSolution={antivirusConfigEditingSolution}
          isCommunity={isCommunity}
          onClose={() => {
      ...
      {antivirusPickerOpen && client?.id && (
        <AntivirusSolutionPickerModal
          open={antivirusPickerOpen}
          client={client}
          solutions={antivirusPickerSolutions}
          onClose={() => {
      ...
      {microsoftTenantConfigModalOpen && client?.id && (
        <MicrosoftTenantConfigModal
          client={client}
          initialSection={microsoftTenantConfigInitialSection}
          onClose={() => setMicrosoftTenantConfigModalOpen(false)}
      ...
      {antispamConfigModalOpen && client?.id && (
        <AntispamConfigModal
          client={client}
          initialSection={antispamConfigInitialSection}
          initialEditingSolution={antispamConfigEditingSolution}
          isCommunity={isCommunity}
          onClose={() => {
      ...
      {antispamPickerOpen && client?.id && (
        <AntispamSolutionPickerModal
          open={antispamPickerOpen}
          client={client}
          solutions={antispamPickerSolutions}
          onClose={() => {
          ...
          onDeleteSolution={async (solution) => {
            ...
            const { modulesData, tenants } = await refreshAntispamState();
            toast.success(copy.toast.antispamAssociationRemoved);
            const remaining = listConfiguredAntispamSolutions(
              client,
              [],
              modulesData,
              tenants
          ...
          onReorderSolutions={async (orderedSolutions) => {
            ...
            const { modulesData, tenants } = await refreshAntispamState();
            setAntispamPickerSolutions(
              listConfiguredAntispamSolutions(client, [], modulesData, tenants)
            );
          }}
        />
      )}

      {backupConfigModalOpen && client?.id && createPortal(
        <>
          <div
            className={styles.modalOverlay}
            onClick={() => setBackupConfigModalOpen(false)}
            aria-hidden
          />
          <MonitoringClientModal
            initialClient={{
              ...client,
              modules: client.modules || { Monitoring: true },
              modules_monitoring: {
      ...
      <CustomEquipmentModal
        isOpen={Boolean(customEquipmentModal?.family)}
        onClose={() => setCustomEquipmentModal(null)}
        family={customEquipmentModal?.family}
        item={customEquipmentModal?.item}
        client={client}
        clientId={client?.id}
        onRefresh={async () => {
      ...
      {/* Modal Détails Noms de domaine */}
      {serviceDetailsModalOpen && (
        <div className={styles.modalOverlay} onClick={() => setServiceDetailsModalOpen(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>
                <Icon icon="mdi:web" className={styles.modalIcon} />
                {interpolate(copy.modals.domainsTitle, { count: domainsData.length })}
              </h2>
              <SmartTooltip
                as="button"
                className={styles.modalCloseButton}
            ...
            <div className={styles.modalBody}>
              {domainsData && domainsData.length > 0 ? (
                <div className={styles.domainsTableSection}>
                  <div className={styles.dataTableWrapper}>
                    <table className={styles.dataTable}>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {domainsData.map((domain, index) => {
                          const isExpired = domain.expirityDate || domain.expiration ?
                            new Date(domain.expirityDate || domain.expiration) < new Date() :
                            false;
                          ...
                          return (
                            <tr key={index}>
                              ...
                              <td>
                                {(domain.expirityDate || domain.expiration) ?
                                  new Date(domain.expirityDate || domain.expiration).toLocaleDateString('fr-FR') :
                                  'N/A'
                                }
                              </td>
                              <td>
      ...
      {/* Modal Détails Utilisateurs Cybersécurité */}
      {userDetailsModalOpen && selectedSecurityItem && (
        <div className={styles.modalOverlay} onClick={() => setUserDetailsModalOpen(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>
                ...
                {interpolate(copy.modals.securityUsersTitle, {
                  name:
                    selectedSecurityItem.nom ||
                    selectedSecurityItem.name ||
                    selectedSecurityItem.solution ||
                    "N/A",
                })}
              </h2>
              <SmartTooltip
            ...
            <div className={styles.modalBody}>
              <div className={styles.securityDetailsGrid}>
                ...
                <div className={styles.securityDetail}>
                  ...
                  <span className={styles.detailValue}>
                    {(selectedSecurityItem.expirityDate || selectedSecurityItem.expiration) ?
                      new Date(selectedSecurityItem.expirityDate || selectedSecurityItem.expiration).toLocaleDateString('fr-FR') :
                      'N/A'
                    }
                  </span>
                </div>
              ...
              <div className={styles.usersTableSection}>
                <h3 className={styles.sectionTitle}>
                  ...
                  {interpolate(copy.modals.usersTitle, {
                    count:
                      selectedSecurityItem.utilisateurs ||
                      selectedSecurityItem.nombre_utilisateurs ||
                      0,
                  })}
                </h3>

                {selectedSecurityItem.utilisateurs_details && selectedSecurityItem.utilisateurs_details.length > 0 ? (
                  <div className={styles.dataTableWrapper}>
                    <table className={styles.dataTable}>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {selectedSecurityItem.utilisateurs_details.map((user, userIndex) => (
                          <tr key={userIndex}>
                            <td>{user.nom || user.name || user.username || 'N/A'}</td>
                            <td>{user.email || user.mail || 'N/A'}</td>
                            <td>
                              {user.derniere_activite || user.last_activity ?
                                new Date(user.derniere_activite || user.last_activity).toLocaleDateString('fr-FR') :
                                'N/A'
                              }
                            </td>
                            <td>
      ...
      {logDetailsModalOpen && selectedLog && (
        <div className={styles.modalOverlay} onClick={() => setLogDetailsModalOpen(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>
      ...
      {/* Modal Détails Antivirus */}
      {antivirusDetailsModalOpen && antivirusDetailData && (
        <div className={styles.modalOverlay} onClick={() => setAntivirusDetailsModalOpen(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()} style={{ maxWidth: '900px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>
                ...
                {interpolate(copy.modals.antivirusTitle, {
                  name:
                    antivirusDetailData.item.nom ||
                    antivirusDetailData.item.name ||
                    antivirusDetailData.item.solution ||
                    "N/A",
                })}
              </h2>
              <SmartTooltip
                as="button"
                className={styles.modalCloseButton}
                onClick={() => setAntivirusDetailsModalOpen(false)}
                content={copy.close}
              >
                <FaTimes />
              </SmartTooltip>
            ...
            <div className={styles.modalBody}>
              {loadingAntivirusDetails ? (
                <div style={{ padding: '2rem', textAlign: 'center', color: '#6b7280' }}>
                  <Icon icon="mdi:loading" style={{ fontSize: '2rem', animation: 'spin 1s linear infinite' }} />
                ...
                <>
                  ...
                  {/* Endpoints */}
                  {antivirusDetailData.endpoints && antivirusDetailData.endpoints.length > 0 && (
                    <div className={styles.usersTableSection} style={{ marginTop: '2rem' }}>
                      <h3 className={styles.sectionTitle}>
                      ...
                      <div className={styles.dataTableWrapper}>
                        <table className={styles.dataTable}>
                          ...
                          <tbody>
                            {antivirusDetailData.endpoints.slice(0, 20).map((endpoint, idx) => (
                              <tr key={idx}>
                                ...
                                <td>
                                  {endpoint.lastUpdate
                                    ? new Date(endpoint.lastUpdate).toLocaleDateString('fr-FR')
                                    : 'N/A'
                                  }
                                </td>
                              </tr>
                      ...
                      {antivirusDetailData.endpoints.length > 20 && (
                  ...
                  {/* Pas d'endpoints */}
                  {(!antivirusDetailData.endpoints || antivirusDetailData.endpoints.length === 0) && (
                    <div className={styles.noUsersMessage} style={{ marginTop: '2rem' }}>

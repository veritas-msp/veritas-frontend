...
import {
  buildInfraBrickGroups,
  buildCustomFamilyBricks,
  buildCustomFamilyHoneycombSlots,
  collectHoneycombSlotsFromItems,
  computeHoneycombClusterMetrics,
  EMPTY_HONEYCOMB_LAYOUT,
  INFRA_BRICK_GROUPS,
  honeycombOffsetRem,
  isHoneycombFeatured,
} from "./infraHoneycombLayout";
import styles from "./InfrastructureMap.module.css";

function InfraHexNode({ node, onClick, copy }) {
  ...
  const tooltipLines = [
    ...
    statusTooltip,
    node.subtitle,
    node.count > 0 ? copy.formatEquipmentCount(node.count) : null,
  ].filter(Boolean);
  ...
  return (
    <SmartTooltip content={tooltipLines.join(" · ")} as="span">
      <button
        ...
        className={[
          styles.hexNode,
          isCustomFamily ? styles.hexNodeCustom : "",
          isCritical ? styles.hexNodeCritical : "",
          isWarning ? styles.hexNodeWarning : "",
          isOk ? styles.hexNodeOk : "",
          isUnmonitored ? styles.hexNodeUnmonitored : "",
        ].filter(Boolean).join(" ")}
        style={{
        ...
        aria-label={`${node.displayName || node.name}${statusTooltip ? `, ${statusTooltip}` : ""}${node.count > 0 ? `, ${copy.formatEquipmentCount(node.count)}` : ""}`}
      >
...
function HoneycombCluster({ items, clusterMetrics }) {
  ...
  return (
    <div
      className={styles.hexClusterViewport}
      style={{
    ...
    >
      ...
      {items.map((item) => {
        ...
        const { x, y } = honeycombOffsetRem(slot.q ?? 0, slot.r ?? 0, metrics.layoutScale);

        return (
          <div
            key={item.key}
            className={`${styles.hexSlot} ${featured ? styles.hexSlotFeatured : ""}`}
            style={{
              "--hex-x": `${x}rem`,
              "--hex-y": `${y}rem`,
            }}
...
function InfraBrickColumn({ bricks, placeholder = false, onBrickClick, ariaLabel, className, isCommunity = false, copy }) {
  return (
...
function InfraMapCanvas({
  ...
  const groups = customFamilyBricks.length > 0
    ? [
        ...brickGroups,
        {
          id: "custom",
  ...
  return (
    <div className={`${styles.mapCard} ${honeycombEmpty ? styles.mapCardEmpty : ""}`}>
...
function InfraPlaceholderHex({ type, featured = false, icon, label, copy }) {
  ...
  return (
    <div
      className={`${styles.hexNode} ${styles.hexNodePlaceholder} ${featured ? styles.hexNodeFeatured : ""}`}
      style={{
        "--hex-accent": meta.color,
        "--hex-soft": meta.soft,
      }}
  ...
  return (
    <div className={styles.map}>
      ...
      <InfraMapCanvas
        ...
        items={EMPTY_HONEYCOMB_LAYOUT.map((slot) => ({
          key: slot.type,
          type: slot.type,
          node: (
            <InfraPlaceholderHex type={slot.type} featured={slot.featured} copy={copy} />
          ),
        }))}
        antivirusItems={antivirusItems}
        antispamItems={antispamItems}
        domainItems={domainItems}
        domainIntegrationReady={domainIntegrationReady}
        sslItems={sslItems}
        licenceItems={licenceItems}
...
function InfraMapLegend({ copy }) {
  return (
    <div className={styles.legend} aria-label={copy.legendAria}>
...
function InfraMapSkeleton({ copy }) {
  return (
    <div className={styles.skeleton} aria-busy="true" aria-label={copy.loadingAria}>
        ...
        <div className={styles.mapLayout}>
          <div className={styles.mapHoneycomb}>
            ...
            {EMPTY_HONEYCOMB_LAYOUT.map((slot) => {
              const { x, y } = honeycombOffsetRem(slot.q, slot.r);
              return (
                <div
                  key={slot.type}
                  className={`${styles.hexSlot} ${slot.featured ? styles.hexSlotFeatured : ""}`}
                  style={{ "--hex-x": `${x}rem`, "--hex-y": `${y}rem` }}
                >
                ...
                <div key={group.id} className={styles.mapModuleGroup}>
                  ...
                  <div className={styles.brickRow}>
                    {group.types.map((type) => (
                      <div key={type} className={styles.skeletonBrick} />
                    ))}
                  </div>
                </div>
              ))}
...
export default function InfrastructureMap({
  ...
  const equipementsCount = useMemo(() => {
    ...
    return Object.values(equipements).reduce(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const [clientEquipment, summaryData] = await Promise.all([
          getClientHardwareEquipment(clientId, {
            client: clientSnapshotRef.current,
            signal: controller.signal,
          }),
          checkmkIntegrationEnabled
            ? getEquipmentMonitoringSummaries({ clientId }, { signal: controller.signal }).catch(() => ({ summaries: {} }))
  ...
  const localizeCategoryNode = useCallback(
    (type, nodes) => {
  ...
  const filteredEquipment = useMemo(
    () => filterBySite(equipment, siteFilter),
    [equipment, siteFilter]
  );
  const filteredBackupInstances = useMemo(
    () => filterBySite(backupInstances, siteFilter),
    [backupInstances, siteFilter]
  );
  const filteredAntivirusItems = useMemo(
    () => filterBySite(antivirusItems, siteFilter),
    [antivirusItems, siteFilter]
  );
  const filteredAntispamItems = useMemo(
    () => filterBySite(antispamItems, siteFilter),
    [antispamItems, siteFilter]
  );
  const filteredDomainItems = useMemo(
    () => filterBySite(domainItems, siteFilter),
    [domainItems, siteFilter]
  );
  const filteredSslItems = useMemo(
    () => filterBySite(sslItems, siteFilter),
    [siteFilter, sslItems]
  );
  const filteredLicenceItems = useMemo(
    () => filterBySite(licenceItems, siteFilter),
    [licenceItems, siteFilter]
  );
  const filteredCampaignItems = useMemo(
    () => filterBySite(campaignItems, siteFilter),
    [campaignItems, siteFilter]
  );
  const filteredCustomFamilyMap = useMemo(
    () => filterCustomFamilyMap(customFamilyMap, siteFilter),
    [customFamilyMap, siteFilter]
  );
  ...
  const honeycombItems = useMemo(() => {
    ...
    const baseItems = EMPTY_HONEYCOMB_LAYOUT.map((slot) => {
      ...
      return {
        ...
        slot,
        node: hasData ? (
          <InfraHexNode node={categoryNode} onClick={onNodeClick} copy={copy} />
        ) : (
          <InfraPlaceholderHex type={slot.type} featured={slot.featured} copy={copy} />
        ),
      };
    ...
    const customItems = buildCustomFamilyHoneycombSlots(customFamilyMap).map(({ family, slot }) => {
      const filteredFamily = filteredCustomFamilyMap.find(
  ...
  const customFamilyBricks = useMemo(
    () => buildCustomFamilyBricks(filteredCustomFamilyMap),
    [filteredCustomFamilyMap]
  );
  ...
  const categorySummary = useMemo(() => {
    ...
    const categories = EMPTY_HONEYCOMB_LAYOUT.map((slot) =>
      localizeCategoryNode(slot.type, nodesByType[slot.type] || [])
    );
  ...
  const mapCanvasProps = {
    ...
    licenceItems: filteredLicenceItems,
  ...
  return (
    <div className={styles.map}>
      {(categorySummary.critical > 0 || categorySummary.warning > 0) && (
        <div className={styles.mapHeader}>
          <div className={styles.mapSummary}>
            {categorySummary.critical > 0 && (
              <span className={`${styles.mapBadge} ${styles.mapBadgeCritical}`}>
                {copy.formatCriticalCategories(categorySummary.critical)}
              </span>
            )}
            {categorySummary.warning > 0 && (
              <span className={`${styles.mapBadge} ${styles.mapBadgeWarning}`}>

export const SSL_STATUS_ORDER = { error: 0, expired: 1, warning: 2, unknown: 3, active: 4 };

export function formatSslDate(value, locale = "fr-FR") {
  if (!value) return "-";
  try {
...
export function formatSslDateTime(value, locale = "fr-FR") {
  if (!value) return "-";
  try {
...
export function getSslCertStatus(cert, statusLabels = DEFAULT_SSL_STATUS_LABELS) {
  if (cert?.error) {
    return { key: "error", text: statusLabels.error, icon: "mdi:alert-circle-outline" };
  }
  if (!cert?.expiration) {
    return { key: "unknown", text: statusLabels.unknown, icon: "mdi:help-circle-outline" };
  }
  const expiry = new Date(cert.expiration);
  if (Number.isNaN(expiry.getTime())) {
    return { key: "unknown", text: statusLabels.unknownInvalid, icon: "mdi:help-circle-outline" };
  }
  const now = new Date();
  if (expiry < now) {
    return { key: "expired", text: statusLabels.expired, icon: "mdi:close-circle-outline" };
  }
  const warnDate = new Date(now);
  warnDate.setDate(warnDate.getDate() + 30);
  if (expiry <= warnDate) {
    return { key: "warning", text: statusLabels.warning, icon: "mdi:clock-alert-outline" };
  }
  return { key: "active", text: statusLabels.active, icon: "mdi:check-circle-outline" };
}

export function computeSslStats(certificates) {
  const stats = { total: certificates.length, active: 0, warning: 0, problem: 0, unknown: 0 };
  for (const cert of certificates) {
    const status = getSslCertStatus(cert);
    if (status.key === "active") stats.active += 1;
    else if (status.key === "warning") stats.warning += 1;
    else if (status.key === "error" || status.key === "expired") stats.problem += 1;

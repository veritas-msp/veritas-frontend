...
export default function ClientNoteModal({
  ...
  const subtitle = clientName
    ? interpolate(modalCopy.subtitle, { name: clientName })
    : modalCopy.subtitleFallback;

  return createPortal(
    <div className={formStyles.overlay} onClick={onClose} role="presentation">
      <div
        className={`${formStyles.shell} ${styles.shellNote}`}
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-labelledby="client-note-modal-title"
      >
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          <div className={`${formStyles.content} ${styles.modalContent}`}>
            <div className={formStyles.field}>
              <label
                className={`${formStyles.label} ${formStyles.labelRequired}`}
                htmlFor="client-note-content"
              >
                {modalCopy.label}
              </label>
              <textarea
          ...
          <footer className={formStyles.footer}>
            ...
            >
              {saving ? (
                ...
                <>
                  <Icon
                    icon={isEdit ? "mdi:content-save-outline" : "mdi:note-plus-outline"}
                    aria-hidden
                  />

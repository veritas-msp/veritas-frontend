/** Dimensions de base (rem) · gardées en sync avec InfrastructureMap.module.css */

export const HONEYCOMB_HEX = {
  w: 6.25,
  h: 7.2,
  /** Marge supplémentaire entre les centres (au-delà de l'emboîtement serré), en rem. */
  spacing: 1,
};

export function honeycombOffsetRem(q, r, layoutScale = 1) {
  const { w, h, spacing } = HONEYCOMB_HEX;
...
export function buildCustomFamilyHoneycombSlots(families = []) {
  const hexFamilies = (Array.isArray(families) ? families : []).filter(
...
export function collectHoneycombSlotsFromItems(items = []) {
  return items
    .map((item) => item.slot || EMPTY_HONEYCOMB_LAYOUT.find((entry) => entry.type === item.type))
    .filter(Boolean);
}
...
export function computeHoneycombClusterMetrics(slots = [], options = {}) {
  ...
  let layoutScale = 1;
  if (count > 16) layoutScale = 0.76;
  else if (count > 13) layoutScale = 0.84;
  else if (count > 11) layoutScale = 0.92;

  const { w, h } = HONEYCOMB_HEX;
  ...
  slots.forEach((slot) => {
    const { x, y } = honeycombOffsetRem(slot.q ?? 0, slot.r ?? 0, layoutScale);
...
/** Ordre unifié des familles matériel (ajout, filtres périphériques, listes). */
export const HARDWARE_TYPE_ORDER = [
  "Internet",
  "Firewalls",
  "Routeur",
  "Serveurs",
...
/** Disposition : centre Internet, anneau réseau, anneau externe (serveurs, alim., TOIP, postes). */
/** Ordre d'affichage des tables périphériques aligné sur la cartographie hexagonale */
export const HONEYCOMB_TABLE_TYPE_ORDER = HARDWARE_TYPE_ORDER.filter(
  (type) => type !== "Ordinateurs"
);

export const EMPTY_HONEYCOMB_LAYOUT = [
  { type: "Internet", q: 0, r: 0 },
  { type: "Switch", q: -1, r: 0, featured: true },
  { type: "Firewalls", q: 0, r: -1 },
  { type: "Routeur", q: 1, r: -1 },
  { type: "Serveurs", q: 2, r: -1 },
  { type: "BorneWifi", q: 1, r: 0 },
  { type: "Stockage", q: 0, r: 1 },
  { type: "Ordinateurs", q: -1, r: 1 },
  { type: "Alimentation", q: -2, r: 0 },
  { type: "TOIP", q: 2, r: 0 },
];
...
export const INFRA_BRICK_GROUPS = [
  ...
  },
  {
    id: "services",
    label: "Services",
    types: ["TenantMicrosoft", "GoogleWorkspace", "Sauvegarde"],
  },
  {
    id: "licensing",
    label: "Licences & abonnements",
    types: ["NDD", "CertificatsSSL", "LicencesAbonnements"],
  },
  {
    id: "campaign",
    label: "Campagne",
...
const BRICK_TYPE_META = {
  Antivirus: { id: "antivirus", label: "Antivirus" },
  Antispam: { id: "antispam", label: "Antispam" },
  Sauvegarde: { id: "sauvegarde", label: "Sauvegarde" },
  TenantMicrosoft: { id: "tenant", label: "Tenant Microsoft" },
  GoogleWorkspace: { id: "google-workspace", label: "Google Workspace" },
  NDD: { id: "ndd", label: "Nom de domaine" },
  CertificatsSSL: { id: "ssl", label: "Certificats SSL" },
  LicencesAbonnements: { id: "licences", label: "Licences & abonnements" },
  Campagne: { id: "campagne", label: "Campagne" },
};

const COMING_SOON_BRICK_TYPES = new Set([
  "Sauvegarde",
  "GoogleWorkspace",
  "Campagne",
]);

const PRO_ONLY_BRICK_TYPES = new Set([
  "TenantMicrosoft",
]);

/** Clé promo Veritas Pro / bientôt disponible par type de brique infra */
...
/** Briques accessibles à toutes les éditions, cliquables même sans donnée */
const ALWAYS_CLICKABLE_BRICK_TYPES = new Set([
  "Antivirus",
  "Antispam",
  "NDD",
  "CertificatsSSL",
...
function applyBrickAccessFlags(brick) {
  ...
  if (COMING_SOON_BRICK_TYPES.has(brick.type)) {
    next = { ...next, comingSoon: true, alwaysClickable: true };
  }
  if (ALWAYS_CLICKABLE_BRICK_TYPES.has(brick.type)) {
    next = { ...next, alwaysClickable: true };
  }
  if (PRO_ONLY_BRICK_TYPES.has(brick.type)) {
    next = { ...next, proOnly: true, alwaysClickable: true };
  }
  return next;
}

/** Liste plate des types de briques (ordre d'affichage) */
export const INFRA_BRICK_TYPES = INFRA_BRICK_GROUPS.flatMap((group) =>
...
function buildSslBrick(items = [], label = "Certificats SSL") {
  ...
  if (list.length > 0) {
    ...
    else if (statuses.includes("ok")) status = "ok";
    else status = "neutral";
  }
...
function buildDomainBrick(items = [], { integrationReady = false, label = "Nom de domaine" } = {}) {
  const list = Array.isArray(items) ? items : [];
...
function buildLicencesBrick(items = [], label = "Licences & abonnements") {
  const list = Array.isArray(items) ? items : [];
...
function buildTenantBrick(tenantInfo = {}, label = "Tenant Microsoft") {
  ...
  let status = "unmonitored";
  if (configured) {
    status = tenantInfo.status === "inactif" ? "neutral" : "ok";
  }
...
function buildGoogleWorkspaceBrick(workspaceInfo = {}, label = "Google Workspace") {
  ...
  let status = "unmonitored";
  if (configured) {
    status = workspaceInfo.status === "inactif" ? "neutral" : "ok";
  }
...
function buildSauvegardeBrick(instances = [], label = "Sauvegarde") {
  const list = Array.isArray(instances) ? instances : [];

  let status = "unmonitored";
  if (list.length > 0) {
    const hasUnmapped = list.some(
      (instance) => instance.jobsCount > 0 && instance.mappedJobsCount === 0
    );
    const hasMapped = list.some(
      (instance) => instance.jobsCount > 0 && instance.mappedJobsCount > 0
    );
    if (hasUnmapped && !hasMapped) status = "unmonitored";
    else if (hasMapped) status = "ok";
    else status = "neutral";
  }
...
function buildCampaignBrick(items = [], label = "Campagne") {
  ...
  if (list.length > 0) {
    const hasActive = list.some(
      (campaign) => campaign.status === "en_cours" || campaign.status === "active"
    );
...
function buildAllInfraBricks({
  ...
  if (empty) {
    return INFRA_BRICK_TYPES.map(({ id, type, label }) =>
  ...
  return [
    buildSecurityBrick("Antivirus", getBrickTypeLabel("Antivirus", "Antivirus"), antivirusItems),
    buildSecurityBrick("Antispam", getBrickTypeLabel("Antispam", "Antispam"), antispamItems),
    buildSauvegardeBrick(backupInstances, getBrickTypeLabel("Sauvegarde", "Sauvegarde")),
    buildTenantBrick(tenantInfo, getBrickTypeLabel("TenantMicrosoft", "Tenant Microsoft")),
    buildGoogleWorkspaceBrick(
      googleWorkspaceInfo,
      getBrickTypeLabel("GoogleWorkspace", "Google Workspace")
    ),
    buildDomainBrick(domainItems, {
      integrationReady: domainIntegrationReady,
      label: getBrickTypeLabel("NDD", "Nom de domaine"),
    }),
    buildSslBrick(sslItems, getBrickTypeLabel("CertificatsSSL", "Certificats SSL")),
    buildLicencesBrick(
      licenceItems,
      getBrickTypeLabel("LicencesAbonnements", "Licences & abonnements")
    ),
    buildCampaignBrick(campaignItems, getBrickTypeLabel("Campagne", "Campagne")),
  ].map(applyBrickAccessFlags);
}

export function buildInfraBrickGroups(options = {}) {
  const { getBrickGroupLabel, getBrickTypeLabel, ...brickOptions } = options;
  ...
  const bricksByType = Object.fromEntries(
    buildAllInfraBricks({
      ...brickOptions,
...
/** @deprecated Utiliser buildInfraBrickGroups */
export function buildInfraBricks(options = {}) {
  return buildAllInfraBricks(options);
}

/** @deprecated Utiliser INFRA_BRICK_TYPES */

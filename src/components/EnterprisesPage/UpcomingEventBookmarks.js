...
function getRelativeLabel(startDate, locale, labels) {
  ...
  if (diffDays > 1 && diffDays < 7) {
    return startDate.toLocaleDateString(getLocaleTag(locale), { weekday: "long" });
  }
  return null;
}
...
function getPastRelativeLabel(endDate, locale, labels) {
  ...
  if (diffDays === 0) return labels.today;
  if (diffDays > 1 && diffDays < 7) {
    return interpolate(labels.daysAgo, { count: String(diffDays) });
  }
  return null;
}
...
function formatBookmarkRibbon(start, locale) {
  ...
  return {
    day: d.toLocaleDateString(localeTag, { day: "2-digit" }),
    month: d.toLocaleDateString(localeTag, { month: "short" }).replace(".", "").toUpperCase(),
...
function formatEventTimeLine(start, end, locale) {
  ...
  const dateFmt = { day: "2-digit", month: "2-digit" };
...
function formatExpandedDateRange(startRaw, endRaw, locale) {
  ...
  if (Number.isNaN(s.getTime())) return "";
  const withWeekday = { weekday: "short", day: "2-digit", month: "short" };
  ...
  const startFmt = { day: "2-digit", month: "short" };
  const endFmt =
...
function getEventDurationLabel(startRaw, endRaw, labels) {
  ...
  return interpolate(labels.durationDays, { count: String(diffDays) });
}
...
function EventActionMenu({
  ...
  return createPortal(
    <div
      ref={menuRef}
      className={styles.eventActionMenu}
      style={{ top: position.top, left: position.left }}
      role="menu"
      aria-label={event?.title || labels.editEvent}
    >
      <button
        type="button"
        className={styles.eventActionMenuItem}
        role="menuitem"
        onClick={() => {
          onClose();
          onEdit?.(event);
...
function EventBookmark({
  ...
  const anchorDate = isRecent
    ? event.event_end ?? event["end"] ?? event.end ?? event.start
    : event.start;
  ...
  const relative = isRecent
    ? getPastRelativeLabel(new Date(endRaw), locale, labels)
    : getRelativeLabel(start, locale, labels);
  ...
  const { display: titleDisplay, full: titleFull, truncated: titleTruncated } = truncateEventTitle(eventTitle);
  ...
  const assigneeLabel =
    assigneeNames.length > 0
      ? interpolate(labels.assignedAgent, { name: assigneeNames.join(", ") })
      : null;
  const linkedTicketLabel = (() => {
    const ticketNumber = resolveLinkedTicketNumber(event);
    if (!ticketNumber) return null;
    return interpolate(labels.linkedTicket, { number: ticketNumber });
  })();
  const sameDay =
    !Number.isNaN(start.getTime()) &&
    !Number.isNaN(new Date(endRaw).getTime()) &&
    start.toDateString() === new Date(endRaw).toDateString();
  ...
  const expandedDetailItems = [
    showExpandedDateRange
      ? { icon: "mdi:calendar-range", text: expandedDateRange, key: "dates" }
      : null,
    scheduleDetail
      ? { icon: "mdi:clock-outline", text: scheduleDetail, key: "schedule" }
      : null,
    assigneeLabel
      ? { icon: "mdi:account-hard-hat", text: assigneeLabel, key: "agent" }
      : null,
    linkedTicketLabel
      ? { icon: "mdi:ticket-outline", text: linkedTicketLabel, key: "ticket" }
      : null,
  ].filter(Boolean).slice(0, 2);

  const titleNode = (
    <span className={cardStyles.eventBookmarkTitle} title={titleTruncated ? undefined : titleFull}>
      {titleDisplay}
    </span>
  );
  ...
  return (
    <button
      ref={buttonRef}
      type="button"
      className={`${styles.bookmark} ${cardStyles.eventBookmark} ${isRecent ? styles.bookmarkPast : ""} ${menuOpen ? `${styles.bookmarkMenuOpen} ${cardStyles.eventBookmarkMenuOpen}` : ""}`}
      style={{ "--bookmark-accent": accent }}
      onClick={() => onClick?.(event)}
      aria-expanded={menuOpen}
      aria-haspopup="menu"
      aria-label={interpolate(labels.formatEventAria, {
        title: eventTitle,
        when: whenLabel,
        type: typeLabel,
      })}
        ...
        <span className={cardStyles.eventBookmarkCompact} aria-hidden>
          ...
          <span className={cardStyles.eventBookmarkCompactMid}>
            <span className={cardStyles.eventBookmarkCompactRelativeSlot}>
              {relative ? (
                <span className={cardStyles.eventBookmarkCompactRelative}>{relative}</span>
              ) : null}
            </span>
          ...
          <span className={cardStyles.eventBookmarkCompactTimeSlot}>
            {compactPeriod?.kind === "range" ? (
              <span className={cardStyles.eventBookmarkCompactPeriod}>
              ...
              <span className={cardStyles.eventBookmarkCompactTime}>{compactPeriod.label}</span>
            ) : null}
          </span>
        </span>
        ...
        <span className={cardStyles.eventBookmarkExpanded}>
          <span className={cardStyles.eventBookmarkTop}>
            <span className={cardStyles.eventBookmarkTopMain}>
              <span
                className={`${styles.bookmarkRibbon} ${cardStyles.eventBookmarkRibbonExpanded}`}
                aria-hidden
              >
          ...
          {titleTruncated ? (
            <SmartTooltip content={titleFull} className={cardStyles.eventBookmarkTitleWrap}>
              {titleNode}
            </SmartTooltip>
          ) : (
            <span className={cardStyles.eventBookmarkTitleWrap}>{titleNode}</span>
          )}
          <span className={cardStyles.eventBookmarkDetails}>
            {expandedDetailItems.map((item) => (
              <span key={item.key} className={cardStyles.eventBookmarkDetailRow}>
...
function BookmarkSkeleton() {
  return (
    <div className={`${styles.bookmarkSkeleton} ${cardStyles.eventBookmarkSkeleton}`} aria-hidden>
...
function SectionLabel({ icon, title, count, variant = "upcoming" }) {
  return (
    <div
      className={`${styles.sectionLabel} ${variant === "recent" ? styles.sectionLabelRecent : ""}`}
      aria-hidden
    >
...
export default function UpcomingEventBookmarks({
  ...
  locale: localeProp,
  proFeatureLabel = "Planning entreprise",
  ...
  const labels = useMemo(
    () => ({ ...DEFAULT_EVENT_BOOKMARK_LABELS, ...labelsProp }),
    [labelsProp]
  );
  ...
  const handleBookmarkClick = useCallback((event) => {
    ...
    setActionMenu((current) => (
      current?.event?.id === event.id ? null : { event, anchorEl }
    ));
  ...
  const recent = proLocked
    ? []
    : Array.isArray(recentEvents)
      ? recentEvents
      : [];
  const upcoming = proLocked
    ? []
  ...
  const { collapsed, toggle } = useHeroBookmarkCollapse(collapseStorageKey, defaultCollapsed);
  ...
  const bar = (
    <div
      className={[
        ...
        proLocked && !inPageHero ? styles.barInProPanel : "",
      ]
        .filter(Boolean)
        .join(" ")}
    ...
    >
      <div className={styles.barInner}>
          ...
          <div className={`${styles.track} ${cardStyles.eventBookmarksTrack}`} ref={trackRef}>
            {showLoading ? (
              ...
              <>
                <SectionLabel
                  icon="mdi:history"
                  title={labels.sectionRecent}
                  count={proLocked ? null : recent.length}
                  variant="recent"
                />
                {recent.length === 0 ? (
                  <span className={styles.sectionEmptyHint}>{labels.noneRecent30Days}</span>
                ) : (
                  recent.map((event) => (
                    <EventBookmark
                      key={`recent-${event.id}`}
                      ...
                      users={users}
                      mode="recent"
                      menuOpen={actionMenu?.event?.id === event.id}
                      buttonRef={(node) => registerBookmarkRef(event.id, node)}
                ...
                <SectionLabel
                  ...
                  title={labels.sectionUpcoming}
                  count={proLocked ? null : upcoming.length}
                />
                {upcoming.length === 0 ? (
                  <span className={styles.sectionEmptyHint}>{labels.noneUpcoming}</span>
                ) : (
                  upcoming.map((event) => (
                    <EventBookmark
        ...
        <div className={styles.barActions}>
          <SmartTooltip
            content={proLocked ? labels.openPlanningProTooltip : labels.openPlanning}
          >
            <button
              type="button"
              className={styles.barActionBtn}
              onClick={handleOpenPlanning}
              aria-label={proLocked ? labels.openPlanningPro : labels.openPlanningAria}
            >
              <FaCalendarAlt aria-hidden />
            </button>
          </SmartTooltip>
          <SmartTooltip
            content={proLocked ? labels.createEventProTooltip : labels.createEvent}
          >
            <button
              type="button"
              className={`${styles.barActionBtn} ${styles.barActionBtnPrimary}`}
              onClick={handleAddEvent}
              aria-label={proLocked ? labels.createEventPro : labels.createEventAria}
            >
              <FaPlus aria-hidden />
            </button>
          </SmartTooltip>
      ...
      {inPageHero ? (
        ...
        >
          <HeroBookmarkCollapseHeader
            ...
            expandAria={labels.expandPanelAria}
            collapseAria={labels.collapsePanelAria}
          />
        ...
        <div className={proLocked ? styles.barProPanel : undefined}>
          {proLocked ? (
            <div className={styles.barProHeader}>

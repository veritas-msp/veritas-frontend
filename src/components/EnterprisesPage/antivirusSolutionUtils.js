...
export function formatAntivirusSolutionLabel(solution) {
  ...
  if (!normalized) return "Solution antivirus";
  return (
...
export function formatAntivirusSolutionSummary(solution) {
  ...
  const providerId =
    normalized?.providerId || (normalized?.companyId ? "bitdefender" : "manual");
  const providerName =
    providerId === "bitdefender"
      ...
      : providerId === "manual"
        ? "Autre solution"
        : providerId;
  return { label, mode, providerName, providerId };
}
...
export function normalizeAntivirusItem(item) {
  if (!item) return null;
  const companyId =
    item.companyId || item.company_id || item.syncData?.company?.id || null;
  ...
  const providerId =
    item.providerId ||
    (companyId ? "bitdefender" : hasManualHints ? "manual" : inferProviderIdFromSolution(item));
  const isManualEntry =
    hasManualHints || (!companyId && providerId === "manual");
...
export function isAntivirusConfigured(item) {
  ...
  const hasLicenseMeta =
    String(normalized.licencesTotales ?? "").trim() ||
    String(normalized.licencesUtilisees ?? "").trim() ||
...
export function listConfiguredAntivirusSolutions(client, antivirusItems = [], modulesData = null) {
  const sources = [
    ...extractAntivirusSolutionsFromModules(modulesData || { equipements: client?.equipements }),
...
export function resolveAntivirusItemForBrick(brick, antivirusItems = [], client, modulesData = null) {
  const configured = listConfiguredAntivirusSolutions(client, [
    ...(brick?.items || []),
    ...(antivirusItems || []),
  ], modulesData);
...
function solutionMatches(a, b) {
  if (a.companyId && b.companyId) {
    return (
      ...
      (a.bitdefenderTenantId || null) === (b.bitdefenderTenantId || null)
    );
  }
...
export async function removeAntivirusSolution(clientId, solution) {
  ...
  if (!clientId || (!normalized?.companyId && !solution?.id && !solution?.solution)) {
    throw new Error("Association antivirus introuvable.");
  }
...
export async function fetchFullAntivirusSyncExtra(companyId, credentialContext) {
  const [dashboard, statisticsRes, enrichedRes] = await Promise.all([
    fetchGravityZoneDashboard(companyId, credentialContext).catch(() => null),
...
export function formatAntivirusSyncPayload(
  syncData,
  companyId,
  companyName,
  mappingMode,
...
export async function syncAndPersistAntivirusSolution(clientId, solution) {
  ...
  const [syncResult, syncExtra] = await Promise.all([
    syncBitdefenderCompany(companyId, credentialContext),
    fetchFullAntivirusSyncExtra(companyId, credentialContext),
  ]);
  ...
  const companyName =
    syncResult.data?.company?.name ||
    syncExtra.dashboard?.sections?.company?.data?.name ||
    normalized.companyName ||
    normalized.solution ||
    "";

  const updatedPayload = formatAntivirusSyncPayload(
    syncResult.data,
    companyId,
    companyName,
    mappingMode,
    normalized.bitdefenderTenantId,
    providerId,
    syncExtra
  );

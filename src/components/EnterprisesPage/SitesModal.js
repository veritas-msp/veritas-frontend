...
import {
  buildSiteAddress,
  buildSiteGeocodeQuery,
  createEmptySite,
  geocodeSiteAddress,
  getSiteDisplayName,
  getSiteId,
  normalizeClientSites,
  enforceSinglePrimarySite,
  sortClientSites,
  assignClientSitesOrder,
} from "../../utils/clientSites";
import styles from "./SitesModal.module.css";

function SiteEditor({ site, onChange, onLocate, locating, copy }) {
  const patch = (fields) => onChange({ ...site, ...fields });
    ...
    <div className={styles.editorForm}>
      <div className={styles.fieldGrid}>
        ...
        <div className={`${styles.field} ${styles.fieldFull}`}>
          ...
          <input
            ...
            type="text"
            className={styles.input}
            value={site.addressStreet}
            onChange={(e) => patch({ addressStreet: e.target.value, latitude: null, longitude: null })}
        ...
        <div className={styles.field}>
          ...
          <input
            ...
            type="text"
            className={styles.input}
            value={site.addressPostalCode}
            onChange={(e) => patch({ addressPostalCode: e.target.value, latitude: null, longitude: null })}
        ...
        <div className={styles.field}>
          ...
          <input
            ...
            type="text"
            className={styles.input}
            value={site.addressCity}
            onChange={(e) => patch({ addressCity: e.target.value, latitude: null, longitude: null })}
        ...
        <div className={styles.field}>
          ...
          <input
            ...
            type="text"
            className={styles.input}
            value={site.addressCountry}
            onChange={(e) => patch({ addressCountry: e.target.value, latitude: null, longitude: null })}
          ...
          <label className={styles.primaryToggle} htmlFor="site-primary">
            ...
            <span className={styles.switchWrap}>
              <input
                ...
                checked={Boolean(site.isPrimary)}
                onChange={(e) => patch({ isPrimary: e.target.checked })}
                role="switch"
                aria-checked={Boolean(site.isPrimary)}
              />
      ...
      <div className={styles.mapPanel}>
        ...
        <SiteMapPreview
          ...
          onLocate={onLocate}
          locating={locating}
          className={styles.mapPreviewCondensed}
        />
      </div>
    </div>
  );
...
function SiteListCard({ site, isActive, onEdit, dragHandleProps = null, dragHandleDisabled = false, copy }) {
  ...
  return (
    <>
      {dragHandleProps ? (
        <button
          type="button"
          className={`${styles.dragHandle} ${dragHandleDisabled ? styles.dragHandleDisabled : ""}`}
          aria-label={copy.formatReorderAria(getSiteDisplayName(site))}
          title={dragHandleDisabled ? copy.drag.finishEditing : copy.drag.dragToReorder}
        ...
        <span className={`${styles.dragHandle} ${styles.dragHandleDisabled}`} aria-hidden>
          <Icon icon="mdi:drag-vertical" />
        </span>
      )}
      <div
        className={`${styles.siteCard} ${isActive ? styles.siteCardActive : ""}`}
        onClick={() => onEdit?.(site)}
        onKeyDown={(event) => {
          if (!onEdit) return;
...
function SortableSiteRow({ site, isActive, canReorder, onEdit, copy }) {
  ...
  return (
    <li
      ...
      className={[
        styles.siteListItem,
        isDragging ? styles.siteListItemDragging : "",
        isActive ? styles.siteListItemActive : "",
      ]
        .filter(Boolean)
        .join(" ")}
    >
...
export default function SitesModal({ sites = [], onSave, onClose }) {
  const locale = useAppLocale();
  const common = useCommonCopy();
  const copy = useMemo(() => getSitesModalCopy(locale), [locale]);
  const { isCommunity, limits } = useVeritasEdition();
  ...
  const activeDragSite = useMemo(
    () => orderedSites.find((site) => site.id === activeDragId) || null,
    [orderedSites, activeDragId]
  );
  ...
  const handleDraftChange = (updatedDraft) => {
    ...
    // En création, le brouillon ne doit pas compter dans la liste ni la limite Community.
    if (isCreating) {
      if (updatedDraft.isPrimary) {
        setCurrentSites((prev) => prev.map((site) => ({ ...site, isPrimary: false })));
    ...
    setCurrentSites((prev) => {
      ...
      return updatedDraft.isPrimary
        ? enforceSinglePrimarySite(merged, updatedDraft.id)
        : enforceSinglePrimarySite(merged);
    });
  };
  ...
  const upsertDraft = async () => {
    ...
    if (!normalized.latitude && buildSiteGeocodeQuery(normalized)) {
      try {
        ...
        normalized = { ...normalized, ...coords };
      } catch {
        // La carte pourra être actualisée manuellement.
      }
    }
    ...
    if (normalized.isPrimary) {
      setCurrentSites((prev) => {
        const next = isCreating
          ? [...prev.filter((site) => site.id !== normalized.id), normalized]
          : prev.map((site) => (site.id === normalized.id ? normalized : site));
        return assignClientSitesOrder(next);
      });
    } else if (isCreating) {
      setCurrentSites((prev) =>
        assignClientSitesOrder([...prev.filter((site) => site.id !== normalized.id), normalized])
      );
    } else {
      setCurrentSites((prev) =>
        assignClientSitesOrder(
  ...
  const removeSite = (siteId) => {
    setCurrentSites((prev) =>
      assignClientSitesOrder(prev.filter((site) => site.id !== siteId))
    );
  ...
  const handleLocate = async () => {
    ...
    try {
      ...
      handleDraftChange({ ...draft, ...coords });
            ...
            <div className={styles.listPanel}>
              <div className={styles.listHead}>
                ...
                <button
                  ...
                  title={
                    atSiteLimit
                      ? copy.formatLimitTooltip(maxSites)
                      : undefined
                  }
                >
              ...
              {atSiteLimit ? (
                <div className={styles.sitesLimitHint} role="note">
              ...
              {orderedSites.length === 0 ? (
                ...
                >
                  ...
                  <DragOverlay dropAnimation={{ duration: 180, easing: "ease-out" }}>
                    {activeDragSite ? (
                      <div className={styles.siteDragOverlay}>

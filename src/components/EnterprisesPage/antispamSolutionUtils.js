...
function resolveAntispamProviderId(item) {
  ...
  return (
    ...
    inferProviderIdFromSolution(normalized) ||
    (normalized.mailinblackTenantId || normalized.customerId ? "mailinblack" : null) ||
    (normalized.mappingMode === "dedicated" || normalized.mappingMode === "reseller"
      ? "mailinblack"
      : null) ||
    "manual"
  );
...
function resolveAntispamProductName(providerId, provider) {
  ...
  if (providerId === "manual") {
    return provider?.label || "Autre solution";
  }
...
function resolveAntispamTenantLabel(item, productName) {
  ...
  const candidates = [
    ...
    normalized.syncData?.customer?.name,
    normalized.label,
    normalized.nom,
    normalized.name,
    normalized.logiciel,
...
function resolveAntispamProviderImage(providerId, provider) {
  ...
  if (provider?.image) {
    return provider.image.startsWith("/")
      ? provider.image
      : `/assets/icons/${provider.image}`;
  }
  return null;
}
...
export function getAntispamSolutionModeLabel(solution) {
  ...
  const mode =
    normalized?.mappingMode ||
    (normalized?.mailinblackTenantId ? "dedicated" : normalized?.customerId ? "reseller" : "manual");
  if (mode === "dedicated") return "Tenant dédié";
...
export function normalizeAntispamItem(item) {
  if (!item) return null;
  const name =
    item.logiciel || item.solution || item.nom || item.name || item.customerName || "";
  const customerId =
    item.customerId ||
    item.customer_id ||
    item.authClientId ||
    item.syncData?.customer?.id ||
    null;
  const hasManualHints =
    item.mappingMode === "manual" ||
    item.isManual === true ||
    item.providerId === "manual";
  const providerId =
    item.providerId ||
    (item.mailinblackTenantId || customerId
      ? "mailinblack"
      : hasManualHints
        ? "manual"
        : inferProviderIdFromSolution(item));
  const mappingMode =
    item.mappingMode ||
    (item.mailinblackTenantId
      ? "dedicated"
      : customerId
        ? "reseller"
        : providerId === "manual" || hasManualHints
          ? "manual"
          : "manual");
  ...
  return {
    ...
    utilisateursProteges:
      item.utilisateursProteges ?? item.utilisateurs ?? item.nombre_utilisateurs ?? null,
    domainesSurveilles:
      item.domainesSurveilles ?? item.domaines ?? item.licences ?? item.nombre_licences ?? null,
  };
}

export function formatAntispamSolutionLabel(solution) {
  const normalized = normalizeAntispamItem(solution);
  if (!normalized) return "Solution antispam";
  return (
    normalized.customerName ||
    normalized.logiciel ||
    normalized.solution ||
...
export function formatAntispamSolutionSummary(solution) {
  ...
  if (domains != null && domains !== "") {
    metaParts.push(`${domains} domaine${Number(domains) > 1 ? "s" : ""}`);
  }
  return { label, mode, providerName, providerId, meta: metaParts.join(" · ") };
}
...
export function isAntispamConfigured(item) {
  ...
  if (
    normalized.providerId === "mailinblack" &&
    normalized.mappingMode === "dedicated" &&
    normalized.mailinblackTenantId
  ) {
    return true;
  }
  ...
  const hasCoverageMeta =
    String(normalized.utilisateursProteges ?? "").trim() ||
    String(normalized.domainesSurveilles ?? "").trim() ||
    String(normalized.expiration ?? "").trim();

  if (
    normalized.mappingMode === "manual" ||
    normalized.isManual ||
    normalized.providerId === "manual"
  ) {
  ...
  return Boolean(
    label &&
      label !== "N/A" &&
      (normalized.utilisateursProteges != null ||
        normalized.domainesSurveilles != null ||
        Boolean(normalized.expiration))
  );
}
...
export function listOverviewAntispamSolutions(solutions = []) {
  return (solutions || [])
    .map((solution) => normalizeAntispamItem(solution))
    .filter((solution) => isAntispamConfigured(solution));
}
...
export function listConfiguredAntispamSolutions(
  client,
  antispamItems = [],
  modulesData = null,
  mailinblackTenants = []
) {
  const moduleSolutions = extractAntispamSolutionsFromModules(
    modulesData || { equipements: client?.equipements }
  );
  const tenantSolutions = buildSolutionsFromMailinblackTenants(mailinblackTenants);
  const sources = [
    ...moduleSolutions,
    ...tenantSolutions,
    ...(antispamItems || []).map((item) => normalizeAntispamItem(item)).filter(Boolean),
  ];
...
export function mergeAntispamSources(apiItems = [], modulesData) {
  ...
  for (const item of moduleItems) {
    const key =
      ...
      item.name;
    if (!key) continue;
    merged.set(String(key), { ...merged.get(String(key)), ...item });
  }
...
function solutionMatches(a, b) {
  ...
  if (a.customerId && b.customerId) {
    return (
      ...
      (a.mappingMode || "reseller") === (b.mappingMode || "reseller") &&
...
export async function removeAntispamSolution(clientId, solution) {
  ...
  if (
    ...
    throw new Error("Association antispam introuvable.");
  }
  ...
  const remaining = existingSolutions.filter(
    (entry) => !solutionMatches(normalizeAntispamItem(entry) || entry, normalized)
  );
...
export async function reorderAntispamSolutions(clientId, orderedItems = []) {
  ...
  for (const item of orderedItems) {
    ...
    const matchIndex = raw.findIndex(
      (entry, index) => !used.has(index) && solutionMatches(entry, normalized)
    );
    if (matchIndex >= 0) {
      used.add(matchIndex);
...
export async function syncAndPersistAntispamSolution(clientId, solution) {
  ...
  if (!syncResult.success) {
    throw new Error(syncResult.error || "Synchronisation échouée");
  }
  ...
  const finalSolutions = hasMatch
    ? existingSolutions.map((entry) =>
        solutionMatches(entry, normalized)
          ? { ...entry, ...updatedPayload, id: entry.id ?? normalized.id }
          : entry
      )
    : [...existingSolutions, { id: normalized.id ?? Date.now(), ...updatedPayload }];
  ...
  return { syncResult, dashboard: syncResult.dashboard, updatedPayload };
}

export function formatAntispamSyncPayload(
  customer,
  mappingMode,
  mailinblackTenantId,
  providerId = "mailinblack"

...
import {
  getDnsProvider,
  getDnsProviderOptions,
  resolveProviderGlobalConfigured,
} from "./dnsFormConfig";
import {
  extractDomainsFromModules,
  formatDomainSummary,
  importOvhDomainsForClient,
  isManualDomain,
  mapOvhApiDomainToMonitored,
  normalizeDomainItem,
  removeMonitoredDomain,
  saveMonitoredDomains,
...
export default function DomainsConfigModal({
  ...
  const selectedProvider = useMemo(
    () => getDnsProvider(selectedProviderId),
    [selectedProviderId]
  );
  ...
  const linkedDomainKeys = useMemo(
    () => new Set(domains.map((d) => (d.nom || "").toLowerCase()).filter(Boolean)),
    [domains]
  );

  const selectableOvhDomains = useMemo(
    () =>
      filteredOvhDomains.filter((domain) => {
        const name = (domain.domain || domain.name || domain.nom || "").toLowerCase();
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [modules, globalStatus, settingsList] = await Promise.all([
  ...
  const loadOvhDomains = useCallback(async ({ refresh = false } = {}) => {
    ...
    try {
      const data = await fetchOvhDomains({ light: true, refresh });
  ...
  const startEditingDomain = (domain) => {
    ...
    setManualForm({
      ...
      registrar:
        ...
        normalized.registrar !== copy.manualProvider.label
          ? normalized.registrar
          : "",
  ...
  const handleSaveManual = async () => {
    ...
    const duplicate = domains.some(
      (entry) =>
        (entry.nom || "").trim().toLowerCase() === nom.toLowerCase() &&
        (!editingDomain ||
          (entry.nom || "").trim().toLowerCase() !==
            (editingDomain.nom || "").trim().toLowerCase())
    );
    if (duplicate) {
    ...
    try {
      ...
      const nextDomains = editingDomain
        ? domains.map((entry) =>
            (entry.nom || "").trim().toLowerCase() ===
            (editingDomain.nom || "").trim().toLowerCase()
              ? { ...entry, ...newDomain, nom: newDomain.nom }
              : entry
          )
        : [...domains, newDomain];
      ...
      setManualForm({ nom: "", registrar: "", expiration: "" });
        ...
        <>
          ...
          <div className={formStyles.sectionHead}>
            ...
            <p className={formStyles.sectionDesc}>
              {globalOvhConfigured
                ? copy.provider.globalDetected
                : copy.provider.globalNotActive}
            </p>
          </div>
          <div className={avStyles.modeGrid}>
            <button
              type="button"
              className={`${avStyles.modeCard} ${!globalOvhConfigured ? avStyles.modeCardDisabled : ""}`}
              onClick={() => {
            ...
            >
              ...
              <span className={avStyles.modeCardDesc}>
                {globalOvhConfigured
                  ? copy.provider.globalAccountDescActive
                  : copy.provider.globalAccountDescInactive}
              </span>
      ...
      {domains.length === 0 ? (
        ...
        <div className={avStyles.solutionList}>
          {domains.map((domain, index) => {
            ...
            return (
              <div key={domainKey} className={avStyles.solutionCard}>
                <SolutionProviderIcon
                  provider={getDnsProvider(summary.providerId)}
                  fallbackIcon="stash:domain"
                />
                ...
                <div className={avStyles.solutionCardButtons}>
                  ...
                  <button
                    ...
                    onClick={() => requestRemoveDomain(domain)}
                    disabled={Boolean(deletingDomainKey)}
                    aria-label={copy.formatRemoveAria(summary.label)}
                    title={copy.overview.removeTitle}
                  >
                    <Icon
  ...
  const renderManual = () => (
    <>
      ...
      <div className={formStyles.field}>
        ...
        <input
          id="manual-domain-registrar"
          type="text"
          className={formStyles.input}
          value={manualForm.registrar}
          onChange={(e) => setManualForm({ ...manualForm, registrar: e.target.value })}
    ...
    <>
      ...
      <div className={dnsStyles.importToolbar}>
        <div className={dnsStyles.importSearchWrap}>
          <input
            id="ovh-domain-search"
            className={formStyles.input}
            value={ovhSearch}
            onChange={(e) => setOvhSearch(e.target.value)}
            placeholder={copy.import.searchPlaceholder}
            disabled={loadingOvhDomains}
          />
        </div>
        <div className={dnsStyles.importToolbarActions}>
          <button
            ...
            onClick={() => loadOvhDomains({ refresh: true })}
            disabled={loadingOvhDomains || saving}
          >
            <Icon
              icon={loadingOvhDomains ? "mdi:loading" : "mdi:refresh"}
              className={loadingOvhDomains ? formStyles.spinning : ""}
              aria-hidden
            />
            {copy.import.refresh}
          </button>
          <button
            type="button"
            className={formStyles.ghostBtn}
            onClick={handleSelectAllOvh}
            disabled={loadingOvhDomains || selectableOvhDomains.length === 0}
          >
          ...
          <button
            type="button"
            className={formStyles.ghostBtn}
            onClick={handleClearOvhSelection}
            disabled={loadingOvhDomains || selectedOvhDomains.size === 0}
          >
            {copy.import.clear}
          </button>
        </div>
      ...
      ) : filteredOvhDomains.length === 0 ? (
        <div className={avStyles.emptyState}>
          {copy.import.empty}
        </div>
      ) : (
        <div className={dnsStyles.importTableWrap}>
          <table className={dnsStyles.importTable}>
            <thead>
              <tr>
                <th scope="col" style={{ width: "2.5rem" }} aria-label={copy.table.selectionAria} />
            ...
            <tbody>
              {filteredOvhDomains.map((domain) => {
                ...
                return (
                  <tr
                    key={domainName}
                    className={`${dnsStyles.importTableRow} ${
                      isSelected ? dnsStyles.importTableRowSelected : ""
                    } ${alreadyLinked ? dnsStyles.importTableRowDisabled : ""}`}
                    onClick={() => !alreadyLinked && toggleOvhDomainSelection(domainName)}
                  >
                    <td>
                      <input
                        type="checkbox"
                        ...
                        onChange={() => toggleOvhDomainSelection(domainName)}
                        onClick={(e) => e.stopPropagation()}
                        aria-label={copy.formatSelectAria(domainName)}
                      />
                    </td>
                    <td>
                    ...
                    <td>
                      {mapped.expiration
                        ? new Date(mapped.expiration).toLocaleDateString(copy.bcp47)
                        : copy.emDash}
                    </td>
                    <td>
                      {mapped.autoRenew == null
                        ? copy.emDash
                        : mapped.autoRenew
                          ? copy.renewal.automatic
                          : copy.renewal.manual}
                    </td>
                    <td>
                      {alreadyLinked ? (
                        <span className={dnsStyles.domainBadge}>{copy.badges.alreadyLinked}</span>
                      ) : isSelected ? (
                        <span className={`${dnsStyles.domainBadge} ${dnsStyles.domainBadgeOk}`}>
                        ...
                        <span className={dnsStyles.domainBadge}>{copy.badges.available}</span>
                      )}
                    </td>
                  </tr>
                );
      ...
      {ovhDomains.length > 0 ? (
        <p className={dnsStyles.importSelectionSummary}>
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="domains-config-title"
      >
        <div className={formStyles.accentBar} aria-hidden />
        <header className={formStyles.header}>
          <div className={formStyles.headerMain}>
            <div
              className={`${formStyles.headerIconWrap} ${dnsStyles.headerIconDns}`}
              aria-hidden
            >
            ...
            {navSections.map((section) => {
              ...
              return (
                <button
                  key={section.id}
                  type="button"
                  className={`${formStyles.navItem} ${isActive ? formStyles.navItemActive : ""} ${
          ...
          <div className={formStyles.footerActions}>
            {showManualActions ? (
              <button
                ...
                disabled={saving || !manualForm.nom.trim()}
              >
                <Icon
            ...
            {showImportActions ? (
              <>
                ...
                <button
                  ...
                  disabled={saving || selectedOvhDomains.size === 0 || loadingOvhDomains}
                >
                  <Icon
                    icon={saving ? "mdi:loading" : "mdi:download"}
                    className={saving ? formStyles.spinning : ""}
                    aria-hidden
                  />
    ...
    document.getElementById("modal-root") || document.body
      ...
      <ConfirmModal
        ...
        message={
          deleteTarget
            ? interpolate(configCopy.confirm.removeMonitoring.message, {
        ...
        loading={Boolean(deletingDomainKey)}

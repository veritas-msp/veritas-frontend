...
export function normalizeDomainItem(item) {
  ...
  const expirationRaw =
    item.expiration || item.expirationDate || item.expirityDate || null;
  const autoRenew = item.autoRenew ?? item.auto_renewal ?? null;
  const hasManualHints =
    item.providerId === "manual" || item.isManual === true;
  const providerId =
    item.providerId ||
    ((item.registrar || "").toLowerCase().includes("ovh")
      ? "ovh"
      : hasManualHints
        ? "manual"
        : inferProviderIdFromDomain({ ...item, nom }));
  ...
  return {
    ...
    registrar:
      item.registrar ||
      (providerId === "ovh" ? "OVH" : providerId === "manual" ? null : "") ||
      "",
...
export function formatDomainSummary(domain) {
  ...
  if (!normalized) return { label: "Domaine", meta: "" };
  const providerId = normalized.providerId || inferProviderIdFromDomain(normalized) || "manual";
  const providerName =
    providerId === "manual"
      ? getDnsProvider("manual")?.label || "Autre registrar"
      : providerId === "ovh"
        ? "OVH"
        : getDnsProvider(providerId)?.label || normalized.registrar || "-";
  const registrarLabel =
    normalized.registrar &&
    normalized.registrar !== "N/A" &&
    providerId !== "manual"
      ? normalized.registrar
      : providerId === "manual" && normalized.registrar
        ? normalized.registrar
        : null;
  const metaParts = [registrarLabel || providerName];
  if (normalized.expiration) {
    metaParts.push(`Expire le ${new Date(normalized.expiration).toLocaleDateString("fr-FR")}`);
  }
  if (normalized.autoRenew != null) {
    metaParts.push(normalized.autoRenew ? "Renouvellement auto" : "Renouvellement manuel");
  }
  if (normalized.hasDnsZone) {
    metaParts.push("Zone DNS OVH");
  }
  return {
    label: normalized.nom || "Domaine",
    providerName,
    providerId,
    meta: metaParts.join(" · "),
  };
...
export function listConfiguredDomains(client, domainItems = [], modulesData = null) {
  const sources = [
    ...extractDomainsFromModules(modulesData || { equipements: client?.equipements }),
...
export async function importOvhDomainsForClient(clientId, ovhDomains = [], existingDomains = []) {
  const existingKeys = new Set(
    (existingDomains || []).map((d) => buildDomainDedupeKey(d)).filter(Boolean)
  );
  const toAdd = (ovhDomains || [])
    .map((d) => mapOvhApiDomainToMonitored(d))
    .filter((d) => {
...
export async function refreshMonitoredDomainsFromOvh(clientId) {
  ...
  const apiResult = await fetchOvhDomains({ light: true });
  const apiDomains = Array.isArray(apiResult.domains) ? apiResult.domains : [];
  const apiByName = new Map(
    apiDomains.map((d) => [
      (d.domain || d.name || "").toLowerCase(),
      mapOvhApiDomainToMonitored(d),
    ])
  );
  ...
  const updated = existing.map((domain) => {
    ...
    return { ...domain, ...fresh, nom: domain.nom };
  });
...
export async function refreshSingleMonitoredDomainFromOvh(clientId, domain) {
  ...
  const merged = existing.map((entry) =>
    domainMatches(entry, normalized) ? { ...entry, ...mapped, nom: normalized.nom } : entry
  );

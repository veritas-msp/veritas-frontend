...
import {
  getVaultSecretAvailabilityLabel,
  getVaultSecretsCopy,
  interpolate,
} from "./vaultSecretsI18n";
  ...
  const handleConfirmRevoke = async () => {
    ...
    try {
      ...
      setSecrets((prev) =>
        prev.map((row) =>
          row.id === secret.id
            ? { ...row, availability: "revoked", status: "revoked", views_remaining: 0 }
            : row
        )
      );
    ...
    <div className={styles.secretsPanel}>
      <div className={styles.toolbar}>
        <div className={styles.toolbarIntro}>
          <p className={styles.introText}>
            ...
            {activeCount > 0
              ? ` ${interpolate(activeCount > 1 ? panelCopy.activeCountPlural : panelCopy.activeCount, {
      ...
      {loading ? (
        ...
        <div className={styles.secretsList}>
          {secrets.map((secret) => (
            <article key={secret.id} className={styles.secretCard}>
              <div className={styles.secretCardMain}>
                <div className={styles.secretCardHead}>
                  ...
                  <div className={styles.secretCardTitleWrap}>
                    ...
                    {secret.description ? (
                      <p className={styles.secretCardDesc}>{secret.description}</p>
                    ) : null}
                  </div>
                ...
                <div className={styles.secretMetaRow}>
                  <span>
                    {interpolate(panelCopy.expiresAt, { date: formatDateTime(secret.expires_at) })}
                  </span>
                  <span>
                    ...
                    {secret.availability === "active" && secret.views_remaining != null
                      ? interpolate(
                          secret.views_remaining > 1
                            ? panelCopy.viewsRemainingPlural
                            : panelCopy.viewsRemaining,
                          { count: String(secret.views_remaining) }
                        )
                      : ""}
                  </span>
                  <span>
                    {interpolate(panelCopy.createdAt, { date: formatDateTime(secret.created_at) })}
                  </span>
                  {secret.deletion_requested_at ? (
                    <span className={styles.secretDeletionRequested}>
                      {panelCopy.deletionRequested}
                    </span>
                  ) : null}
                </div>
              </div>
              {secret.availability === "active" ? (
                <button
                  type="button"
                  className={`${pageLayout.iconBtn} ${styles.secretRevokeBtn}`}
                  onClick={() => setRevokeTarget(secret)}
                  disabled={Boolean(revokingId)}
                  title={panelCopy.revokeTitle}
                >
                  <Icon
                    icon={revokingId === secret.id ? "mdi:loading" : "mdi:key-remove"}
                    className={revokingId === secret.id ? styles.spinning : undefined}
                    aria-hidden
                  />
                </button>
              ) : null}
      ...
      <VaultSecretRevokeModal
        open={Boolean(revokeTarget)}
        secret={revokeTarget}
        contactName={contactName}
        saving={Boolean(revokeTarget && revokingId === revokeTarget.id)}
        onClose={() => {
...
function NumberStepper({ id, value, onChange, min, max, disabled = false, stepperCopy }) {
  ...
  return (
    <div className={styles.numberStepper}>
      <input
        ...
        max={max}
        disabled={disabled}
        onChange={(e) => onChange(clampStepperValue(e.target.value, min, max, min))}
      />
      <div className={styles.numberStepperActions}>
        <button
          ...
          aria-label={stepperCopy.increase}
        >
        ...
        <button
          ...
          aria-label={stepperCopy.decrease}
        >
...
function VaultShareSecretModal({ contactId, clientId, locale, onClose, onCreated }) {
  const copy = useMemo(() => getVaultSecretsCopy(locale), [locale]);
  ...
  return createPortal(
    <div className={formStyles.overlay} onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="vault-share-secret-title"
      >
        ...
        <form className={styles.modalForm} onSubmit={handleSubmit}>
          <div className={styles.modalFormBody}>
            ...
            <div className={formStyles.field}>
              ...
              <input
                id="vault-secret-description"
                className={formStyles.input}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder={modalCopy.contextPlaceholder}
                maxLength={2000}
                disabled={saving}
              />
            </div>
            ...
            <div className={formStyles.field}>
              ...
              <input
                id="vault-secret-login"
                className={formStyles.input}
                value={login}
                onChange={(e) => setLogin(e.target.value)}
                placeholder={modalCopy.loginPlaceholder}
                maxLength={320}
                disabled={saving}
                autoComplete="off"
              />
            ...
            <div className={formStyles.field}>
              ...
              <div className={styles.secretInputRow}>
                <input
                  ...
                  placeholder={modalCopy.secretPlaceholder}
                  maxLength={4000}
                  disabled={saving}
                ...
                <button
                  ...
                  disabled={saving}
                  title={showSecret ? modalCopy.hide : modalCopy.show}
                >

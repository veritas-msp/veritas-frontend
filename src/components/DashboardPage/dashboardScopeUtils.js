...
/** Filtre effectif pour l'API (incomplet → tout Veritas). */
export function normalizeScopeFilter(value) {
  const parsed = parseScopeFilter(value);
  if (parsed.type === "all") return { ...DEFAULT_SCOPE_FILTER };
  if (parsed.type === "agent" && parsed.agentId) {
    return { type: "agent", agentId: parsed.agentId, clientId: null, contactId: null };
  }
  if (parsed.type === "client" && parsed.clientId) {
    return { type: "client", agentId: null, clientId: parsed.clientId, contactId: null };
  }
  if (parsed.type === "contact" && parsed.contactId) {
    return { type: "contact", agentId: null, clientId: null, contactId: parsed.contactId };
  }

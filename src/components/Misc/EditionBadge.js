...
export default function EditionBadge({ variant = "dark", className = "" }) {
  const { edition, loaded } = useVeritasEdition();
  ...
  return (
    <span
      className={`${styles.badge} ${styles[variant]} ${styles[edition]} ${className}`.trim()}
      title={meta.title}
    >
      {meta.label}
    </span>

...
export function buildEventMovePayload(calendarEvent, newStart, newEnd, { assignedUserId } = {}) {
...
export function applyLocalEventMove(events, eventId, { start, end, assignedUserId, users = [] }) {
    ...
    return {
      ...
      _rawData: ev._rawData
        ? {
            ...
            description: buildEventMovePayload(ev, start, end, { assignedUserId }).description,
          }
        : ev._rawData,
    };
  });
...
export function computeMovedAllDayRange(event, targetDay) {
  ...
  return { start: newStart.toDate(), end: newEnd.toDate() };
}
...
export function computeMonthResizeRange(event, edge, deltaDays) {
  ...
  if (edge === "left") {
    ...
    return { start: nextStart.toDate(), end: endM.toDate() };
  }

  const nextEnd = endM.clone().add(deltaDays, "days");
  if (nextEnd.isBefore(startM)) return null;
  return { start: startM.toDate(), end: nextEnd.toDate() };
}

/** Repositionnement d'un événement sur une case jour (vue mois, y compris entre calendriers). */
export function computeMonthDropRange(event, targetDate) {
  const targetDay = moment(targetDate).startOf("day");
  const startM = moment(event.start);
  const endM = moment(event.end);
  const inferredAllDay =
    startM.format("HH:mm") === "00:00" && endM.format("HH:mm") === "23:59";
  if (inferredAllDay) {

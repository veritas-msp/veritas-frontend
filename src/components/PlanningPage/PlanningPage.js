...
function serializeEventDescription(text, meta) {
  ...
  const hasMeta = Array.isArray(meta?.linkedItems) && meta.linkedItems.length > 0 || Array.isArray(meta?.todos) && meta.todos.length > 0 || meta?.schedule?.allDay === true || meta?.schedule?.businessDaysOnly === false;
...
export default function PlanningPage({
  ...
  const planningCopy = useMemo(() => getPlanningPageCopy(locale, eventFormCopy.eventTypes), [locale, eventFormCopy]);
  ...
  const renderEventPreviewCard = event => ({
    close
  }) => <PlanningEventHoverCard event={event} users={users} onNavigate={onNavigate} onClose={close} onAssignAgent={handleAssignAgentFromTooltip} copy={hoverCardCopy} onEdit={() => {
    close();
  ...
  return <div className={`${mspStyles.mspPage} ${layout.page} msp-page-grid`}>
      <div className={mspStyles.mspLayout}>
        <div className={mspStyles.mspMain}>
          <MspPageHero eyebrow={planningCopy.eyebrow} title={planningCopy.title} subtitle={loading ? planningCopy.loading : planningCopy.formatEventsSubtitle(displayEvents.length, portfolioTotal)} icon="mdi:calendar-clock-outline" actions={<button type="button" className={`${layout.primaryBtn} ${layout.primaryBtnIconOnly}`} onClick={() => openCreateEventModal()} aria-label={planningCopy.newEvent}>
                <FaPlus />
              </button>} />
        ...
        <div className={styles.viewsLayout}>
          ...
          <div className={styles.mainColumn}>
            ...
            {loading ? <div className={layout.stateBox}>
                ...
                {view === "month" ? <>
                    <PlanningToolbar />
                    <PlanningMultiMonthView currentDate={currentDate} monthsShown={monthsShown} events={displayEvents} localizer={localizer} calendarHeight={getMonthViewHeight(monthsShown)} EventComponent={EventComponent} eventPropGetter={eventStyleGetter} onSelectSlot={handleSelectSlot} onNavigate={handleNavigate} onMonthEventDrop={handleMonthEventDrop} onMonthEventResize={handleCalendarEventResize} calendarMessages={planningCopy.calendarMessages} culture={locale} />
                  </> : view === "week" ? <>
                    <PlanningToolbar />
                    <PlanningWeekResourceView currentDate={currentDate} events={displayEvents} agents={weekAgents} renderEvent={renderWeekEvent} onSelectSlot={handlePlanningSelectSlot} onEventMove={handleWeekEventMove} onEventResize={handleCalendarEventResize} isEventDraggable={isPlanningEventDraggable} getEventColors={(event, agentId) => getPlanningEventColors(event, agentId)} onDragInteractionEnd={() => markSuppressSlotSelect(500)} weekCopy={hoverCardCopy} />
                  </> : view === "agenda" ? <>
                    <PlanningToolbar />
                    ...
                    <div className={styles.calendarDayWrapper}>
                      <DragAndDropCalendar localizer={localizer} events={displayEvents} startAccessor="start" endAccessor="end" view="day" date={currentDate} toolbar={false} selectable draggableAccessor={isPlanningEventDraggable} resizableAccessor={isPlanningEventDraggable} resizable onEventDrop={handleCalendarEventDrop} onEventResize={handleCalendarEventResize} onNavigate={handleNavigate} onSelectSlot={handleSelectSlot} tooltipAccessor={() => ""} style={{
                          height: "100%"
      ...
      {daySlotAction?.date && <div className={styles.dayActionOverlay} onClick={() => setDaySlotAction(null)} role="presentation">
          <div className={styles.dayActionCard} onClick={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="planning-day-action-title">
            ...
            <div className={styles.dayActionButtons}>
              <button type="button" className={styles.dayActionBtnPrimary} onClick={handleCreateEventOnSelectedDay}>
                <Icon icon="mdi:calendar-plus" aria-hidden />
                {planningCopy.dayAction.createEvent}
              </button>
              {view !== "day" ? <button type="button" className={styles.dayActionBtnSecondary} onClick={handleGoToSelectedDay}>

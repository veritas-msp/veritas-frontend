...
export function assignAllDayLanes(segments) {
  ...
  return { segments: sorted, laneCount: Math.max(0, laneEnds.length) };
}
...
export function allDaySegmentToPercentStyle(segment) {
  ...
  return { left: `${left}%`, width: `${Math.max(width, 100 / WEEK_DAY_COUNT)}%` };
}

/** Style live pendant un resize (variant: "allDay" | "timed"). */
export function weekResizeBoundsToPercentStyle(variant, startBound, endBound) {
  if (variant === "allDay") {
    return allDaySegmentToPercentStyle({ startDay: startBound, endDay: endBound });
  }
  return segmentToPercentStyle({ startMin: startBound, endMin: endBound });
}

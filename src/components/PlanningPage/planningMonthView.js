...
export function createPlanningMonthWeekWrapper(monthDate) {
  ...
  function PlanningMonthWeekWrapper({ children, slotMetrics }) {
    const range = slotMetrics?.range || [];

    return (
      <div className={styles.monthWeekWrap}>
        {children}
        <div className={styles.offRangeMaskRow} aria-hidden>
          {range.map((day, index) => {
            ...
            return (
              <div
                key={+day}
                className={styles.offRangeMaskCell}
                style={{
                  left: `${(index / 7) * 100}%`,
                  width: `${100 / 7}%`,
                }}
...
export function createPlanningMonthDateCellWrapper(monthDate, onEventDrop) {
  ...
  function PlanningMonthDateCellWrapper({ value, children }) {
    ...
    return (
      <div
        className={styles.monthDayDropCell}
        data-off-range={isOffRange ? "true" : undefined}

...
import {
  ...
  WEEK_HOURS_PER_DAY,
  WEEK_TOTAL_MINUTES,
  allDaySegmentDaysToRange,
  allDaySegmentToPercentStyle,
  buildAgentWeekLayout,
  getWeekRange,
  segmentToPercentStyle,
  timedSegmentMinutesToRange,
  weekResizeBoundsToPercentStyle,
} from "./planningWeekLayout";
...
function WeekEventBlock({
  ...
  const basePositionStyle =
    variant === "allDay"
      ? allDaySegmentToPercentStyle(segment)
      : segmentToPercentStyle(segment);
  const positionStyle = previewBounds
    ? weekResizeBoundsToPercentStyle(variant, previewBounds.nextStart, previewBounds.nextEnd)
    : basePositionStyle;
  const { backgroundColor, borderColor } = getEventColors(segment.event, agentId);
  ...
  const beginResize = (edge, pointerEvent) => {
    ...
    const onMove = (moveEvent) => {
      ...
      if (variant === "allDay") {
        ...
        if (edge === "left") {
          ...
          nextEnd = Math.min(
            WEEK_DAY_COUNT - 1,
            Math.max(initialStart, initialEnd + deltaDays)
          );
        }
        resizeRef.current = { nextStart, nextEnd, variant: "allDay" };
        setPreviewBounds({ nextStart, nextEnd });
        return;
      }
      ...
      if (edge === "left") {
        ...
        nextEnd = Math.min(
          WEEK_TOTAL_MINUTES,
          Math.max(initialStart + MIN_TIMED_MINUTES, initialEnd + deltaMinutes)
        );
      }
      resizeRef.current = { nextStart, nextEnd, variant: "timed" };
      setPreviewBounds({ nextStart, nextEnd });
    };
    ...
    const finishResize = () => {
      ...
      if (state.variant === "allDay") {
        range = allDaySegmentDaysToRange(
          weekStart,
          state.nextStart,
          state.nextEnd,
          segment.event
      ...
      if (!range) return;
      onEventResize({ event: segment.event, start: range.start, end: range.end });
    };
  ...
  return (
    <div
      className={`${styles.eventBlock} ${variant === "allDay" ? styles.eventBlockAllDay : ""} ${
        canDrag ? styles.eventBlockDraggable : ""
      } ${resizable ? styles.eventBlockResizable : ""} ${isResizing ? styles.eventBlockResizing : ""}`}
      style={{
        ...positionStyle,
    ...
    >
      {resizable ? (
        <button
          type="button"
          className={`${styles.eventResizeHandle} ${styles.eventResizeHandleLeft}`}
          aria-label={weekCopy?.weekView?.resizeStart || "Modifier le début"}
      ...
      <div className={styles.eventBlockBody}>
        {renderEvent ? (
          ...
          <span className={styles.eventBlockTitle}>
            {segment.event.title || weekCopy?.defaults?.event || "Événement"}
          </span>
        )}
      </div>
      {resizable ? (
        <button
          type="button"
          className={`${styles.eventResizeHandle} ${styles.eventResizeHandleRight}`}
          aria-label={weekCopy?.weekView?.resizeEnd || "Modifier la fin"}
...
export default function PlanningWeekResourceView({
  ...
  const weekDays = Array.from({ length: WEEK_DAY_COUNT }, (_, index) =>
  ...
  const getRowHeight = (layout) => {
    const allDayHeight =
      layout.allDayLaneCount > 0
        ? layout.allDayLaneCount * ALL_DAY_LANE_HEIGHT + 6
        : 0;
    const timedHeight =
      layout.timedLaneCount > 0
        ? layout.timedLaneCount * TIMED_LANE_HEIGHT + 6
        : ROW_MIN_HEIGHT - allDayHeight;
  ...
  const reassignEventToAgent = (eventId, agentId) => {
    ...
    const movedEvent = findEventById(events, eventId);
    if (!movedEvent || (isEventDraggable && !isEventDraggable(movedEvent))) return;
    const currentAgentIds = Array.isArray(movedEvent.assignedUserIds)
      ? movedEvent.assignedUserIds.map(String)
      : movedEvent.assignedUserId
        ? [String(movedEvent.assignedUserId)]
        : [];
    if (
      currentAgentIds.length === 1 &&
      currentAgentIds[0] === String(agentId)
    ) {
      return;
    }
    onEventMove({
    ...
    <div className={styles.wrap}>
      <div className={styles.scrollArea}>
        <div
          ...
          style={{ "--week-grid-width": `${GRID_WIDTH}px` }}
        >
              ...
              <div className={styles.dayHeaderRow}>
                {weekDays.map((day) => (
                  <div
                    ...
                    className={`${styles.dayHeaderCell} ${day.isSame(moment(), "day") ? styles.dayHeaderToday : ""} ${allowDrop ? styles.dayHeaderDropTarget : ""}`}
                ...
                {weekDays.map((day) =>
                  hours.map((hour) => (
                    <div
                      ...
                      className={styles.hourHeaderCell}
          ...
          {agentRows.length === 0 ? (
            ...
            agentRows.map(({ agent, layout }) => {
              const rowHeight = getRowHeight(layout);
              const isDragOverRow = dragOverAgentId === String(agent.id);
              const makeAssignHandler = (agentId) => (eventId) =>
                reassignEventToAgent(eventId, agentId);

              return (
                <div
                  key={agent.id}
                  className={`${styles.bodyRow} ${isDragOverRow ? styles.bodyRowDragOver : ""}`}
                >
                  <div
                    className={`${styles.resourceCell} ${isDragOverRow ? styles.resourceCellDragOver : ""}`}
                    title={formatAgentLabel(agent)}
                    onDragOver={(event) => handleAgentDragOver(event, agent.id)}
                    onDragLeave={clearDragOverAgent}
                    onDrop={(event) => {
                      event.preventDefault();
                      clearDragOverAgent();
                      const eventId = event.dataTransfer.getData(PLANNING_EVENT_DRAG_MIME);
                  ...
                  >
                    <span
                      className={styles.resourceAgentDot}
                  ...
                  <div
                    className={`${styles.timelineCell} ${isDragOverRow ? styles.timelineCellDragOver : ""}`}
                  ...
                  >
                    <div className={styles.gridBackground}>
                      {weekDays.map((day) =>
                        hours.map((hour) => (
                          <div
                            key={`${agent.id}-${day.format("YYYY-MM-DD")}-${hour}`}
                            className={`${styles.gridCell} ${day.isSame(moment(), "day") ? styles.gridCellToday : ""} ${onSelectSlot ? styles.gridCellInteractive : ""}`}
                            style={{ width: HOUR_COLUMN_WIDTH }}
                            onClick={() => {
                            ...
                            title={
                              onSelectSlot
                                ? `${day.format("dddd D MMMM")} · ${String(hour).padStart(2, "0")}:00`
                                : undefined
                            }
                          />
                        ))
                    ...
                    {layout.allDay.length > 0 && (
                      <div
                        className={styles.allDayBand}
                        style={{ height: layout.allDayLaneCount * ALL_DAY_LANE_HEIGHT + 4 }}
                      >
                        {layout.allDay.map((segment) => (
                          <WeekEventBlock
                            key={`${segment.event.id}-allday-${segment.startDay}`}
                            segment={segment}
                            variant="allDay"
                            ...
                            weekStart={weekStart}
                            draggable={
                              !isEventDraggable || isEventDraggable(segment.event)
                            }
                            resizable={
                              Boolean(onEventResize) &&
                              (!isEventDraggable || isEventDraggable(segment.event))
                            }
                            onEventResize={onEventResize}
                            onAssignToAgent={makeAssignHandler(agent.id)}
                      ...
                      style={{
                        minHeight:
                          layout.timedLaneCount > 0
                            ...
                            : rowHeight -
                              (layout.allDayLaneCount > 0
                                ? layout.allDayLaneCount * ALL_DAY_LANE_HEIGHT + 4
                                : 0),
                      }}
                    >
                      {layout.timed.map((segment) => (
                        <WeekEventBlock
                          key={`${segment.event.id}-${segment.startMin}-${segment.lane}`}
                          segment={segment}
                          variant="timed"
                          renderEvent={renderEvent}
                          getEventColors={getEventColors}

...
function AgendaEventCard({ item, renderEventPreview, defaultEventTitle, planningTypes }) {
  const { event, timeLabel, allDay } = item;
  const { backgroundColor } = getPlanningEventColors(event);
  ...
  const card = (
    <button type="button" className={styles.eventCard}>
      <span className={styles.eventAccent} style={{ backgroundColor }} aria-hidden />
      ...
      <div className={styles.mainCol}>
        ...
        <div className={styles.metaRow}>
          ...
          {event.clientName ? (
            <span className={styles.metaChip} title={event.clientName}>
              <Icon icon="mdi:domain" className={styles.metaChipIcon} aria-hidden />
              {event.clientName}
            </span>
          ) : null}
          {event.assignedUserName ? (
            <span className={styles.metaChip} title={event.assignedUserName}>
  ...
  return (
    <SmartTooltip
      trigger="click"
      interactive
      content={renderEventPreview(event)}
      tooltipClassName={pageStyles.eventHoverPortal}
...
export default function PlanningAgendaView({
  ...
  const agenda = useMemo(
    () =>
      buildAgendaDayGroups(events, currentDate, {
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.summaryBar}>
        <span>
          {copy?.agenda?.weekOf || "Semaine du"}{" "}
          <strong>{capitalizeLabel(weekLabel)}</strong>
        </span>
        <span className={styles.summaryCount}>
          {copy?.formatAgendaEventCount
            ? copy.formatAgendaEventCount(agenda.totalEvents)
            : `${agenda.totalEvents} événement${agenda.totalEvents > 1 ? "s" : ""}`}
        </span>
      </div>
        ...
        {!agenda.hasEvents ? (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyTitle}>
              {copy?.agenda?.emptyTitle || "Aucun événement cette semaine"}
            </p>
            <p className={styles.emptyHint}>
              {copy?.agenda?.emptyHint ||
                "Ajustez les filtres agents ou types, ou naviguez vers une autre semaine avec les flèches du calendrier."}
            </p>
          </div>
        ) : (
          agenda.groups.map((group) => (
            <section key={group.dateKey} className={styles.daySection} aria-label={group.label}>
              <div className={styles.dayHeader}>
                <h3
                  className={`${styles.dayTitle} ${group.isToday ? styles.dayTitleToday : ""}`}
                >
                  {capitalizeLabel(group.label)}
                </h3>
                {group.isToday ? (
                  <span className={styles.dayBadgeToday}>
                    {copy?.agenda?.today || "Aujourd'hui"}
                  </span>
                ) : group.isWeekend ? (
                  <span className={styles.dayBadgeWeekend}>
                    {copy?.agenda?.weekend || "Week-end"}
                  </span>
                ) : null}
              </div>
              <div className={styles.eventList}>
                {group.events.map((item) => (
                  <AgendaEventCard
                    key={`${group.dateKey}-${item.event.id}-${moment(item.event.start).format("HHmm")}`}
                    item={item}
                    renderEventPreview={renderEventPreview}
                    defaultEventTitle={copy?.defaults?.event || "Événement"}

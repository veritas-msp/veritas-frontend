...
function NotificationItem({ item, onOpen, onArchive, sectionKey, archivingId, hideArchive, copy, locale }) {
  const isArchiving = String(archivingId) === String(item.id);

  return (
    <div
      className={`${styles.itemRow} ${item.isRead ? styles.itemRead : styles.itemUnread} ${
        isArchiving ? styles.itemArchiving : ""
      }`.trim()}
    >
      ...
      {!hideArchive ? (
        <button
          ...
          aria-label={copy.archiveItemAria}
          title={copy.archiveItemTitle}
        >
...
export default function NotificationBell({
  ...
  const sections = useMemo(
    () =>
      showArchived
        ? groupArchivedNotifications(items, copy.sections)
        : groupNotificationsByPriority(items, copy.sections),
    [items, showArchived, copy.sections]
  );
  ...
  const button = (
    <button
      type="button"
      ref={triggerRef}
      className={`${triggerClassName || styles.trigger} ${open ? styles.triggerOpen : ""} ${
        !triggerClassName && (!isCollapsed || isMobile) ? styles.triggerExpanded : ""
      }`}
      ...
      aria-label={
        unreadCount > 0
          ? interpolate(unreadCount > 1 ? copy.triggerUnreadAriaPlural : copy.triggerUnreadAria, {
              count: unreadCount,
            })
          : copy.triggerAria
      }
    >
      <Icon
        ...
        className={triggerIconClassName || styles.triggerIcon}
        aria-hidden
      />
  ...
  return (
    <div className={`${styles.root} ${rootClassName || ""}`.trim()}>
      {showIconTooltip && TooltipComponent ? (
        <TooltipComponent as="span" content={copy.title} className={styles.tooltipHost}>
          {button}
        </TooltipComponent>
      ) : (
        button
      ...
      {open &&
        fixedStyle &&
        createPortal(
          <div ref={dropdownRef} className={styles.dropdown} style={fixedStyle} role="menu" aria-label={copy.title}>
            <div className={styles.dropdownHeader}>
              <div className={styles.dropdownHeaderTop}>
                ...
                <div className={styles.viewToggle} role="group" aria-label={copy.filterAria}>
                  <button
                    type="button"
                    className={`${styles.viewToggleBtn} ${!showArchived ? styles.viewToggleBtnActive : ""}`.trim()}
              ...
              {!showArchived && (unreadCount > 0 || items.length > 0) ? (
                <div className={styles.dropdownHeaderActions}>
                  ...
                  <div className={styles.headerActionBtns}>
                    {unreadCount > 0 ? (
                      <button type="button" className={styles.headerActionBtn} onClick={() => markAllRead()}>
                        {copy.markAllRead}
                      </button>
                    ) : null}
                    {items.length > 0 ? (
            ...
            <div className={styles.list} style={{ maxHeight: listMaxHeight }}>
              {loading && items.length === 0 ? (
                ...
                <div className={styles.emptyState}>
                  <div className={`${styles.emptyIconWrap} ${showArchived ? styles.emptyIconWrapMuted : ""}`.trim()} aria-hidden>
                    <Icon
                      icon={showArchived ? "mdi:archive-outline" : "mdi:check-circle-outline"}

...
import {
  buildContactFormFromInitial,
  cloneContactFormSnapshot,
  contactFormsEqual,
...
function resolvePrimaryEmail(source) {
  ...
  return (
    getPrimaryCommunicationValue(list, "email") ||
...
export default function ContactFormModal({
  ...
  const [form, setForm] = useState(() =>
    buildContactFormFromInitial(initialContact, fixedClientId ?? defaultClientId)
  );
  const [initialSnapshot, setInitialSnapshot] = useState(() =>
    cloneContactFormSnapshot(buildContactFormFromInitial(initialContact, fixedClientId ?? defaultClientId))
  );
  ...
  const isEnterpriseLocked = Boolean(
    lockedClientId || lockedEnterpriseLabel || hideEnterpriseSection
  );
  ...
  const hasChanges = useMemo(
    () => !contactFormsEqual(form, initialSnapshot),
  ...
  const visibleSections = useMemo(
    () => {
      const sections = isEditing
        ? formSections
        : formSections.filter((section) => section.id !== "status");
      return sections;
    },
  ...
  const performSave = async (payload, primaryEmailChanged) => {
    try {
      ...
      if (isEditing) {
        ...
        showSuccess(
          primaryEmailChanged ? copy.portalEmailSuccess : copy.successUpdate
        );
        onSuccess?.(updated);
  ...
  const handleSubmit = async () => {
    ...
    const preparedCommunications = enforcePrimaryCommunications(
      (form.communications || []).filter((entry) => String(entry.value ?? "").trim())
    );
  ...
  const modalTitle = draftMode
    ? copy.draftPrimaryTitle
    : copy.modalTitle(isEditing);
  const modalSubtitle = draftMode
    ? copy.draftPrimarySubtitle
    : copy.modalSubtitle(isEditing);
  const formValid =
    Boolean(form.nom?.trim()) &&
    (draftMode || hasEnterprise) &&
    !hasEmptyCommunicationDrafts;
  const submitDisabled =
    saving ||
    !formValid ||
    (isEditing && !hasChanges && !draftMode);
            ...
            <div className={styles.fieldGrid2}>
              <div className={`${styles.field} ${styles.fieldFull}`}>
                ...
                <div
                  ...
                  role="group"
                  aria-labelledby="contact-form-sexe-label"
                >
                    ...
                    return (
                      <button
                        ...
                        className={`${styles.moduleTile} ${
                          isActive ? styles.moduleTileActive : ""
                        }`}
                        onClick={() =>
                          patchForm({ sexe: isActive ? "" : option.value })
                        }
        ...
        return (
          <>
            ...
            <ContactCommunicationsEditor
              ...
              onChange={(communications) => patchForm({ communications })}
            />
          </>
        );
                ...
                {isEnterpriseLocked ? (
                  <>
                    ...
                    <input
                      ...
                      value={
                        lockedClientId
                          ? getClientLabel(selectedClient, copy) || copy.currentClient
                          : lockedEnterpriseLabel || copy.enterprisePendingName
                      }
                      readOnly
                      disabled
                    />
                            ...
                            filteredClients.map((client) => (
                              <button
                                ...
                                className={`${styles.dropdownOption} ${
                                  String(form.client_id) === String(client.id)
          ...
          <>
            ...
            <div className={styles.modulesGrid}>
              <button
                type="button"
                className={`${styles.moduleTile} ${
                  form.statut === "actif" ? styles.moduleTileActive : ""
                }`}
              ...
              <button
                type="button"
                className={`${styles.moduleTile} ${
                  form.statut === "inactif" ? styles.moduleTileActive : ""
                }`}
  ...
  return createPortal(
    <div
      ...
      onClick={portalEmailConfirm || saving ? undefined : onClose}
      role="presentation"
    >
      <div
      ...
      >
        ...
        <header className={styles.header}>
          ...
          <button
            ...
            onClick={onClose}
            disabled={saving || Boolean(portalEmailConfirm)}
          ...
          <nav className={styles.nav} aria-label={copy.navAria}>
            {visibleSections.map((section) => (
              ...
              >
                ...
                <span className={styles.navItemText}>
                  <span
                    className={`${styles.navItemLabel} ${
                      isSectionIncomplete(section.id) ? styles.navItemLabelRequired : ""
                    }`}
                  >
                    {section.label}
                  </span>
                ...
                {section.id === "identity" && sectionMeta.identity && (
                  <span className={styles.navBadge}>✓</span>
                )}
                {section.id === "enterprise" && sectionMeta.enterprise && (
                  <span className={styles.navBadge}>✓</span>
                )}
              </button>
            ))}
        ...
        <footer className={styles.footer}>
          <span className={styles.footerHint}>
            {isEditing &&
              (hasChanges ? copy.footerUnsaved : copy.footerNoChanges)}
          </span>
        ...
        <ContactPortalEmailChangeModal
          embedded
          open={Boolean(portalEmailConfirm)}
          previousEmail={portalEmailConfirm?.previousPrimaryEmail || ""}
          nextEmail={portalEmailConfirm?.nextPrimaryEmail || ""}
          saving={saving}
          onClose={() => {
            if (saving) return;
            setPortalEmailConfirm(null);

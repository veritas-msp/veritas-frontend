...
export default function ClientDashboard() {
  const { dashboard: data } = useOutletContext() || {};
  ...
  const mappedComputers = useMemo(() => {
    ...
    return mapClientHardwareEquipment({
      ...
      equipements: { Ordinateurs: data.computers },
    }).filter((eq) => eq.type === "Ordinateurs");
  }, [data?.computers, data?.client]);

  if (!data) return null;

  const { client, commercial, stats, tickets, actionRequiredTickets, pendingValidationTickets } = data;

  const actionRequiredRows = actionRequiredTickets?.length
    ? actionRequiredTickets
    : pendingValidationTickets || [];

  const actionRequiredCount =
    stats.actionRequiredCount ?? stats.pendingValidationCount ?? actionRequiredRows.length ?? 0;

  const healthPct = stats.totalEquipment
  ...
  return (
    <div className={styles.mainContent}>
      ...
      <div className={styles.dashboardTopRow}>
        {kpiItems.map((item) => (
          <Link key={item.key} to={item.to} className={styles.kpiCard}>
                ...
                <tr key={ticket.id}>
                  ...
                  <td>
                    {(() => {
                      ...
                      if (actionBadge) {
                        return (
                          <span className={portalStyles.validationPendingBadge}>{actionBadge.label}</span>
                        );
                      }

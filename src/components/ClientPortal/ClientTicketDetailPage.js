...
import {
  ...
  validatePortalTicketResolution,
...
import {
  ...
  createEmptySatisfactionRatings,
  isSatisfactionComplete,
...
function toAbsoluteAttachmentUrl(rawPath) {
  ...
  const relativePath =
    uploadsIndex >= 0 ? normalizedSlashes.slice(uploadsIndex) : normalizedSlashes.startsWith("/")
    ? normalizedSlashes
    : `/${normalizedSlashes}`;
...
function isOwnPortalComment(comment, user) {
  ...
  if (
    ...
    user.id &&
    String(comment.author_user_id) === String(user.id)
  ) {
    return true;
  }
  if (
    user.email &&
    String(comment.author_name || "").toLowerCase() === String(user.email).toLowerCase()
  ) {
    return true;
  }
  if (
    user.username &&
    String(comment.author_name || "").toLowerCase() === String(user.username).toLowerCase()
  ) {
    return true;
  }
  return false;
...
function StarRatingInput({ value, onChange, disabled = false, label, copy }) {
  return (
    <div className={portalStyles.starRating} role="radiogroup" aria-label={label}>
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          key={star}
          type="button"
          ...
          aria-label={copy.formatStarAria(star)}
          className={`${portalStyles.starBtn} ${star <= value ? portalStyles.starBtnActive : ""}`.trim()}
...
function StarRatingDisplay({ rating, copy }) {
  const safeRating = Math.max(0, Math.min(5, Number(rating) || 0));
  return (
    <div className={portalStyles.starRatingDisplay} aria-label={copy.formatStarDisplayAria(safeRating)}>
      {[1, 2, 3, 4, 5].map((star) => (
        <Icon
          key={star}
          icon={star <= safeRating ? "mdi:star" : "mdi:star-outline"}
          className={star <= safeRating ? portalStyles.starIconActive : portalStyles.starIcon}
          aria-hidden
        />
      ))}
    </div>
...
function SatisfactionCriteriaDisplay({ ratings, criteria, copy }) {
  if (!ratings) return null;
  const average = computeSatisfactionAverage(ratings);
  return (
    <div className={portalStyles.satisfactionCriteriaList}>
      ...
      {average > 0 ? (
        <div className={portalStyles.satisfactionAverageRow}>
...
function SatisfactionCriteriaForm({ ratings, onChange, disabled = false, criteria, copy }) {
      ...
      {criteria.map(({ key, label, hint }) => (
        <div key={key} className={portalStyles.satisfactionCriterionFormRow}>
          ...
          <StarRatingInput
            ...
            onChange={(star) => onChange(key, star)}
            disabled={disabled}
            label={interpolate(td.starNoteFor, { label })}
            copy={copy}
          />
        </div>
      ))}
...
export default function ClientTicketDetailPage() {
  ...
  const resolutionPending =
    ticket?.status === "resolved" && Boolean(ticket?.resolutionValidation?.isPending);
  ...
  const handleSatisfactionRatingChange = (key, star) => {
    setSatisfactionRatings((prev) => ({ ...prev, [key]: star }));
  };
  ...
  const handleSubmitSatisfaction = async () => {
    ...
    try {
      ...
      setTicket((prev) => (prev ? { ...prev, satisfaction } : prev));
  ...
  const openSatisfactionModal = ({ edit = false } = {}) => {
    if (hasSatisfaction) {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const comment = await addPortalTicketComment(ticketId, { content, files });
      setTicket((prev) =>
        prev
          ? {
              ...prev,
        ...
        prev
          ? {
              ...
              comments: (prev.comments || []).map((comment) =>
                String(comment.id) === String(editingCommentId) ? updatedComment : comment
              ),
            }
          : prev
      );
  ...
  return (
    <div className={`${portalLayout.mainScrollFill} ${layout.page}`}>
      <div className={`${tdStyles.shell} ${portalStyles.portalDetailShell}`}>
        <header className={`${tdStyles.ticketChromeBar} ${tdStyles.ticketHeaderBar}`}>
          <div className={tdStyles.ticketHeroTrack}>
            ...
            <p className={tdStyles.ticketHeroEyebrow}>
              {interpolate(td.ticketRef, { id: ticket.ticket_number || ticket.id })}
            </p>
            <h1 className={tdStyles.conversationTitle} style={{ margin: 0, flex: 1, minWidth: 0 }}>
              {ticket.title}
            </h1>
          </div>
        </header>
        ...
        <div className={portalStyles.portalWorkspace}>
          <section className={`${tdStyles.centerPane} ${portalStyles.portalCenterPane}`}>
            {showSatisfaction ? (
              <div className={portalStyles.portalChatTopBanners}>
                {!hasSatisfaction ? (
                  <div className={portalStyles.satisfactionInviteBanner}>
                    ...
                    <button
                      type="button"
                      className={portalStyles.satisfactionInviteBtn}
                  ...
                  <div className={portalStyles.satisfactionThanksBanner}>
                    <div className={portalStyles.satisfactionThanksMain}>
                      ...
                      <div>
                        ...
                        <p className={portalStyles.satisfactionThanksText}>
                          {td.thanksText}
                          {submittedAverage > 0
                            ? interpolate(td.thanksAverage, { avg: String(submittedAverage) })
                            : ""}
                          {ticket.satisfaction.createdAt
                            ? interpolate(td.thanksOnDate, {
                    ...
                    <div className={portalStyles.satisfactionThanksActions}>
                      <button
                        type="button"
                        className={portalStyles.satisfactionThanksLink}
                        onClick={() => openSatisfactionModal()}
                      >
                        {td.viewRating}
                      </button>
                      <button
                        type="button"
                        className={portalStyles.satisfactionThanksEditBtn}
                        onClick={() => openSatisfactionModal({ edit: true })}
                      >
                        {td.editRating}
                      </button>
                    </div>
            ...
            {ticket.description ? (
              <article className={`${tdStyles.descriptionSticky} ${tdStyles.commentItem} ${tdStyles.descriptionItem}`}>
            ...
            <div className={tdStyles.timelineWrap}>
              <div className={tdStyles.timeline} ref={timelineRef}>
                {(ticket.comments || []).length === 0 ? (
                  <p className={layout.emptyStateHint}>{td.noMessages}</p>
                ) : (
                  ticket.comments.map((comment) => {
                    ...
                    const authorAvatar =
                      isOwn && user?.avatar ? user.avatar : comment.author_avatar || null;
                    return (
                      <article key={comment.id} className={tdStyles.commentItem}>
                        <div className={tdStyles.commentHeader}>
                          <div className={tdStyles.commentHeaderMain}>
                            <UserAvatar
                              name={comment.author_name || td.supportAuthor}
                              avatar={authorAvatar}
                              size={26}
                              variant={isOwn ? "client" : "agent"}
                            />
                            <div className={tdStyles.commentMeta}>
                          ...
                          <div className={tdStyles.commentHeaderRight}>
                            <span className={tdStyles.commentTimestamp}>
                              ...
                              {isCommentEdited(comment) ? (
                                <span className={tdStyles.commentEditedMark}> · {copy.common.edited}</span>
                              ) : null}
                            </span>
                            {showEditAction ? (
                              <button
                                type="button"
                                className={tdStyles.commentEditBtn}
                                onClick={() => startEditComment(comment)}
                                disabled={savingCommentEdit}
                                aria-label={td.editMessageAria}
                                title={td.editMessageTitle}
                              >
                        ...
                        {isEditingComment ? (
                          <div className={tdStyles.commentEditBox}>
                            <textarea
                              className={tdStyles.commentEditEditor}
                            ...
                            <div className={tdStyles.commentEditActions}>
                              <button
                                type="button"
                                className={tdStyles.secondaryBtn}
                          ...
                          <div className={tdStyles.commentBody}>{comment.content}</div>
                        ) : null}
                        {Array.isArray(comment.attachments) && comment.attachments.length > 0 ? (
                          <div className={tdStyles.attachmentsList}>
                            {comment.attachments.map((file) => {
                              ...
                              if (isImageAttachment(attachment)) {
                                return (
                                  <a
                                    ...
                                    className={tdStyles.attachmentPreviewLink}
                                    title={openTitle}
                                  >
                                    <img
                                      src={attachment.url}
                                      alt={label}
                                      className={tdStyles.attachmentPreviewImage}
                                      loading="lazy"
                                    />
                                  </a>
                                );
                              ...
                              return (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className={tdStyles.attachmentLink}
                                  title={openTitle}
                                >
            ...
            {resolutionPending ? (
              <div className={portalStyles.portalValidationPanel}>
                ...
                ticket.resolutionValidation?.interventionType ||
                ticket.resolutionValidation?.actionType ? (
                  <div className={portalStyles.portalValidationSolution}>
                    {ticket.resolutionValidation?.interventionType ? (
                      <span className={portalStyles.portalValidationTag}>
                        {getLocalizedSolutionCatalogLabel(
                          ticket.resolutionValidation.interventionType,
                          locale
                        )}
                      </span>
                    ...
                    {ticket.resolutionValidation?.resolutionReason ? (
                      <span className={portalStyles.portalValidationSolutionText}>
                        {ticket.resolutionValidation.resolutionReason}
                      </span>
                    ) : null}
                  </div>
                ...
                <textarea
                  className={portalStyles.portalValidationTextarea}
                  value={validationMessage}
                  onChange={(e) => setValidationMessage(e.target.value)}
                  placeholder={td.validationCommentPlaceholder}
                ...
                <div className={portalStyles.portalValidationActions}>
                  ...
                  <button
                    type="button"
                    className={tdStyles.primaryBtn}
                    onClick={() => handleSubmitValidation(true)}
                    disabled={submittingValidation}
                  >
              ...
              <div className={tdStyles.replyBox}>
                <textarea
                  className={tdStyles.editor}
                  value={reply}
                ...
                <div className={portalStyles.portalReplyFooter}>
                  <div className={portalStyles.portalReplyFiles}>
                    ...
                    {files.length > 0 ? (
                      <span>
                        {interpolate(
                          files.length === 1 ? td.filesSelectedOne : td.filesSelectedMany,
              ...
              <div className={portalStyles.portalValidationDone}>
      ...
      {satisfactionModalOpen && showSatisfaction ? (
        <div
          className={portalStyles.satisfactionModalOverlay}
          onClick={closeSatisfactionModal}
          role="presentation"
        >
          <div
            className={portalStyles.satisfactionModal}
            onClick={(event) => event.stopPropagation()}
            role="dialog"
            aria-modal="true"
            aria-labelledby="portal-satisfaction-modal-title"
          >
            <header className={portalStyles.satisfactionModalHeader}>
              <div className={portalStyles.satisfactionModalTitleWrap}>
                ...
                <div>
                  ...
                  {!hasSatisfaction ? (
                    <p className={portalStyles.satisfactionModalSubtitle}>{td.modalSubtitle}</p>
                  ) : null}
                </div>
              </div>
              <button
                type="button"
                className={portalStyles.satisfactionModalClose}
                onClick={closeSatisfactionModal}
              ...
              {hasSatisfaction && !satisfactionEditing ? (
                ...
                <>
                  ...
                  <div className={portalStyles.satisfactionCommentField}>
                    ...
                    <textarea
                      id="portal-satisfaction-comment"
                      className={portalStyles.satisfactionTextarea}
                      value={satisfactionMessage}
              ...
              {hasSatisfaction && !satisfactionEditing ? (
                <>
                  ...
                  <button
                    type="button"
                    className={tdStyles.primaryBtn}
                    onClick={() => setSatisfactionEditing(true)}
                  >
                    <Icon icon="mdi:pencil-outline" aria-hidden />
                    {td.editMyRating}
                  </button>
                ...
                <>
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSatisfactionRatings(
                        resolveDisplayRatings(ticket.satisfaction) || createEmptySatisfactionRatings()
                      );
                      setSatisfactionMessage(String(ticket.satisfaction?.message || ""));
                    }}
                    disabled={submittingSatisfaction}
                  ...
                  <button
                    ...
                    disabled={submittingSatisfaction || !isSatisfactionComplete(satisfactionRatings)}
                  >

...
const NAV_ITEMS = [
  { to: "/client", end: true, icon: "mdi:view-dashboard-outline", labelKey: "navDashboard" },
  { to: "/client/tickets", icon: "mdi:ticket-outline", labelKey: "navSupport", badgeKey: "actionRequired" },
  { to: "/client/devices", icon: "mdi:devices", labelKey: "navDevices" },
  { to: "/client/services", icon: "mdi:cloud-outline", labelKey: "navServices" },
  { to: "/client/contract", icon: "mdi:file-document-outline", labelKey: "navContract" },
  { to: "/client/documents", icon: "mdi:safe-square-outline", labelKey: "navVault" },
];

function PortalNavItem({ item, t, showTooltip, isCollapsed, isMobile, badge, onNavigate }) {
  ...
  const content = (
    <>
      ...
      {badge > 0 ? (
        <span className={portalStyles.navBadge}>{badge > 99 ? "99+" : badge}</span>
      ) : null}
    </>
  );
...
export default function ClientPortalSidebar({ copy, user, clientName, actionRequiredCount, onLogout }) {
  const t = copy.layout;
  ...
  const currentLocale = useMemo(
    () => APP_LOCALES.find((entry) => entry.code === locale) ?? APP_LOCALES[0],
    [locale]
  );
  ...
  const updateFloatingMenuPosition = useCallback(
    (open, rootRef, dropdownRef, setStyle) => {
      ...
      if (openMenuToRight) {
        ...
        const clampedCenter =
          maxCenter >= minCenter ? Math.max(minCenter, Math.min(maxCenter, centerY)) : window.innerHeight / 2;
        setStyle({
          position: "fixed",
          top: clampedCenter,
          left: rect.right + gap,
  ...
  useLayoutEffect(() => {
    ...
    const update = () =>
      updateFloatingMenuPosition(userMenuOpen, userMenuRef, userMenuDropdownRef, setUserMenuFixedStyle);
    update();
  ...
  useLayoutEffect(() => {
    ...
    const update = () =>
      updateFloatingMenuPosition(langMenuOpen, langMenuRef, langMenuDropdownRef, setLangMenuFixedStyle);
    update();
    const id = requestAnimationFrame(update);
    const onResize = () => update();
    window.addEventListener("resize", onResize);
  ...
  const renderPreferencesSection = () => {
    const renderThemeToggleBtn = () => {
      const btn = (
        <button type="button" className={portalStyles.settingsSquareBtn} onClick={toggleTheme} aria-label={themeTooltip}>
      ...
      if (showIconTooltip) {
        return (
          <SidebarTooltip as="span" content={themeTooltip}>
            {btn}
          </SidebarTooltip>
        );
      }
    ...
    const renderLangBtn = () => {
      const btn = (
        <button
          type="button"
          className={`${portalStyles.settingsSquareBtn} ${langMenuOpen ? portalStyles.settingsSquareBtnActive : ""}`.trim()}
          onClick={() => {
      ...
      if (showIconTooltip) {
        return (
          <SidebarTooltip as="span" content={t.languageTitle}>
            {btn}
          </SidebarTooltip>
        );
      }
    ...
    return (
      <div className={portalStyles.settingsSection} ref={langMenuRef}>
  ...
  return (
    <>
      {isMobile ? (
        <button type="button" className={sidebarStyles.burgerButton} onClick={() => setShowMenu((prev) => !prev)}>
          ☰
        </button>
      ) : null}
      ...
      {(showMenu || !isMobile) && (
        <motion.nav
          className={`${sidebarStyles.sidebar} ${portalStyles.portalSidebar} ${isMobile ? sidebarStyles.mobileSidebar : ""}`.trim()}
        ...
        >
          <div className={sidebarStyles.sidebarContent}>
            <div className={sidebarStyles.logoHeader}>
              {showIconTooltip ? (
                <SidebarTooltip as="span" content={t.brandSub} className={sidebarStyles.sidebarTooltipHost}>
              ...
              {(!isCollapsed || isMobile) && clientName ? (
                <div className={sidebarStyles.userMenuLabelSubtitle} style={{ textAlign: "center", padding: "0 4px" }}>
                  {clientName}
                </div>
              ) : null}
            </div>
            ...
            {isMobile ? (
              <button type="button" className={sidebarStyles.closeButton} onClick={() => setShowMenu(false)}>
                ✖
              </button>
            ) : null}
            ...
            <ul className={sidebarStyles.navList}>
              {NAV_ITEMS.map((item) => (
                <PortalNavItem
                  ...
                  isMobile={isMobile}
                  badge={item.badgeKey === "actionRequired" ? actionRequiredCount : 0}
            ...
            <div className={sidebarStyles.userSection} ref={userMenuRef}>
              <div
                className={`${sidebarStyles.userMenuTriggerRow} ${
                  isCollapsed && !isMobile ? sidebarStyles.userMenuTriggerRowCollapsed : ""
                }`}
              >
                {showIconTooltip ? (
                  <SidebarTooltip as="span" content={t.accountMenu} className={sidebarStyles.userAvatarTooltipHost}>
                    <button
                      type="button"
                      className={`${sidebarStyles.userAvatarButton} ${userMenuOpen ? sidebarStyles.userAvatarButtonOpen : ""} ${
                        isProfileActive ? sidebarStyles.userAvatarButtonActive : ""
                      }`}
                      onClick={() => {
                        closeLangMenu();
                        setUserMenuOpen((o) => !o);
                ...
                {!isCollapsed && (
                  <button
                    type="button"
                    className={sidebarStyles.userMenuLabelButton}
                    onClick={() => {
                      closeLangMenu();
                      setUserMenuOpen((o) => !o);
                    }}
                  ...
                  >
                    ...
                    {user?.email ? (
                      <span className={sidebarStyles.userMenuLabelSubtitle} title={user.email}>
                        {user.email}
                      </span>
                    ) : null}
                  </button>
              ...
              {userMenuOpen &&
                ...
                createPortal(
                  <div
                    ...
                    className={sidebarStyles.userMenuDropdown}
                  ...
                  >
                    ...
                    <button
                      type="button"
                      role="menuitem"
                      className={`${sidebarStyles.userMenuItem} ${isProfileActive ? sidebarStyles.userMenuItemActive : ""}`}
                      onClick={() => runUserMenuAction(() => navigate("/client/profile"))}
                    >
                    ...
                    <button
                      type="button"
                      role="menuitem"
                      className={`${sidebarStyles.userMenuItem} ${sidebarStyles.userMenuItemDanger}`}
      ...
      {langMenuOpen &&
        ...
        createPortal(
          <div
            ref={langMenuDropdownRef}
            className={portalStyles.langMenuDropdown}
            style={langMenuFixedStyle}
            role="listbox"
            aria-label={t.languageAria}
          >
            ...
            {APP_LOCALES.map(({ code, flag, label }) => (
              <button
                ...
                className={`${portalStyles.langMenuItem} ${locale === code ? portalStyles.langMenuItemActive : ""}`.trim()}
      ...
      {isMobile && showMenu ? (
        <div className={sidebarStyles.overlay} onClick={() => setShowMenu(false)} aria-hidden />
      ) : null}
    </>
  );
}

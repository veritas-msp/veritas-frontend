...
export function buildAvailabilityContactSlots({ mode, date, startTime, endTime, note }) {
  if (mode === "none") return [];
...
export function buildPortalClientEquipments(dashboard) {
  ...
  const groups = [
    ...(Array.isArray(dashboard.infrastructure) ? dashboard.infrastructure : []),
    ...(Array.isArray(dashboard.cloudServices) ? dashboard.cloudServices : []),
  ];

  groups.forEach((group) => {
    const groupLabel = group?.label || group?.type || "Équipement";
...
export function getEquipmentTypeLabel(type, copy) {
  ...
  if (type === "Équipement") return copy?.fleet?.chartLabels?.equipment ?? type;
  return type;
}
...
export function getEquipmentLinkSearchText(equipment) {
  return [equipment?.type, equipment?.name, equipment?.serial, equipment?.id]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
}

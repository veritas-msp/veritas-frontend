/**
 * Catalogue des offres affichées dans la vue d'ensemble portail client.
 */

export const PORTAL_INFRA_CATALOG = [
  { type: "internet", labelKey: "internet", icon: "mdi:web", moduleKeys: ["Internet"] },
  { type: "firewall", labelKey: "firewall", icon: "mdi:shield-lock", moduleKeys: ["Firewall", "Firewalls"] },
  { type: "servers", labelKey: "servers", icon: "mdi:server", moduleKeys: ["Serveurs"] },
  { type: "stockage", labelKey: "stockage", icon: "mdi:harddisk", moduleKeys: ["Stockage", "NAS"] },
  { type: "switch", labelKey: "switch", icon: "mdi:lan", moduleKeys: ["Switch"] },
  { type: "wifi", labelKey: "wifi", icon: "mdi:wifi", moduleKeys: ["BorneWifi", "WiFi"] },
  { type: "alimentation", labelKey: "alimentation", icon: "mdi:power-plug", moduleKeys: ["Alimentation"] },
  { type: "routeur", labelKey: "routeur", icon: "mdi:router-wireless", moduleKeys: ["Routeur"] },
  { type: "toip", labelKey: "toip", icon: "mdi:phone-voip", moduleKeys: ["TOIP"] },
];

export const PORTAL_CLOUD_CATALOG = [
  { type: "o365", labelKey: "o365", icon: "mdi:microsoft-office", moduleKeys: ["Office365", "O365"] },
  { type: "save", labelKey: "save", icon: "mdi:backup-restore", moduleKeys: ["Sauvegarde"] },
  { type: "antivirus", labelKey: "antivirus", icon: "mdi:shield-check", moduleKeys: ["Antivirus"] },
  { type: "antispam", labelKey: "antispam", icon: "mdi:email-lock", moduleKeys: ["Antispam"] },
  { type: "ndd", labelKey: "ndd", icon: "mdi:domain", moduleKeys: ["NDD"] },
];
...
function isComputerMonitored(computer) {
  return Boolean(
    computer?.monitored ||
      computer?.agentManaged ||
      computer?.checkmk_host_name ||
      computer?.agent_id ||
      computer?.agentId
  );
}
...
export function computeWorkstationOverview(computers = [], mappedComputers = []) {
  ...
  return { total, rmmManaged, monitored };
}

function buildWorkstationEntry(flags, stats) {
  const subscribed =
    isModuleEnabled(flags, ["Ordinateurs", "Hardware"]) || stats.total > 0;
...
function summarizeCloudItems(type, items) {
  ...
  if (type === "o365") {
    ...
    if (licenseCount > 0) return { key: "licenseCount", count: licenseCount };
    const users = items.reduce((max, item) => Math.max(max, item.details?.userCount || 0), 0);
    if (users > 0) return { key: "userCount", count: users };
    return { key: "serviceCount", count: items.length };
  }
  if (type === "antivirus" || type === "antispam") {
    const products = items.map((item) => item.product || item.name).filter(Boolean);
    if (products.length) return { key: "products", products: products.slice(0, 2) };
    return { key: "serviceCount", count: items.length };
  }
  if (type === "save") {
    ...
    return { key: "backupSummary", instances, jobs, count: items.length };
  }
  if (type === "ndd") {
    return { key: "serviceCount", count: items.length };
  }
...
export function buildPortalOverview(data, copy) {
  ...
  const workstations = buildWorkstationEntry(
    flags,
    computeWorkstationOverview(data?.computers, data?.mappedComputers)
  );

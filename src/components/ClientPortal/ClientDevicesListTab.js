...
function SupervisionBadge({ monitored, agentOnline, copy }) {
  if (!monitored) {
...
function WorkstationsTable({ rows, copy }) {
  if (!rows.length) {
  ...
  return (
    <div className={styles.root}>
      <div className={styles.filterBar}>
        ...
        <div className={styles.filterChips} role="tablist" aria-label={t.filterByType}>
          <button
            ...
            className={`${styles.chip} ${typeFilter === "all" ? styles.chipActive : ""}`.trim()}
          ...
          {categories.map((cat) => (
            <button
              ...
              type="button"
              role="tab"
              aria-selected={typeFilter === cat.key}
              className={`${styles.chip} ${typeFilter === cat.key ? styles.chipActive : ""}`.trim()}
      ...
      {visibleCategories.map((cat) => {
        const rows =
          cat.kind === "workstations"
            ? cat.items.map(mapWorkstationRow)
            : cat.items.map(mapInfraRow);

        return (
          <section key={cat.key} className={portalStyles.panel}>
            ...
            {cat.kind === "workstations" ? (
              mappedComputers.length === 0 && rawComputers.length > 0 ? (
                <>
                  <p className={styles.hint}>{t.noWorkstationsData}</p>
                  <InfraTable
                    rows={rawComputers.map((item) => ({
                ...
                <WorkstationsTable rows={rows} copy={t} />
              )
            ) : (
              <InfraTable rows={rows} copy={t} />
            )}
          </section>
        );
      })}

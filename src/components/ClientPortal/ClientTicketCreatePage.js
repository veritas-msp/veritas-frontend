...
import {
  ...
  buildPortalClientEquipments,
  formatContactSlotLabel,
  getEquipmentLinkLabel,
  getEquipmentLinkSearchText,
  getEquipmentTypeLabel,
  getTodayDateString,
  serializeContactSlots,
} from "./clientPortalTicketCreateUtils";
...
function validateAttachmentFiles(files = [], copy) {
  ...
  for (const file of files) {
    ...
    if (!ALLOWED_ATTACHMENT_EXTENSIONS.has(ext)) {
      throw new Error(interpolate(tc.invalidFileType, { formats: formatsLabel }));
    }
    if (Number(file?.size || 0) > MAX_ATTACHMENT_SIZE_BYTES) {
      throw new Error(interpolate(tc.fileTooLarge, { name }));
    }
  }
}
...
function TicketCreateTipsTooltip({ tips, title }) {
  return (
    <div className={s.tipsTooltip}>
      <p className={s.tipsTooltipTitle}>{title}</p>
      <ul className={s.tipList}>
        {tips.map((text, index) => (
          <li key={TIP_ICONS[index] || index} className={s.tipItem}>
  ...
  return (
    <div className={s.recapBody}>
      ...
      <div className={s.recapTable}>
        ...
        {availabilitySummary && availabilitySummary !== copy.noneFeminine ? (
          <RecapRow label={tc.recapAvailability} value={availabilitySummary} />
        ) : null}
        {equipmentSummary && equipmentSummary !== copy.none ? (
          <RecapRow label={tc.recapEquipment} value={equipmentSummary} />
        ) : null}
        {linkedTicketSummary && linkedTicketSummary !== copy.none ? (
          <RecapRow label={tc.recapLinkedTicket} value={linkedTicketSummary} />
        ) : null}
        {attachmentsSummary && attachmentsSummary !== copy.none ? (
          <RecapRow label={tc.recapDocuments} value={attachmentsSummary} />
        ) : null}
      </div>
...
export default function ClientTicketCreatePage() {
  ...
  const { dashboard: outletDashboard } = useOutletContext() || {};
  ...
  useEffect(() => {
    if (!outletDashboard) {
      fetchPortalDashboard()
        .then(setPortalDashboard)
        .catch(() => toast.error(tc.loadDashboardError));
    }
  }, [outletDashboard]);
  ...
  useEffect(() => {
    ...
    fetchPortalTickets({ limit: 200 })
      .then((rows) => setPortalTickets(Array.isArray(rows) ? rows : []))
      .catch(() => setPortalTickets([]))
  ...
  const clientEquipments = useMemo(
    () => buildPortalClientEquipments(portalDashboard),
    [portalDashboard]
  );
  ...
  const filteredEquipments = useMemo(() => {
    ...
    return clientEquipments
      .filter((eq) => getEquipmentLinkSearchText(eq).includes(query))
      .slice(0, 20);
  ...
  const filteredLinkableTickets = useMemo(() => {
    ...
    return rows
      .filter((ticket) => getTicketLinkSearchText(ticket).includes(query))
      .slice(0, 20);
  }, [portalTickets, linkedTicketSearch]);

  const selectedLinkedTicket = useMemo(
    () => portalTickets.find((ticket) => String(ticket.id) === String(linkedTicketId)) || null,
    [portalTickets, linkedTicketId]
  );
  ...
  const applySelectedAttachments = useCallback((selectedFiles = []) => {
    ...
    setAttachmentFiles((prev) => {
      ...
      if (merged.length >= MAX_ATTACHMENT_FILES && prev.length + selectedFiles.length > MAX_ATTACHMENT_FILES) {
        toast.warning(interpolate(tc.maxFilesWarning, { max: String(MAX_ATTACHMENT_FILES) }));
      }
      return merged;
    });
  ...
  const validate = () => {
    ...
    if (availabilityMode === "from") {
      ...
      if (!availabilityDate || !availabilityStart || !availabilityEnd) errors.contactSlots = true;
      else if (availabilityStart >= availabilityEnd) errors.contactSlots = true;
    }
  ...
  const handleConfirmCreate = async () => {
    ...
    try {
      ...
      if (attachmentFiles.length > 0) {
        try {
          await addPortalTicketComment(ticket.id, { content: "", files: attachmentFiles });
        } catch (error) {
  ...
  const availabilityModeOptions = useMemo(
    () => [
      { key: "none", label: tc.availabilityNone, icon: "mdi:calendar-remove-outline" },
      { key: "from", label: tc.availabilityFrom, icon: "mdi:clock-time-four-outline" },
      { key: "range", label: tc.availabilityRange, icon: "mdi:calendar-clock-outline" },
    ],
  ...
  return (
    <div
      className={`${portalLayout.mainScrollFill} ${layout.page}`}
    ...
    >
        ...
        <div className={account.contentScroll}>
          <div className={account.contentGridWide}>
            <div className={s.formStack}>
              <SectionPanel
                title={tc.detailsTitle}
                headerExtra={
                  <SmartTooltip
                    content={<TicketCreateTipsTooltip tips={tc.tips} title={tc.tipsTitle} />}
                    tooltipClassName={s.tipsPortalTooltip}
                    trigger="click"
                    as="button"
                    type="button"
                    className={s.tipsHelpBtn}
                    aria-label={tc.tipsAria}
                  >
              ...
              >
                <div className={s.fieldBlock}>
                  ...
                  <div
                    ...
                    className={`${s.fieldShell} ${fieldErrors.title ? s.fieldShellError : ""} ${fieldErrors.title ? s.fieldErrorPulse : ""}`.trim()}
                  >
                    <input
                      ...
                      placeholder={tc.titlePlaceholders[type]}
                      maxLength={200}
                    />
                  </div>
                ...
                <div className={s.fieldBlock}>
                  ...
                  <div
                    ...
                    className={`${s.fieldShell} ${s.fieldShellMultiline} ${fieldErrors.description ? s.fieldShellError : ""} ${fieldErrors.description ? s.fieldErrorPulse : ""}`.trim()}
                  >
                    <textarea
                      ...
                      placeholder={tc.descriptionPlaceholders[type]}
                      rows={6}
                      maxLength={5000}
                    />
                  </div>
                ...
                <div className={s.fieldBlock}>
                  ...
                  <div className={s.segmentedGroup} role="radiogroup" aria-label={tc.issueNature}>
                    {issueNatureOptions.map((item) => (
                      <button
                        key={item.key}
                        type="button"
                        role="radio"
                        aria-checked={issueNature === item.key}
                        className={`${s.segmentedBtn} ${issueNature === item.key ? s.segmentedBtnActive : ""}`.trim()}
                        onClick={() => handleIssueNatureChange(item.key)}
                      >
                        <Icon icon={item.icon} aria-hidden />
                        {item.label}
                      </button>
                ...
                <div className={s.fieldBlock}>
                  ...
                  <div
                    ...
                    className={`${s.attachmentDropZone} ${isAttachmentDragOver ? s.attachmentDropZoneActive : ""} ${fieldErrors.attachments ? s.attachmentDropZoneError : ""} ${fieldErrors.attachments ? s.fieldErrorPulse : ""}`.trim()}
                  >
                    ...
                    <label className={s.attachmentUploadBtn}>
                      ...
                      <input
                        ...
                        aria-labelledby="portal-ticket-attachments-label"
                        onChange={(e) => {
                        ...
                        return (
                          <li key={fileKey} className={s.attachmentFileItem}>
                            ...
                            <button
                              ...
                              aria-label={copy.formatRemoveAttachmentAria(file.name)}
                            >
                ...
                <div className={s.demandeurBlock}>
                  ...
                  <div className={s.availabilityComposer}>
                    <div className={s.availabilityModeChips} role="radiogroup" aria-label={tc.availabilityAria}>
                      {availabilityModeOptions.map((item) => (
                        <button
                          key={item.key}
                          type="button"
                          role="radio"
                          aria-checked={availabilityMode === item.key}
                          className={`${s.availabilityModeChip} ${availabilityMode === item.key ? s.availabilityModeChipActive : ""}`.trim()}
                          onClick={() => handleAvailabilityModeChange(item.key)}
                        >
                          <Icon icon={item.icon} aria-hidden />
                          {item.label}
                        </button>
                    ...
                    {availabilityMode !== "none" ? (
                      <div
                        ...
                        className={`${s.availabilitySlotBar} ${fieldErrors.contactSlots ? s.availabilitySlotBarError : ""} ${fieldErrors.contactSlots ? s.fieldErrorPulse : ""}`.trim()}
                      >
                        ...
                        {availabilityMode === "from" ? (
                          ...
                          <div className={`${s.slotField} ${s.slotFieldTimeRange}`} role="group" aria-label={tc.timeRangeAria}>
                            <Icon icon="mdi:clock-outline" className={s.slotFieldIcon} aria-hidden />
                            <input
                              type="time"
                              className={s.slotInput}
              ...
              {equipmentConcerned ? (
                <SectionPanel title={tc.equipmentTitle} className={s.panelAllowOverflow}>
                  <div className={s.equipmentPanel}>
                    <div className={s.segmentedGroup} role="radiogroup" aria-label={tc.equipmentSourceAria}>
                      <button
                        ...
                        className={`${s.segmentedBtn} ${equipmentSource === "veritas" ? s.segmentedBtnActive : ""}`.trim()}
                        onClick={() => {
                      ...
                      <button
                        ...
                        className={`${s.segmentedBtn} ${equipmentSource === "external" ? s.segmentedBtnActive : ""}`.trim()}
                        onClick={() => {
                    ...
                    <div className={s.equipmentSubPanel}>
                      {equipmentSource === "veritas" ? (
                        clientEquipments.length === 0 ? (
                          <p className={s.equipmentHint}>{tc.noEquipmentHint}</p>
                        ) : (
                          <>
                            <div className={s.equipmentField}>
                              ...
                              <div className={s.linkTicketPicker} ref={equipmentDropdownRef}>
                                <div
                                  ...
                                  className={`${s.contactInputWrap} ${fieldErrors.equipmentId ? s.contactInputWrapError : ""} ${fieldErrors.equipmentId ? s.fieldErrorPulse : ""}`.trim()}
                                >
                                  <Icon icon="mdi:magnify" className={s.contactInputIcon} aria-hidden />
                                  <input
                                    id="portal-equipment-search"
                                    ...
                                    placeholder={tc.equipmentSearchPlaceholder}
                                ...
                                {showEquipmentDropdown ? (
                                  <div className={s.contactDropdown} role="listbox">
                                    {filteredEquipments.length === 0 ? (
                                      <div className={s.contactEmpty}>{tc.noEquipmentFound}</div>
                                    ) : (
                                      filteredEquipments.map((eq, idx) => (
                                        <button
                                          ...
                                          className={`${s.contactOption} ${idx === equipmentHighlight ? s.contactOptionActive : ""}`.trim()}
                        ...
                        <div className={s.equipmentExternalGrid}>
                          <div className={s.equipmentField}>
                            ...
                            <input
                              type="text"
                              className={`${s.input} ${fieldErrors.equipmentBrand ? s.inputError : ""}`.trim()}
                              value={equipmentBrand}
                              placeholder={tc.brandPlaceholder}
                              onChange={(e) => {
                          ...
                          <div className={s.equipmentField}>
                            ...
                            <input
                              type="text"
                              className={`${s.input} ${fieldErrors.equipmentModel ? s.inputError : ""}`.trim()}
                          ...
                          <div className={`${s.equipmentField} ${s.equipmentExternalFull}`}>
                            ...
                            <input
                              type="text"
                              className={`${s.input} ${fieldErrors.equipmentSerial ? s.inputError : ""}`.trim()}
                              value={equipmentSerial}
                              placeholder={tc.serialPlaceholder}
                              onChange={(e) => {
                ...
                <div className={s.linkTicketPanel}>
                  <div className={s.segmentedGroup} role="radiogroup" aria-label={tc.linkAria}>
                    <button
                      ...
                      className={`${s.segmentedBtn} ${!linkedTicketEnabled ? s.segmentedBtnActive : ""}`.trim()}
                  ...
                  {linkedTicketEnabled ? (
                    <div className={s.linkTicketSubPanel}>
                      {loadingTickets ? (
                        <p className={s.equipmentHint}>{tc.loadingTickets}</p>
                      ) : portalTickets.length === 0 ? (
                        <p className={s.equipmentHint}>{tc.noLinkableTickets}</p>
                      ) : (
                        <>
                          <div className={s.equipmentField}>
                            ...
                            <div className={s.linkTicketPicker} ref={linkedTicketDropdownRef}>
                              <div
                                ...
                                className={`${s.contactInputWrap} ${fieldErrors.linkedTicketId ? s.contactInputWrapError : ""} ${fieldErrors.linkedTicketId ? s.fieldErrorPulse : ""}`.trim()}
                              >
                                <Icon icon="mdi:magnify" className={s.contactInputIcon} aria-hidden />
                                <input
                                  id="portal-linked-ticket"
                                  className={s.contactInput}
                                  type="text"
                                  ...
                                  placeholder={tc.ticketSearchPlaceholder}
                              ...
                              {showLinkedTicketDropdown ? (
                                <div className={s.contactDropdown} role="listbox">
                                  {filteredLinkableTickets.length === 0 ? (
                                    <div className={s.contactEmpty}>{tc.noTicketFound}</div>
                                  ) : (
                                    filteredLinkableTickets.map((ticket, idx) => (
                                      <button
                                        ...
                                        className={`${s.contactOption} ${idx === linkedTicketHighlight ? s.contactOptionActive : ""}`.trim()}
      ...
      {confirmModalOpen &&
        createPortal(
          <div className={s.confirmOverlay} onClick={handleCloseConfirm} role="presentation">
            <div
              className={s.confirmShell}
              onClick={(e) => e.stopPropagation()}
              role="dialog"
              aria-modal="true"
              aria-labelledby="portal-ticket-create-recap-title"
            >

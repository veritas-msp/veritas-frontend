...
export default function AuthPage() {
  ...
  const activeBranding = useMemo(() => mergeBrandingWithAuthCopy(loginBranding?.[accountType], authPanel, accountType), [loginBranding, accountType, authPanel]);
  const panelHeadline = <>
      {activeBranding.headlineLine1}
      <br />
      {activeBranding.headlineLine2}
    </>;
  const brandingStyleVars = useMemo(() => activeBranding.custom ? buildLoginBrandingStyleVars(activeBranding, accountType) : null, [activeBranding, accountType]);
  ...
  return <div className={styles.wrapper} style={brandingStyleVars ? {
    "--login-accent": activeBranding.colors.accentColor
      ...
      {}
      <aside className={`${styles.left} ${!activeBranding.custom && accountType === "client" ? styles.leftClient : ""} ${activeBranding.custom ? styles.leftBranded : ""}`} style={brandingStyleVars || undefined}>
      ...
      background: activeBranding.colors.rightBgColor
        ...
        <div className={styles.card}>
          ...
          <header className={styles.cardHeader}>
            {!mfaPending && view !== "login" && <button type="button" className={styles.backBtn} onClick={() => goTo("login")} disabled={busy}>
                {copy.back}
              </button>}
            {mfaPending && <button type="button" className={styles.backBtn} onClick={cancelMfaLogin} disabled={busy}>
                {copy.back}
              </button>}
          ...
          <div className={styles.viewBody} key={mfaPending ? "mfa" : `${view}-${accountType}`}>
            {mfaPending ? <form className={styles.form} onSubmit={e => {
            e.preventDefault();
            if (mfaCode.length === 6) handleMfaLogin(mfaCode);
          }}>
                <div className={styles.field}>
                  <label htmlFor="mfa-code">{copy.mfa.codeLabel}</label>
                  <input id="mfa-code" type="text" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={mfaCode} onChange={e => setMfaCode(e.target.value.replace(/\D/g, ""))} disabled={busy} required placeholder="000000" className={styles.mfaCodeInput} />
                </div>
            ...
            {!mfaPending && view === "forgot" && <form className={styles.form} onSubmit={handleForgot}>
                <Field label={copy.fields.email} id="forgot-email" type="email" autoComplete="email" value={email} onChange={setEmail} disabled={busy} required />
                <button type="submit" className={styles.btnPrimary} disabled={busy}>

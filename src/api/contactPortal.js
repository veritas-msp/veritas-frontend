...
export async function fetchContactPortal(contactId) {
  const res = await fetch(`${API_BASE_URL}/contacts/${contactId}/portal`, { credentials: "include" });
...
export async function impersonateContactPortal(contactId, { signal } = {}) {
...
export async function fetchClientPortalUsers() {
  const res = await fetch(`${API_BASE_URL}/client-portal-users`, { credentials: "include" });
  if (!res.ok) await parseError(res);
  return res.json();
}

export async function fetchClientPortalUsage() {
  const res = await fetch(`${API_BASE_URL}/contacts/portal/usage`, { credentials: "include" });
  if (!res.ok) await parseError(res);
  return res.json();
}

...
export async function getGlobalOvhStatus() {
  const res = await fetch(`${OVH_URL}/config`, { credentials: "include" });
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({ error: "Erreur inconnue" }));
    throw new Error(errorData.error || "Erreur lors de la vérification de la configuration OVH");
  }
  return res.json();
}

export async function fetchOvhDomains({ light = true, refresh = false } = {}) {
  ...
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({ error: "Erreur inconnue" }));
    throw new Error(errorData.error || errorData.details || "Impossible de récupérer les domaines OVH");
  }
  return res.json();
}
...
export async function fetchOvhDomainDetails(domainName) {
  ...
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({ error: "Erreur inconnue" }));
    throw new Error(errorData.error || "Impossible de récupérer le domaine OVH");
  }
  return res.json();
}
...
export async function testOvhConnection(credentials = {}) {
  ...
  if (!res.ok || data.success === false) {
    const message = data.error || data.details || "Échec du test de connexion OVH";

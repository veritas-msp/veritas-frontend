...
export async function fetchPortalVaultFiles({ category, search, limit, offset } = {}) {
  ...
  const data = await handleJsonResponse(response, "Erreur lors de la récupération des documents.");
  return {
...
export async function fetchPortalVaultSecrets() {
  const response = await fetch(SECRETS_BASE, { credentials: "include" });
  const data = await handleJsonResponse(response, "Erreur lors de la récupération des accès.");
  return {
...
export async function revealPortalVaultSecret(secretId) {
  ...
  return handleJsonResponse(response, "Impossible d'afficher cet accès.");
}
...
export async function requestPortalVaultSecretRevocation(secretId) {
  ...
  return handleJsonResponse(response, "Impossible de supprimer cet accès.");
}

...
export async function downloadRmmWindowsAgentSetupZip() {
  ...
  return downloadRmmFile(`/rmm/agent/download/windows/zip?v=${cacheBust}`, "VeritasAgent-Windows-Setup.zip");
}
export async function downloadRmmWindowsAgentSetup() {
  const cacheBust = Date.now();
  return downloadRmmFile(`/rmm/agent/download/windows?v=${cacheBust}`, "VeritasAgent-Windows-Setup.cmd");
}

...
export function getVeritasCommunitySupportLinks() {
  const discord =
    String(process.env.REACT_APP_VERITAS_DISCORD_URL || VERITAS_DISCORD_URL).trim() || null;
  const githubRepo = (
    String(process.env.REACT_APP_VERITAS_GITHUB_REPO || DEFAULT_GITHUB_REPO).trim().replace(/\/+$/, "") ||
    DEFAULT_GITHUB_REPO
  );

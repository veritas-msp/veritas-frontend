...
import {
  ...
  resolveActiveLocale,
  resolveServerDefaultLocale,
  syncLocaleToServerDefault,
} from "../i18n/locales";
import {
  formatAppDate,
  formatAppDateTime,
  ...
  formatAppLongDate,
  formatAppTime,
  parseDefaultPageSize,
  resolveEffectiveDateFormat,
  setAppRegionalSettings,
} from "../utils/appRegionalSettings";
...
function applySettingsSideEffects(settings, { preferServerDefault = false } = {}) {
  setAppRegionalSettings(settings);

  const locale = preferServerDefault
    ? syncLocaleToServerDefault(settings.app_default_locale)
    : resolveActiveLocale(settings.app_default_locale);

  applyDocumentLocale(locale);
  applyAdminDefaultThemeIfAllowed(settings.app_default_theme);
  window.dispatchEvent(
    new CustomEvent("appAdminThemeSetting", { detail: settings.app_default_theme || "light" })
  );
  window.dispatchEvent(new CustomEvent("appLocaleChanged", { detail: { locale } }));
  return locale;
}

export function AppGeneralSettingsProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const onUpdated = (event) => {
      ...
      setSettings(next);
      setLocale(applySettingsSideEffects(next, { preferServerDefault: true }));
    };
  ...
  const defaultPageSize = useMemo(
    () => parseDefaultPageSize(settings),
    [settings.app_default_page_size]
  );
  ...
  return (
    <AppGeneralSettingsContext.Provider value={value}>
      {children}
...
export function useAppGeneralSettings() {
  const { settings, loaded, defaultPageSize } = useAppGeneralSettingsContext();
  return { settings, loaded, defaultPageSize };
}

export function useAppFormatters() {
  const { settings, locale } = useAppGeneralSettingsContext();
...
export function setUserLocaleOverride(locale) {
  ...
  setAppRegionalSettings({ app_default_locale: nextLocale });
  window.dispatchEvent(new CustomEvent("appLocaleChanged", { detail: { locale: nextLocale } }));
}

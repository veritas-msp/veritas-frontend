...
export function useOnboarding(user, userRole) {
  ...
  const complete = useCallback(() => {
    if (!user?.id) return;
    writeState(user.id, { completed: true, pausedAtStep: null });
    setShowWizard(false);
    setShowResumeFab(false);
  }, [user?.id]);

  const pauseAtStep = useCallback(
    (nextStep) => {
      if (!user?.id) return;
      writeState(user.id, { pausedAtStep: nextStep });
      setShowWizard(false);
      setShowResumeFab(true);
    },
    [user?.id]
  ...
  const goToStep = useCallback((nextStep) => {
    setStep(nextStep);
    if (user?.id) {
      writeState(user.id, { pausedAtStep: null });
    }
  }, [user?.id]);

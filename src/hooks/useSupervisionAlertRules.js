...
import { buildDefaultSupervisionAlertRules } from "../components/EquipementPage/supervisionAlertRulesConfig";

let sharedRules = null;
let sharedPromise = null;

export function useSupervisionAlertRules() {
  const [rules, setRules] = useState(sharedRules || buildDefaultSupervisionAlertRules());
  ...
  const applyRules = useCallback((nextRules) => {
    ...
    setRules(nextRules);
    if (catalog) {
      setCatalog({ ...catalog, rules: nextRules });
    }
  }, [catalog]);

  return { rules, catalog, loading, error, reload: () => load(true), applyRules };
}

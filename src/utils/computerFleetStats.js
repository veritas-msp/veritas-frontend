import {
  ...
  getWorstDiskUsage,
} from "../components/EquipementPage/rmmMonitoringUtils";
...
export const POWER_PROFILES = {
  desktop: { watts: 75, label: "Poste fixe" },
  laptop: { watts: 40, label: "Portable" },
  unknown: { watts: 55, label: "Poste type" },
};
...
export function inferBrand(equipment, inventory) {
  ...
  if (explicit) return normalizeBrand(explicit) || "Non renseigné";

  const hostname = String(equipment?.name || inventory?.hostname || "").toLowerCase();
  const osName = String(equipment?.systeme || inventory?.os?.name || "").toLowerCase();
  const cpu = String(hw.cpu || "").toLowerCase();

  const hostnameBrands = [
    ["dell", "Dell"],
    ["latitude", "Dell"],
    ["optiplex", "Dell"],
    ["hp-", "HP"],
...
export function inferModelLabel(equipment, inventory) {
  ...
  if (profile === "laptop") return "Portable (modèle inconnu)";
  if (profile === "desktop") return "Fixe (modèle inconnu)";
  return "Non renseigné";
}

function inferCpuFamily(inventory) {
  const cpu = String(inventory?.hardware?.cpu || "").toLowerCase();
  if (!cpu) return "Non renseigné";
...
function ramBucket(ramGb) {
  ...
  if (!Number.isFinite(value) || value <= 0) return "Non renseigné";
...
export function normalizeOsFamily(systeme) {
  ...
  if (s.includes("windows")) return "Windows (autre)";
...
function increment(map, key) {
  const label = String(key || "").trim() || "Non renseigné";
  map[label] = (map[label] || 0) + 1;
}

function toDistribution(counts, total, { limit = 8, otherLabel = "Autres" } = {}) {
...
export function buildComputerFleetStats(computers, options = {}) {
  ...
  const opts = { ...DEFAULT_FLEET_STATS_OPTIONS, ...options };
  ...
  const powerByProfile = { desktop: 0, laptop: 0, unknown: 0 };
  ...
  for (const equipment of list) {
    ...
    increment(
      formFactorCounts,
      profile === "laptop" ? "Portable" : profile === "desktop" ? "Fixe" : "Non identifié"
    );
    ...
    const agentVersion = String(
      inventory.agentVersion || inventory.agent_version || equipment?.rawData?.agent_version || ""
    ).trim();
    if (agentVersion) increment(agentVersionCounts, `v${agentVersion.replace(/^v/i, "")}`);
    else increment(agentVersionCounts, "Inconnu");
    ...
    if (machineDiskGb > 0) {
      ...
      diskKnownCount += 1;
      if (machineDiskGb < 256) increment(diskTierCounts, "< 256 Go");
      else if (machineDiskGb < 512) increment(diskTierCounts, "256 – 512 Go");
      else if (machineDiskGb < 1024) increment(diskTierCounts, "512 Go – 1 To");
      else increment(diskTierCounts, "1 To et +");
    } else {
      increment(diskTierCounts, "Non renseigné");
    }
    ...
    if (statusKey === "online") online += 1;
    else if (statusKey === "offline") offline += 1;
    ...
    if (lastAt) {
      ...
      if (Number.isFinite(ageDays) && ageDays > opts.staleInventoryDays) staleInventory += 1;
      else freshInventory += 1;
    } else {
      staleInventory += 1;
    }

    const updates = getWindowsUpdateStatus(inventory);
    if (updates.pendingCount === 0) updatesUpToDate += 1;
    else if (updates.pendingCount != null) {
      updatesPending += 1;
    ...
    const domainLabel =
      equipment?.domaine ||
      inventory.domaine ||
      (domain.joined ? domain.name : domain.workgroup) ||
      "";
    if (domain.joined === true || (domainLabel && !domain.workgroup)) domainJoined += 1;
    else if (domain.workgroup || String(domainLabel).toUpperCase() === "WORKGROUP") workgroupOnly += 1;
    else domainUnknown += 1;
  ...
  let monthlyKwh = 0;
  const powerBreakdown = Object.entries(powerByProfile)
  ...
  const attentionCount =
    manual + offline + updatesPending + staleInventory + highDiskUsage + licenseInactive;
  ...
  return {
    ...
    agentStatus: { online, offline, unknown: unknownStatus },
    ...
    formFactorDistribution: toDistribution(formFactorCounts, total, { limit: 4 }),
    ramDistribution: toDistribution(ramCounts, total, { limit: 5 }),
    cpuDistribution: toDistribution(cpuCounts, total, { limit: 5 }),
    diskDistribution: toDistribution(diskTierCounts, total, { limit: 5 }),
    agentVersionDistribution: toDistribution(agentVersionCounts, total, { limit: 5 }),
    domain: { joined: domainJoined, workgroup: workgroupOnly, unknown: domainUnknown },
    windowsUpdates: {
    ...
    fleetHealth: {
      ...
      label:
        attentionCount === 0
          ? "Parc sous contrôle"
          : attentionCount <= Math.max(1, Math.floor(total * 0.2))
          ? "Points de vigilance"
          : "Parc à surveiller",
    },
  };
...
export async function loadClientEquipmentForFleetStats(
  clientId,
  ...
  const [modulesData, clientRes] = await Promise.all([

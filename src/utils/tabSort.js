/**
 * Catégories d'onglets (alignées sur la sidebar) · tri et dossiers.
 */
export const FOLDER_MODE_THRESHOLD = 8;

export const TAB_FOLDER_GROUPS = [
  { key: "enterprise", types: ["ContratDetail", "Contrat"], icon: "mdi:office-building" },
  { key: "contact", types: ["ContactDetail", "Contact"], icon: "mdi:account-group" },
  { key: "ticket", types: ["TicketDetail", "Ticket"], icon: "mdi:ticket-outline" },
  { key: "ticketSales", types: ["TicketSalesCreate", "TicketSales"], icon: "mdi:briefcase-edit-outline" },
  { key: "planning", types: ["Planning"], icon: "mdi:calendar-clock" },
  { key: "hardware", types: ["Hardware", "Equipment", "EquipmentDetail", "ComputerFleetStats"], icon: "mdi:monitor-dashboard" },
  { key: "cyber", types: ["CampaignDetail", "AntivirusDetail", "AntispamDetail", "Cybersecurite"], icon: "mdi:shield-lock" },
  { key: "service", types: ["TenantDetail", "Service"], icon: "mdi:cloud-outline" },
  { key: "monitoring", types: ["MonitoringDetail", "Mon", "Rapport"], icon: "mdi:chart-line" },
  { key: "documents", types: ["DocumentsHub"], icon: "mdi:folder-outline" },
];
...
const OTHER_FOLDER = { key: "other", icon: "mdi:folder-outline" };
...
/**
 * @returns {{ key: string, icon: string, tabs: object[] }[]}
 */
export function groupTabsIntoFolders(tabs, locale = "fr") {
  ...
  tabs.forEach((tab, index) => {
    ...
    buckets.get(folderKey).push({ tab, index });
  });

  const orderedKeys = [
    ...TAB_FOLDER_GROUPS.map((g) => g.key),
    ...(buckets.has(OTHER_FOLDER.key) ? [OTHER_FOLDER.key] : []),
  ];

  return orderedKeys
    .filter((key) => buckets.has(key))
    .map((key) => {
      const meta = key === OTHER_FOLDER.key
        ? OTHER_FOLDER
        : TAB_FOLDER_GROUPS.find((g) => g.key === key);
      const sortedTabs = buckets
        .get(key)
        .sort((a, b) => {

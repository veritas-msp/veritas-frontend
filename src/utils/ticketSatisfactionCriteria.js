...
/** @deprecated Utiliser getTicketSatisfactionCriteria(locale) pour les libellés localisés. */
export const TICKET_SATISFACTION_CRITERIA = TICKET_SATISFACTION_CRITERION_KEYS.map((key) => ({ key }));
...
export function computeSatisfactionAverage(ratings) {
  if (!ratings) return 0;
  const values = TICKET_SATISFACTION_CRITERION_KEYS.map((key) => Number(ratings[key])).filter(
    (v) => v >= 1 && v <= 5
  );

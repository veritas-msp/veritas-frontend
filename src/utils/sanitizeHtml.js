...
/** Commentaires tickets : images inline autorisées, pas d'attribut style (vecteur XSS). */
...
/** Sanitise du HTML riche avant dangerouslySetInnerHTML. */
...
/** Sanitise le HTML des commentaires / modèles de tickets. */
export function sanitizeTicketCommentHtml(raw) {
  return sanitizeHtml(raw, TICKET_COMMENT_CONFIG);
}

/** HTML Microsoft Graph (insights/remediation). */
...
/** Options fetch standard · cookie httpOnly uniquement (plus de localStorage token). */

/**
 * Identification stable des périphériques · ne jamais matcher uniquement par nom
 * (plusieurs clients peuvent avoir un serveur « HYPERV-01 »).
 */
...
/**
 * Recherche une ligne API modules (déjà filtrée par client_id côté route).
 */
export function findEquipmentInApiList(list, equipment, { type, equipmentName } = {}) {
  const arr = Array.isArray(list) ? list : [];
  const dbId = getEquipmentDbId(equipment);
  if (dbId) {
  ...
  const targetName =
    equipmentName ||
    equipment?.name ||
    equipment?.rawData?.nom ||
    equipment?.rawData?.name ||
    "";
  ...
  if (type === "Internet") {
    const nameMatches = arr.filter((row) => {
      ...
      const equipmentFournisseur =
        equipment?.rawData?.fournisseur || equipment?.fournisseur || "";
      const equipmentInternetType =
        equipment?.rawData?.type || equipment?.internetType || "";
      if (equipmentFournisseur && equipmentInternetType && eqFournisseur && eqType) {
        return (
          ...
          eqType.toUpperCase() === equipmentInternetType.toUpperCase()
        );
      }
      return false;
    });
  ...
  const nameMatches = arr.filter((row) => {
    ...
    return (
      eqName === targetName ||
      eqName === (equipment?.name || equipment?.rawData?.nom)
    );
  });

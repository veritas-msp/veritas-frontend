...
const DEFAULT_COMMENT_TEMPLATES = [
  {
    ...
    content:
      "Bonjour,\n\nNous confirmons la bonne prise en charge de votre demande. Nous revenons vers vous rapidement avec un point d'avancement.\n\nCordialement,",
  },
  {
    id: "tpl-need-info",
    name: "Demande d'informations",
    content:
      "Bonjour,\n\nAfin d'avancer sur votre ticket, pouvez-vous nous transmettre les informations complémentaires suivantes :\n- \n- \n\nMerci d'avance,\nCordialement,",
  },
  {
    id: "tpl-resolved",
    name: "Résolution",
    content:
      "Bonjour,\n\nLe ticket est désormais résolu. N'hésitez pas à revenir vers nous si vous constatez un comportement anormal.\n\nCordialement,",
  },
];
...
const DEFAULT_MACROS = [
  {
    ...
    actions: [
      { id: "a1", type: "set_field", field: "status", value: "in_progress" },
      { id: "a2", type: "set_field", field: "assigned_to_me", value: "true" },
    ],
  },
  {
    id: "macro-resolve-notify",
    name: "Résoudre + notification",
    actions: [
      { id: "a1", type: "set_field", field: "status", value: "resolved" },
      { id: "a2", type: "set_field", field: "assigned_to_me", value: "true" },
      {
        id: "a3",
        type: "open_email",
        emailSubject: "Ticket #{{ticketNumber}} - Résolution",
        emailBody: "Bonjour,\n\nLe ticket \"{{title}}\" est maintenant résolu.\n\nCordialement,",
      },
    ],
  },
];
...
const getDefaults = () => ({
  ...
  autoReplyRules: [],
  autoReplyTemplate:
    "Bonjour {{demandeur.prenom}},\n\nVotre demande a bien été reçue et un ticket vient d'être créé.\nNous revenons vers vous dans les meilleurs délais.\n\nCordialement,",
...
const normalizeNotificationSettings = (row) => ({
  ...
  webhooks: Array.isArray(row?.webhooks)
    ...
    : [],
  notificationEvents: (Array.isArray(row?.notificationEvents) ? row.notificationEvents : DEFAULT_NOTIFICATION_EVENTS).map(
    (eventItem, idx) => {
      const sourceRaw = String(eventItem?.source || "").trim().toLowerCase();
      const legacyKey = String(eventItem?.key || "").trim().toLowerCase();
      const source =
        sourceRaw ||
        (legacyKey.includes("ticket") ? "tickets" : legacyKey.includes("equipment") ? "infrastructure" : "tickets");
      const element =
        String(eventItem?.element || "").trim().toLowerCase() ||
        (legacyKey.includes("created")
          ? "created"
          : legacyKey.includes("resolved")
          ? "resolved"
          : legacyKey.includes("comment")
          ? "commented"
          : "updated");
      return {
...
export async function fetchTicketAutomationConfig() {
  try {
    ...
    if (!response.ok) throw new Error("Erreur lors du chargement de la configuration ticket");
    const payload = await response.json();
    cachedConfig = {
      commentTemplates: Array.isArray(payload?.commentTemplates)
        ? payload.commentTemplates.map(normalizeTemplate)
        : getDefaults().commentTemplates,
      ...
      scheduledAlertRules: Array.isArray(payload?.scheduledAlertRules)
        ? payload.scheduledAlertRules.map(normalizeScheduledAlertRule)
        : [],
...
export async function saveTicketAutomationConfig(nextConfig) {
  const sanitized = {
    commentTemplates: Array.isArray(nextConfig?.commentTemplates)
      ? nextConfig.commentTemplates.map(normalizeTemplate)
      : [],
    macros: Array.isArray(nextConfig?.macros) ? nextConfig.macros.map(normalizeMacro) : [],
    emailInboxes: Array.isArray(nextConfig?.emailInboxes)
      ? nextConfig.emailInboxes.map(normalizeEmailInbox)
      : [],
    exclusionRules: Array.isArray(nextConfig?.exclusionRules)
      ? nextConfig.exclusionRules.map(normalizeExclusionRule)
      : [],
    autoReplyRules: Array.isArray(nextConfig?.autoReplyRules)
      ? nextConfig.autoReplyRules.map(normalizeAutoReplyRule)
      : [],
    ...
    scheduledAlertRules: Array.isArray(nextConfig?.scheduledAlertRules)
      ? nextConfig.scheduledAlertRules.map(normalizeScheduledAlertRule)
      : [],
    mailCollectors: Array.isArray(nextConfig?.mailCollectors)
      ? nextConfig.mailCollectors.map(normalizeMailCollector)
      : [],
  ...
  if (!response.ok) {
    ...
    throw new Error(payload?.error || "Erreur lors de la sauvegarde de la configuration ticket");
  }
  ...
  cachedConfig = {
    ...
    emailInboxes: Array.isArray(payload?.emailInboxes)
      ? payload.emailInboxes.map(normalizeEmailInbox)
      : sanitized.emailInboxes,
    exclusionRules: Array.isArray(payload?.exclusionRules)
      ? payload.exclusionRules.map(normalizeExclusionRule)
      : sanitized.exclusionRules,
    autoReplyRules: Array.isArray(payload?.autoReplyRules)
      ? payload.autoReplyRules.map(normalizeAutoReplyRule)
      : sanitized.autoReplyRules,
    autoReplyTemplate: String(payload?.autoReplyTemplate || sanitized.autoReplyTemplate),
    notificationSettings: normalizeNotificationSettings(
      payload?.notificationSettings || sanitized.notificationSettings
    ),
    scheduledAlertRules: Array.isArray(payload?.scheduledAlertRules)
      ? payload.scheduledAlertRules.map(normalizeScheduledAlertRule)
      : sanitized.scheduledAlertRules,
    mailCollectors: Array.isArray(payload?.mailCollectors)
      ? payload.mailCollectors.map(normalizeMailCollector)
      : sanitized.mailCollectors,
    mailCollectSettings: normalizeMailCollectSettings(
      payload?.mailCollectSettings || sanitized.mailCollectSettings
    ),
  };
  window.dispatchEvent(new CustomEvent(UPDATE_EVENT));
  return cachedConfig;

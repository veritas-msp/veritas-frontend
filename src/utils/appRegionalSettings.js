...
/** Format d'affichage par défaut selon la langue UI (en = en-GB). */
...
let cachedRegional = { ...DEFAULT_REGIONAL };
...
export function formatAppLongDate(value, regional = cachedRegional) {
  ...
  const capitalizedWeekday =
    weekday.charAt(0).toLocaleUpperCase(localeTag(regional.locale)) + weekday.slice(1);
...
export function formatAppEventRange(start, end, regional = cachedRegional) {
  ...
  const endTime = !endDate || Number.isNaN(endDate.getTime())
    ? "-"
    : new Intl.DateTimeFormat(localeTag(regional.locale), timeOpts).format(endDate);

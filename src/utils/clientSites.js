...
/** Réindexe sortOrder selon l'ordre du tableau (0…n-1), sans re-trier. */
export function assignClientSitesOrder(sites) {
  const list = Array.isArray(sites) ? [...sites] : [];
  return enforceSinglePrimarySite(
    list.map((site, index) => ({ ...site, sortOrder: index }))
  );
}
...
export function normalizeClientSites(sites) {
  ...
  if (!Array.isArray(sites)) {
    ...
    normalized = sites
      .map((site, index) => normalizeClientSite(site, index))
      .filter(Boolean);
  }
...
/** Un seul lieu principal à la fois ; preferredPrimaryId force la bascule si renseigné. */
export function enforceSinglePrimarySite(sites, preferredPrimaryId = null) {
  ...
  if (!primaryId) {
    return list.map((site) => ({ ...site, isPrimary: false }));
  }
...
export function findClientSiteByLocation(sites, locationName) {
  ...
  return (
    normalizeClientSites(sites).find((site) => {
...
export function serializeSitesForCompare(sites) {
  return JSON.stringify(
    normalizeClientSites(sites).map((site) => ({
      id: site.id,
      name: site.name,
...
export async function geocodeSiteAddress(site) {
  ...
  if (!query) {
    throw new Error("Adresse insuffisante pour la localisation.");
  }
  ...
  if (!response.ok) {
    throw new Error("Impossible de contacter le service de cartographie.");
  }

  const results = await response.json();
  if (!Array.isArray(results) || results.length === 0) {
    throw new Error("Aucune localisation trouvée pour cette adresse.");
  }
...
export function buildStaticMapUrl(latitude, longitude, { width = 280, height = 160, zoom = 14 } = {}) {
...
/** Vignette fiable via l'embed officiel OpenStreetMap (le service staticmap.de est instable). */
